...
//   node studio-data/build.mjs --all           -> construit les deux
...
const targets = args.includes('--all') ? ['v7', 'v11'] : [tIdx >= 0 ? args[tIdx + 1] : 'v7'];
let rc = 0;
for (const target of targets) {
  const manifest = JSON.parse(fs.readFileSync(path.join(srcDir, target === 'v11' ? 'manifest-v11.json' : 'manifest.json'), 'utf8'));
  ...
  const v = target === 'v11' ? ((outHtml.match(/const V11_VERSION = '([^']+)'/) || [])[1] || '?') : ((outHtml.match(/V11_VERSION : '([^']+)'/) || [])[1] || '?');
  ...
  if (check) {
    ...
    else { console.error('ÉCART : ' + name + ' ne correspond pas aux sources — lancez node studio-data/build.mjs' + (target === 'v11' ? ' --target v11' : '')); rc = 1; }
    continue;
  }
