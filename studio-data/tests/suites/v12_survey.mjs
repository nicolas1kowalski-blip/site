...
await p.setContent(html,{waitUntil:'domcontentloaded'}); await p.addStyleTag({ path: D+'tw/tw_built.css' }); await p.waitForTimeout(300);
...
for (const [n,name] of steps) { try { await p.evaluate(n=>switchTab(n), n); } catch(e) { console.log('ERR', name, String(e).slice(0,150)); } await p.waitForTimeout(500); await p.evaluate(()=>document.querySelectorAll('.v11-toast').forEach(t=>t.remove())); await p.screenshot({ path: D+'s12_'+name+'.png' }); }
for (const [fn,name] of [['wizOpen("menu")','wizmenu'],['wizClose(); openBackupCenter()','backup'],['toggleDbModal()','dbmodal'],['toggleDbModal(); openMergeModal()','merge']]) { try { await p.evaluate(fn=>eval(fn), fn); } catch(e) { console.log('ERR', name, String(e).slice(0,150)); } await p.waitForTimeout(400); await p.screenshot({ path: D+'s12_'+name+'.png' }); }
