        ...
        const RC_STEPS = {
            ...
            std: '☎️ Standardiser (tél/email)',
            enrich: '🌍 Enrichir (référentiel)',
            calc: '🧮 Colonne calculée',
            dedup: '♊ Dédoublonner',
            rename: '✏️ Renommer',
            drop: '🗑 Supprimer colonnes'
        };
        ...
        function rcAdd() {
            const r = { id: 'rc_' + generateId(), name: 'Nouvelle préparation', src: '', out: '', steps: [] };
            rcList().push(r);
            rcState.openId = r.id;
            persistAppState();
            renderRecipes();
        ...
        function rcById(id) {
            return rcList().find(x => x.id === id);
        }
        function rcSet(id, f, v) {
            const recipe = rcById(id);
            if (!recipe) return;
            recipe[f] = v;
            if (f === 'src' && !recipe.out) recipe.out = 'PROPRE_' + String(v).replace(/\.[^.]+$/, '');
            persistAppState();
            if (f === 'src') renderRecipes();
        }
        function rcDel(id) {
            state.recipes = rcList().filter(x => x.id !== id);
            persistAppState();
            renderRecipes();
        }
        function rcAddStep(id, type) {
            const recipe = rcById(id);
            if (!recipe) return;
            recipe.steps.push({ id: 'st_' + generateId(), type, enabled: true, p: {} });
            persistAppState();
            renderRecipes();
        }
        function rcSetStep(id, sid, f, v) {
            const recipe = rcById(id);
            const st = recipe && recipe.steps.find(x => x.id === sid);
            if (!st) return;
            if (f.startsWith('p.')) st.p[f.slice(2)] = v;
            else st[f] = v;
            persistAppState();
            if (f === 'enabled' || f === 'type') renderRecipes();
        }
        function rcDelStep(id, sid) {
            const recipe = rcById(id);
            if (!recipe) return;
            recipe.steps = recipe.steps.filter(x => x.id !== sid);
            persistAppState();
            renderRecipes();
        }
        function rcMoveStep(id, sid, dir) {
            const recipe = rcById(id);
            if (!recipe) return;
            const i = recipe.steps.findIndex(x => x.id === sid);
            const j = i + dir;
            if (i < 0 || j < 0 || j >= recipe.steps.length) return;
            const [st] = recipe.steps.splice(i, 1);
            recipe.steps.splice(j, 0, st);
            persistAppState();
            renderRecipes();
        }
        // SQL d'une recette, jusqu'à l'étape uptoIdx incluse (déterministe : même entrée -> même sortie).
        function rcBuildSql(r, uptoIdx) {
            const table = tableByName(r.src);
            if (!table || table.status !== 'ready') throw new Error(`La source "${r.src}" n'est pas chargée.`);
            let sql = `SELECT * EXCLUDE (__rn) FROM ${sqlIdent(duckTableName(table.id))}`;
            const steps = r.steps.filter(st => st.enabled);
            const upto = uptoIdx == null ? steps.length : Math.min(uptoIdx + 1, steps.length);
            for (let i = 0; i < upto; i++) {
                const step = steps[i];
                const P = step.p || {};
                const c = P.col ? sqlIdent(P.col) : null;
                const raw = c ? `CAST(${c} AS VARCHAR)` : null;
                switch (step.type) {
                    case 'filter': {
                        const f = P.col && P.op ? qualCondSql({ col: P.col, op: P.op, val: P.val || '' }) : '';
                        if (f) sql = `SELECT * FROM (\n${sql}\n) q WHERE ${f}`;
                        break;
                    }
                    case 'clean': {
                        if (!c) break;
                            ...
                            A === 'trim'
                                ...
                                : A === 'upper'
                                  ...
                                  : A === 'lower'
                                    ...
                                    : A === 'noaccents'
                                      ? `strip_accents(${raw})`
                                      : `regexp_replace(TRIM(${raw}), ' +', ' ', 'g')`;
                        sql = `SELECT * REPLACE (${ex} AS ${c}) FROM (\n${sql}\n) q`;
                        break;
                    }
                    case 'normalize': {
                        if (!c || !P.fmt) break;
                        const ne = tdNormExpr(P.fmt, raw);
                        if (!ne) break;
                        sql = `SELECT * REPLACE (COALESCE(${ne}, ${raw}) AS ${c}) FROM (\n${sql}\n) q`;
                        break;
                    }
                    case 'calc': {
                        if (!P.name || !String(P.formula || '').trim()) break;
                    ...
                    case 'dedup': {
                        ...
                        const k = keys
                            .map(k2 => `COALESCE(UPPER(TRIM(CAST(${sqlIdent(k2)} AS VARCHAR))), '')`)
                            .join(` || chr(1) || `);
                        sql = `SELECT * FROM (\n${sql}\n) q QUALIFY row_number() OVER (PARTITION BY ${k} ORDER BY md5(CAST(q AS VARCHAR))) = 1`;
                        break;
                    }
                    case 'std': {
                        if (!c) break;
                    ...
                    case 'rename': {
                        if (!c || !String(P.to || '').trim()) break;
                        sql = `SELECT * RENAME (${c} AS ${sqlIdent(P.to.trim())}) FROM (\n${sql}\n) q`;
                        break;
                    }
                    case 'drop': {
        ...
        async function rcRun(id, btn) {
            const recipe = rcById(id);
            if (!recipe) return;
            if (!recipe.src) return showError('Choisissez la source à préparer.');
            if (!String(recipe.out || '').trim()) return showError('Donnez un nom à la table produite.');
            const clash = Object.values(state.tables).find(t => t.name === recipe.out && t.id !== recipe.targetId);
            if (clash) return showError(`Le nom "${recipe.out}" est déjà utilisé par une autre source/table.`);
            if (btn) btn.disabled = true;
            bgTaskStart(`Préparation « ${recipe.name} » en cours`);
            try {
                const { conn } = await getDB();
                const inner = rcBuildSql(recipe, null);
                const tId = recipe.targetId && state.tables[recipe.targetId] ? recipe.targetId : 'tb_' + generateId();
                if (!state.tables[tId]) {
                    state.tables[tId] = {
                        id: tId,
                        name: recipe.out,
                ...
                recipe.lastRows = Number(arrowResultToObjects(cRes)[0].n);
                recipe.lastAt = Date.now();
                recipe.targetId = tId;
                // E8 : taux d'appariement de chaque étape d'enrichissement (rapport de correspondance).
                for (const st of recipe.steps.filter(
                    s2 => s2.enabled && s2.type === 'enrich' && (s2.p || {}).ref && (s2.p || {}).col
                )) {
                    try {
                        const R2 = RC_REFS[st.p.ref];
                        const added = R2.cols.filter(c2 => c2 !== R2.key)[0];
                        const mRes = await conn.query(
                            `SELECT COUNT(*)::BIGINT AS n, SUM(CASE WHEN ${sqlIdent((st.p.prefix || '') + added)} IS NOT NULL THEN 1 ELSE 0 END)::BIGINT AS m FROM ${sqlIdent(duckTableName(tId))} WHERE ${sqlIdent(st.p.col)} IS NOT NULL AND TRIM(CAST(${sqlIdent(st.p.col)} AS VARCHAR)) <> ''`
                        );
                        const rows = arrowResultToObjects(mRes)[0];
                        st.lastMatch = Number(rows.n) ? Math.round((100 * Number(rows.m)) / Number(rows.n)) : 100;
                    } catch (e2) {
                        st.lastMatch = null;
                    }
                }
                // Journalisation lineage : la table produite est alimentée par la source.
                state.governance.lineage[recipe.out] = { from: [recipe.src] };
                renderTables();
                ...
                renderRecipes();
                bgTaskEnd(
                    `🧹 Préparation « ${recipe.name} » : table propre « ${recipe.out} » produite (${recipe.lastRows.toLocaleString('fr-FR')} lignes).`
                );
                try {
                    await qrAutoRun(recipe.out);
                } catch (e2) {}
            } catch (e) {
                bgTaskEnd();
                showError('Préparation impossible : ' + e.message);
            } finally {
                if (btn) btn.disabled = false;
            }
        }
        async function rcAutoRun(srcName) {
            for (const r of rcList().filter(x => x.src === srcName && x.steps.length)) {
                try {
                    await rcRun(r.id, null);
                } catch (e) {
                    console.warn('Préparation auto :', e);
                }
            }
        }
        async function rcPreview(id, uptoIdx) {
            ...
            try {
                const { conn } = await getDB();
                const res = await conn.query(`SELECT * FROM (\n${rcBuildSql(recipe, uptoIdx)}\n) p LIMIT 8`);
                const rows = arrowResultToObjects(res);
                const box = el('rcPreview-' + id);
                if (!box) return;
                if (!rows.length) {
                    box.innerHTML = '<p class="text-[11px] text-slate-400 italic p-2">Aucune ligne après cette étape.</p>';
                    return;
                }
                ...
                box.innerHTML = `<div class="text-[10px] font-bold text-teal-700 mb-1">Aperçu après l'étape ${uptoIdx == null ? 'finale' : uptoIdx + 1} (8 premières lignes)</div>
                    <div class="border border-slate-200 rounded overflow-x-auto"><table class="w-full text-left text-[10px]"><thead class="bg-slate-50 font-bold"><tr>${cols.map(c2 => `<th class="p-1 whitespace-nowrap">${escapeHTML(c2)}</th>`).join('')}</tr>
                        </thead>
                    <tbody>${rows.map(rw => `<tr class="border-t border-slate-100">${cols.map(c2 => `<td class="p-1 whitespace-nowrap max-w-[160px] truncate">${escapeHTML(rw[c2] == null ? '' : String(rw[c2]))}</td>`).join('')}</tr>`).join('')}</tbody>
                        </table>
                        </div>`;
            } catch (e) {
                const box = el('rcPreview-' + id);
                if (box)
                    box.innerHTML = `<p class="text-[11px] text-red-600 p-2">Aperçu impossible : ${escapeHTML(e.message)}</p>`;
            }
        }
        function rcExport() {
            const blob = new Blob(
                [
                    JSON.stringify(
                        {
                            kind: 'studio-data-recipes',
                            recipes: rcList().map(r => {
                                const c2 = { ...r };
                                delete c2.targetId;
                                return c2;
                            })
            ...
            anchorElement.download = `Preparations_${Date.now()}.json`;
        ...
        function rcImport(inputEl) {
            ...
            reader.onload = () => {
                try {
                    ...
                    const list = j && j.kind === 'studio-data-recipes' ? j.recipes : Array.isArray(j) ? j : null;
                    ...
                    persistAppState();
                    renderRecipes();
                    showSuccess(`🧹 ${count} préparation(s) importée(s).`);
                } catch (e) {
        ...
        function rcStepParamsHtml(r, st) {
            ...
            const P = st.p || {};
            const colSel = (f, cur) =>
                `<select onchange="rcSetStep('${r.id}','${st.id}','${f}',this.value)" class="border border-slate-200 rounded px-1 py-0.5 text-[11px] bg-white max-w-[130px]"><option value="">— colonne —</option>${hs.map(h => `<option value="${escapeHTML(h)}" ${cur === h ? 'selected' : ''}>${escapeHTML(h)}</option>`).join('')}</select>`;
            const inp = (f, cur, ph, w) =>
                `<input type="text" value="${escapeHTML(cur == null ? '' : String(cur))}" onchange="rcSetStep('${r.id}','${st.id}','p.${f}',this.value)" placeholder="${ph}" class="border border-slate-200 rounded px-1.5 py-0.5 text-[11px] ${w || 'w-32'}">`;
            switch (st.type) {
                case 'filter':
                    return (
                        colSel('p.col', P.col) +
                        ` <select onchange="rcSetStep('${r.id}','${st.id}','p.op',this.value)" class="border border-slate-200 rounded px-1 py-0.5 text-[11px] bg-white">${QUAL_FILTER_OPS.map(o => `<option value="${o.v}" ${P.op === o.v ? 'selected' : ''}>${o.t}</option>`).join('')}</select> ` +
                        inp('val', P.val, 'valeur')
                    );
                case 'clean':
                    return (
                        colSel('p.col', P.col) +
                        ` <select onchange="rcSetStep('${r.id}','${st.id}','p.action',this.value)" class="border border-slate-200 rounded px-1 py-0.5 text-[11px] bg-white">${Object.entries(
                            RC_CLEAN
                        )
                            .map(
                                ([k, l]) => `<option value="${k}" ${(P.action || 'trim') === k ? 'selected' : ''}>${l}</option>`
                            )
                            .join('')}</select>`
                    );
                case 'normalize':
                    return (
                        colSel('p.col', P.col) +
                        ` <select onchange="rcSetStep('${r.id}','${st.id}','p.fmt',this.value)" class="border border-slate-200 rounded px-1 py-0.5 text-[11px] bg-white"><option value="">— format —</option><option value="date" ${P.fmt === 'date' ? 'selected' : ''}>date → AAAA-MM-JJ</option><option value="dec" ${P.fmt === 'dec' ? 'selected' : ''}>décimal (1 234,5 → 1234.5)</option><option value="int" ${P.fmt === 'int' ? 'selected' : ''}>entier</option><option value="code" ${P.fmt === 'code' ? 'selected' : ''}>code (MAJ, sans espaces)</option><option value="bool" ${P.fmt === 'bool' ? 'selected' : ''}>booléen</option></select>`
                    );
                case 'calc':
                    return (
                        inp('name', P.name, 'nom de la colonne', 'w-36') +
                        ' = ' +
                        inp('formula', P.formula, 'ex : [PRIX] * [QTE]', 'w-64')
                    );
                case 'dedup':
                    return 'clé : ' + inp('keys', P.keys, 'colonnes séparées par ;', 'w-56');
                case 'std':
                    return (
                        colSel('p.col', P.col) +
                        ` <select onchange="rcSetStep('${r.id}','${st.id}','p.what',this.value)" class="border border-slate-200 rounded px-1 py-0.5 text-[11px] bg-white"><option value="phone" ${(P.what || 'phone') === 'phone' ? 'selected' : ''}>téléphone → +33…</option><option value="email" ${P.what === 'email' ? 'selected' : ''}>email → minuscules</option></select>`
                    );
                case 'enrich':
                    return (
                        colSel('p.col', P.col) +
                        ` <select onchange="rcSetStep('${r.id}','${st.id}','p.ref',this.value)" class="border border-slate-200 rounded px-1 py-0.5 text-[11px] bg-white"><option value="">— référentiel —</option>${Object.entries(
                            RC_REFS
                        )
                            .map(([k, R2]) => `<option value="${k}" ${P.ref === k ? 'selected' : ''}>${R2.label}</option>`)
                            .join('')}</select> préfixe ` +
                        inp('prefix', P.prefix, 'REF_', 'w-16') +
                        (st.lastMatch != null
                            ? ` <span class="text-[10px] font-bold ${st.lastMatch >= 90 ? 'text-emerald-600' : 'text-amber-600'}">${st.lastMatch} % appariés</span>`
                            : '')
                    );
                case 'rename':
                    return colSel('p.col', P.col) + ' → ' + inp('to', P.to, 'nouveau nom', 'w-36');
                case 'drop':
                    return inp('cols', P.cols, 'colonnes à supprimer, séparées par ;', 'w-72');
                default:
                    return '';
            }
        }
        function renderRecipes() {
            const rcContentElement = el('rcContent');
            if (!rcContentElement) return;
                ...
                html += `<details class="border border-teal-200 rounded-xl bg-white mb-3 group" ${open ? 'open' : ''}>
                    ...
                    <div class="p-4">
                        ...
                        ${r.steps
                            .map(
                                (
                                    st,
                                    i
                                ) => `<div class="flex items-center gap-1.5 mb-1.5 flex-wrap ${st.enabled ? '' : 'opacity-40'}">
                            ...
                            <select onchange="rcSetStep('${r.id}','${st.id}','type',this.value)" class="border border-slate-200 rounded px-1 py-0.5 text-[11px] bg-white font-bold">${Object.entries(
                                RC_STEPS
                            )
                                .map(([k, l]) => `<option value="${k}" ${st.type === k ? 'selected' : ''}>${l}</option>`)
                                .join('')}</select>
                        ...
                        <div class="flex items-center gap-1.5 flex-wrap mt-2">
                            ...
                            ${Object.entries(RC_STEPS)
                                .map(
                                    ([k, l]) =>
                                        `<button onclick="rcAddStep('${r.id}','${k}')" class="text-[11px] bg-white border border-slate-300 rounded px-2 py-0.5 font-bold text-slate-600 hover:bg-teal-50">${l}</button>`
                                )
                                .join('')}
                        </div>
            ...
            rcContentElement.innerHTML = html;
        }
        ...
        function cbShowBadge(sel) {
            if (!_cbBadge) {
                _cbBadge = document.createElement('div');
                _cbBadge.id = 'comboBadge';
                _cbBadge.setAttribute('role', 'status');
                document.body.appendChild(_cbBadge);
            }
            const rct = sel.getBoundingClientRect();
            _cbBadge.style.cssText =
                'position:fixed;left:' +
                Math.round(rct.left) +
                'px;top:' +
                Math.round(Math.max(2, rct.top - 27)) +
                'px;z-index:10000;background:#0f172a;color:#fff;font:bold 11px system-ui;padding:3px 9px;border-radius:6px;box-shadow:0 2px 8px rgba(0,0,0,.25);pointer-events:none;max-width:340px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis';
            return _cbBadge;
        }
        ...
        function cbApply(sel) {
            const normalized = cbNorm(_cbQ);
            const opts = Array.from(sel.options).filter(o => o.value !== '' && !o.disabled);
            const hit =
                opts.find(o => cbNorm(o.textContent).startsWith(normalized)) ||
                opts.find(o => cbNorm(o.textContent).includes(normalized));
        ...
        document.addEventListener(
            ...
            e => {
                ...
                if (e.key === 'Enter' || e.key === 'Tab') {
                    if (_cbSel === sel) cbReset(true);
                    return;
                }
                if (e.key === 'Escape') {
                    if (_cbSel === sel) {
                        if (_cbOrig !== null) sel.value = _cbOrig;
                        cbReset(false);
                        e.stopPropagation();
                    }
                    return;
                ...
                if (e.key === 'Backspace') {
                    if (_cbSel === sel && _cbQ) {
                        ...
                        _cbQ = _cbQ.slice(0, -1);
                        if (_cbQ) cbApply(sel);
                        else cbShowBadge(sel).textContent = '🔍 tapez pour chercher…';
                    }
                    return;
                }
        ...
        document.addEventListener(
            'focusout',
            e => {
                if (e.target === _cbSel) cbReset(true);
            },
            true
        );
