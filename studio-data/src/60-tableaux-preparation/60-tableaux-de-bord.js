        ...
        function dbList() {
            return (state.dashboards = state.dashboards || []);
        }
        function dbById(id) {
            return dbList().find(x => x.id === id);
        }
        function dbAdd() {
            const d = { id: 'db_' + generateId(), name: 'Nouveau tableau de bord', filters: [], tiles: [] };
            dbList().push(d);
            dbState.openId = d.id;
            persistAppState();
            renderDashboards();
        ...
        function dbDup(id) {
            const dashboard = dbById(id);
            if (!dashboard) return;
            const parsed = JSON.parse(JSON.stringify(dashboard));
            parsed.id = 'db_' + generateId();
            parsed.name = dashboard.name + ' (copie)';
            parsed.tiles.forEach(t => (t.id = 'tl_' + generateId()));
            dbList().push(parsed);
            persistAppState();
            renderDashboards();
        }
        function dbDel(id) {
            state.dashboards = dbList().filter(x => x.id !== id);
            if (dbState.openId === id) dbState.openId = null;
            persistAppState();
            renderDashboards();
        }
        function dbSet(id, f, v) {
            const dashboard = dbById(id);
            if (!dashboard) return;
            dashboard[f] = v;
            persistAppState();
        }
        function dbAddTile(id) {
            const dashboard = dbById(id);
            if (!dashboard) return;
        ...
        function dbSetTile(id, tid, f, v) {
            const dashboard = dbById(id);
            const t = dashboard && dashboard.tiles.find(x => x.id === tid);
            if (!t) return;
            t[f] = v;
            persistAppState();
            if (f === 'table' || f === 'kind') renderDashboards();
            else dbRunTile(id, tid);
        }
        function dbDelTile(id, tid) {
            const dashboard = dbById(id);
            if (!dashboard) return;
            dashboard.tiles = dashboard.tiles.filter(x => x.id !== tid);
            persistAppState();
            renderDashboards();
        }
        function dbAddFilter(id) {
            const dashboard = dbById(id);
            if (!dashboard) return;
            dashboard.filters.push({ col: '', op: 'eq', val: '', conn: 'AND' });
            persistAppState();
            renderDashboards();
        }
        function dbSetFilter(id, i, f, v) {
            const dashboard = dbById(id);
            if (!dashboard || !dashboard.filters[i]) return;
            dashboard.filters[i][f] = v;
            persistAppState();
            if (f === 'op') renderDashboards();
        }
        function dbDelFilter(id, i) {
            const dashboard = dbById(id);
            if (!dashboard) return;
        ...
        function dbWhereFor(d, tableName) {
            ...
            let sql = '';
            (d.filters || []).forEach(c => {
                if (!c.col || !table.headers.includes(c.col)) return;
        ...
        function dbTileSql(d, t) {
            const table = tableByName(t.table);
            if (!table || table.status !== 'ready') return null;
            const T = sqlIdent(duckTableName(table.id));
            const w = dbWhereFor(d, t.table);
            const W = w ? ` WHERE ${w}` : '';
            ...
            const agg =
                t.agg === 'count'
                    ? 'COUNT(*)'
                    : t.agg === 'countd'
                      ? `COUNT(DISTINCT ${t.aggCol ? sqlIdent(t.aggCol) : '1'})`
                      : `${(t.agg || 'sum').toUpperCase()}(${t.aggCol ? num(t.aggCol) : 'NULL'})`;
        ...
        async function dbRunTile(id, tid) {
            ...
            const t = dashboard && dashboard.tiles.find(x => x.id === tid);
            if (!t) return;
            const body = el('dbTileBody-' + tid);
            if (!body) return;
            const sql = dbTileSql(dashboard, t);
            if (!sql) {
                body.innerHTML =
                    '<p class="text-[11px] text-slate-300 italic p-3">Configurez la tuile (table' +
                    (t.kind !== 'kpi' ? ', axe' : '') +
                    (t.agg !== 'count' ? ', colonne agrégée' : '') +
                    ').</p>';
                return;
            }
            try {
                ...
                if (t.kind === 'kpi') {
                    ...
                    t._v = v;
                    const st2 = dbKpiStatus(t, v);
                    t._st = st2;
                    const cls = st2 === 'crit' ? 'text-red-600' : st2 === 'warn' ? 'text-amber-500' : 'text-sky-700';
                    const cmp = (t.thDir || 'max') === 'min' ? '≤' : '≥';
                    const badge = st2
                        ? `<div class="text-center text-[10px] font-bold pb-3 ${st2 === 'crit' ? 'text-red-600' : st2 === 'warn' ? 'text-amber-600' : 'text-emerald-600'}">${st2 === 'crit' ? '⛔ Seuil critique atteint (' + cmp + ' ' + t.thCrit + ')' : st2 === 'warn' ? "⚠️ Seuil d'alerte atteint (" + cmp + ' ' + t.thWarn + ')' : '✅ Dans les seuils'}</div>`
                        : '';
                ...
                if (t.kind === 'table') {
                    body.innerHTML = `<div class="overflow-auto max-h-[240px]"><table class="w-full text-left text-[11px]"><thead class="bg-slate-50 font-bold sticky top-0"><tr><th class="p-1.5">${escapeHTML(t.dim)}</th>
                        <th class="p-1.5 text-right">${DB_AGGS[t.agg] || t.agg}${t.aggCol ? ' ' + escapeHTML(t.aggCol) : ''}</th>
                        </tr></thead>
                        <tbody>${rows
                            .map(
                                r => `<tr class="border-t border-slate-100"><td class="p-1.5">${escapeHTML(String(r.d))}</td><td class="p-1.5 text-right font-bold">${Number(r.v).toLocaleString('fr-FR')}</td>
                            </tr>`
                            )
                            .join('')}</tbody>
                            </table></div>`;
                ...
                if (window._dbCharts[tid]) {
                    try {
                        window._dbCharts[tid].destroy();
                    } catch (e2) {}
                }
                if (typeof Chart === 'undefined') {
                    body.innerHTML =
                        '<p class="text-[11px] text-slate-400 p-3">Graphique indisponible (bibliothèque non chargée).</p>';
                    return;
                }
                ...
                window._dbCharts[tid] = new Chart(el('dbCv-' + tid).getContext('2d'), {
                    ...
                    data: {
                        labels: rows.map(r => String(r.d)),
                        datasets: [
                            {
                                label: DB_AGGS[t.agg] || '',
                                data: rows.map(r => Number(r.v)),
                                backgroundColor: t.kind === 'line' ? '#0284c755' : colors,
                                borderColor: t.kind === 'line' ? '#0284c7' : colors,
                                fill: t.kind === 'line'
                            }
                        ]
                    },
                    options: {
                        ...
                        scales: t.kind === 'pie' ? {} : { y: { beginAtZero: true } }
                    }
                });
            } catch (e) {
                body.innerHTML = `<p class="text-[11px] text-red-600 p-3">Erreur : ${escapeHTML(e.message)}</p>`;
            }
        }
        async function dbRunAll(id) {
            const dashboard = dbById(id);
            if (!dashboard) return;
            for (const t of dashboard.tiles) await dbRunTile(id, t.id);
        }
        function dbExportHtml(id) {
            const dashboard = dbById(id);
            if (!dashboard) return;
            const parts = dashboard.tiles
                .map(t => {
                    let inner = '';
                    if (t.kind === 'kpi')
                        inner = `<div style="font-size:42px;font-weight:900;color:#0369a1;text-align:center;padding:24px">${((t._rows || [])[0] ? Number(t._rows[0].v) : 0).toLocaleString('fr-FR')}</div>`;
                    else if (t.kind === 'table' || !el('dbCv-' + t.id))
                        inner = `<table style="width:100%;border-collapse:collapse;font:11px system-ui">${(t._rows || [])
                            .map(
                                r => `<tr><td style="border-top:1px solid #eee;padding:4px">${escapeHTML(String(r.d))}</td>
                            <td style="border-top:1px solid #eee;padding:4px;text-align:right;font-weight:700">${Number(r.v).toLocaleString('fr-FR')}</td>
                            </tr>`
                            )
                            .join('')}</table>`;
                    else {
                        try {
                            inner = `<img src="${el('dbCv-' + t.id).toDataURL('image/png')}" style="max-width:100%">`;
                        } catch (e) {
                            inner = '(graphique)';
                        }
                    }
                ...
                .join('');
            const filt = (dashboard.filters || [])
                .filter(f => f.col)
                .map(f => `${f.col} ${f.op} ${f.val || ''}`)
                .join(' · ');
            const html2 = `<!DOCTYPE html><html lang="fr"><head><meta charset="utf-8"><title>${escapeHTML(dashboard.name)}</title></head><body style="font-family:system-ui;background:#f8fafc;padding:24px;margin:0">
                <h1 style="font-size:20px;color:#0f172a">📋 ${escapeHTML(dashboard.name)}</h1>
            ...
            anchorElement.download = `${dashboard.name.replace(/[^a-zA-Z0-9]/g, '_')}_${Date.now()}.html`;
            document.body.appendChild(anchorElement);
            anchorElement.click();
            anchorElement.remove();
        }
        function renderDashboards() {
            const dbContentElement = el('dbContent');
            if (!dbContentElement) return;
            const tables = Object.values(state.tables).filter(t => t.status === 'ready');
            const d = dbState.openId ? dbById(dbState.openId) : null;
            if (!d) {
                dbContentElement.innerHTML = `<div class="flex items-center gap-2 mb-4 flex-wrap"><button onclick="dbAdd()" class="bg-sky-600 hover:bg-sky-700 text-white font-bold py-2 px-4 rounded-lg text-sm">+ Tableau de bord</button>
                    ...
                    ${
                        dbList().length
                            ? `<div class="grid grid-cols-1 md:grid-cols-3 gap-3">${dbList()
                                  .map(
                                      d2 => `<div class="border border-slate-200 rounded-xl p-4 bg-slate-50/50 hover:border-sky-300 cursor-pointer" onclick="dbOpen('${d2.id}')">
                        ...
                        <div class="flex gap-2 mt-2"><button onclick="event.stopPropagation(); dbDup('${d2.id}')" class="text-[10px] bg-white border border-slate-300 px-2 py-0.5 rounded font-bold text-slate-500">⧉ Dupliquer</button><button onclick="event.stopPropagation(); dbDel('${d2.id}')" class="text-[10px] text-red-400 hover:text-red-600 font-bold">✕ Supprimer</button></div>
                            ...
                            : emptyStateHtml(
                                  ...
                                  "Composez des tuiles (indicateurs, barres, camemberts) sur vos tables, ajoutez des seuils d'alerte, puis exportez un livrable HTML autonome à partager.",
            ...
            <div class="border border-slate-200 rounded-lg p-2.5 bg-slate-50/50 mb-4">
                ...
                ${
                    (d.filters || [])
                        .map(
                            (f, i) => `<div class="flex items-center gap-1.5 mb-1">
                    ...
                    <button onclick="dbDelFilter('${d.id}',${i})" class="text-slate-300 hover:text-red-500 font-bold">✕</button>
                        ...
                        .join('') || '<p class="text-[10px] text-slate-300 italic">Aucun filtre global.</p>'
                }
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">${d.tiles
                .map(t => {
                    const table = tableByName(t.table);
                    const hs = table ? table.headers : [];
                    const colSel = (f, cur, ph) =>
                        `<select onchange="dbSetTile('${d.id}','${t.id}','${f}',this.value)" class="border border-slate-200 rounded px-1 py-0.5 text-[10px] bg-white max-w-[110px]"><option value="">${ph}</option>${hs.map(h => `<option value="${escapeHTML(h)}" ${cur === h ? 'selected' : ''}>${escapeHTML(h)}</option>`).join('')}</select>`;
                ...
                <div class="flex items-center gap-1.5 px-3 py-1.5 bg-slate-50/60 flex-wrap text-[10px]">
                    ...
                    <select onchange="dbSetTile('${d.id}','${t.id}','kind',this.value)" class="border border-slate-200 rounded px-1 py-0.5 bg-white">${Object.entries(
                        DB_KINDS
                    )
                        .map(([k, l]) => `<option value="${k}" ${t.kind === k ? 'selected' : ''}>${l}</option>`)
                        .join('')}</select>
                    ${t.kind !== 'kpi' ? `<span class="text-slate-400">axe</span>${colSel('dim', t.dim, '— axe —')}` : ''}
                    <select onchange="dbSetTile('${d.id}','${t.id}','agg',this.value)" class="border border-slate-200 rounded px-1 py-0.5 bg-white">${Object.entries(
                        DB_AGGS
                    )
                        .map(([k, l]) => `<option value="${k}" ${t.agg === k ? 'selected' : ''}>${l}</option>`)
                        .join('')}</select>
                    ...
                    ${
                        t.kind === 'kpi'
                            ? `<select onchange="dbSetTile('${d.id}','${t.id}','thDir',this.value)" class="border border-slate-200 rounded px-1 py-0.5 bg-white" title="Sens de l'alerte"><option value="max" ${(t.thDir || 'max') === 'max' ? 'selected' : ''}>alerte si ≥</option><option value="min" ${t.thDir === 'min' ? 'selected' : ''}>alerte si ≤</option></select>
            ...
            dbContentElement.innerHTML = html;
            dbRunAll(d.id);
        }
