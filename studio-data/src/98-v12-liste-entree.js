        ...
        ['advCount', 'advPreview', 'advQuality', 'advGenerate'].forEach(nm => { const o = window[nm]; if (typeof o !== 'function') return; window[nm] = async function () { if (!(await v12ListEnsure())) return; return o.apply(this, arguments); }; });
        const _v12lRemove = advRemoveFilter;
        advRemoveFilter = function (id) { const f = (state.advExtract.filters || []).find(x => x.id === id); if (f && f.op === 'list') { (async () => { try { const { conn } = await getDB(); await conn.query(`DROP TABLE IF EXISTS ${v12ListTable(f)}`); } catch (e) {} })(); if (v12State.listReady) delete v12State.listReady[id]; } return _v12lRemove(id); };
        ...
        function v12ListReshape() {
            ...
            const targets = v12ListTargets(); const s = state.advExtract;
        ...
        const _v12lLayout = v12ExtractLayout;
        v12ExtractLayout = function () { const r = _v12lLayout.apply(this, arguments); try { v12ListAfterLayout(); } catch (e) { console.warn('v12 liste', e); } return r; };
