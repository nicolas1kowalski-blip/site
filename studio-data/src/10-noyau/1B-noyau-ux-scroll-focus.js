        ...
        (function () {
            ...
            const detKey = d => {
                if (d.id) return '#' + d.id;
                const element = d.querySelector('summary');
                return element ? element.textContent.replace(/\d+/g, '').replace(/\s+/g, ' ').trim() : '';
            };
            const wrap = fn =>
                function (...a) {
                    ...
                    const dets = [];
                    let dord = 0;
                    document.querySelectorAll(DET_SEL).forEach(d => {
                        const o2 = dord++;
                        if (d.open) dets.push({ key: detKey(d), ord: o2 });
                    });
                    const scrolled = [];
                    let ord = 0;
                    document.querySelectorAll('main *, .drawer-ux *').forEach(e2 => {
                        ...
                        if (!sc) return;
                        const o2 = ord++;
                        if (e2.scrollTop || e2.scrollLeft)
                            scrolled.push({ id: e2.id || null, ord: o2, t: e2.scrollTop, l: e2.scrollLeft });
                    });
                    const ae = document.activeElement;
                    const focusId =
                        ae && ae.id && (ae.tagName === 'INPUT' || ae.tagName === 'SELECT' || ae.tagName === 'TEXTAREA')
                            ? ae.id
                            : null;
                    ...
                    const restore = () => {
                        ...
                        if (scrolled.length) {
                            ...
                            scrolled.forEach(s2 => {
                                const e2 = (s2.id && el(s2.id)) || cand[s2.ord];
                                if (e2) {
                                    e2.scrollTop = s2.t;
                                    e2.scrollLeft = s2.l;
                                }
                        ...
                        if (focusId) {
                            const element = el(focusId);
                            if (element && document.activeElement !== element && element.focus) {
                                try {
                                    element.focus({ preventScroll: true });
                                    if (selStart != null && element.setSelectionRange)
                                        element.setSelectionRange(selStart, selStart);
                                } catch (er) {}
                            }
                        }
                    };
            ...
            Studio.extendAll(
                KEEP.filter(nm => typeof window[nm] === 'function' && !window[nm].__keepScroll),
                base => {
                    const w = wrap(base);
                    w.__keepScroll = true;
                    return w;
                },
                { motif: 'conserver le défilement et le focus pendant un rendu' }
            );
        })();
