        ...
        function guideRail(phase, idx) {
            const element = document.querySelector(`.v7-grp[data-phase="${phase}"]`);
            if (element) element.classList.add('open');
            const its = element ? element.querySelectorAll('.v7-it') : [];
            return its[idx] || null;
        }
        const GUIDE_FLOWS = {
            extract: {
                ...
                steps: [
                    ...
                    {
                        target: () => {
                            const advColTblElement = el('adv-col-tbl');
                            return advColTblElement ? advColTblElement.closest('div') : null;
                        },
                    ...
                    {
                        target: () => {
                            const advFltTblElement = el('adv-flt-tbl');
                            return advFltTblElement ? advFltTblElement.closest('div') : null;
                        },
                    ...
                    },
                    {
                        ...
                        until: () => {
                            const advPreviewElement = el('adv-preview');
                            return !!(advPreviewElement && advPreviewElement.children.length);
                        }
                    },
                    {
            ...
            quality: {
                ...
                steps: [
                    ...
                    },
                    {
                        ...
                        until: d => {
                            if (qrRules().length > d.n0) {
                                d.newId = qrRules()[qrRules().length - 1].id;
                                return true;
                            }
                            return false;
                        }
                    ...
                    {
                        target: d => {
                            const element = el('qrContent') && el('qrContent').querySelector(`[onchange*="${d.newId}"]`);
                        ...
                        until: d => {
                            const rules = qrRules().find(x => x.id === d.newId);
                            return !!(rules && (rules.table || rules.bo));
                        }
                    },
                    {
                        target: d => {
        ...
        function guideStart(kind) {
            const wizOverlayElement = el('wizOverlay');
            if (wizOverlayElement) wizOverlayElement.classList.add('hidden');
            guideStop();
        ...
        function guideStop(msg) {
            ...
            guideState = null;
            ['gdB0', 'gdB1', 'gdB2', 'gdB3', 'gdRing', 'gdTip'].forEach(id => {
                const element = el(id);
                if (element) element.remove();
            });
            if (msg) showSuccess(msg);
        }
        function guideFlowSteps() {
            return GUIDE_FLOWS[guideState.kind].steps;
        }
        function guideNext() {
            ...
            if (i >= steps.length) {
                const m = GUIDE_FLOWS[guideState.kind].done;
                return guideStop(m);
            }
            guideState.idx = i;
            try {
                if (steps[i].pre) steps[i].pre(guideState.data);
            } catch (e) {}
            guidePlace(true);
        }
        function guideTick() {
        ...
        function guidePlace(fresh) {
            if (!guideState) return;
            const steps = guideFlowSteps(),
                st = steps[guideState.idx];
            let element = null;
            try {
                element = st.target ? st.target(guideState.data) : null;
            } catch (e) {}
            ...
            if (r && r.width > 0) {
                const maximum = Math.max(0, r.left - PAD),
                    y0 = Math.max(0, r.top - PAD),
                    minimum = Math.min(W, r.right + PAD),
                    y1 = Math.min(H, r.bottom + PAD);
                set(b[0], 0, 0, W, y0); // dessus
                set(b[1], 0, y1, W, H - y1); // dessous
                set(b[2], 0, y0, maximum, y1 - y0); // gauche
                set(b[3], minimum, y0, W - minimum, y1 - y0); // droite
                set(ring, maximum, y0, minimum - maximum, y1 - y0);
                // bulle : sous la cible si la place y est, sinon au-dessus
                tip.style.display = 'block';
                const tw = Math.min(380, W - 24);
                tip.style.width = tw + 'px';
                tip.style.left = Math.max(12, Math.min(W - tw - 12, maximum)) + 'px';
                ...
                set(b[0], 0, 0, W, H);
                [b[1], b[2], b[3], ring].forEach(n => (n.style.display = 'none'));
                tip.style.display = 'block';
                const minimum = Math.min(380, W - 24);
                tip.style.width = minimum + 'px';
                if (fresh || tip.dataset.step !== String(guideState.idx)) guideTipFill(tip, st, steps);
                tip.style.left = (W - minimum) / 2 + 'px';
        ...
        function guideTipFill(tip, st, steps) {
            ...
            tip.innerHTML = `
                ...
                <div class="gd-tip-foot">
                    ${
                        st.next
                            ? `<button onclick="guideNext()" class="gd-btn-main">${st.nextLabel || 'Étape suivante →'}</button>`
        ...
        function wizOpen(kind) {
            ...
            const wizOverlayElement = el('wizOverlay');
            if (!wizOverlayElement) return;
            wizOverlayElement.classList.remove('hidden');
            el('wizTitle').textContent = '🧭 Par où commencer ?';
            el('wizSub').textContent =
                'Deux guides pas à pas, sur les écrans réels : le guide surligne où cliquer, le reste devient non cliquable, et il avance quand l’action est faite.';
            ...
            el('wizFoot').innerHTML =
                `<button onclick="wizClose()" class="ml-auto text-sm bg-white border border-slate-300 px-3 py-2 rounded-lg font-bold">Fermer</button>`;
            try {
                v7WatchIcons();
            } catch (e) {}
        }
