        ...
        function cellMeter(pct) {
            const maximum = Math.max(0, Math.min(100, pct));
            const c = meterColor(maximum);
            return `<div class="meter"><div class="bar"><i style="width:${Math.max(maximum > 0 ? 1 : 0, Math.round(maximum))}%;background:${c}"></i></div><span class="pct" style="color:${c};width:52px">${fmtPctUx(maximum)}</span></div>`;
        }
        ...
        function uxTypeOf(sem) {
            ...
            if (text.includes('num')) return ['t-num', '123', 'Nombre'];
            if (text.includes('date')) return ['t-date', 'CAL', 'Date'];
            if (text.includes('mail')) return ['t-mail', '@', 'E-mail'];
            if (text.includes('phone') || text.includes('tél') || text.includes('tel')) return ['t-tel', 'TEL', 'Téléphone'];
            if (text.includes('ident') || text.includes('clé') || text.includes('key') || text.includes('code'))
                return ['t-key', 'KEY', 'Clé'];
            return ['t-txt', 'ABC', 'Texte'];
        }
        function typeIconUx(sem) {
            const [cls, lab] = uxTypeOf(sem);
            return `<span class="typ ${cls}" aria-hidden="true">${lab}</span>`;
        }
        ...
        function colDefFor(tableName, col) {
            ...
            const term = (governance.glossary || []).find(t2 =>
                (t2.links || []).some(l => l.table === tableName && l.col === col)
            );
            if (term && term.definition) return { def: term.definition, src: 'Glossaire : ' + term.term };
            for (const bo of governance.businessObjects || []) {
                for (const attribute of bo.elements || []) {
                    if ((attribute.mappings || []).some(m => m.table === tableName && m.col === col))
                        return {
                            def:
                                attribute.definition ||
                                'Attribut « ' + attribute.name + " » de l'objet métier « " + bo.name + ' ».',
                            src: 'Objet métier : ' + bo.name
                        };
                }
            }
            return null;
        ...
        function renderContextBar() {
            ...
            const CTX_SEL = {
                ...
                4: 'vizBaseTable',
                5: 'expBaseTable',
                6: 'browserTableSelect',
                7: 'compTableA',
                8: 'qualTable'
            };
            const selId = CTX_SEL[currentTab];
            const sel2 = selId ? el(selId) : null;
            if (sel2 && sel2.value) table = state.tables[sel2.value] || tableByName(sel2.value);
            if (!table && currentTab === 8 && currentProfilingTableId && state.tables[currentProfilingTableId])
                table = state.tables[currentProfilingTableId];
            const ready = Object.values(state.tables).filter(x => x.status === 'ready');
            el('ctxSource').textContent = table
                ? table.name
                : ready.length
                  ? ready.length + ' source(s) chargée(s)'
                  : 'aucune source';
            el('ctxCols').textContent = table ? String((table.headers || []).length) : '—';
            el('ctxDomain').textContent = table && String(table.theme || '').trim() ? table.theme.trim() : '—';
            if (table && table.status === 'ready') {
                const cached = _ctxRowsCache[table.id];
                el('ctxRows').textContent = typeof cached === 'number' ? cached.toLocaleString('fr-FR') : '…';
                if (cached === undefined) {
                    _ctxRowsCache[table.id] = null;
                    (async () => {
                        try {
                            ...
                            _ctxRowsCache[table.id] = Number(rows[0].n);
                            renderContextBar();
                        } catch (e) {
                            delete _ctxRowsCache[table.id];
                            el('ctxRows').textContent = '—';
                        }
                    })();
                }
            } else el('ctxRows').textContent = '—';
            const ph = phaseOfTab(currentTab);
            el('ctxCrumb').innerHTML = NAV_PHASES.map((p2, i) =>
                p2.id === ph.id
                    ? `<b>${escapeHTML(p2.badge + ' ' + p2.label)}</b>`
                    : `<span>${escapeHTML(p2.badge + ' ' + p2.label)}</span>`
            ).join(' <span aria-hidden="true">›</span> ');
        }
        document.addEventListener('change', e => {
            const id = e.target && e.target.id;
            if (
                ['qualTable', 'baseTableSelect', 'vizBaseTable', 'expBaseTable', 'browserTableSelect', 'compTableA'].includes(
                    id
                )
            )
                renderContextBar();
        ...
        function openUxDrawer(o, row) {
            ...
            el('uxDrawerTyp').className = 'typ ' + cls;
            el('uxDrawerTyp').textContent = lab;
            ...
            el('uxDrawer').classList.add('on');
            el('uxScrim').classList.add('on');
        }
        function closeUxDrawer() {
            el('uxDrawer').classList.remove('on', 'wide');
            el('uxScrim').classList.remove('on');
            document.querySelectorAll('tr.sel').forEach(r => r.classList.remove('sel'));
        }
        document.addEventListener('keydown', e => {
            if (e.key === 'Escape') closeUxDrawer();
        });
        function uxSetDensity(compact) {
            const uxProfilTableElement = el('uxProfilTable');
            if (!uxProfilTableElement) return;
            uxProfilTableElement.classList.toggle('dense', !!compact);
        ...
        function uxColQuality(c) {
            ...
            if (pct > 0 && pct < 0.05)
                return ['bad', 'quasi vide (' + Number(c.distinctCount || 0).toLocaleString('fr-FR') + ' valeur(s))'];
        ...
        function openColumnDrawer(cName, row) {
            ...
            const table = state.tables[currentProfilingTableId];
            const tName = table ? table.name : currentProfilingStats.tableName || '';
            ...
            const anoms = [];
            if (c.completeness < 100)
                anoms.push(
                    `<div class="anom-ux warn"><div class="ic">◑</div><div><div class="t">${(100 - c.completeness).toFixed(1)} % de valeurs manquantes</div><div class="d">${Number(c.nullCount || 0).toLocaleString('fr-FR')} ligne(s) vide(s)</div>
                        </div>
                        </div>`
                );
            if (c.placeholders > 0)
                anoms.push(
                    `<div class="anom-ux warn"><div class="ic">⚠️</div><div><div class="t">${Number(c.placeholders).toLocaleString('fr-FR')} valeur(s) « bouche-trou »</div>
                        <div class="d">N/A, -, ?, INCONNU…</div>
                        </div>
                        </div>`
                );
            if (c.caseDupGroups > 0)
                anoms.push(
                    `<div class="anom-ux warn"><div class="ic">Aa</div><div><div class="t">${Number(c.caseDupGroups).toLocaleString('fr-FR')} doublon(s) de casse</div>
                        <div class="d">PARIS / Paris comptés comme différents</div>
                        </div>
                        </div>`
                );
            if (c.multiSpace > 0)
                anoms.push(
                    `<div class="anom-ux warn"><div class="ic">␣</div><div><div class="t">${Number(c.multiSpace).toLocaleString('fr-FR')} valeur(s) avec espaces parasites</div>
                        <div class="d">Espaces multiples ou en bord de valeur</div>
                        </div>
                        </div>`
                );
            if (c.outliers > 0)
                anoms.push(
                    `<div class="anom-ux bad"><div class="ic">✕</div><div><div class="t">${Number(c.outliers).toLocaleString('fr-FR')} valeur(s) aberrante(s)</div>
                        <div class="d">À plus de 3 écarts-types de la moyenne</div>
                        </div>
                        </div>`
                );
            const maxF = c.frequentValues && c.frequentValues[0] ? c.frequentValues[0].count : 1;
            const tops = (c.frequentValues || [])
                .slice(0, 5)
                .map(v => {
                    const text = String(v.val !== undefined ? v.val : v.value);
                    return `<div class="row"><div class="lbl" title="${escapeHTML(text)}">${escapeHTML(text)}</div><div class="track"><i style="width:${Math.max(4, Math.round((100 * v.count) / maxF))}%"></i></div>
                        <div class="n">${Number(v.count).toLocaleString('fr-FR')}</div>
                        </div>`;
                })
                .join('');
            const rules = (typeof qrRules === 'function' ? qrRules() : []).filter(
                r =>
                    r.table === tName &&
                    (r.col === cName ||
                        String((r.p || {}).cols || '')
            ...
            const rulesHtml = rules.length
                ? rules
                      .map(r => {
                          const okR = r.last ? (r.last.fails === 0 ? 'ok' : 'ko') : 'wa';
                          const ic = okR === 'ok' ? '✓' : okR === 'ko' ? '✕' : '—';
                          return `<div class="rule-ux"><span class="st ${okR}">${ic}</span> ${escapeHTML(r.name)} <span class="ml-auto text-slate-400">${r.last ? (r.last.fails === 0 ? 'conforme' : r.last.fails.toLocaleString('fr-FR') + ' échec(s)') : 'jamais exécutée'}</span></div>`;
                      })
                      .join('')
            ...
            const body = `
                <div class="dsect"><div class="kv-ux">
                    <div class="cell"><div class="k">Complétude</div><div class="v" style="color:${meterColor(c.completeness)}">${fmtPctUx(c.completeness)}</div>
                        </div>
                    <div class="cell"><div class="k">Valeurs distinctes</div>
                        <div class="v">${Number(c.distinctCount || 0).toLocaleString('fr-FR')}${c.hasMoreDistinct ? '+' : ''}</div>
                        </div>
                    <div class="cell"><div class="k">État qualité</div>
                        <div class="v" style="font-size:12.5px">${cellQuality(qs, ql)}</div></div>
                    <div class="cell"><div class="k">Sensibilité</div>
                        <div class="v" style="font-size:12.5px;color:${lvl === 'personnel' ? 'var(--bad)' : 'var(--muted)'}">${escapeHTML(lvl ? lvlLab : '—')}</div>
                        </div>
                </div>
                            </div>
                <div class="dsect"><h4>Définition métier</h4>
                    ${
                        d
                            ? `<div class="text-[12px] bg-slate-50 border border-slate-200 rounded-lg p-3">${escapeHTML(d.def)}<div class="text-[10px] text-slate-400 mt-1.5">${escapeHTML(d.src)}</div>
                                </div>`
            ...
            openUxDrawer(
                {
                    sem: c.semanticType,
                    title: cName,
                    sub: `${uxTypeOf(c.semanticType)[2]} · table « ${tName} »${dict.owner ? ' · propriétaire : ' + dict.owner : ''}`,
                    body,
                    foot
                },
                row
        ...
        function uxGoDictionary() {
            if (_uxDrawerCtx) {
                govState.dictTable = _uxDrawerCtx.tName;
                closeUxDrawer();
        ...
        function switchTab(num) {
            for (let i = 1; i <= 16; i++) {
                const element = el(`step-${i}`);
                if (element) element.classList.add('hidden');
            }
            const stepEl = el(`step-${num}`);
            ...
            if (num === 3) {
                ...
                advMigrateClassic();
                renderAdvExtract();
                comboifyTableSelect('baseTableSelect');
            }
            if (num === 4) {
                updateVizBaseTableSelect();
                covPopulate();
                ['covBase', 'covDimTable', 'covRel'].forEach(comboifyTableSelect);
            }
            if (num === 5) {
                populateExpTableSelect();
                ['expBaseTable', 'expConfigTable'].forEach(comboifyTableSelect);
            }
            if (num === 6) {
                populateBrowserTableSelect();
                comboifyTableSelect('browserTableSelect');
            }
            if (num === 7) {
                populateCompareTables();
                ['compTableA', 'compTableB'].forEach(comboifyTableSelect);
            }
            if (num === 7) {
                renderCompKeys();
                renderCompMappings();
