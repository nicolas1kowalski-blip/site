        ...
        function isTransientLockError(e) {
            return /unique_lock|not locked|Operation not permitted|being used by another|conflict|InvalidInputException.*lock/i.test(
        ...
        async function getDB() {
            ...
            if (!result || result.error) {
                throw new Error(
                    'Moteur de données DuckDB indisponible : ' +
                        (result?.error?.message || 'échec de chargement (vérifiez la connexion internet).')
                );
            }
            ...
            if (result.conn && !result.conn.__serialized) {
                ...
                result.conn.query = sql => {
                    const r = __dbQueue.then(() => raw(sql));
                    __dbQueue = r.then(
                        () => {},
                        () => {}
                    );
        ...
        function isSpillWriteError(e) {
            return /do not support writing|temp_directory|temporary directory|temporary file|failed to write|No space left/i.test(
                String((e && e.message) || e)
            );
        }
        ...
        function arrowResultToObjects(res) {
            ...
            for (const row of res) {
                ...
                fields.forEach(f => {
                    ...
                    obj[f] = value === undefined ? null : value;
                });
                out.push(obj);
            }
            return out;
        ...
        async function duckDropTable(tId) {
            ...
            try {
                ...
                try {
                    await conn.query(`DROP TABLE IF EXISTS ${sqlIdent(duckTableName(tId))}`);
                } catch (e) {}
                try {
                    await conn.query(`DROP VIEW IF EXISTS ${sqlIdent(duckTableName(tId))}`);
                } catch (e) {}
            } catch (e) {
                /* moteur jamais démarré */
            }
        }
        ...
        function sniffEncodingFromBytes(buf) {
            ...
            if (b.length >= 3 && b[0] === 0xef && b[1] === 0xbb && b[2] === 0xbf)
                return { enc: 'UTF-8', why: "marque d'ordre UTF-8" };
            if (b.length >= 2 && ((b[0] === 0xff && b[1] === 0xfe) || (b[0] === 0xfe && b[1] === 0xff)))
                return { enc: 'UTF-16', why: "marque d'ordre UTF-16" };
            // UTF-16 sans marque : un octet sur deux est nul sur un texte latin.
            let nul = 0;
            const minimum = Math.min(b.length, 4096);
            for (let i = 0; i < minimum; i++) if (b[i] === 0) nul++;
            if (minimum > 32 && nul / minimum > 0.2)
                return { enc: 'UTF-16', why: "un octet sur deux est nul (UTF-16 sans marque d'ordre)" };
            // Validation UTF-8 stricte : c'est elle qui distingue l'UTF-8 du Windows-1252.
            try {
                new TextDecoder('utf-8', { fatal: true }).decode(b);
                return { enc: 'UTF-8', why: 'octets valides en UTF-8' };
            } catch (e) {
                return { enc: 'ISO-8859-1', why: 'octets invalides en UTF-8 — fichier Windows/ANSI' };
            }
        }
        ...
        function ingestLooksWrong(headers, sample) {
            ...
            if (hs.length === 1 && /[\uFFFE\uFEFF]|ÿþ|þÿ/.test(hs[0]))
                return "une seule colonne portant une marque d'ordre (encodage incorrect)";
        ...
        function csvReadClauses(config) {
            config = config || {};
            let text = '';
            if (config.delim) text += `, delim=${sqlLiteral(config.delim)}`;
            if (config.enc === 'ISO-8859-1') text += `, encoding='latin-1'`;
            else if (config.enc === 'UTF-16') text += `, encoding='utf-16'`;
            if (config.quote === 'none') text += `, quote=''`;
            if (config.ignoreErrors) text += `, ignore_errors=true`;
            return text;
        }
        ...
        function csvViewThreshold() {
            return window.__duckdbOpfs ? CSV_VIEW_THRESHOLD_DISK : CSV_VIEW_THRESHOLD_MEM;
        }
        ...
        async function ingestCsvIntoDuckDB(tId) {
            ...
            const create = async (kind, withFrn) => {
                await duckDropTable(tId);
                await conn.query(`CREATE ${kind} ${sqlIdent(tName)} AS ${readExpr(withFrn)}`);
            };
            try {
                if (asView) {
                    await create('VIEW', true);
                    table.storage = 'view';
                } else {
                    await create('TABLE', true);
                    table.storage = 'table';
                }
            } catch (e) {
        ...
        async function ingestRowsIntoDuckDB(tId, rows, headers) {
            ...
            if (!cols.length) {
                const seen = new Set();
                rows.forEach(r => {
                    if (r && typeof r === 'object') Object.keys(r).forEach(k => seen.add(cleanHeader(k)));
                });
            ...
            if (got.length === 1 && got[0] === 'json')
                throw new Error(
                    "Structure de source non reconnue : impossible d'en déduire des colonnes. Vérifiez que le fichier contient bien un tableau de lignes."
                );
            return got;
        }
        ...
        function xlsxSheetNote(pick, fileName) {
            ...
            return (
                `« ${fileName} » contient ${pick.others.length + 1} feuilles. La feuille « ${pick.name} » a été retenue (la plus fournie).` +
        ...
        async function ingestFileTable(tId) {
            ...
            if (Array.isArray(table.files) && table.files.length) {
                ...
                if (table.files.every(isXlsx)) {
                    ...
                    for (const file of table.files) {
                        ...
                        if (!pk.name) throw new Error(`« ${file.name} » ne contient aucune feuille.`);
                        const nt = xlsxSheetNote(pk, file.name);
                        if (nt) table.xlsxNote = ((table.xlsxNote || '') + ' ' + nt).trim();
                        const raw = XLSX.utils.sheet_to_json(wb.Sheets[pk.name], { defval: '' });
                        raw.forEach(rr => {
                            const c = {};
                            Object.keys(rr).forEach(k => {
                                const ch = cleanHeader(k);
                                headersSet.add(ch);
                                c[ch] = rr[k];
                            });
                            rows.push(c);
                ...
                throw new Error(
                    'Fusion impossible : gardez le même format pour tous les fichiers (tous CSV/TXT, ou tous Excel).'
                );
            }
            if (table.type === 'csv' || table.type === 'txt') {
                ...
                table.xlsxSheet = pick.name;
                table.xlsxNote = xlsxSheetNote(pick, table.name);
                if (rawJson.length === 0)
                    throw new Error(
                        `La feuille « ${pick.name} » ne contient aucune ligne.` +
                ...
                if (!rows.length)
                    throw new Error(
                        "Aucune ligne trouvée dans ce JSON (attendu : un tableau d'objets, éventuellement imbriqué sous une clé)."
                    );
                ...
                const flat = rows.map(r => {
                    ...
                    if (r && typeof r === 'object' && !Array.isArray(r))
                        Object.keys(r).forEach(k => {
                            ...
                            headSet.add(ch);
                            const value = r[k];
                            o[ch] = value !== null && typeof value === 'object' ? JSON.stringify(value) : value;
                        });
                    return o;
                });
        ...
        function isCsvScanError(e) {
            return /CSV Error|unterminated quote|Value with unterminated|read_csv/i.test(String((e && e.message) || e));
        }
        // Erreur typique d'un fichier lu dans le mauvais encodage (octets non décodables en UTF-8).
        function isEncodingError(e) {
            return /invalid unicode|byte sequence mismatch|not valid utf-?8|invalid utf-?8|codepoint|encoding/i.test(
                String((e && e.message) || e)
            );
        }
        async function autoFixCsvRead(tId, err) {
            const table = state.tables[tId];
            if (!table || (!isCsvScanError(err) && !isEncodingError(err))) return false;
            const isCsvSource =
                table.type === 'csv' ||
                table.type === 'txt' ||
                (Array.isArray(table.files) && table.files.length && table.files.every(f => /\.(csv|txt)$/i.test(f.name)));
            ...
            if (isEncodingError(err)) {
                ...
                let first = null;
                try {
                    const sn = await sniffFileEncoding(table.file);
                    if (sn) first = sn.enc;
                } catch (e0) {}
                const encOrder = first ? [first, 'ISO-8859-1', 'UTF-16', 'UTF-8'] : ['ISO-8859-1', 'UTF-16', 'UTF-8'];
                const seen = new Set();
                encOrder.forEach(en => {
                    if (seen.has(en) || en === table.config.enc) return;
                    seen.add(en);
                    attempts.push({
                        patch: { enc: en },
                        label:
                            en === 'ISO-8859-1'
                                ? 'encodage Windows/ANSI (Latin-1)'
                                : en === 'UTF-16'
                                  ? 'encodage UTF-16'
                                  : 'encodage UTF-8'
                    });
            ...
            if (isCsvScanError(err)) {
                if (table.config.quote !== 'none')
                    attempts.push({ patch: { quote: 'none' }, label: 'guillemets traités comme du texte brut (aucune perte)' });
                if (!table.config.ignoreErrors)
                    attempts.push({ patch: { ignoreErrors: true }, label: 'lignes malformées ignorées' });
                if (table.config.quote !== 'none' && !table.config.ignoreErrors)
                    attempts.push({
                        patch: { quote: 'none', ignoreErrors: true },
                        label: 'guillemets bruts + lignes malformées ignorées'
                    });
            }
            const { conn } = await getDB();
            for (const a of attempts) {
                const prev = { ...table.config };
                Object.assign(table.config, a.patch);
                try {
                    ...
                    const cnt = Number(
                        arrowResultToObjects(
                            await conn.query(`SELECT COUNT(*)::BIGINT AS n FROM ${sqlIdent(duckTableName(tId))}`)
                        )[0].n
                    );
                    let smp = [];
                    try {
                        smp = await duckSampleRows(tId, 6);
                    } catch (e2) {}
                    ...
                    showSuccess(
                        `⚙️ Lecture de « ${escapeHTML(table.name)} » réparée automatiquement : ${a.label}.` +
                            (perte
                                ? ` ${cnt.toLocaleString('fr-FR')} ligne(s) conservée(s) — des lignes ont pu être écartées, vérifiez le total.`
                                : '')
                    );
                    return true;
                } catch (e2) {
        ...
        async function duckStreamRows(tId, limit, onRow, onProgress) {
            ...
            if (typeof conn.send === 'function') {
                try {
                    ...
                    console.warn('Curseur natif indisponible, repli par bornes :', e);
                }
            }
