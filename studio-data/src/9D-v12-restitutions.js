        ...
        function v12RepCardHtml(a) {
            const gens = (state.governance.assets || []).filter(x => x.kind !== 'report'); const dest = gens;
        ...
        const _v12repAssets = renderGovAssets;
        renderGovAssets = function () {
        ...
        const _v12repBo = buildBoLineageGraph;
        buildBoLineageGraph = function () { const gr = _v12repBo.apply(this, arguments); try { v12RepExtend(gr); } catch (e) {} return gr; };
        const _v12repAttr = buildAttrLineageGraph;
        buildAttrLineageGraph = function () { const gr = _v12repAttr.apply(this, arguments); try { v12RepExtend(gr); } catch (e) {} return gr; };
        ...
        if (typeof v12OrgLineageEdges === 'function') { const _v12repOrg = v12OrgLineageEdges; v12OrgLineageEdges = function (d) { _v12repOrg.apply(this, arguments); try { v12RepMapEdges(d); } catch (e) {} }; }
        // ---- lineage repliable ----
        ...
        const _v12repOpenBo = openBoLineage;
        ...
        const _v12repOpenAttr = openAttrLineage;
        openAttrLineage = function (boId, stId, elId) {
        ...
        if (typeof v11AssetRead === 'function') { const _v12repRead = v11AssetRead; v11AssetRead = function (a) { let h = _v12repRead.apply(this, arguments); if (a && a.kind === 'report') { h = h.replace('🖥', '📊').replace('>Application<', '>Restitution<'); const gens = (a.producedBy || []).map(id => assetById(id)).filter(Boolean).map(x => escapeHTML(assetLabel(x))).join(', '); const dst = (a.deliveredTo || []).map(id => assetById(id)).filter(Boolean).map(x => escapeHTML(assetLabel(x))).join(', '); h += `<div class="v12rep-read"><b>📊 Restitution</b> · générée par : ${gens || '<i>non renseigné</i>'} · diffusée à : ${[dst, escapeHTML(a.recipients || '')].filter(Boolean).join(', ') || '<i>non renseigné</i>'}${a.frequency ? ' · ' + escapeHTML(a.frequency.toLowerCase()) : ''}${a.format ? ' · ' + escapeHTML(a.format) : ''} · utilise ${v12RepAttrUses(a).length} attribut(s)</div>`; } return h; }; }
        if (typeof v12BoLinSynth === 'function') { const _v12repSynth = v12BoLinSynth; v12BoLinSynth = function (bo, gr) { const rows = _v12repSynth.apply(this, arguments); rows.forEach(r => { const m = /^(?:as|use):(.+)$/.exec(r.id); const a = m && assetById(m[1]); if (a && a.kind === 'report') r.kind = 'restitution'; if (/^rcp:/.test(r.id)) r.kind = 'destinataires'; }); return rows; }; }
        // catalogue : sous-titre « Restitution » au lieu de « Application »
        if (typeof catBuildIndex === 'function') { const _v12repIdx = catBuildIndex; catBuildIndex = function () { const ix = _v12repIdx.apply(this, arguments); try { (ix || []).forEach(e => { if (e.type !== 'asset') return; const a = assetById(e.id); if (a && a.kind === 'report') { e.sub = 'Restitution' + (a.frequency ? ' · ' + a.frequency.toLowerCase() : '') + (a.format ? ' · ' + a.format : ''); e.tags = (e.tags || []).map(t => t === 'application' ? 'restitution' : t); } }); } catch (e) {} return ix; }; }
        // export en masse : type « restitution » ; import : une ligne « restitution » redevient une restitution
        if (typeof giTemplateRows === 'function') { const _v12repTpl = giTemplateRows; giTemplateRows = function (target) { const out = _v12repTpl.apply(this, arguments); try { if (target === 'assets' && Array.isArray(out)) { const g = state.governance; const rows = [out[0]]; (g.assets || []).forEach(a => rows.push([a.name, a.kind === 'process' ? 'processus' : (a.kind === 'report' ? 'restitution' : 'application'), a.criticality || '', a.description || '', a.owner || '', a.domain || '', (a.appIds || []).map(id => (assetById(id) || {}).name).filter(Boolean).join(' ; ')])); if (rows.length === 1) return out; return rows; } } catch (e) {} return out; }; }
        if (typeof giApply === 'function') { const _v12repApply = giApply; giApply = function () { const r = _v12repApply.apply(this, arguments); try { if (giState && giState.target === 'assets') (giState.rows || []).forEach(row => { const k = typeof giVal === 'function' ? giVal(row, 'kind') : ''; const nm = typeof giVal === 'function' ? giVal(row, 'name') : ''; if (/restit|rapport|tableau de bord/i.test(k) && nm) { const a = (state.governance.assets || []).find(x => giNorm(x.name) === giNorm(nm)); if (a && a.kind !== 'report') { a.kind = 'report'; a.producedBy = a.producedBy || []; a.deliveredTo = a.deliveredTo || []; } } }); persistAppState(); } catch (e) {} return r; }; }
        ...
        if (typeof exportGovernanceReport === 'function') { const _v12repDossier = exportGovernanceReport; exportGovernanceReport = function () { const args = arguments; return v12WithBlobHook(() => _v12repDossier.apply(this, args), html => html.includes('</body>') ? html.replace('</body>', v12RepDossierHtml() + '</body>') : html); }; }
        // liste V11 des actifs : icône 📊 sur les lignes des restitutions
        if (typeof v11ApplyReadMode === 'function') { const _v12repRM = v11ApplyReadMode; v11ApplyReadMode = function () { const r = _v12repRM.apply(this, arguments); try { if (currentTab === 9 && govState.tab === 'assets') v12Reports().forEach(a => { const c = el('v11-as-' + a.id); const ic = c && c.classList.contains('v11-rowcard') && c.querySelector(':scope > span'); if (ic && !ic.dataset.v12rep) { ic.dataset.v12rep = '1'; ic.textContent = '📊'; } }); } catch (e) {} return r; }; }
