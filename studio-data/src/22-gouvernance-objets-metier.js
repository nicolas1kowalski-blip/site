        ...
        function renderBoDetail(bo, names) {
            ...
            let html = `<div class="border-2 border-emerald-200 rounded-xl bg-white overflow-hidden">
                <div class="bg-emerald-50/70 border-b border-emerald-100 p-4">
                    ...
                    <div class="flex items-center gap-2 flex-wrap mt-2 text-[11px] text-slate-500"><span class="font-bold">Contributeurs :</span><input type="text" value="${escapeHTML((bo.contributors || []).join(', '))}" onchange="updateBusinessObject('${bo.id}','contributors',this.value.split(',').map(x=>x.trim()).filter(Boolean))" placeholder="ex : DSI, BU Sud (virgules)" class="border border-slate-200 rounded-lg px-2 py-1 text-[11px] bg-white w-72">${govFeatureOn() ? `<span class="ml-auto text-[11px] ${govCanEditBo(bo) ? 'text-emerald-700' : (govCanProposeBo(bo) ? 'text-sky-700' : 'text-slate-500')} font-bold">${govCanEditBo(bo) ? (govIsOwnerOfBo(bo) ? '👑 vous êtes propriétaire de cet objet : vous validez' : '👑 vous validez ce domaine') : (govCanProposeBo(bo) ? '✍️ vos modifications seront des propositions' : '👁 lecture seule')}</span>` : ''}</div>
