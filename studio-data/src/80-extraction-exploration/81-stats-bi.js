        ...
        function handleVizBaseTableChange() {
            ...
            Object.values(state.tables).forEach(t => {
                opts += `<optgroup label="${escapeHTML(t.name)}">${t.headers.map(h => `<option value="${t.id}|${escapeHTML(h)}">${escapeHTML(h)}</option>`).join('')}</optgroup>`;
            });
            el('vizDim').innerHTML = opts;
            el('vizMeasure').innerHTML = opts;
        }
        function toggleVizMeasure() {
            el('vizMeasure').disabled = el('vizAgg').value === 'count';
        }

        async function generateChart() {
            const dimRaw = el('vizDim').value;
            if (!dimRaw) return showError("Sélectionnez l'Axe X.");
            hideError();
            const measureRaw = el('vizMeasure').value;
            const agg = el('vizAgg').value;
            const [dimTId, dimCol] = dimRaw.split('|');
            const [, measCol] = agg !== 'count' ? measureRaw.split('|') : [null, null];
            ...
            currentChart = new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [
                        {
                            label: `${agg} de ${measCol || 'Lignes'} par ${dimCol}`,
                            data: dataValues,
                            backgroundColor: [
                                '#4f46e5',
                                '#10b981',
