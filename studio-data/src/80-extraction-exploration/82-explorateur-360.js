        ...
        //  UI SELECTORS
        // ==========================================
        function populateBrowserTableSelect() {
            el('browserTableSelect').innerHTML = tableOptionsHtml({ placeholder: 'Table...' });
        }
        function populateCompareTables() {
            const opts = tableOptionsHtml({ placeholder: 'Sélectionner une table...' });
            el('compTableA').innerHTML = opts;
            el('compTableB').innerHTML = opts;
        }
        function populateExpTableSelect() {
            const opts = tableOptionsHtml({ placeholder: 'Sélectionnez...' });
            el('expBaseTable').innerHTML = opts;
            el('expConfigTable').innerHTML = opts;
        }

        // ==========================================
        //  STEP 5: EXPLORATEUR 360
        // ==========================================
        function populateExpColumns() {
            const element = el('expBaseTable').value;
            el('expCol').innerHTML =
                '<option value="">Colonne...</option>' +
                (state.tables[element]?.headers
                    .map(h => `<option value="${escapeHTML(h)}">${escapeHTML(h)}</option>`)
                    .join('') || '');
        }
        function renderExpConfigCols() {
            const tId = el('expConfigTable').value;
            const cont = el('expConfigCols');
            if (!tId) {
                cont.innerHTML = '';
                return;
            }
            if (!state.graphConfig) state.graphConfig = {};
            if (!state.graphConfig[tId]) state.graphConfig[tId] = [];
            cont.innerHTML = state.tables[tId].headers
                .map(h => {
        ...
        function toggleGraphConfigCol(tId, col, isChecked) {
            if (isChecked) state.graphConfig[tId].push(col);
            else state.graphConfig[tId] = state.graphConfig[tId].filter(c => c !== col);
            persistAppState();
            if (expNetInstance) {
        ...
        function buildNodeLabel(tId, row) {
            ...
            const title = table.name.length > 20 ? table.name.substring(0, 17) + '...' : table.name;
            let content = '';
            const conf = state.graphConfig && state.graphConfig[tId];
            if (conf && conf.length > 0) {
                content = conf.map(c => `${c}: ${row[c] || ''}`).join(' | ');
            } else {
                for (const h of table.headers) {
                    if (row[h] && String(row[h]).trim() !== '') {
                        content = String(row[h]).substring(0, 25);
                        break;
                    }
                }
            }
        ...
        function applyGraphFilter() {
            ...
            expNetInstance.getEdges().forEach(edge => {
                ...
                const src = expNetInstance.findById(model.source),
                    tgt = expNetInstance.findById(model.target);
                if (src && tgt && src.isVisible() && tgt.isVisible()) edge.show();
                else edge.hide();
            });
        }
        async function fetchRowsByCondition(tableId, colName, targetValue) {
            ...
            const results = [];
            const targetUpper = String(targetValue).trim().toUpperCase();
            if (!table) return results;
            await processDataStream(table, 0, row => {
                const val = row[colName];
                if (val !== undefined && val !== null && String(val).trim().toUpperCase() === targetUpper) {
                    results.push(row);
                }
            });
            return results;
        ...
        async function expandNodeRelationships(nodeId) {
            const nodeInfo = expDataStore[nodeId];
            if (!nodeInfo) return;
            const tId = nodeInfo.tableId;
            const rowData = nodeInfo.data;
            const relevantRelations = state.relations.filter(
                r => r.sourceCol && r.targetCol && (r.sourceTable === tId || r.targetTable === tId)
            );

            for (const rel of relevantRelations) {
                if (rel.sourceTable === tId) {
                    const targetTableId = rel.targetTable,
                        targetCol = rel.targetCol,
                        joinVal = rowData[rel.sourceCol],
                        edgeLabel = `${rel.sourceCol} = ${rel.targetCol}`;
                    if (joinVal !== undefined && joinVal !== null && String(joinVal).trim() !== '') {
                        ...
                        for (const cRow of connectedRows) {
                            const targetNodeId = addGraphNode(targetTableId, cRow);
                            addGraphEdge(nodeId, targetNodeId, edgeLabel);
                        }
                    }
                }
                if (rel.targetTable === tId) {
                    const targetTableId = rel.sourceTable,
                        targetCol = rel.sourceCol,
                        joinVal = rowData[rel.targetCol],
                        edgeLabel = `${rel.targetCol} = ${rel.sourceCol}`;
                    if (joinVal !== undefined && joinVal !== null && String(joinVal).trim() !== '') {
                        const connectedRows = await fetchRowsByCondition(targetTableId, targetCol, joinVal);
                        for (const cRow of connectedRows) {
                            const targetNodeId = addGraphNode(targetTableId, cRow);
                            addGraphEdge(targetNodeId, nodeId, edgeLabel);
                        }
                    }
                }
            }
        ...
        async function startExploration() {
            ...
            if (!expNetInstance) {
                        ...
                        defaultEdge: {
                            ...
                            labelCfg: {
                                ...
                                style: {
                                    fill: '#475569',
                                    fontSize: 10,
                                    background: { fill: '#fff', padding: [1, 3, 1, 3], radius: 2 }
                                }
                            }
                        }
                    }
                ...
                expNetInstance.on('node:click', e => {
                    const info = expDataStore[e.item.getModel().id];
                    if (!info) return;
                    const d = info.data;
                    el('expNodeDetails').innerHTML = Object.keys(d)
                        .map(
                            k =>
                                `<div class="p-2.5 border-b border-slate-100 bg-white mb-2 rounded shadow-sm"><span class="text-[10px] text-slate-500 uppercase block">${escapeHTML(k)}</span><span class="text-sm font-medium break-all text-slate-800">${escapeHTML(d[k]) || '(Vide)'}</span></div>`
                        )
                        .join('');
                });
            ...
            expNetInstance.changeData({ nodes: [], edges: [] });
            expDataStore = {};
            el('expGraphFilter').value = '';
            try {
                const rows = await fetchRowsByCondition(tId, col, val);
                if (rows.length) {
                    for (const row of rows) {
                        const rId = addGraphNode(tId, row);
                        await expandNodeRelationships(rId);
                    }
                } else {
                    showError('Aucun résultat.');
                }
            } catch (e) {
                showError("Erreur d'exploration");
            } finally {
                el('expLoading').classList.add('hidden');
                if (expNetInstance) {
                    expNetInstance.layout();
                    expNetInstance.fitView(20);
                }
            }
        }
