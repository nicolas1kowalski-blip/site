        ...
        function pvCfg() {
            ...
            governance.privacy.levels = governance.privacy.levels || {};
            governance.privacy.actions = governance.privacy.actions || {
                personnel: 'pseudo',
                confidentiel: 'mask',
                interne: 'none',
                public: 'none'
        ...
        function pvSuggestLevel(tn, col) {
            ...
            try {
                if ((typeof PII_HEADER_RX !== 'undefined' && PII_HEADER_RX.test(col)) || /naiss|birthd/i.test(col))
                    return 'personnel';
            } catch (e) {}
            return 'interne';
        }
        function pvLevelOf(tn, col) {
            const lv = (pvCfg().levels[tn] || {})[col];
            return lv || pvSuggestLevel(tn, col);
        }
        function pvSetLevel(tn, col, v) {
            const P = pvCfg();
            (P.levels[tn] = P.levels[tn] || {})[col] = v;
            persistAppState();
            renderGovernance();
        }
        function pvSetAction(level, v) {
            pvCfg().actions[level] = v;
            persistAppState();
            renderGovernance();
        }
        ...
        function pvAnonExpr(tn, col, action, salt) {
            ...
            const raw = `CAST(${c} AS VARCHAR)`;
            switch (action) {
                case 'mask':
                    return `CASE WHEN ${c} IS NULL OR TRIM(${raw}) = '' THEN NULL ELSE substr(TRIM(${raw}), 1, 1) || '•••' END AS ${c}`;
                case 'pseudo':
                    return `CASE WHEN ${c} IS NULL OR TRIM(${raw}) = '' THEN NULL ELSE 'P_' || substr(md5(${sqlLiteral(salt)} || UPPER(TRIM(${raw}))), 1, 12) END AS ${c}`;
                case 'generalize': {
                    if (covColIsDate(null, col)) {
                        const d = `COALESCE(TRY_CAST(${tdNormExpr('date', raw)} AS DATE), CAST(TRY_CAST(${raw} AS TIMESTAMP) AS DATE))`;
                        return `CAST(YEAR(${d}) AS VARCHAR) AS ${c}`;
                    }
        ...
        function pvPlan(tn) {
            ...
            return table.headers.map(col => {
                const level = pvLevelOf(tn, col);
                const action = acts[level] || 'none';
                return { col, level, action };
            });
        }
        async function pvExport(btn) {
            const element = el('pvTable') ? el('pvTable').value : '';
            const table = tableByName(element);
            if (!table) return showError('Choisissez une table.');
            const plan = pvPlan(element);
            const salt = Array.from(crypto.getRandomValues(new Uint8Array(16)))
                .map(x => x.toString(16).padStart(2, '0'))
                .join('');
            const sel = plan
                .filter(p2 => p2.action !== 'drop')
                .map(p2 => pvAnonExpr(element, p2.col, p2.action, salt) || sqlIdent(p2.col));
            ...
            try {
                ...
                await conn.query(
                    `CREATE TABLE ${sqlIdent(duckTableName(tmp))} AS SELECT row_number() OVER () AS __rn, ${sel.join(', ')} FROM ${sqlIdent(duckTableName(table.id))}`
                );
                const headers = await duckTableHeaders(tmp);
                const parts = [headers.map(escapeCSV).join(';') + '\n'];
                let buf = [];
                await duckStreamRows(tmp, 0, async row => {
                    buf.push(headers.map(h => escapeCSV(row[h])).join(';'));
                    if (buf.length >= 5000) {
                        parts.push(buf.join('\n') + '\n');
                        buf = [];
                    }
                ...
                anchorElement.download = `${element.replace(/[^a-zA-Z0-9]/g, '_')}_ANONYMISE_${Date.now()}.csv`;
                ...
                const rep2 = `<!DOCTYPE html><html lang="fr"><head><meta charset="utf-8"><title>Rapport d'anonymisation</title></head><body style="font-family:system-ui;padding:24px">
                    <h1 style="font-size:18px">🔐 Rapport d'anonymisation — ${escapeHTML(element)}</h1>
                    ...
                    ${plan
                        .map(
                            p2 => `<tr><td style="padding:5px;border-bottom:1px solid #e2e8f0"><code>${escapeHTML(p2.col)}</code></td><td style="padding:5px;border-bottom:1px solid #e2e8f0">${PV_LEVELS[p2.level][0]} ${PV_LEVELS[p2.level][1]}</td>
                        <td style="padding:5px;border-bottom:1px solid #e2e8f0">${PV_ACTIONS[p2.action]}</td>
                        </tr>`
                        )
                        .join('')}</table>
                        </body></html>`;
                const b2 = new Blob([rep2], { type: 'text/html' });
                const reportAnchorElement = document.createElement('a');
                reportAnchorElement.href = URL.createObjectURL(b2);
                reportAnchorElement.download = `${element.replace(/[^a-zA-Z0-9]/g, '_')}_RAPPORT_ANONYMISATION_${Date.now()}.html`;
                document.body.appendChild(reportAnchorElement);
                reportAnchorElement.click();
                reportAnchorElement.remove();
                bgTaskEnd(`🔐 Export anonymisé de « ${element} » téléchargé, accompagné de son rapport.`);
            } catch (e) {
                bgTaskEnd();
                showError('Export anonymisé impossible : ' + e.message);
            } finally {
                try {
                    await duckDropTable(tmp);
                } catch (e2) {}
        ...
        function renderGovPrivacy() {
            ...
            if (!tables.length)
                return '<p class="text-sm text-slate-400 italic py-8 text-center">Chargez des sources pour classifier leurs colonnes.</p>';
            if (!govState.pvTable || !tableByName(govState.pvTable)) govState.pvTable = tables[0].name;
            const tn = govState.pvTable;
            const acts = pvCfg().actions;
                ...
                ${Object.entries(PV_LEVELS)
                    .map(
                        ([
                            k,
                            [ic, lbl]
                        ]) => `<div class="border border-slate-200 rounded-lg p-2.5 bg-slate-50/50"><div class="text-xs font-bold mb-1">${ic} ${lbl}</div>
                    <select onchange="pvSetAction('${k}',this.value)" class="w-full border border-slate-300 rounded p-1.5 text-[11px] bg-white">${Object.entries(
                        PV_ACTIONS
                    )
                        .map(([a2, l2]) => `<option value="${a2}" ${acts[k] === a2 ? 'selected' : ''}>${l2}</option>`)
                        .join('')}</select></div>`
                    )
                    .join('')}
            </div>
            ...
            <div class="border border-slate-200 rounded-lg overflow-x-auto"><table class="w-full text-left text-xs">
                <thead class="bg-slate-50 text-[10px] uppercase font-bold text-slate-500"><tr><th class="p-2">Colonne</th>
                    <th class="p-2 w-56">Classification</th>
                    <th class="p-2">Règle qui s'appliquera à l'export</th>
                    </tr>
                    </thead>
                <tbody class="divide-y divide-slate-100">${plan
                    .map(
                        p2 => `<tr class="hover:bg-slate-50 ${p2.level === 'personnel' ? 'bg-red-50/30' : ''}">
                    <td class="p-2 font-bold font-mono">${escapeHTML(p2.col)}</td>
                    <td class="p-2"><select onchange="pvSetLevel('${escapeHTML(tn)}','${escapeHTML(p2.col.replace(/'/g, "\\'"))}',this.value)" class="border border-slate-200 rounded p-1 bg-white">${Object.entries(
                        PV_LEVELS
                    )
                        .map(
                            ([k, [ic, lbl]]) => `<option value="${k}" ${p2.level === k ? 'selected' : ''}>${ic} ${lbl}</option>`
                        )
                        .join('')}</select></td>
