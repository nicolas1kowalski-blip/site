        ...
        function svgBuildNodeModels(visibleIds, schemaMode) {
            ...
            const keyCols = {};
            state.relations
                .filter(r => r.sourceCol && r.targetCol)
                .forEach(r => {
            ...
            return Object.values(state.tables)
                ...
                .map(t => {
                    ...
                    const cols = schemaMode
                        ? t.headers.slice()
                        : [...t.headers].sort((a, b) => (pk.has(b) || ks.has(b) ? 1 : 0) - (pk.has(a) || ks.has(a) ? 1 : 0));
                    const lineOf = h =>
                        (pk.has(h) ? '🔑 ' : fkAttrs.has(h) ? '🔐 ' : ks.has(h) ? '🔗 ' : '· ') +
                        h +
                        (fmts[h] ? '  〈' + tdFormatLabel(fmts[h]) + '〉' : '');
                    ...
                    const hierTxt = hierNodeContent(hier, t.id);
                    if (hierTxt) rows.unshift('🌳 ' + hierTxt);
                    const isH = isHierTable(hier, t.id),
                        isD = t.type === 'designed';
        ...
        function svgBorderPoint(b, tx, ty) {
            ...
            const minimum = Math.min(dx ? b.w / 2 / Math.abs(dx) : Infinity, dy ? b.h / 2 / Math.abs(dy) : Infinity);
            return { x: b.cx + dx * minimum, y: b.cy + dy * minimum };
        }
        function svgApplyTransform() {
            const mcdVpElement = el('mcdVp');
            if (mcdVpElement) mcdVpElement.setAttribute('transform', `translate(${svgG.tx},${svgG.ty}) scale(${svgG.k})`);
            try {
                svgRenderMinimap();
            } catch (e) {}
        }
        function renderGraphSVG(cont) {
            ...
            if (missing.length === nodes.length) {
                computeDomainPositions(nodes);
                nodes.forEach(n => (svgG.pos[n.id] = { x: n.x, y: n.y }));
                svgG.needFit = true;
            } else if (missing.length) {
                const clone = nodes.map(n => ({ id: n.id, size: n.size }));
                computeDomainPositions(clone);
                clone.forEach(c => {
                    if (!svgG.pos[c.id]) svgG.pos[c.id] = { x: c.x, y: c.y };
                });
            }
            nodes.forEach(n => {
                const position = svgG.pos[n.id];
                n.x = position.x;
                n.y = position.y;
            });
            const boxOf = id => {
                ...
                return { cx: position.x, cy: position.y, w: s[0], h: s[1] };
            };
            const idset = new Set(nodes.map(n => n.id));
            const _hierRelIds = new Set();
            Object.values(tableHierarchyInfo()).forEach(h => h.relIds.forEach(id2 => _hierRelIds.add(id2)));
            const edges = state.relations
                .filter(r => r.sourceCol && r.targetCol && idset.has(r.sourceTable) && idset.has(r.targetTable))
                .map(r => ({
                    id: r.id,
                    source: r.sourceTable,
                    target: r.targetTable,
                    label: relEdgeLabel(r),
                    hier: _hierRelIds.has(r.id)
                }));

            // Zones par domaine (rectangles derrière les blocs).
            const byDom = {};
            nodes.forEach(n => {
                const d = n.theme || '(sans domaine)';
                (byDom[d] = byDom[d] || []).push(n);
            });
            let zonesSvg = '';
            Object.keys(byDom).forEach(d => {
                if (d === '(sans domaine)') return;
                const pal = themePalette(d);
                if (!pal) return;
                let x1 = 1e9,
                    y1 = 1e9,
                    x2 = -1e9,
                ...
                byDom[d].forEach(n => {
                    ...
                    x1 = Math.min(x1, box.cx - box.w / 2);
                    y1 = Math.min(y1, box.cy - box.h / 2);
                    x2 = Math.max(x2, box.cx + box.w / 2);
                    y2 = Math.max(y2, box.cy + box.h / 2);
                });
                const P = 22;
            ...
            let edgesSvg = '';
            edges.forEach(e => {
                const box = boxOf(e.source),
                    b = boxOf(e.target);
                const p1 = svgBorderPoint(box, b.cx, b.cy),
                    p2 = svgBorderPoint(b, box.cx, box.cy);
            ...
            nodes.forEach(n => {
                ...
                n.rows.forEach((r, i) => {
                    inner += `<text x="10" y="${headH + 12 + i * rowH}" font-size="10" fill="${String(r).startsWith('…') ? '#94a3b8' : '#475569'}">${escapeHTML(svgTrunc(r, w))}</text>`;
                });
        ...
        function svgTrunc(s, w) {
            const max = Math.max(6, Math.floor((w - 20) / 6.2));
            s = String(s);
        ...
        function svgBindEvents(cont) {
            ...
            const ptGraph = ev => {
                ...
                return { x: (ev.clientX - rect.left - svgG.tx) / svgG.k, y: (ev.clientY - rect.top - svgG.ty) / svgG.k };
            };
            // Index construit UNE fois par rendu : accès O(1) pendant les déplacements.
            const nodeEls = {};
            svg.querySelectorAll('.svgnode').forEach(g => (nodeEls[g.getAttribute('data-id')] = g));
            const edgesByNode = {};
            ...
            let rafId = 0,
                zoneIds = null,
                zoneEdges = null;
            const applyNodeXY = id => {
                const elx = nodeEls[id];
                if (!elx) return;
                const position = svgG.pos[id],
                    sz = svgG.size[id] || [180, 60];
                elx.setAttribute('transform', `translate(${position.x - sz[0] / 2},${position.y - sz[1] / 2})`);
            };
            const flushNode = () => {
                rafId = 0;
                if (!dragId) return;
                applyNodeXY(dragId);
                svgUpdateEdgesFor(dragId);
            };
            const flushZone = () => {
                rafId = 0;
                if (!zoneIds) return;
                zoneIds.forEach(applyNodeXY);
                zoneEdges.forEach(svgUpdateEdgeRec);
                if (zoneEl) zoneEl.setAttribute('transform', `translate(${zoneAcc.x},${zoneAcc.y})`);
            };
            ...
            svg.addEventListener('mousedown', ev => {
                ...
                if (ng) {
                    ...
                    mode = 'zone';
                    zoneDom = zn.getAttribute('data-dom');
                    zoneEl = zn;
                    zoneAcc = { x: 0, y: 0 };
                    startG = ptGraph(ev);
                    svgG.userMoved = true;
            ...
            window.addEventListener(
                ...
                (_svgWin.mm = ev => {
                    if (mode === 'pan') {
                        svgG.tx += ev.clientX - last.x;
                        svgG.ty += ev.clientY - last.y;
                        last = { x: ev.clientX, y: ev.clientY };
                        svgApplyTransform();
                    } else if (mode === 'node') {
                        // Les positions sont mises à jour à chaque événement, le DOM UNE fois par image.
                        const g = ptGraph(ev);
                        const position = svgG.pos[dragId];
                        if (!position) return;
                        position.x += g.x - startG.x;
                        position.y += g.y - startG.y;
                        startG = g;
                        moved++;
                        svgG.userMoved = true;
                        ...
                        const g = ptGraph(ev);
                        const dx = g.x - startG.x,
                            dy = g.y - startG.y;
                        startG = g;
                        if (!zoneIds) {
                            zoneIds = Object.keys(svgG.pos).filter(id => {
                                const table = state.tables[id];
                                return table && (table.theme || '').trim() === zoneDom;
                            });
                            const set = new Set();
                            zoneIds.forEach(id => (edgesByNode[id] || []).forEach(r => set.add(r)));
            ...
            window.addEventListener(
                ...
                (_svgWin.mu = ev => {
                    ...
                    if (mode === 'node') {
                        flushNode();
                        const ng = nodeEls[dragId];
                        if (ng) ng.style.cursor = 'grab';
                        if (moved < 3) {
                            if (mcdLinkMode) mcdNodeClicked(dragId);
                            else if (mcdPathMode) mcdPathClick(dragId);
                            else svgHighlightNeighbors(dragId);
                        } else persistAppState();
                    }
                    if (mode === 'zone') {
                        flushZone();
                        persistAppState();
            ...
            svg.addEventListener(
                ...
                ev => {
                    ...
                    const maximum = Math.max(0.15, Math.min(3, svgG.k * f));
            ...
            svg.addEventListener('click', ev => {
                if (!ev.target.closest('.svgnode') && !ev.target.closest('.svgedge')) svgClearHighlight();
            });
            svg.addEventListener('contextmenu', ev => {
                const ng = ev.target.closest('.svgnode');
                if (ng) {
                    ev.preventDefault();
                    mcdHideTable(ng.getAttribute('data-id'));
                }
            });
            svg.addEventListener('click', ev => {
                const eg = ev.target.closest('.svgedge');
                if (eg) {
                    const id = eg.getAttribute('data-id');
                    if (id && id.startsWith('rel_')) openEdgeDeleteModal(id);
                }
            });
        }
        function svgUpdateEdgeRec(rec) {
            const boxOf = i => {
                const position = svgG.pos[i],
                    s = svgG.size[i] || [180, 60];
                return { cx: position.x, cy: position.y, w: s[0], h: s[1] };
            };
            const box = boxOf(rec.s),
                b = boxOf(rec.t);
            const p1 = svgBorderPoint(box, b.cx, b.cy),
                p2 = svgBorderPoint(b, box.cx, box.cy);
            rec.paths.forEach(pt => pt.setAttribute('d', `M ${p1.x} ${p1.y} L ${p2.x} ${p2.y}`));
            if (rec.text) {
                rec.text.setAttribute('x', (p1.x + p2.x) / 2);
                rec.text.setAttribute('y', (p1.y + p2.y) / 2 - 3);
            }
        }
        function svgUpdateEdgesFor(id) {
            ((_svgIdx.edgesByNode || {})[id] || []).forEach(svgUpdateEdgeRec);
        }
        // Focus (domaine) + surlignage du voisinage en SVG (via classe .svg-dim).
        function svgSetDim(keepIds) {
            const svg = el('mcdSvg');
            if (!svg) return;
            svg.querySelectorAll('.svgnode').forEach(g =>
                g.classList.toggle('svg-dim', keepIds && !keepIds.has(g.getAttribute('data-id')))
            );
            svg.querySelectorAll('.svgedge').forEach(g =>
                g.classList.toggle(
                    'svg-dim',
                    keepIds && !(keepIds.has(g.getAttribute('data-s')) && keepIds.has(g.getAttribute('data-t')))
                )
            );
        }
        ...
        function svgHighlightNeighbors(id) {
            ...
            state.relations.forEach(r => {
                if (r.sourceTable === id) keep.add(r.targetTable);
                else if (r.targetTable === id) keep.add(r.sourceTable);
            });
            svgSetDim(keep);
        }
        function svgClearHighlight() {
            mcdHighlightId = null;
            svgApplyFocus();
        }
        ...
        function svgCenterOn(id) {
            const cont = el('networkGraph');
            const position = svgG.pos[id];
            if (!cont || !position) return;
            const W = cont.clientWidth || 800,
                H = cont.clientHeight || 500;
            if (svgG.k < 0.5) svgG.k = 0.8;
            svgG.tx = W / 2 - position.x * svgG.k;
            svgG.ty = H / 2 - position.y * svgG.k;
            svgApplyTransform();
            mcdHighlightId = null;
            svgHighlightNeighbors(id);
        }
        function mcdGotoTable(name) {
            const table = Object.values(state.tables).find(x => x.name === name);
            if (!table) return;
            if (!mcdVisibleIds().has(table.id)) {
                state.graphConfig.mcdTheme = '';
                state.graphConfig.mcdHidden = [];
                renderGraph();
            }
            svgCenterOn(table.id);
        }
        // ---- 🧭 Chemin entre deux tables : surligne la chaîne de jointures ----
        let mcdPathMode = false,
            mcdPathA = null;
        function mcdTogglePathMode() {
            mcdPathMode = !mcdPathMode;
            mcdPathA = null;
            mcdLinkMode = false;
            renderMcdControls();
            mcdLinkHint(
                mcdPathMode
                    ? '🧭 Mode chemin — cliquez la <strong>première</strong> table, puis la <strong>seconde</strong> : la chaîne de jointures qui les relie sera surlignée.'
                    : ''
            );
        ...
        function mcdPathClick(id) {
            if (!mcdPathA) {
                mcdPathA = id;
                mcdLinkHint(`🧭 1️⃣ <strong>${escapeHTML(state.tables[id].name)}</strong> — cliquez la table d'arrivée.`);
                return;
            }
            const a = mcdPathA;
            mcdPathA = null;
            ...
            state.relations.forEach(r => {
                ...
                (adj[r.sourceTable] = adj[r.sourceTable] || []).push({ o: r.targetTable, r });
                (adj[r.targetTable] = adj[r.targetTable] || []).push({ o: r.sourceTable, r });
            });
            const prev = { [a]: null };
            const q = [a];
            let ok = false;
            ...
            if (!ok) {
                mcdLinkHint(
                    `❌ Aucun chemin de jointures entre <strong>${escapeHTML(state.tables[a].name)}</strong> et <strong>${escapeHTML(state.tables[id].name)}</strong> — ajoutez un lien dans le modèle.`
                );
                return;
            }
            const path = [];
            ...
            mcdLinkHint(
                `🧭 Chemin : ${path.map(x => `<strong>${escapeHTML(state.tables[x].name)}</strong>`).join(' → ')} <button onclick="svgClearHighlight(); mcdLinkHint(mcdPathMode ? '🧭 Mode chemin — cliquez deux tables.' : '')" class="ml-2 text-[10px] bg-white border border-indigo-300 rounded px-1.5 py-0.5">effacer</button>`
            );
        }
        // ---- 🗺 Mini-carte : aperçu global + fenêtre courante, clic pour se déplacer ----
        function svgRenderMinimap() {
            const cont = el('networkGraph');
            if (!cont || !el('mcdSvg')) return;
            let element = cont.querySelector('.mcdMini');
            const ids = Object.keys(svgG.pos);
            if (!ids.length) {
            ...
            const MW = 168,
                MH = 104,
                minimum = Math.min(MW / Math.max(1, x2 - x1), MH / Math.max(1, y2 - y1)) * 0.92;
            const ox = (MW - (x2 - x1) * minimum) / 2 - x1 * minimum,
                oy = (MH - (y2 - y1) * minimum) / 2 - y1 * minimum;
            const W = cont.clientWidth || 800,
                H = cont.clientHeight || 500;
            const vx = (0 - svgG.tx) / svgG.k,
                vy = (0 - svgG.ty) / svgG.k,
                vw = W / svgG.k,
                vh = H / svgG.k;
            const rects = ids
                .map(id => {
                    const position = svgG.pos[id],
                        s = svgG.size[id] || [180, 60];
                    const table = state.tables[id];
                    const pal = table && table.theme ? themePalette((table.theme || '').trim()) : null;
                    return `<rect x="${(position.x - s[0] / 2) * minimum + ox}" y="${(position.y - s[1] / 2) * minimum + oy}" width="${Math.max(2, s[0] * minimum)}" height="${Math.max(2, s[1] * minimum)}" fill="${pal ? pal.zone : '#94a3b8'}" fill-opacity="0.75" rx="1"/>`;
                })
                .join('');
            const view = `<rect x="${vx * minimum + ox}" y="${vy * minimum + oy}" width="${vw * minimum}" height="${vh * minimum}" fill="none" stroke="#4f46e5" stroke-width="1.5"/>`;
            if (!element) {
                element = document.createElement('div');
                element.className = 'mcdMini';
                element.style.cssText =
                    'position:absolute;right:10px;bottom:10px;width:168px;height:104px;background:rgba(255,255,255,.92);border:1px solid #cbd5e1;border-radius:8px;box-shadow:0 4px 10px -4px rgba(15,23,42,.3);cursor:crosshair;z-index:5;overflow:hidden';
                cont.style.position = 'relative';
                cont.appendChild(element);
                element.addEventListener('mousedown', ev => {
                    ev.stopPropagation();
                    const r = element.getBoundingClientRect();
                    const gx = (ev.clientX - r.left - element.__ox) / element.__sc,
                        gy = (ev.clientY - r.top - element.__oy) / element.__sc;
                    const networkGraphElement = el('networkGraph');
                    const W2 = networkGraphElement.clientWidth || 800,
                        H2 = networkGraphElement.clientHeight || 500;
                    svgG.tx = W2 / 2 - gx * svgG.k;
                    svgG.ty = H2 / 2 - gy * svgG.k;
                    svgApplyTransform();
                });
            }
            element.__sc = minimum;
            element.__ox = ox;
            element.__oy = oy;
            element.innerHTML = `<svg width="${MW}" height="${MH}">${rects}${view}</svg>`;
        }
        ...
        function getReachableTables(startNodeId) {
            ...
            state.relations.forEach(r => {
                if (r.sourceCol && r.targetCol) {
                    adj[r.sourceTable].push(r.targetTable);
                    adj[r.targetTable].push(r.sourceTable);
                }
            });
        ...
        function updateBaseTableSelect() {
            ...
            const currentVal = baseTableSelectElement.value;
            baseTableSelectElement.innerHTML = tableOptionsHtml({
                placeholder: 'Sélectionnez la table principale...',
                readyOnly: true
            });
            if (currentVal && state.tables[currentVal]) baseTableSelectElement.value = currentVal;
        }

        function handleBaseTableChange() {
            const val = el('baseTableSelect').value;
            if (val && state.tables[val]) advSetBase(val);
            else renderAdvExtract();
        }
