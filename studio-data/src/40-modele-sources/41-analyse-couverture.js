        ...
        function covPopulate() {
            ...
            const cur = covBaseElement.value;
            covBaseElement.innerHTML =
                '<option value="">— table de base —</option>' +
                ready
                    .map(t => `<option value="${t.id}" ${t.id === cur ? 'selected' : ''}>${escapeHTML(t.name)}</option>`)
                    .join('');
            covBaseChanged(true);
        }
        function covBaseChanged(keep) {
            ...
            const covDimTableElement = el('covDimTable'),
                covRelElement = el('covRel');
            if (!bId) {
                covDimTableElement.innerHTML = '';
                covRelElement.innerHTML = '';
                el('covDimCol').innerHTML = '';
                covRenderFilters('base');
                covRenderFilters('rel');
                return;
            }
            const nb = covNeighbors(bId);
            const curD = covDimTableElement.value,
                curR = covRelElement.value;
            covDimTableElement.innerHTML =
                `<option value="${bId}">${escapeHTML(state.tables[bId].name)} (elle-même)</option>` +
                nb
                    .map(
                        id =>
                            `<option value="${id}" ${id === curD ? 'selected' : ''}>${escapeHTML(state.tables[id].name)}</option>`
                    )
                    .join('');
            covRelElement.innerHTML =
                (nb.length ? '' : '<option value="">— aucune table liée dans le modèle —</option>') +
                nb
                    .map(
                        id =>
                            `<option value="${id}" ${id === curR ? 'selected' : ''}>${escapeHTML(state.tables[id].name)}</option>`
                    )
                    .join('');
            const covDim2TableElement = el('covDim2Table');
            if (covDim2TableElement) {
                const cur2 = covDim2TableElement.value;
                covDim2TableElement.innerHTML =
                    `<option value="">— aucune —</option><option value="${bId}" ${cur2 === bId ? 'selected' : ''}>${escapeHTML(state.tables[bId].name)}</option>` +
                    nb
                        .map(
                            id =>
                                `<option value="${id}" ${id === cur2 ? 'selected' : ''}>${escapeHTML(state.tables[id].name)}</option>`
                        )
                        .join('');
                covDim2TableChanged();
            }
            const covDim3TableElement = el('covDim3Table');
            if (covDim3TableElement) {
                const cur3 = covDim3TableElement.value;
                covDim3TableElement.innerHTML =
                    `<option value="">— aucune —</option><option value="${bId}" ${cur3 === bId ? 'selected' : ''}>${escapeHTML(state.tables[bId].name)}</option>` +
                    nb
                        .map(
                            id =>
                                `<option value="${id}" ${id === cur3 ? 'selected' : ''}>${escapeHTML(state.tables[id].name)}</option>`
                        )
                        .join('');
                covDim3TableChanged();
            }
        ...
        function covDimColOptions(tId, cur) {
            ...
            const table = state.tables[tId].headers;
            let html = table
            ...
            const dts = table.filter(h => covColIsDate(tId, h));
            if (dts.length)
                html += dts
                    .map(h => {
                        const v = '__annee__|' + h;
                        return `<option value="${escapeHTML(v)}" ${v === cur ? 'selected' : ''}>📅 Année de ${escapeHTML(h)}</option>`;
                    })
                    .join('');
            return html;
        }
        function covDimTableChanged() {
            const tId = el('covDimTable').value;
            const covDimColElement = el('covDimCol');
            covDimColElement.innerHTML = covDimColOptions(tId, covDimColElement.value);
        }
        function covDim2TableChanged() {
            const tId = el('covDim2Table') ? el('covDim2Table').value : '';
            const covDim2ColElement = el('covDim2Col');
            if (!covDim2ColElement) return;
            covDim2ColElement.innerHTML = tId ? covDimColOptions(tId, covDim2ColElement.value) : '<option value="">—</option>';
        }
        function covDim3TableChanged() {
            const tId = el('covDim3Table') ? el('covDim3Table').value : '';
            const covDim3ColElement = el('covDim3Col');
            if (!covDim3ColElement) return;
            covDim3ColElement.innerHTML = tId ? covDimColOptions(tId, covDim3ColElement.value) : '<option value="">—</option>';
        }
        function covRelChanged() {
            covFilters.rel = [];
            covRenderFilters('rel');
        }
        function covScopeTable(scope) {
            return scope === 'base' ? el('covBase').value : el('covRel').value;
        }
        function covAddFilter(scope) {
            covFilters[scope].push({ col: '', op: 'eq', val: '', conn: scope === 'rel' ? 'OR' : 'AND' });
            covRenderFilters(scope);
        }
        function covSetFilter(scope, i, f, v) {
            if (covFilters[scope][i]) {
                covFilters[scope][i][f] = v;
                if (f === 'op' || f === 'col') covRenderFilters(scope);
            }
        }
        function covDelFilter(scope, i) {
            covFilters[scope].splice(i, 1);
            covRenderFilters(scope);
        }
        ...
        function covColIsDate(tId, col) {
            ...
            try {
                const vals = tdValueCache[(tId || '') + '|' + col];
                if (vals && vals.length) {
                    const d = vals
                        .slice(0, 8)
                        .filter(v => /^\d{4}-\d{2}-\d{2}/.test(v) || /^\d{1,2}[\/.\-]\d{1,2}[\/.\-]\d{4}/.test(v));
                    return d.length >= Math.ceil(Math.min(vals.length, 8) / 2);
                }
            } catch (e) {}
            return false;
        }
        function covRenderFilters(scope) {
            const box = el('covFilterRows-' + scope);
            if (!box) return;
            const tId = covScopeTable(scope);
            const headers = tId && state.tables[tId] ? state.tables[tId].headers : [];
            const rows = covFilters[scope];
            if (!rows.length) {
                box.innerHTML = '<p class="text-[10px] text-slate-400 italic">Aucun filtre.</p>';
                return;
            }
            box.innerHTML = rows
                .map((c, i) => {
                    const conn =
                        i === 0
                            ? '<span class="text-[10px] font-bold text-slate-400 w-9 text-center shrink-0">Où</span>'
            ...
            if (tId && state.tables[tId])
                rows.forEach((c, i) => {
                    if (c.col && !covColIsDate(tId, c.col))
                        tdFillDatalist(`covfval-${scope}-${i}`, state.tables[tId].name, c.col);
                });
        }
        function covWhere(scope) {
            const valid = covFilters[scope].filter(
        ...
        function covRelBetween(x, y) {
            return state.relations.find(
                r =>
                    r.sourceCol &&
                    r.targetCol &&
                    ((r.sourceTable === x && r.targetTable === y) || (r.sourceTable === y && r.targetTable === x))
            );
        }
        async function runCoverageAnalysis(btn) {
            hideError();
            const bId = el('covBase').value,
                dT = el('covDimTable').value,
                dC = el('covDimCol').value,
                rId = el('covRel').value;
            const d2T = el('covDim2Table') ? el('covDim2Table').value : '',
                d2C = el('covDim2Col') ? el('covDim2Col').value : '';
            const dim2On = !!(d2T && d2C);
            const d3T = el('covDim3Table') ? el('covDim3Table').value : '',
                d3C = el('covDim3Col') ? el('covDim3Col').value : '';
            ...
            if (!relation)
                return showError(
                    `Aucun lien du modèle entre "${state.tables[bId].name}" et "${state.tables[rId].name}" — créez-le dans le Modèle de données.`
                );
            let rd = null;
            if (dT !== bId) {
                rd = covRelBetween(bId, dT);
                if (!rd)
                    return showError(`Aucun lien du modèle entre "${state.tables[bId].name}" et "${state.tables[dT].name}".`);
            }
            let rd2 = null;
            if (dim2On && d2T !== bId && d2T !== dT) {
                rd2 = covRelBetween(bId, d2T);
                if (!rd2)
                    return showError(
                        `Aucun lien du modèle entre "${state.tables[bId].name}" et "${state.tables[d2T].name}" (dimension 2).`
                    );
            }
            let rd3 = null;
            if (dim3On && d3T !== bId && d3T !== dT && !(dim2On && d3T === d2T)) {
                rd3 = covRelBetween(bId, d3T);
                if (!rd3)
                    return showError(
                        `Aucun lien du modèle entre "${state.tables[bId].name}" et "${state.tables[d3T].name}" (dimension 3).`
                    );
            }
            ...
            try {
                ...
                const rKey = relation.sourceTable === bId ? relation.targetCol : relation.sourceCol;
                const fb = covWhere('base'),
                    fr = covWhere('rel');
                ...
                const dimExpr = (alias, col) => {
                    if (String(col).startsWith('__annee__|')) {
                        const text = String(col).slice(10);
                        const raw2 = `CAST(${alias}.${sqlIdent(text)} AS VARCHAR)`;
                ...
                if (dim2On) {
                    if (d2T === bId) dim2Sel = dimExpr('b', d2C);
                        ...
                        joins.push(dimJoin(rd2, 'd2'));
                        dim2Sel = dimExpr('d2', d2C);
                    }
                }
                let dim3Sel = null;
                if (dim3On) {
                    if (d3T === bId) dim3Sel = dimExpr('b', d3C);
                        ...
                        joins.push(dimJoin(rd3, 'd3'));
                        dim3Sel = dimExpr('d3', d3C);
                    }
                }
                ...
                if (compareTotal) {
                    ...
                    totMap = new Map();
                    arrowResultToObjects(resT).forEach(r =>
                        totMap.set(
                            (dim3Sel ? String(r.dim3) : '') + '' + (dim2Sel ? String(r.dim2) : '') + '' + String(r.dim),
                            Number(r.total)
                        )
                    );
                }
                ...
                const rows = arrowResultToObjects(res).map(r => ({
                    dim: String(r.dim),
                    dim2: dim2Sel ? String(r.dim2) : null,
                    dim3: dim3Sel ? String(r.dim3) : null,
                    total: Number(r.total),
                    avec: Number(r.avec),
                    sans: Number(r.total) - Number(r.avec)
                }));
                if (!rows.length) {
                    bgTaskEnd();
                    showError('Population vide avec ces filtres.');
                    return;
                }
                const gT = rows.reduce((a, r) => a + r.total, 0),
                    gA = rows.reduce((a, r) => a + r.avec, 0);
                ...
                try {
                    if (covChartInst) {
                        covChartInst.destroy();
                        covChartInst = null;
                    }
                    (window.covFacetCharts || []).forEach(ch => {
                        try {
                            ch.destroy();
                        } catch (e2) {}
                    });
                    window.covFacetCharts = [];
                    const oldFac = el('covFacets');
                    if (oldFac) oldFac.remove();
                    ...
                    const buildChartData = (subset, facetVal) => {
                        const totOf = (d2v, d1v) =>
                            totMap ? totMap.get((facetVal || '') + '' + (d2v || '') + '' + d1v) || 0 : 0;
                        if (!dim2Sel) {
                            const labels2 = subset.map(r => r.dim);
                            const ds = [
                            ...
                            if (totMap)
                                ds.push({
                                    label: '⬜ Écarté par les filtres',
                                    data: subset.map(r => Math.max(0, totOf('', r.dim) - r.total)),
                        ...
                        const totBy = (arr, key) => {
                            const m = new Map();
                            arr.forEach(r => m.set(r[key], (m.get(r[key]) || 0) + r.total));
                            return m;
                        };
                        const d1Top = [...totBy(subset, 'dim')]
                            .sort((a, b2) => (String(dC).startsWith('__annee__|') ? (a[0] < b2[0] ? -1 : 1) : b2[1] - a[1]))
                            .slice(0, 15)
                            .map(x => x[0]);
                        const d2Top = [...totBy(subset, 'dim2')]
                            .sort((a, b2) => b2[1] - a[1])
                            .slice(0, 5)
                            .map(x => x[0]);
                        ...
                        const cell = {};
                        subset.forEach(r => {
                            if (!d1Top.includes(r.dim)) return;
                            const k = r.dim + '' + d2Of(r);
                            cell[k] = cell[k] || { avec: 0, sans: 0, tot: 0 };
                            cell[k].avec += r.avec;
                            cell[k].sans += r.sans;
                            cell[k].tot += totOf(r.dim2, r.dim);
                        });
                        const datasets = [];
                        d2Vals.forEach((v, i) => {
                            ...
                            datasets.push({
                                label: `${v} ✔`,
                                data: d1Top.map(d1 => (cell[d1 + '' + v] || {}).avec || 0),
                                backgroundColor: col,
                                stack: v
                            });
                            datasets.push({
                                label: `${v} ✖`,
                                data: d1Top.map(d1 => (cell[d1 + '' + v] || {}).sans || 0),
                            ...
                            if (totMap)
                                datasets.push({
                                    ...
                                    data: d1Top.map(d1 => {
                                        const c2 = cell[d1 + '' + v];
                                        return c2 ? Math.max(0, c2.tot - c2.avec - c2.sans) : 0;
                                    }),
                                    backgroundColor: '#cbd5e1',
                                    stack: v
                                });
                    ...
                    const mkChart = (canvas, data, title) =>
                        new Chart(canvas.getContext('2d'), {
                            ...
                            options: {
                                ...
                                scales: {
                                    x: { stacked: true, ticks: { autoSkip: false, maxRotation: 60, font: { size: 9 } } },
                                    y: { stacked: true, beginAtZero: true }
                                },
                                plugins: {
                                    legend: { position: 'top', labels: { boxWidth: 10, font: { size: 9 } } },
                                    title: title
                                        ? { display: true, text: title, font: { size: 12, weight: 'bold' } }
                                        : undefined
                                }
                            }
                        });
                    if (!dim3Sel) {
                        ...
                        const totBy3 = new Map();
                        rows.forEach(r => totBy3.set(r.dim3, (totBy3.get(r.dim3) || 0) + r.total));
                        const d3Top = [...totBy3]
                            .sort((a, b2) => b2[1] - a[1])
                            .slice(0, 6)
                            .map(x => x[0]);
                        ...
                        d3Vals.forEach(v3 => {
                            ...
                            card.firstChild.appendChild(canvasElement);
                            fac.appendChild(card);
                            window.covFacetCharts.push(
                                mkChart(
                                    canvasElement,
                                    buildChartData(subset, v3),
                                    `${v3} — ${tot.toLocaleString('fr-FR')} entité(s)`
                                )
                            );
                        });
                    }
                } catch (eC) {
                    console.warn('Graphique indisponible :', eC);
                }
                // Tableau + consultation/export des lignes
                qualInspectRegistry = qualInspectRegistry || [];
                const rowsSql = neg =>
                    `${withSql} SELECT ${dimSel} AS "Axe d'analyse"${dim2Sel ? `, ${dim2Sel} AS "Dimension 2"` : ''}${dim3Sel ? `, ${dim3Sel} AS "Dimension 3"` : ''}, b.* EXCLUDE (__rn) FROM b ${dJoin} WHERE ${neg ? 'NOT (' + hasCond + ')' : hasCond}`;
                ...
                el('covTable').innerHTML =
                    `<div class="border border-slate-200 rounded-lg overflow-x-auto max-h-[420px] overflow-y-auto"><table class="w-full text-left text-xs">
                    <thead class="bg-slate-50 text-[10px] uppercase font-bold text-slate-500 sticky top-0"><tr><th class="p-2">${escapeHTML(dC)}</th>${dim2Sel ? `<th class="p-2">${escapeHTML(d2C)}</th>` : ''}${dim3Sel ? `<th class="p-2">${escapeHTML(d3C)}</th>` : ''}<th class="p-2 text-right">Population</th>${totMap ? '<th class="p-2 text-right">Totalité (sans filtres)</th>' : ''}<th class="p-2 text-right">✔ Avec</th>
                        <th class="p-2 text-right">✖ Sans</th>
                        <th class="p-2 text-right">Couverture</th>
                        </tr>
                        </thead>
                    <tbody class="divide-y divide-slate-100">${rows
                        .map(
                            r => `<tr class="hover:bg-teal-50/40"><td class="p-2 font-bold">${escapeHTML(r.dim)}</td>${dim2Sel ? `<td class="p-2">${escapeHTML(r.dim2)}</td>` : ''}${dim3Sel ? `<td class="p-2">${escapeHTML(r.dim3)}</td>` : ''}<td class="p-2 text-right">${r.total.toLocaleString('fr-FR')}</td>${totMap ? `<td class="p-2 text-right text-slate-500">${(totMap.get((r.dim3 || '') + '' + (r.dim2 || '') + '' + r.dim) || 0).toLocaleString('fr-FR')}</td>` : ''}<td class="p-2 text-right text-emerald-700 font-bold">${r.avec.toLocaleString('fr-FR')}</td>
                        <td class="p-2 text-right text-red-600 font-bold">${r.sans.toLocaleString('fr-FR')}</td>
                        <td class="p-2 text-right font-black">${((r.avec / r.total) * 100).toFixed(1)} %</td>
                        </tr>`
                        )
                        .join('')}</tbody>
                        </table></div>
                ...
                bgTaskEnd(
                    `🎯 Analyse terminée : ${((gA / gT) * 100).toFixed(1)} % de couverture sur ${gT.toLocaleString('fr-FR')} entités.`
                );
            } catch (e) {
                bgTaskEnd();
