        ...
        const CN_KINDS = {
            json: '🌐 API / JSON',
            csv: '📄 CSV par URL',
            parquet: '🧊 Parquet par URL',
            gsheet: '📗 Google Sheets'
        };
        function cnList() {
            return (state.connectors = state.connectors || []);
        }
        function cnById(id) {
            return cnList().find(x => x.id === id);
        }
        function cnAdd() {
            cnList().push({
                id: 'cn_' + generateId(),
                name: 'Nouveau connecteur',
                kind: 'json',
                url: '',
                path: '',
                keyCol: '',
        ...
        function cnSet(id, f, v) {
            const c = cnById(id);
            if (!c) return;
            c[f] = v;
            persistAppState();
            if (f === 'kind') renderSourceUpdatePanel();
        }
        function cnDel(id) {
            state.connectors = cnList().filter(x => x.id !== id);
            persistAppState();
            renderSourceUpdatePanel();
        }
        function cnSheetUrl(u) {
            const text = String(u).match(/docs\.google\.com\/spreadsheets\/d\/([\w-]+)/);
            if (!text) return u;
            const gid = (String(u).match(/[#&?]gid=(\d+)/) || [])[1] || '0';
            return `https://docs.google.com/spreadsheets/d/${text[1]}/export?format=csv&gid=${gid}`;
        }
        async function cnFetch(c) {
            ...
            try {
                ...
                throw new Error(
                    'téléchargement impossible — hors ligne, URL invalide, ou le serveur refuse les requêtes navigateur (CORS).'
                );
            }
            if (!resp.ok)
                throw new Error(
                    'HTTP ' +
                        ...
                        " — vérifiez l'URL et les droits d'accès."
                );
        ...
        function cnJsonAtPath(text, path) {
            ...
            if (!Array.isArray(parsed)) throw new Error(`le chemin « ${path} » ne pointe pas vers un tableau JSON.`);
        ...
        async function cnRun(id, btn) {
            ...
            try {
                ...
                if (payload.buf) await db.registerFileBuffer(fname, payload.buf);
                else await db.registerFileText(fname, payload.text);
                const reader =
                    c.kind === 'json'
                        ? `read_json_auto('${fname}')`
                        : c.kind === 'parquet'
                          ? `read_parquet('${fname}')`
                          : `read_csv_auto('${fname}', header=true, all_varchar=true)`;
                const stage = sqlIdent('cn_stage_' + c.id);
                await conn.query(`CREATE OR REPLACE TABLE ${stage} AS SELECT * FROM ${reader}`);
                const newHeaders =
                    arrowResultToObjects(await conn.query(`SELECT * FROM ${stage} LIMIT 0`)) &&
                    (await conn.query(`SELECT * FROM ${stage} LIMIT 0`)).schema.fields.map(f => f.name);
                const old =
                    c.targetId && state.tables[c.targetId] && state.tables[c.targetId].status === 'ready'
                        ? state.tables[c.targetId]
                        : null;
                ...
                if (old) {
                    ...
                    if (!common.length)
                        throw new Error(
                            'aucune colonne commune avec la table existante — supprimez le connecteur ou changez le nom de la table produite.'
                        );
                    const hash = al =>
                        `md5(concat_ws(chr(1), ${common.map(h => `COALESCE(CAST(${al}${sqlIdent(h)} AS VARCHAR), '')`).join(', ')}))`;
                    ...
                    if (key) {
                        ...
                        const rows = arrowResultToObjects(queryResult)[0];
                        diff = { added: Number(rows.a), removed: Number(rows.r), changed: Number(rows.m), key };
                    } else {
                        const queryResult = await conn.query(`SELECT
                        ...
                        const rows = arrowResultToObjects(queryResult)[0];
                        diff = { added: Number(rows.a), removed: Number(rows.r), changed: null, key: null };
                    }
                }
                let t;
                if (old && (c.mode || 'replace') === 'append' && diff && diff.key) {
                    ...
                    await conn.query(
                        `CREATE TABLE ${sqlIdent(duckTableName(tId))} AS SELECT row_number() OVER () AS __rn, * FROM ${stage}`
                    );
                    c.targetId = tId;
                }
                await conn.query(`DROP TABLE IF EXISTS ${stage}`);
                t.headers = await duckTableHeaders(t.id);
                t.storage = 'table';
                t.sampleData = await duckSampleRows(t.id, 6);
                t.status = 'ready';
                const cRes = await conn.query(`SELECT COUNT(*)::BIGINT AS n FROM ${sqlIdent(duckTableName(t.id))}`);
                c.lastRows = Number(arrowResultToObjects(cRes)[0].n);
                c.lastAt = Date.now();
                c.lastDiff = diff;
                state.governance.lineage[t.name] = {
                    from: [(CN_KINDS[c.kind] || c.kind) + ' — ' + String(c.url).slice(0, 100)]
                };
                // E9 : colonnes disparues -> alerte listant les objets aval impactés.
                try {
                    oldHeaders
                        .filter(h => !t.headers.includes(h))
                        .forEach(h => {
                            const deps = modelImpact(t.name, h);
                            if (deps.length)
                                showError(
                                    `⚠️ La colonne « ${h} » a disparu de « ${t.name} » — ${deps.length} objet(s) dépendant(s) : ${deps
                                        .slice(0, 5)
                                        .map(d2 => d2.type + ' ' + d2.name)
                                        .join(' · ')}${deps.length > 5 ? '…' : ''}`
                                );
                        });
                } catch (e2) {}
                ...
                populateQualTables();
                try {
                    await persistTableData(t.id);
                } catch (e2) {}
                persistAppState();
                renderSourceUpdatePanel();
                bgTaskEnd(
                    `🔌 « ${t.name} » : ${c.lastRows.toLocaleString('fr-FR')} ligne(s)${diff ? ` — différentiel : +${diff.added} ajoutée(s), −${diff.removed} disparue(s)${diff.changed != null ? ', ' + diff.changed + ' modifiée(s)' : ''}${(c.mode || 'replace') === 'append' && diff.key ? ' (mode ajout : seules les nouvelles clés ont été insérées)' : ''}` : ' (premier import)'}.`
                );
                try {
                    await rcAutoRun(t.name);
                } catch (e2) {}
                try {
                    await qrAutoRun(t.name);
                } catch (e2) {}
            } catch (e) {
                bgTaskEnd();
                showError('Connecteur « ' + c.name + ' » : ' + e.message);
            } finally {
        ...
        function cnPanelHtml() {
            const rows = cnList()
                .map(
                    c => `<div class="bg-white border border-slate-200 rounded-lg p-2 mb-1.5">
                <div class="flex items-center gap-1.5 flex-wrap text-[11px]">
                    ...
                    <select onchange="cnSet('${c.id}','kind',this.value)" class="border border-slate-200 rounded px-1 py-1 bg-white">${Object.entries(
                        CN_KINDS
                    )
                        .map(([k, l]) => `<option value="${k}" ${c.kind === k ? 'selected' : ''}>${l}</option>`)
                        .join('')}</select>
