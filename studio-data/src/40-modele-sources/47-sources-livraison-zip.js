        ...
        function zipListEntries(buf) {
            ...
            for (let k = 0; k < count; k++) {
                ...
                const mtime = dv.getUint16(off + 12, true),
                    mdate = dv.getUint16(off + 14, true);
                ...
                if (!name.endsWith('/')) {
                    ...
                    entries.push({
                        name,
                        folder: parts.length > 1 ? parts[0] : '(racine)',
        ...
        async function zipExtractEntry(buf, e) {
            ...
            const nlen = dv.getUint16(e.lho + 26, true),
                elen = dv.getUint16(e.lho + 28, true);
        ...
        async function zipDeliveryLoad(inputEl) {
            ...
            try {
                ...
                const entries = zipListEntries(buf).filter(
                    e => ZIP_EXTS.includes(e.base.split('.').pop().toLowerCase()) && !e.base.startsWith('.')
                );
                if (!entries.length) throw new Error('Aucun fichier CSV / TXT / Excel dans ce ZIP.');
                _zipDelivery = { name: file.name, buf, entries };
                ...
                folders.forEach(fo => {
                    if (!state.zipMappings[fo])
                        state.zipMappings[fo] = { mode: 'folder', dest: fo === '(racine)' ? '' : fo, files: {} };
                });
                renderZipMapping();
                showSuccess(
                    `📦 « ${file.name} » : ${entries.length} fichier(s) dans ${folders.length} dossier(s) — vérifiez le mapping puis importez.`
                );
            } catch (e) {
                showError('Lecture du ZIP impossible : ' + e.message);
            }
        }
        function zipSetMap(folder, field, v) {
            const zipMapping = state.zipMappings[folder];
            if (!zipMapping) return;
            zipMapping[field] = v;
            persistAppState();
            renderZipMapping();
        }
        function zipSetFileMap(folder, base, v) {
            const zipMapping = state.zipMappings[folder];
            if (!zipMapping) return;
            zipMapping.files[base] = v;
            persistAppState();
        }
        function renderZipMapping() {
            const area = el('zipMapArea');
            if (!area || !_zipDelivery) return;
            ...
            const destOpts = (cur, folder) => {
                ...
                return (
                    `<option value="__ignore__" ${cur === '__ignore__' ? 'selected' : ''}>⤫ ignorer</option>` +
                    cands
                        .map(
                            d =>
                                `<option value="${escapeHTML(d)}" ${cur === d ? 'selected' : ''}>→ domaine ${escapeHTML(d)}</option>`
                        )
                        .join('') +
            ...
            area.innerHTML = `<div class="text-[11px] font-bold text-slate-600 mb-1">📦 ${escapeHTML(_zipDelivery.name)} — mapping (mémorisé pour les prochains ZIP)</div>
                ${folders
                    .map(fo => {
                        const zipMapping = state.zipMappings[fo];
                        const files = _zipDelivery.entries.filter(e => e.folder === fo);
                    ...
                    <div class="flex items-center gap-2 flex-wrap text-[11px]">
                        ...
                        <select onchange="if(this.value==='__custom__'){const d=prompt('Nom du domaine :'); if(d) zipSetMap('${escapeHTML(fo)}','dest',d); else renderZipMapping();} else zipSetMap('${escapeHTML(fo)}','dest',this.value)" class="border border-slate-300 rounded px-1 py-0.5 text-[11px] bg-white" ${zipMapping.mode === 'files' ? 'disabled' : ''}>${destOpts(zipMapping.dest, fo)}</select>
                        <label class="flex items-center gap-1 ml-auto cursor-pointer text-slate-500"><input type="checkbox" ${zipMapping.mode === 'files' ? 'checked' : ''} onchange="zipSetMap('${escapeHTML(fo)}','mode',this.checked?'files':'folder')"> détail par fichier</label>
                    </div>
                    ${
                        zipMapping.mode === 'files'
                            ? `<div class="mt-1.5 space-y-1 pl-4">${files
                                  .map(e => {
                                      const cur = e.base in zipMapping.files ? zipMapping.files[e.base] : zipMapping.dest;
                                      return `<div class="flex items-center gap-2 text-[11px]"><span class="text-slate-600 truncate max-w-[220px]">${escapeHTML(e.base)}</span>
                        <select onchange="if(this.value==='__custom__'){const d=prompt('Nom du domaine :'); if(d) { zipSetFileMap('${escapeHTML(fo)}','${escapeHTML(e.base.replace(/'/g, "\\'"))}',d); renderZipMapping(); } else renderZipMapping();} else zipSetFileMap('${escapeHTML(fo)}','${escapeHTML(e.base.replace(/'/g, "\\'"))}',this.value)" class="border border-slate-200 rounded px-1 py-0.5 text-[11px] bg-white">${destOpts(cur, fo)}</select></div>`;
                                  })
                                  .join('')}</div>`
                            : ''
                    }
        ...
        async function zipDeliveryImport(btn) {
            ...
            try {
                for (const e of _zipDelivery.entries) {
                    const zipMapping = state.zipMappings[e.folder] || { mode: 'folder', dest: '', files: {} };
                    const dest =
                        zipMapping.mode === 'files'
                            ? e.base in zipMapping.files
                                ? zipMapping.files[e.base]
                                : zipMapping.dest
                            : zipMapping.dest;
                    if (dest === '__ignore__') {
                        ignored++;
                    ...
                    try {
                        ...
                        const existing = Object.keys(state.tables).find(
                            tid => state.tables[tid].name.toLowerCase() === e.base.toLowerCase()
                        );
                        if (existing) {
                            const table = state.tables[existing];
                            if (table && !table.theme && dest) table.theme = dest;
                        ...
                        errors++;
                        console.warn('ZIP : import impossible pour', e.name, e2);
                    }
                }
                persistAppState();
                renderTables();
                bgTaskEnd(
                    `📦 Livraison « ${_zipDelivery.name} » : ${refreshed} mise(s) à jour, ${imported} import(s), ${ignored} ignoré(s)${errors ? ', ' + errors + ' erreur(s)' : ''}.`
                );
            } catch (e) {
                bgTaskEnd();
                showError('Import du ZIP impossible : ' + e.message);
            } finally {
