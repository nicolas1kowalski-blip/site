        ...
        async function refreshTableFromFile(tId, file, opts) {
            ...
            const oldHeaders = (table.headers || []).slice(); // E9 : détection des colonnes disparues
            const ext = file.name.split('.').pop().toLowerCase();
            if (!['csv', 'txt', 'xlsx', 'xls'].includes(ext)) throw new Error('Type non pris en charge : ' + file.name);
            if (opts && opts.skipUnchanged && table.srcModified && file.lastModified && file.lastModified <= table.srcModified)
                return { ok: true, skipped: true };
            table.file = file;
            table.size = file.size;
            table.type = ext;
            ...
            table.status = 'ready';
            table.lastRefresh = Date.now();
            table.srcModified = file.lastModified || Date.now();
            renderTables();
            ...
            try {
                await tdAutoRebuildFor(table.name);
            } catch (e) {
                console.warn('Reconstruction auto ignorée :', e);
            }
            // E9 : colonnes disparues -> alerte listant les objets aval qui vont casser.
            try {
                const gone = (oldHeaders || []).filter(h => !table.headers.includes(h));
                gone.forEach(h => {
                    const deps = modelImpact(table.name, h);
                    if (deps.length)
                        showError(
                            `⚠️ La colonne « ${h} » a disparu de « ${table.name} » au rafraîchissement — ${deps.length} objet(s) dépendant(s) risquent de casser : ${deps
                                .slice(0, 5)
                                .map(d => d.type + ' ' + d.name)
                                .join(' · ')}${deps.length > 5 ? '…' : ''}`
                        );
            ...
            try {
                await rcAutoRun(table.name);
            } catch (e) {
                console.warn('Recettes non relancées :', e);
            }
            // E1 : les règles de qualité de cette table sont rejouées automatiquement.
            try {
                await qrAutoRun(table.name);
            } catch (e) {
                console.warn('Règles qualité non rejouées :', e);
            }
        ...
        function manualUpdateSource(tId) {
            ...
            inp.onchange = async e => {
                ...
                if (!file) return;
                try {
                    await refreshTableFromFile(tId, file);
                    showSuccess(`Source "${table.name}" mise à jour (${file.name}).`);
                } catch (err) {
                    showError('Mise à jour impossible : ' + err.message);
                }
            };
            inp.click();
        }
        async function saveWatchedDirs() {
            try {
                await idbPut(
                    'meta',
                    'watchedDirs',
                    state.watchedDirs.map(d => ({ id: d.id, name: d.name, handle: d.handle }))
                );
            } catch (e) {}
        }
        async function saveUpdatePrefs() {
            try {
                await idbPut('meta', 'updatePrefs', { autoRefresh: state.autoRefresh });
            } catch (e) {}
        }
        ...
        async function addWatchedDir() {
            if (!fsDirSupported())
                return showError(
                    'La surveillance de dossier nécessite un navigateur compatible (Chrome ou Edge sur ordinateur).'
                );
            let handle;
            try {
            ...
            if (state.watchedDirs.some(d => d.name === handle.name)) {
                const ex = state.watchedDirs.find(d => d.name === handle.name);
                ex.handle = handle;
            } else state.watchedDirs.push({ id: 'wd_' + generateId(), name: handle.name, handle });
            await saveWatchedDirs();
            renderSourceUpdatePanel();
            scheduleAutoRefresh();
            const id = state.watchedDirs.find(d => d.name === handle.name).id;
            await scanWatchedDir(id, true, true);
        }
        function removeWatchedDir(id) {
            state.watchedDirs = state.watchedDirs.filter(d => d.id !== id);
            saveWatchedDirs();
            scheduleAutoRefresh();
            renderSourceUpdatePanel();
        }
        ...
        async function _scanDirImpl(wd, importNew, interactive) {
            if (!(await ensureDirPermission(wd.handle, interactive))) {
                if (interactive) showError(`Accès au dossier "${wd.name}" refusé.`);
                return { refreshed: 0, imported: 0, skipped: 0, denied: true };
            }
            let refreshed = 0,
                imported = 0,
                skipped = 0;
            try {
                for await (const entry of wd.handle.values()) {
                    ...
                    if (!['csv', 'txt', 'xlsx', 'xls'].includes(ext)) continue;
                    const existing = Object.keys(state.tables).find(
                        tid => state.tables[tid].name.toLowerCase() === entry.name.toLowerCase()
                    );
                    const file = await entry.getFile();
                    // Le nom du dossier surveillé initialise le THÈME de la source (modifiable ensuite).
                    if (existing) {
                        const table = state.tables[existing];
                        if (table && !table.theme) table.theme = wd.name;
                        try {
                            const r = await refreshTableFromFile(existing, file, { skipUnchanged: true });
                        ...
                        if (nid) {
                            imported++;
                            const table = state.tables[nid];
                            if (table && !table.theme) {
                                table.theme = wd.name;
                                persistAppState();
                                renderTables();
                            }
                ...
                if (interactive) showError('Lecture du dossier impossible : ' + e.message);
        ...
        async function scanWatchedDir(id, importNew, interactive) {
            const wd = state.watchedDirs.find(d => d.id === id);
            if (!wd) return;
            if (_srcScanBusy) {
                if (interactive) showSuccess('⏳ Un scan est déjà en cours — patientez, inutile de recliquer.');
                return;
            }
            _srcScanBusy = true;
            renderSourceUpdatePanel();
            ...
            try {
                ...
                if (interactive)
                    bgTaskEnd(
                        r && !r.denied
                            ? `📂 Dossier "${wd.name}" : ${r.refreshed} mise(s) à jour, ${r.imported} import(s), ${r.skipped} inchangé(s).`
                            : undefined
                    );
                return r;
            } finally {
        ...
        async function scanAllWatchedDirs(importNew, interactive) {
            if (!state.watchedDirs.length) {
                if (interactive) showError("Aucun dossier surveillé. Ajoutez-en un d'abord.");
                return;
            }
            if (_srcScanBusy) {
                if (interactive) showSuccess('⏳ Un scan est déjà en cours — patientez, inutile de recliquer.');
            ...
            try {
                let R = 0,
                    ...
                    S = 0,
                    any = false;
                for (const d of state.watchedDirs) {
                    const r = await _scanDirImpl(d, importNew, interactive);
                    if (r) {
                        R += r.refreshed || 0;
                        I += r.imported || 0;
                        S += r.skipped || 0;
                ...
                if (interactive)
                    bgTaskEnd(
                        any ? `🔄 Rafraîchissement terminé : ${R} mise(s) à jour, ${I} import(s), ${S} inchangé(s).` : undefined
                    );
            } finally {
                _srcScanBusy = false;
                renderSourceUpdatePanel();
        ...
        function setAutoRefresh(on) {
            state.autoRefresh.on = on;
            scheduleAutoRefresh();
            saveUpdatePrefs();
            renderSourceUpdatePanel();
        }
        function setAutoRefreshMinutes(m) {
            state.autoRefresh.minutes = parseInt(m) || 5;
            scheduleAutoRefresh();
            saveUpdatePrefs();
        }
        function scheduleAutoRefresh() {
            ...
            if (state.autoRefresh.on && state.watchedDirs.length) {
                _autoRefreshTimer = setInterval(
                    () => {
                        scanAllWatchedDirs(false, false).catch(() => {});
                    },
                    Math.max(1, state.autoRefresh.minutes) * 60000
                );
            }
        }
        function toggleSourceUpdatePanel() {
            ...
            const chev = el('srcUpdateChevron');
        ...
        function renderSourceUpdatePanel() {
            ...
            if (badge)
                badge.textContent = state.watchedDirs.length
                    ? `— ${state.watchedDirs.length} dossier(s)${state.autoRefresh.on ? ', auto ' + state.autoRefresh.minutes + ' min' : ''}`
                    : '';
            ...
            try {
                ...
                if (Array.isArray(dirs))
                    state.watchedDirs = dirs
                        .filter(d => d && d.handle)
                        .map(d => ({ id: d.id || 'wd_' + generateId(), name: d.name, handle: d.handle }));
                const prefs = await idbGet('meta', 'updatePrefs');
                if (prefs && prefs.autoRefresh)
                    state.autoRefresh = { on: !!prefs.autoRefresh.on, minutes: prefs.autoRefresh.minutes || 5 };
                scheduleAutoRefresh();
            } catch (e) {}
        }

        async function updateTableConfig(tId, f, v) {
            state.tables[tId].config[f] = v;
            state.tables[tId].status = 'loading';
            renderTables();
            try {
                const headers = await ingestFileTable(tId);
                const table = state.tables[tId];
        ...
        function removeTable(tId) {
            ...
            state.relations = state.relations.filter(r => r.sourceTable !== tId && r.targetTable !== tId);
            delete state.selectedCols[tId];
            delete state.pivotMode[tId];
            delete state.filters[tId];
            delete state.hierarchyConfig[tId];
            delete state.graphConfig[tId];
        ...
        function comboifyTableSelect(selId) {
            ...
            const inp = document.createElement('input');
            inp.type = 'text';
            inp.placeholder = '🔍 taper pour filtrer les tables…';
            inp.className =
                'w-full border border-slate-200 rounded px-2 py-1 text-xs mb-1 bg-slate-50 focus:bg-white focus:border-indigo-400';
            inp.addEventListener('input', () => {
                const normalized = srcNorm(inp.value);
                let first = null;
                Array.from(sel.options).forEach(o => {
                    const m = !o.value || !normalized || srcNorm(o.textContent).includes(normalized);
                    o.hidden = !m;
                    if (m && o.value && !first) first = o;
                });
                if (normalized && first) {
                    if (sel.value !== first.value) {
        ...
        function renderTables() {
            ...
            if (!grid) return;
            let _gridHtml = '';
            const themeDatalistElement = el('themeDatalist');
            if (themeDatalistElement)
                themeDatalistElement.innerHTML = themeList()
                    .map(th => `<option value="${escapeHTML(th)}">`)
                    .join('');
            ...
            const _shown = !_q
                ...
                : _all.filter(
                      t =>
                          srcNorm(t.name).includes(_q) ||
                          srcNorm(t.theme || '').includes(_q) ||
                          (t.headers || []).some(h => srcNorm(h).includes(_q))
                  );
            const _cnt = el('srcSearchCount');
            if (_cnt)
                _cnt.textContent = _q
                    ? `${_shown.length} / ${_all.length} source(s)`
                    : _all.length
                      ? `${_all.length} source(s)`
                      : '';
            const _clr = el('srcSearchClear');
            if (_clr) _clr.classList.toggle('hidden', !_q);
            if (_q && !_shown.length)
                _gridHtml = `<div class="col-span-full text-center text-sm text-slate-400 italic py-10 bg-white border border-dashed border-slate-200 rounded-xl">Aucune source ne correspond à « ${escapeHTML(window.srcSearchQ)} ».</div>`;
            if (!_q && !_all.length)
                _gridHtml = emptyStateHtml(
                    '📥',
                    ...
                    "el('fileUploader') && el('fileUploader').click()"
                );
            _shown.forEach(t => {
                    ...
                    t.type === 'api'
                        ...
                        : t.type === 'extraction'
                          ...
                          : t.type === 'designed'
                            ? 'layers'
                            : 'file';
                html += `<div class="flex items-center gap-3 mb-3"><div class="${t.type === 'designed' ? 'bg-blue-100 p-2 rounded text-blue-600' : 'bg-indigo-100 p-2 rounded text-indigo-600'}"><i data-lucide="${typeIcon}" class="w-5 h-5"></i></div>
                    <div class="truncate"><h3 class="font-bold text-sm" title="${escapeHTML(t.name)}">${escapeHTML(t.name)}</h3>${t.type === 'extraction' ? '<span class="text-[9px] uppercase font-bold text-purple-600 bg-purple-50 border border-purple-100 px-1.5 py-0.5 rounded">Source dérivée (Extraction)</span>' : t.type === 'designed' ? '<span class="text-[9px] uppercase font-bold text-blue-600 bg-blue-50 border border-blue-100 px-1.5 py-0.5 rounded">🧱 Table conçue — gérée dans l\'onglet Tables</span>' : t.lastRefresh ? `<span class="text-[9px] font-bold text-emerald-600" title="Dernière mise à jour">🔄 maj ${new Date(t.lastRefresh).toLocaleString('fr-FR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}</span>` : ''}</div>
                    </div>`;
                if (t.status === 'loading') html += `<div class="text-indigo-600 text-sm">Analyse...</div>`;
                    ...
                    if ((t.type === 'csv' || t.type === 'txt' || Array.isArray(t.files)) && isQuote)
                        html += `<button onclick="repairCsvSource('${t.id}')" class="mt-2 text-xs bg-amber-500 hover:bg-amber-600 text-white px-3 py-1.5 rounded font-bold w-full">🔧 Réparer : lire sans guillemets + ignorer les lignes fautives</button>`;
                } else {
                    const storageBadge =
                        t.storage === 'parquet'
                            ? `<div class="text-[10px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-100 rounded px-1.5 py-0.5 mb-1.5 inline-block" title="Source convertie en Parquet compressé : format colonnaire, analyses 5 à 20× plus rapides, lu hors de la mémoire.">🗜️ Optimisée (Parquet)${t.pqSize ? ` · ${(t.pqSize / 1048576).toFixed(1)} Mo` : ''}</div>
                                <br>`
                            : t.storage === 'view'
                              ? `<div class="text-[10px] font-bold text-cyan-700 bg-cyan-50 border border-cyan-100 rounded px-1.5 py-0.5 mb-1.5 inline-block" title="Gros fichier : lu à la demande depuis le disque, jamais copié en mémoire — les analyses relisent le fichier (un peu plus lentes mais sans limite de taille mémoire). Cliquez « Optimiser » pour accélérer.">⚡ Lecture directe (gros fichier)</div>
                                  <br>`
                              : '';
                    const mergedBadge =
                        Array.isArray(t.files) && t.files.length
                            ? `<div class="text-[10px] font-bold text-purple-600 bg-purple-50 border border-purple-100 rounded px-1.5 py-0.5 mb-1.5 inline-block">🧩 Source fusionnée · ${t.files.length} fichiers</div>
                                <br>`
                            : '';
                ...
                if (
                    ...
                    if (_mi)
                        html += `<div class="mt-3 flex items-center gap-2 flex-wrap text-[11px] bg-amber-50 border border-amber-200 rounded-lg px-2.5 py-2">
        ...
        function addManualRelation() {
            ...
            state.relations.push({
                id: 'r_' + generateId(),
                sourceTable: tableIds[0],
                sourceCol: '',
                targetTable: tableIds[1] || tableIds[0],
                targetCol: ''
            });
            renderRelationsList();
            renderGraph();
        ...
        function updateRelation(id, f, v) {
            const relation = state.relations.find(x => x.id === id);
            if (relation) {
                relation[f] = v;
                renderRelationsList();
                renderGraph();
        ...
        function fillRelColSel(sel, tId) {
            ...
            sel.innerHTML =
                ...
                (table ? table.headers.map(h => `<option value="${escapeHTML(h)}">${escapeHTML(h)}</option>`).join('') : '');
            sel.value = cur;
        }
        function renderRelationsList() {
            ...
            scoped.forEach(r => {
                const curOpt = (v, lbl) =>
                    v
                        ? `<option value="${escapeHTML(v)}" selected>${escapeHTML(lbl)}</option>`
                ...
                _rowsHtml += `<div class="px-6 py-3 hover:bg-slate-50 border-b border-slate-50">
                    ...
                    <div class="flex items-center gap-3 mt-2 flex-wrap pl-1">
                        ...
                        <label class="text-[10px] uppercase font-bold text-slate-400 flex items-center gap-1">Nature
                            <select onchange="updateRelation('${r.id}','kind',this.value)" class="border border-slate-200 p-1 rounded text-xs bg-white">${[
                                ['', '— association'],
                                ['composition', '◆ composition (fait partie de)'],
                                ['aggregation', '◇ agrégation'],
                                ['reference', '→ référence']
                            ]
                                .map(([v, l]) => `<option value="${v}" ${(r.kind || '') === v ? 'selected' : ''}>${l}</option>`)
                                .join('')}</select>
                        </label>
        ...
        function tableHierarchyInfo() {
            ...
            state.relations.forEach(r => {
                if (r.sourceCol && r.targetCol && r.sourceTable === r.targetTable && info[r.sourceTable]) {
                    info[r.sourceTable].self = r;
                    info[r.sourceTable].relIds.add(r.id);
                }
            });
            const pairRels = {};
            state.relations.forEach(r => {
            ...
            Object.entries(pairRels).forEach(([key, rels]) => {
                ...
                let subject = null,
                    link = null;
                if (colsX.size === 1 && colsY.size > 1) {
                    subject = x;
                    link = y;
                } else if (colsY.size === 1 && colsX.size > 1) {
                    subject = y;
                    link = x;
                }
                if (subject && info[subject]) {
                    ...
                    rels.forEach(r => {
                        info[subject].relIds.add(r.id);
                        if (info[link]) info[link].relIds.add(r.id);
                    });
                }
            });
            return info;
        }
        function isHierTable(info, id) {
            return !!(info[id] && (info[id].self || info[id].viaLinks.length));
        }
        function hierNodeContent(info, id) {
            const h = info[id];
            if (!h) return '';
            if (h.self) return `hiérarchie interne (${h.self.sourceCol} ↔ ${h.self.targetCol})`;
            if (h.viaLinks.length)
                return (
                    'hiérarchie via ' +
                    h.viaLinks.map(v => (state.tables[v.linkId] ? state.tables[v.linkId].name : '?')).join(', ')
                );
            return '';
        }
        ...
        function exportGraphImage(which) {
            ...
            try {
                g.downloadFullImage('StudioData_' + which + '_' + Date.now(), 'image/png', {
                    backgroundColor: '#ffffff',
                    padding: [20, 20, 20, 20]
                });
            } catch (e) {
                showError('Export du graphe impossible : ' + e.message);
            }
        }
        ...
        function svgExportImage() {
            try {
                const svg = el('mcdSvg');
                if (!svg) return showError("Affichez d'abord le graphe.");
                const ids = Object.keys(svgG.pos);
                if (!ids.length) return showError('Rien à exporter.');
                let x1 = 1e9,
                    y1 = 1e9,
                    x2 = -1e9,
                    y2 = -1e9;
                ids.forEach(id => {
                    const position = svgG.pos[id],
                        s = svgG.size[id] || [180, 60];
                    x1 = Math.min(x1, position.x - s[0] / 2);
                    y1 = Math.min(y1, position.y - s[1] / 2);
                    x2 = Math.max(x2, position.x + s[0] / 2);
                    y2 = Math.max(y2, position.y + s[1] / 2);
                });
                const M = 40,
                    w = Math.ceil(x2 - x1 + 2 * M),
                    h = Math.ceil(y2 - y1 + 2 * M);
                const element = el('mcdVp').cloneNode(true);
                element.setAttribute('transform', `translate(${M - x1},${M - y1})`);
                const out = `<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}"><defs><marker id="svgArrow" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse"><path d="M0 0 L10 5 L0 10 z" fill="#94a3b8"/></marker></defs><rect width="${w}" height="${h}" fill="#fff"/>${new XMLSerializer().serializeToString(element)}</svg>`;
                const img = new Image();
                img.onload = () => {
                    ...
                    anchorElement.href = canvasElement.toDataURL('image/png');
                    anchorElement.download = 'StudioData_modele_' + Date.now() + '.png';
                ...
                showError('Export image impossible : ' + e.message);
            }
        }

        // Symbole UML d'une relation pour les graphes et libellés.
        function relKindSymbol(r) {
            return r.kind === 'composition' ? '◆' : r.kind === 'aggregation' ? '◇' : r.kind === 'reference' ? '→' : '';
        }
        ...
        async function measureRelation(r) {
            ...
            const smax = Number(row.smax),
                tmax = Number(row.tmax);
            return {
                stotal: Number(row.stotal),
                ttotal: Number(row.ttotal),
                smax,
                tmax,
                sorph: Number(row.sorph),
                torph: Number(row.torph),
        ...
        async function measureRelationCardinality(relId) {
            ...
            if (!relation || !relation.sourceCol || !relation.targetCol) return;
            const btn = el('btn-meas-' + relId);
            if (btn) {
                btn.disabled = true;
            ...
            try {
                ...
                if (!relation.cardinality) relation.cardinality = relation.measured.suggested;
                persistAppState();
                renderRelationsList();
                renderGraph();
            } catch (e) {
                showError('Mesure impossible : ' + e.message);
                if (btn) {
                    btn.disabled = false;
                    btn.textContent = '📐 Mesurer sur les données';
                }
            }
        }
        ...
        function renderGraph() {
            const cont = el('networkGraph');
            if (!cont || !Object.keys(state.tables).length) return;
            if (networkInstance) {
                try {
            ...
            try {
                ...
                console.error('MCD SVG error', e);
                cont.innerHTML = graphUnavailableHtml('Rendu du modèle impossible : ' + escapeHTML(e.message));
            }
            renderMcdControls();
        }
        // Contrôles communs du modèle (périmètre, filtre domaine, boîte à outils).
        function renderMcdControls() {
            const mcdToolbarElement = el('mcdToolbar');
            if (mcdToolbarElement && !mcdToolbarElement.innerHTML) mcdToolbarElement.innerHTML = graphToolbarHtml('mcd');
            document.querySelectorAll('.mcdScopeWrap').forEach(w => {
                w.innerHTML = mcdScopeButtonsHtml() + mcdThemeFilterHtml();
            });
            document.querySelectorAll('.mcdScopeMsg').forEach(w => {
                const emptyDesigned =
                    mcdScope() === 'designed' && !Object.values(state.tables).some(t => t.type === 'designed');
                w.classList.toggle('hidden', !emptyDesigned);
                w.innerHTML = emptyDesigned
                    ? "🧱 Aucune table conçue pour l'instant — créez-en une dans l'onglet <strong>Tables</strong> ; son modèle apparaîtra ici, séparé de celui des sources."
                    : '';
            });
        }
