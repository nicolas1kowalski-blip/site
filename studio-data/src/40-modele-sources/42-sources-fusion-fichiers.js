        ...
        function openMergeModal() {
            _mergePicked = [];
            renderMergeModal();
            const mergeModalElement = el('mergeModal');
            mergeModalElement.classList.remove('hidden');
            mergeModalElement.classList.add('flex');
        }
        function closeMergeModal() {
            const mergeModalElement = el('mergeModal');
            mergeModalElement.classList.add('hidden');
            mergeModalElement.classList.remove('flex');
        }
        function mergePickFiles(e) {
            for (const f of Array.from(e.target.files)) _mergePicked.push(f);
            e.target.value = '';
            renderMergeModal();
        }
        function removeMergePicked(i) {
            _mergePicked.splice(i, 1);
            renderMergeModal();
        }
        function renderMergeModal() {
            const body = el('mergeModalBody');
            if (!body) return;
            const fileSrcs = Object.values(state.tables).filter(
                t => t.status === 'ready' && (t.file || (Array.isArray(t.files) && t.files.length))
            );
            body.innerHTML = `
        ...
        async function createMergedSource() {
            ...
            if (Object.values(state.tables).some(t => t.name.toLowerCase() === name.toLowerCase()))
                return showError(`Une source "${name}" existe déjà.`);
            ...
            chosenIds.forEach(id => {
                ...
                if (table) {
                    if (Array.isArray(table.files)) files.push(...table.files);
                    else if (table.file) files.push(table.file);
                }
            });
            ...
            if (!allCsv && !allXlsx)
                return showError('Gardez le même format pour tous les fichiers (tous CSV/TXT, ou tous Excel).');
            const tId = 'tb_' + generateId();
            state.tables[tId] = {
                id: tId,
                name,
                files,
                parts: files.map(f => f.name),
                type: allCsv ? 'csv' : 'xlsx',
            ...
            try {
                ...
                table.status = 'ready';
                if (removeOrig && chosenIds.length)
                    chosenIds.forEach(id => {
                        if (state.tables[id]) removeTable(id);
                    });
                renderTables();
