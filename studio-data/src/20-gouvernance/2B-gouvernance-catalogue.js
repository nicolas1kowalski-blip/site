        ...
        function catOpenByKey(k) {
            const find = () =>
                catRes.findIndex(
                    r =>
                        r.type === k.type &&
                        (k.type === 'column'
                            ? r.tbl === k.tbl && r.col === k.col
                            : k.type === 'bo'
                              ? r.bo === k.bo
                              : k.type === 'attr'
                                ? r.bo === k.bo && r.elId === k.elId
                                : k.type === 'term'
                                  ? (r.termId || r.id) === k.id
                                  : k.tbl
                                    ? r.tbl === k.tbl
            ...
            if (i < 0 && k.type === 'attr') {
                ...
                const e2 = ix.find(r => r.type === 'attr' && r.bo === k.bo && r.elId === k.elId);
                if (e2) {
                    catRes.push(e2);
                    i = catRes.length - 1;
                }
            }
            if (i < 0 && CAT_TECH_TYPES.includes(k.type) && catState.layer !== 'tout') {
                catState.layer = 'tout';
                renderGovernance();
                i = find();
            }
            if (i < 0) {
                CAT_FACETS.forEach(f => (catState.f[f.key] = []));
                catState.q = '';
                renderGovernance();
                i = find();
            }
        ...
        function catKeyAttr(k) {
            return JSON.stringify(k).replace(/"/g, '&quot;');
        }
        const CAT_TYPES = {
            table: ['t-table', '▦', 'Table'],
            view: ['t-view', '◧', 'Vue dérivée'],
            column: ['t-attr', '◈', 'Colonne'],
            attr: ['t-obj', '🔹', 'Attribut métier'],
            bo: ['t-obj', '🏛', 'Objet métier'],
            report: ['t-report', '📊', 'Rapport'],
            term: ['t-ref', '📖', 'Terme'],
            perimeter: ['t-ref', '🧩', 'Périmètre'],
            usecase: ['t-report', '🎯', "Cas d'usage"],
            asset: ['t-ref', '🖥', 'Appli / processus'],
            rule: ['t-ref', '📏', 'Règle qualité'],
            recipe: ['t-view', '🧹', 'Préparation'],
            linkage: ['t-ref', '🤝', 'Rapprochement']
        };
        const CAT_SENS = {
            perso: ['s-perso', '🛡 Personnelle'],
            conf: ['s-conf', '🔒 Confidentiel'],
            non: ['s-non', 'Non sensible']
        };
        const CAT_VAL = { ok: ['v-ok', '✓ Validé'], todo: ['v-todo', '◷ À valider'], draft: ['v-draft', '◌ Brouillon'] };
        function catNorm(s2) {
            return String(s2 == null ? '' : s2)
                .normalize('NFD')
                .replace(/[\u0300-\u036f]/g, '')
                .toLowerCase();
        }
        function catOwnColor(nm) {
            const P2 = ['#2563eb', '#059669', '#8b5cf6', '#f59e0b', '#0ea5e9', '#dc2626', '#0f172a'];
            let h = 0;
            for (const ch of String(nm)) h = (h * 31 + ch.charCodeAt(0)) >>> 0;
            return P2[h % P2.length];
        }
        function catSensOf(level) {
            return level === 'personnel' ? 'perso' : level === 'confidentiel' ? 'conf' : level ? 'non' : null;
        }
        function catValOf(st2) {
            return st2 === 'Validé' ? 'ok' : st2 === 'Proposé' ? 'todo' : st2 ? 'draft' : null;
        }
        function catQOf(tableName) {
            ...
            if (sn && sn.tables && sn.tables[tableName] && sn.tables[tableName].score != null)
                return Math.round(sn.tables[tableName].score);
        ...
        function termById(id) {
            return (state.governance.glossary || []).find(t => t.id === id) || null;
        }
        function termNameOf(id) {
        ...
        function termTargets(t2) {
            ...
            (t2.boIds || []).forEach(id => {
                ...
                if (bo) out.push({ kind: 'bo', boId: bo.id, label: bo.name, via: 'objet métier rattaché' });
            });
            // V9.2 : attributs — liens du glossaire ET champ « Terme » de l'attribut, dédoublonnés
            const seenAttr = new Set();
            (t2.attrLinks || []).forEach(l => {
                const h = termAttrRow(l);
                if (h && !seenAttr.has(l.elId)) {
                    seenAttr.add(l.elId);
                    out.push({
                        kind: 'attr',
                        boId: h.bo.id,
                        elId: l.elId,
                        stId: h.r.stId || '',
                        label: h.bo.name + ' · ' + h.r.el.name,
                        via: 'attribut désigné par ce terme'
                    });
                }
            });
            (state.governance.businessObjects || []).forEach(bo =>
                (boAllAttrRows(bo) || []).forEach(r => {
                    if (r.el.term === t2.id && !seenAttr.has(r.el.id)) {
                        seenAttr.add(r.el.id);
                        out.push({
                            kind: 'attr',
                            boId: bo.id,
                            elId: r.el.id,
                            stId: r.stId || '',
                            label: bo.name + ' · ' + r.el.name,
                            via: 'attribut portant ce terme'
                        });
                    }
                })
            );
            (t2.assetIds || []).forEach(id => {
                const asset = assetById(id);
                if (asset)
                    out.push({
                        kind: 'asset',
                        assetId: asset.id,
                        label: (ASSET_KINDS[asset.kind] ? ASSET_KINDS[asset.kind][0] + ' ' : '') + asset.name,
                        via: asset.kind === 'process' ? 'processus rattaché' : 'application rattachée'
                    });
            });
            (t2.links || []).forEach(l =>
                out.push({
                    ...
                    col: l.col,
                    label: l.table + '.' + l.col,
                    via: 'ancien lien fichier (héritage)'
                })
            );
            Object.entries(state.governance.dictionary || {}).forEach(([tn, d]) =>
                Object.entries(d.columns || {}).forEach(([cn, cm]) => {
        ...
        function termsOfColumn(tn, cn) {
            ...
            (state.governance.glossary || []).forEach(t2 => {
                if (
                    (t2.links || []).some(l => l.table === tn && l.col === cn) ||
                    cm.term === t2.id ||
                    attrs.some(
                        a => a.r.el.term === t2.id || (t2.attrLinks || []).some(x => x.boId === a.boId && x.elId === a.elId)
                    )
                )
                    out.push(t2);
            });
        ...
        function catBuildIndex() {
            ...
            const push = e2 => {
                e2.blob = catNorm(
                    [e2.title, e2.sub, e2.desc, e2.dom, e2.own, (e2.tags || []).join(' '), (e2.kw || []).join(' ')].join(' ')
                );
                ix.push(e2);
            };
            Object.values(state.tables)
                ...
                .forEach(t => {
                    const d = governance.dictionary[t.name] || {};
                    let worst = null;
                    (t.headers || []).forEach(h => {
                        const lv = pvLevelOf(t.name, h);
                        if (lv === 'personnel') worst = 'personnel';
                        else if (lv === 'confidentiel' && worst !== 'personnel') worst = 'confidentiel';
                        else if (!worst) worst = lv;
                    });
                    push({
                        type: t.type === 'extraction' ? 'view' : 'table',
                        id: t.id,
                        title: t.name,
                        sub: (t.headers || []).length + ' colonnes',
                        desc: d.description || '',
                        dom: String(t.theme || '').trim() || '—',
                        own: d.owner || '',
                        q: catQOf(t.name),
                        sens: catSensOf(worst),
                        val: catValOf(d.status),
                        fresh: t.lastRefresh || null,
                        rows: t.lastRows != null ? t.lastRows : null,
                        tags: [t.type === 'extraction' ? 'dérivée' : t.type || 'table', String(t.theme || '').trim()].filter(
                            Boolean
                        ),
                        tbl: t.name
                    });
                    ...
                    (t.headers || []).forEach(h => {
                        const term = (d.columns || {})[h] || {};
                        const def = term.definition || (colDefFor(t.name, h) || {}).def || '';
                        const comp =
                            lq && lq.columnsSummary && lq.columnsSummary[h] ? Math.round(lq.columnsSummary[h].comp) : null;
                        ...
                        push({
                            ...
                            own: d.owner || '',
                            q: comp,
                            sens: catSensOf(pvLevelOf(t.name, h)),
                            val: term.term || def ? 'ok' : null,
                            fresh: t.lastRefresh || null,
                            tags: [...syns.map(x => '📖 ' + x), term.technicalType].filter(Boolean),
                            tbl: t.name,
                            col: h,
                            syns
                        });
            ...
            (governance.businessObjects || []).forEach(bo => {
                const syns = (governance.glossary || []).filter(t2 => (t2.boIds || []).includes(bo.id)).map(x => x.term);
                ...
                const attrTerms = Array.from(
                    new Set((boAllAttrRows(bo) || []).flatMap(r => termsOfAttr(bo.id, r.el.id).map(x => x.term)))
                );
                push({
                    type: 'bo',
                    id: bo.id,
                    title: bo.name,
                    sub: (boAllAttrRows(bo) || []).length + ' attribut(s)',
                    desc: bo.definition || '',
                    dom: String(bo.domain || '').trim() || '—',
                    own: bo.globalOwner || '',
                    q: null,
                    sens: null,
                    val: catValOf(bo.status),
                    fresh: null,
                    tags: ['objet métier', ...syns.map(x => '📖 ' + x)],
                    kw: attrTerms,
                    bo: bo.id,
                    syns
                });
                ...
                (boAllAttrRows(bo) || []).forEach(r => {
                    const ts = termsOfAttr(bo.id, r.el.id).map(x => x.term);
                    const prov = boAttrProv(r);
                    push({
                        type: 'attr',
                        id: bo.id + '\u0001' + r.el.id,
                        title: r.el.name,
                        sub: 'attribut de ' + bo.name + (r.facet ? ' ◆ ' + r.facet : ''),
                        desc: r.el.definition || '',
                        dom: '—',
                        own: r.el.owner || bo.globalOwner || '',
                        q: null,
                        sens: r.el.sensitivity
                            ? /personnel/i.test(r.el.sensitivity)
                                ? 'perso'
                                : /sensible|confid/i.test(r.el.sensitivity)
                                  ? 'conf'
                                  : 'non'
                            : null,
                        val: r.el.definition ? 'ok' : null,
                        fresh: null,
                        tags: ['attribut métier', ...ts.map(x => '📖 ' + x)],
                        kw: [prov, bo.name],
                        bo: bo.id,
                        elId: r.el.id,
                        stId: r.stId || '',
            ...
            (governance.glossary || []).forEach(t2 => {
                ...
                push({
                    ...
                    id: t2.id,
                    title: t2.term,
                    sub: tg.length ? 'désigne ' + tg.length + ' donnée(s)' : 'aucune donnée désignée',
                    desc: t2.definition || '',
                    dom: '—',
                    own: '',
                    q: null,
                    sens: null,
                    val: t2.definition ? 'ok' : null,
                    fresh: null,
                    tags: ['glossaire', ...tg.slice(0, 6).map(x => x.label)],
                    termId: t2.id
                });
            });
            (governance.perimeters || []).forEach(p2 =>
                push({
                    type: 'perimeter',
                    id: p2.id,
                    title: p2.name,
                    sub: (p2.tables || []).length + ' table(s)',
                    desc: p2.description || '',
                    dom: '—',
                    own: '',
                    q: null,
            ...
            (governance.assets || []).forEach(a => {
                const syns = termsOfAsset(a.id).map(x => x.term);
                push({
                    type: 'asset',
                    id: a.id,
                    title: a.name,
                    sub:
                        a.kind === 'process'
                            ? 'Processus' + ((a.appIds || []).length ? ' · ' + (a.appIds || []).length + ' appli(s)' : '')
                            : 'Application',
                    desc: a.description || '',
                    dom: assetDomainEff(a) || '—',
                    own: a.owner || '',
                    q: null,
                    sens: null,
                    val: null,
                    fresh: null,
                    tags: [a.kind === 'process' ? 'processus' : 'application', ...syns.map(x => '📖 ' + x)],
                    syns
                });
            });
            (governance.qualityRules || []).forEach(r =>
                push({
                    type: 'rule',
                    id: r.id,
                    title: r.name,
                    sub: (r.bo ? '🏛 ' + ((qrBoById(r.bo) || {}).name || '?') : r.table || '') + (r.col ? ' · ' + r.col : ''),
                    desc: (QR_TYPES[r.type] || r.type) + ' — criticité ' + (r.crit || '—'),
                    dom: '—',
                    own: '',
                    q: r.last ? Math.round(100 * r.last.rate) : null,
                    sens: null,
                    val: null,
                    fresh: r.last ? r.last.at : null,
                    tags: ['règle'],
                    tbl: r.table
                })
            );
            (state.recipes || []).forEach(r =>
                push({
                    ...
                    desc:
                        (r.steps || []).length +
                        ' étape(s) de nettoyage, rejouées automatiquement à chaque mise à jour de la source.',
                    dom: '—',
                    own: '',
                    q: null,
                    sens: null,
            ...
            (state.dashboards || []).forEach(d2 =>
                push({
                    type: 'report',
                    id: d2.id,
                    title: d2.name,
                    sub: (d2.tiles || []).length + ' tuile(s)',
                    desc:
                        'Tableau de bord composable : ' +
                        (d2.tiles || [])
                            .map(t2 => t2.title)
                            .filter(Boolean)
                            .slice(0, 4)
            ...
            (state.linkages || []).forEach(L =>
                push({
                    ...
                    title: L.name,
                    sub: (L.a || '?') + ' ↔ ' + (L.b || '?'),
                    desc: 'Rapprochement inter-sources (golden record).',
                    dom: '—',
                    own: '',
                    q: null,
                    sens: null,
        ...
        const CAT_FACETS = [
            {
                ...
                title: "Type d'actif",
                of: e2 => e2.type,
                opts: () => Object.keys(CAT_TYPES),
                lab: v => (CAT_TYPES[v] ? CAT_TYPES[v][2] : v)
            },
            {
                key: 'dom',
                title: 'Domaine métier',
                of: e2 => e2.dom,
                opts: ix => Array.from(new Set(ix.map(e2 => e2.dom))).sort(),
                lab: v => v
            },
            {
                key: 'sens',
            ...
            },
            {
                key: 'qb',
                title: 'Qualité',
                of: e2 => (e2.q == null ? null : e2.q >= 90 ? 'hi' : e2.q >= 70 ? 'mid' : 'lo'),
                opts: () => ['hi', 'mid', 'lo'],
                lab: v => ({ hi: 'Bonne (≥ 90)', mid: 'Moyenne (70–90)', lo: 'À surveiller (< 70)' })[v]
            },
            { key: 'val', title: 'Validation', of: e2 => e2.val, opts: () => ['ok', 'todo', 'draft'], lab: v => CAT_VAL[v][1] },
            {
                ...
                title: 'Propriétaire',
                of: e2 => e2.own || null,
                opts: ix => Array.from(new Set(ix.map(e2 => e2.own).filter(Boolean))).sort(),
                lab: v => v
            }
        ];
        function catMatch(e2, skipKey, ignoreLayer) {
            if (!ignoreLayer && skipKey !== 'type' && !catLayerOk(e2)) return false;
            if (e2.type === 'attr' && skipKey !== 'type' && !catState.q && !(catState.f.type || []).includes('attr'))
                return false;
            if (catState.q) {
                const toks = catNorm(catState.q).split(/\s+/).filter(Boolean);
                if (!toks.every(tk => e2.blob.includes(tk))) return false;
            }
            for (const f of CAT_FACETS) {
                if (f.key === skipKey) continue;
                const sel2 = catState.f[f.key];
                if (!sel2.length) continue;
                if (!sel2.includes(f.of(e2))) return false;
            }
            return true;
        }
        function catToggleFacet(key, v) {
            const a = catState.f[key];
            const i = a.indexOf(v);
            if (i >= 0) a.splice(i, 1);
            else a.push(v);
            renderGovernance();
        }
        function catClearFacets() {
            CAT_FACETS.forEach(f => (catState.f[f.key] = []));
            catState.q = '';
            renderGovernance();
        }
        ...
        function catSort(list) {
            ...
            if (catState.sort === 'qualite') l2.sort((a, b) => (b.q == null ? -1 : b.q) - (a.q == null ? -1 : a.q));
                ...
                const toks = catNorm(catState.q).split(/\s+/).filter(Boolean);
                const sc = e2 => {
                    const normalized = catNorm(e2.title);
                    return toks.reduce(
                        (a2, tk) =>
                            a2 + (normalized === tk ? 30 : normalized.startsWith(tk) ? 20 : normalized.includes(tk) ? 10 : 1),
                        0
                    );
                };
        ...
        function catCardHtml(e2, i) {
            ...
            return `<div class="cat-card" onclick="catOpenFiche(${i})" tabindex="0" role="button" aria-label="Fiche de ${escapeHTML(e2.title)}" onkeydown="if(event.key==='Enter')catOpenFiche(${i})">
                ...
                <div class="flex items-center gap-1.5 mt-2.5 flex-wrap">
                    ...
                    ${
                        e2.own
                            ? `<span class="inline-flex items-center gap-1.5 text-[11px] text-slate-500"><span class="cat-oav" style="background:${catOwnColor(e2.own)}">${escapeHTML(
                                  e2.own
                                      .split(/\s+/)
                                      .map(x => x[0] || '')
                                      .join('')
        ...
        function renderGovCatalog() {
            ...
            const docd = tablesN.filter(t => (governance.dictionary[t.name] || {}).description).length;
            ...
            const techHint =
                catState.layer !== 'tout' && techHidden
                    ...
                    : '';
            const chip = [];
            CAT_FACETS.forEach(f =>
                catState.f[f.key].forEach(v =>
                    chip.push(
                        `<button data-ro="keep" onclick="catToggleFacet('${f.key}',${JSON.stringify(String(v)).replace(/"/g, '&quot;')})" class="achip-ux" aria-label="Retirer le filtre ${escapeHTML(f.lab(v))}">${escapeHTML(f.lab(v))} ✕</button>`
                    )
                )
            );
            const facetsHtml = CAT_FACETS.map(f => {
                ...
                const rows = opts
                    ...
                    .join('');
                return rows
                    ? `<div class="border-b border-slate-100 pb-3 mb-3"><div class="text-[10px] uppercase font-black tracking-wide text-slate-400 mb-1.5">${f.title}</div>${rows}</div>`
                    : '';
            }).join('');
            const exs = [
                tablesN[0] ? tablesN[0].name.replace(/\..*$/, '') : 'client',
                'données personnelles',
                (governance.businessObjects[0] || {}).name,
                Object.values(governance.dictionary)
                    .map(d => d.owner)
                    .find(Boolean)
            ]
                .filter(Boolean)
            ...
            <div class="flex gap-4 items-start">
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 mb-2.5 flex-wrap">
                        ...
                        <select data-ro="keep" onchange="catSet('sort', this.value)" class="border border-slate-200 rounded-lg px-2 py-1.5 text-xs font-bold bg-white" aria-label="Tri des résultats">
                            ${[
                                ...
                                .map(([v, l]) => `<option value="${v}" ${catState.sort === v ? 'selected' : ''}>${l}</option>`)
                                .join('')}</select>
                    ...
                    ${
                        list.length
                            ? `<div class="grid gap-3 ${catState.view === 'grid' ? 'md:grid-cols-2' : 'grid-cols-1'}">${list
                                  .slice(0, 60)
                                  .map((e2, i) => catCardHtml(e2, i))
                                  .join('')}</div>
                        ${list.length > 60 ? `<p class="text-[11px] text-slate-400 italic mt-2">… ${list.length - 60} autre(s) — précisez la recherche.</p>` : ''}`
                            : techHidden && catState.layer !== 'tout'
                              ? emptyStateHtml(
                                    '🏛',
                                    'Rien côté métier — mais ' + techHidden + ' donnée(s) technique(s) correspondent',
                                    'La vue métier ne montre que les objets, applications, processus, termes et règles. Les tables et colonnes issues des sources sont masquées par défaut.',
                              ...
                              : emptyStateHtml(
                                    ...
                                    "Essayez d'élargir vos filtres ou une autre recherche — ou chargez des sources et documentez-les pour peupler le catalogue.",
                                    chip.length || catState.q ? 'Tout effacer' : '',
                                    'catClearFacets()'
                                )
                    }
                </div>
        ...
        function catRealTypeLabel(duckType, colName, dictType) {
            ...
            const text = String(duckType || '').toUpperCase();
            if (/INT|DOUBLE|DECIMAL|FLOAT|HUGE/.test(text)) return 'nombre';
            ...
            try {
                ...
                if (!types) {
                    ...
                    const res = arrowResultToObjects(
                        await conn.query(
                            `SELECT column_name, data_type FROM information_schema.columns WHERE table_name = ${sqlLiteral(duckTableName(tId))}`
                        )
                    );
                    types = {};
                    res.forEach(r => (types[r.column_name] = r.data_type));
                    _catTypeCache[tId] = types;
                }
                const dictionaryEntry = state.governance.dictionary[tName] || {};
                const dc2 = dictionaryEntry.columns || {};
                const table = state.tables[tId];
                box.innerHTML = (table.headers || [])
                    .map(h => {
                        const lbl = catRealTypeLabel(types[h], h, (dc2[h] || {}).technicalType);
                        return `<div class="flex items-center gap-2.5 py-1.5 border-b border-slate-50 text-[12px]">${catTypeChip(lbl)}<span class="font-semibold flex-1 truncate">${escapeHTML(h)}</span><span class="text-[11px] text-slate-400">${escapeHTML(lbl)}</span>${(dc2[h] || {}).definition ? '<span class="text-emerald-500 text-[10px]" title="Documentée">📖</span>' : ''}</div>`;
                    })
                    .join('');
            } catch (e) {
                box.innerHTML = '<p class="text-[11px] text-slate-300 italic">Types indisponibles (moteur non démarré).</p>';
            }
        }
        function catLineageCounts(e2) {
            ...
            if (e2.tbl && (e2.type === 'table' || e2.type === 'view')) {
                count = ((governance.lineage[e2.tbl] || {}).from || []).length;
                Object.keys(governance.lineage).forEach(k => {
                    if (k !== e2.tbl && ((governance.lineage[k] || {}).from || []).includes(e2.tbl)) down++;
                });
                (governance.businessObjects || []).forEach(bo => {
                    if ((bo.sources || []).some(s2 => s2.table === e2.tbl)) down++;
                });
                (governance.assets || []).forEach(a2 => {
                    if (
                        a2.kind === 'process' &&
                        ((a2.tables || []).includes(e2.tbl) || (a2.columns || []).some(c2 => c2.table === e2.tbl))
                    )
                        down++;
                });
            } else if (e2.type === 'column' && typeof modelImpact === 'function') down = modelImpact(e2.tbl, e2.col).length;
            else if (e2.type === 'bo') {
                const bo = governance.businessObjects.find(x => x.id === e2.bo);
                if (bo) {
                    count = (bo.sources || []).length;
                    down = new Set(boAllAttrRows(bo).flatMap(r => r.el.usedBy || [])).size;
                }
            } else if (e2.type === 'attr') {
                const bo = governance.businessObjects.find(x => x.id === e2.bo);
                const attributeRow = bo && boAllAttrRows(bo).find(x => x.el.id === e2.elId);
                if (attributeRow) {
                    count = attributeRow.stId ? (attributeRow.el.col ? 1 : 0) : (attributeRow.el.mappings || []).length;
                    down = (attributeRow.el.usedBy || []).length;
                }
            }
            return [count, down];
        }
        ...
        function catKeyOf(e2) {
            ...
            return e2.type === 'bo'
                ? { type: 'bo', bo: e2.bo }
                : e2.type === 'attr'
                  ? { type: 'attr', bo: e2.bo, elId: e2.elId }
                  : e2.type === 'column'
                    ? { type: 'column', tbl: e2.tbl, col: e2.col }
                    : e2.type === 'term'
                      ? { type: 'term', id: e2.termId || e2.id }
                      : { type: e2.type, id: e2.id, tbl: e2.tbl };
        }
        function catSameKey(a, b2) {
        ...
        function catOpenFiche(i) {
            ...
            const prevLabel = (() => {
                if (!prevKey) return '';
                const ix = catBuildIndex();
                const pe = ix.find(r => catSameKey(catKeyOf(r), prevKey));
                return pe ? pe.title : '';
            })();
            const navBar = prevKey
                ? `<div class="cat-nav"><button data-ro="keep" onclick="catBack()" class="cat-back" title="Revenir à la fiche précédente">← ${escapeHTML(prevLabel || 'Retour')}</button><span class="crumb">${_catHist
                      .slice(-3)
                      .map((k2, j, arr) => {
                          const ix2 = catBuildIndex();
                          const pe = ix2.find(r => catSameKey(catKeyOf(r), k2));
                          const lb = pe ? pe.title : '…';
                          return j === arr.length - 1
                              ? `<b>${escapeHTML(lb)}</b>`
                              : `<button data-ro="keep" onclick="catOpenByKey(${catKeyAttr(k2)})">${escapeHTML(lb)}</button> ›`;
                      })
                      .join(' ')}</span></div>`
                : '';
            ...
            const rel = catRes
                .filter(
                    x =>
                        ...
                        x.dom === e2.dom &&
                        e2.dom !== '—' &&
                        (x.type === 'table' || x.type === 'view' || x.type === 'bo')
                )
                .slice(0, 3);
            ...
            const body =
                ...
                <div class="dsect"><h4>À quoi ça sert</h4>
                    <div class="text-[12.5px] bg-slate-50 border border-slate-200 rounded-lg p-3">${e2.desc ? escapeHTML(e2.desc) : '<span class="text-slate-400 italic">Pas encore documenté — ajoutez une description dans le dictionnaire (ou via ⬆ Import en masse) pour aider les consommateurs de cette donnée.</span>'}</div>
                        </div>
                <div class="dsect"><h4>Fiche d'identité</h4>
                        <div class="kv-ux">
                    ${e2.rows != null ? `<div class="cell"><div class="k">Lignes</div><div class="v">${Number(e2.rows).toLocaleString('fr-FR')}</div></div>` : ''}
                    <div class="cell"><div class="k">Fraîcheur</div>
                        <div class="v" style="font-size:12.5px">↻ ${e2.fresh ? new Date(e2.fresh).toLocaleDateString('fr-FR') : '—'}</div>
                        </div>
                    <div class="cell"><div class="k">Propriétaire</div>
                            <div class="v" style="font-size:12.5px">${
                                e2.own
                                    ? `<span class="cat-oav" style="background:${catOwnColor(e2.own)}">${escapeHTML(
                                          e2.own
                                              .split(/\s+/)
                                              .map(x => x[0] || '')
                                              .join('')
                                              .toUpperCase()
                                              .slice(0, 2)
                                      )}</span> ${escapeHTML(e2.own)}`
                                    : '<span class="text-amber-600">⚠ à désigner</span>'
                            }</div></div>
                ...
                ${
                    t
                        ...
                        <div class="cell"><div class="k">Type réel</div>
                                <div class="v" id="catColType" style="font-size:12.5px">…</div>
                                </div>
                        ${(() => {
                            const cm = ((state.governance.dictionary[e2.tbl] || {}).columns || {})[e2.col] || {};
                            return (
                                (cm.technicalType
                                    ? `<div class="cell"><div class="k">Type déclaré</div><div class="v" style="font-size:12.5px">${escapeHTML(cm.technicalType)}</div>
                                        </div>`
                                    : '') +
                                (e2.q != null
                                    ? `<div class="cell"><div class="k">Complétude</div><div class="v" style="font-size:12.5px">${e2.q} %</div></div>`
                                    : '') +
                                (cm.term && termNameOf(cm.term)
                                    ? `<div class="cell"><div class="k">Terme de glossaire</div><div class="v" style="font-size:12.5px">📖 ${escapeHTML(termNameOf(cm.term))}</div>
                                        </div>`
                                    : '') +
                            ...
                            : `<div class="dsect"><h4>Structure (${(t.headers || []).length} colonnes — types réels)</h4><div id="catSchemaBox"><p class="text-[11px] text-slate-300 italic">Lecture des types…</p>
                                </div>
                                </div>`
                        : ''
                }
                <div class="dsect"><h4>Traçabilité (lineage)</h4><div class="flex items-center gap-2 text-center text-[12px]">
                    <div class="flex-1 bg-slate-50 border border-slate-200 rounded-lg py-2"><div class="text-base font-black">${up}</div><div class="text-[10px] text-slate-400">source(s) amont</div>
                        </div>
                    <span class="text-slate-300" aria-hidden="true">→</span>
                    <div class="flex-1 bg-emerald-50 border border-emerald-200 rounded-lg py-2"><div class="text-[12px] font-black text-emerald-700 truncate px-1">${escapeHTML(e2.title)}</div>
                        <div class="text-[10px] text-slate-400">cet actif</div>
                        </div>
                    <span class="text-slate-300" aria-hidden="true">→</span>
                    <div class="flex-1 bg-slate-50 border border-slate-200 rounded-lg py-2"><div class="text-base font-black">${down}</div><div class="text-[10px] text-slate-400">usage(s) aval</div>
                        </div>
                </div>
                        </div>
                ${
                    e2.type === 'term'
                        ? (() => {
                              const term = termById(e2.termId || e2.id);
                              if (!term) return '';
                              const tg = termTargets(term);
                              if (!tg.length)
                                  return `<div class="dsect"><h4>Ce terme désigne</h4><p class="text-[11.5px] text-slate-400 italic">Aucune donnée pour l'instant. Reliez-le : dans le <b>Glossaire</b> (colonnes), ou en choisissant ce terme dans la colonne « Terme » du <b>Dictionnaire</b>.</p>
                                      </div>`;
                              return `<div class="dsect"><h4>Ce terme désigne (${tg.length})</h4>${tg
                                  .map(x => {
                                      const key =
                                          x.kind === 'attr'
                                              ? { type: 'attr', bo: x.boId, elId: x.elId }
                                              : x.kind === 'bo'
                                                ? { type: 'bo', bo: x.boId }
                                                : x.kind === 'asset'
                                                  ? { type: 'asset', id: x.assetId }
                                                  : { type: 'column', tbl: x.table, col: x.col };
                                      const ic =
                                          x.kind === 'col' ? '▦' : x.kind === 'attr' ? '🔹' : x.kind === 'asset' ? '' : '🏛️';
                ...
                ${
                    e2.type === 'bo'
                        ? (() => {
                              const bo = (state.governance.businessObjects || []).find(x => x.id === e2.bo);
                              if (!bo) return '';
                ...
                ${
                    e2.type === 'attr'
                        ? (() => {
                              const bo = (state.governance.businessObjects || []).find(x => x.id === e2.bo);
                              if (!bo) return '';
                              const attributeRow = (boAllAttrRows(bo) || []).find(x => x.el.id === e2.elId);
                              if (!attributeRow) return '';
                              const facet = attributeRow.stId ? getBoFacets(bo).find(x => x.id === attributeRow.stId) : null;
                              const maps = facet
                                  ? attributeRow.el.col
                                      ? [{ table: facet.table, col: attributeRow.el.col }]
                                      : []
                                  : attributeRow.el.mappings || [];
                              const cnt = boAttrMulti(bo, {
                                  fold: false,
                                  members: [{ el: attributeRow.el, stId: attributeRow.stId || '' }],
                                  idx: [],
                                  stId: attributeRow.stId || ''
                              });
                              ...
                              return `<div class="dsect"><h4>Appartient à</h4>${link('🏛️', bo.name, 'objet métier' + (facet ? ' · composant « ' + facet.name + ' »' : ''), { type: 'bo', bo: bo.id })}</div>
                        <div class="dsect"><h4>Détail de l'attribut</h4>
                                  <div class="kv-ux">
                            <div class="cell"><div class="k">Nombre de valeurs</div>
                                  <div class="v" style="font-size:12.5px">${escapeHTML(cnt.multi ? cnt.label : '1 valeur')}${cnt.from === 'déclaré' ? '' : ' <span class="text-slate-400">(observé)</span>'}</div>
                                </div>
                            <div class="cell"><div class="k">Sensibilité</div>
                                <div class="v" style="font-size:12.5px">${escapeHTML(attributeRow.el.sensitivity || '—')}</div>
                                </div>
                            ${
                                attributeRow.el.examples
                                    ? `<div class="cell" style="grid-column:1/-1"><div class="k">Exemples</div><div class="v" style="font-size:12.5px">${escapeHTML(attributeRow.el.examples)}</div>
                                </div>`
                                    : ''
                            }
                        </div>
                                </div>
                        <div class="dsect"><h4>Termes du glossaire</h4>${termTagsHtml('attr', { boId: bo.id, elId: attributeRow.el.id })}</div>
                        ...
                        <div class="dsect"><h4>Utilisé par</h4>${
                            (attributeRow.el.usedBy || []).length
                                ? (attributeRow.el.usedBy || [])
                                      .map(id => {
                                          const asset = assetById(id);
                                          return asset
                                              ? link(
                                                    asset.kind === 'process' ? '⚙️' : '🖥',
                                                    asset.name,
                                      ...
                                      .join('')
                                : '<p class="text-[11.5px] text-slate-400 italic">Aucun usage déclaré.</p>'
                        }</div>`;
                          })()
                        : ''
                }
                ${
                    e2.type === 'column'
                        ? (() => {
                              const hits = attrsOfColumn(e2.tbl, e2.col);
                              return hits.length
                                  ? `<div class="dsect"><h4>Alimente l'attribut métier</h4>${hits.map(h => `<button data-ro="keep" onclick="catOpenByKey(${catKeyAttr({ type: 'attr', bo: h.boId, elId: h.r.el.id })})" class="w-full flex items-center gap-2.5 border border-slate-100 rounded-lg px-2.5 py-2 mb-1.5 text-left hover:bg-slate-50"><span class="min-w-0"><span class="block text-[12px] font-semibold truncate">🔹 ${escapeHTML(h.r.el.name)}</span><span class="block text-[10px] text-slate-400">attribut de ${escapeHTML(h.bo.name)}</span></span><span class="ml-auto text-slate-300">›</span></button>`).join('')}</div>`
                                  : '';
                          })()
                        : ''
                }
                ...
                ${
                    e2.type === 'bo'
                        ? (() => {
                              const bo = (state.governance.businessObjects || []).find(x => x.id === e2.bo);
                              if (!bo) return '';
                              const prod = (bo.producedBy || []).map(assetById).filter(Boolean),
                                  cons = (bo.consumedBy || []).map(assetById).filter(Boolean);
                              ...
                              const row = (ic, lbl, sub, key) =>
                                  `<button data-ro="keep" onclick="catOpenByKey(${catKeyAttr(key)})" class="w-full flex items-center gap-2.5 border border-slate-100 rounded-lg px-2.5 py-2 mb-1.5 text-left hover:bg-slate-50"><span class="min-w-0"><span class="block text-[12px] font-semibold truncate">${ic} ${escapeHTML(lbl)}</span><span class="block text-[10px] text-slate-400">${escapeHTML(sub)}</span></span><span class="ml-auto text-slate-300">›</span></button>`;
                              if (!prod.length && !cons.length && !srcs.length)
                                  return `<div class="dsect"><h4>Chaîne de l'objet</h4><p class="text-[11.5px] text-slate-400 italic">Aucune source ni application rattachée. Complétez la fiche de l'objet (onglet <b>Sources</b>) pour voir sa chaîne amont → aval.</p>
                                      </div>`;
                ...
                ${
                    rel.length
                        ? `<div class="dsect"><h4>Actifs liés (${escapeHTML(e2.dom)})</h4>${rel
                              .map(x => {
                                  const [xc, xi, xl] = CAT_TYPES[x.type];
            ...
            openUxDrawer({
                sem: '',
                title: e2.title,
                sub: tl + (e2.dom !== '—' ? ' · Domaine ' + e2.dom : '') + (e2.own ? ' · propriétaire ' + e2.own : ''),
                body,
                foot: `
            ...
            el('uxDrawerTyp').className = 'cat-tico ' + tc;
            el('uxDrawerTyp').textContent = ti;
        ...
        function catGoLineage() {
            ...
            if (ficheContext.type === 'column' && ficheContext.tbl && ficheContext.col) {
                catShowAttrLineage(ficheContext);
                return;
            }
            // V8.6 — Objet métier : même présentation que pour un attribut, dans la fiche.
            if (ficheContext.type === 'bo' && (state.governance.businessObjects || []).some(x => x.id === ficheContext.bo)) {
                catShowAttrLineage(ficheContext);
                return;
            }
            if (ficheContext.type === 'attr') {
                catShowAttrLineage(ficheContext);
                return;
            }
            closeUxDrawer();
        ...
        function catEditInObject() {
            const ficheContext = _catFicheCtx;
            if (!ficheContext || !ficheContext.bo) return;
            closeUxDrawer();
            govState.selectedBoId = ficheContext.bo;
            govState.boTab = 'structure';
            govState.structView = 'fiche';
            if (ficheContext.type === 'attr')
                govState.boSel = { kind: 'attr', stId: ficheContext.stId || '', elId: ficheContext.elId };
            openGovTab('objects');
        }
        ...
        function catGoLineageTab() {
            ...
            if (!ficheContext) return;
            closeUxDrawer();
            if (ficheContext.type === 'bo') {
                govState.lineageView = 'attr';
                govState.lineageAttrBo = ficheContext.bo;
            }
            openGovTab('lineage');
        }
        function catShowAttrLineage(e2) {
            ...
            const data = isBo
                ? buildBoLineageGraph((state.governance.businessObjects || []).find(x => x.id === e2.bo))
                : e2.type === 'attr'
                  ? (() => {
                        const bo = (state.governance.businessObjects || []).find(x => x.id === e2.bo);
                        const attributeRow = bo && boAllAttrRows(bo).find(x => x.el.id === e2.elId);
                        const facet =
                            attributeRow && attributeRow.stId ? getBoFacets(bo).find(x => x.id === attributeRow.stId) : null;
                        return attributeRow ? buildAttrLineageGraph(bo, facet, attributeRow.el) : { nodes: [], edges: [] };
                    })()
                  : buildColumnLineageGraph(e2.tbl, e2.col);
            ...
            try {
                if (catAttrLineageGraph) {
                    try {
                        catAttrLineageGraph.destroy();
                    } catch (e3) {}
                    catAttrLineageGraph = null;
                }
                ...
                el('catLineageWrap').innerHTML =
                    '<p class="text-[11px] text-slate-400 italic p-4">Lineage indisponible : ' +
                    escapeHTML(e3.message || '') +
                    '</p>';
            }
            box.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
        }
        function catFitLineage(delay) {
            setTimeout(() => {
                const w = el('catLineageWrap');
                if (!w || !catAttrLineageGraph) return;
                try {
                    catAttrLineageGraph.changeSize(w.clientWidth, w.clientHeight);
                    catAttrLineageGraph.fitView(20);
                } catch (e) {}
            }, delay || 0);
        }
        function catCloseLineage() {
            const box = el('catLineageBox');
        ...
        function catUseData() {
            ...
            closeUxDrawer();
            if (!ficheContext) return;
            let tblName = ficheContext.tbl;
            if (ficheContext.type === 'attr' && !tblName) {
                const bo = (state.governance.businessObjects || []).find(x => x.id === ficheContext.bo);
                const attributeRow = bo && boAllAttrRows(bo).find(x => x.el.id === ficheContext.elId);
                const facet = attributeRow && attributeRow.stId ? getBoFacets(bo).find(x => x.id === attributeRow.stId) : null;
                tblName = facet
                    ? facet.table
                    : attributeRow && (attributeRow.el.mappings || [])[0]
                      ? attributeRow.el.mappings[0].table
                      : null;
            }
            if (ficheContext.type === 'bo' && !tblName) {
                const bo = (state.governance.businessObjects || []).find(x => x.id === ficheContext.bo);
                const m = bo && boMasterTable(bo);
                tblName = m ? m.name : null;
            }
            const t = tblName ? tableByName(tblName) : null;
            switchTab(3);
            if (t) {
                const sel2 = el('baseTableSelect');
                if (sel2) {
                    sel2.value = t.id;
                    try {
