        ...
        function wfBadge(entry) {
            const status = entry.status || 'Brouillon';
            const cls =
                status === 'Validé'
            ...
            return `<span class="text-[10px] px-2 py-0.5 rounded font-bold ${cls}" title="${entry.statusAt ? escapeHTML('Depuis le ' + new Date(entry.statusAt).toLocaleString('fr-FR') + ' par ' + (entry.statusBy || '?')) : ''}">${status}</span>`;
        }
        function wfHistoryHtml(entry) {
            ...
            return `<details class="mt-1"><summary class="text-[10px] text-slate-400 cursor-pointer">🕓 Historique des statuts (${entry.history.length})</summary>
                <div class="mt-1 space-y-0.5">${entry.history
                    .slice()
                    .reverse()
                    .map(
                        h =>
                            `<div class="text-[10px] text-slate-500">${new Date(h.at).toLocaleString('fr-FR')} — <strong>${escapeHTML(h.from)}</strong> → <strong>${escapeHTML(h.to)}</strong> par ${escapeHTML(h.by)}${h.comment ? ' · « ' + escapeHTML(h.comment) + ' »' : ''}</div>`
                    )
                    .join('')}</div></details>`;
        }
        function wfSetDictStatus(tn, v) {
            const dictionaryEntry = ensureDictEntry(tn);
            const c = v === 'Validé' ? prompt('Commentaire de validation (optionnel) :') || '' : '';
            wfTransition(dictionaryEntry, v, c);
            persistAppState();
            renderGovernance();
        }
        function wfSetBoStatus(boId, v) {
        ...
        function wfStats() {
            ...
            const all = dicts
                .map(([n, d]) => ({ kind: 'dict', name: n, e: d }))
                .concat(bos.map(b => ({ kind: 'bo', name: b.name, e: b })));
            return {
                all,
                toReview: all.filter(x => (x.e.status || 'Brouillon') === 'Proposé'),
                validated: all.filter(x => x.e.status === 'Validé'),
                obsolete: all.filter(x => x.e.status === 'Obsolète')
            };
        }
        ...
        function readyTableNames() {
            return Object.values(state.tables)
                .filter(t => t.status === 'ready')
                .map(t => t.name);
        }
        function tableByName(n) {
            return Object.values(state.tables).find(t => t.name === n);
        }
        ...
        function renderGovDictionaryBo(bos) {
            if (!bos.length)
                return '<p class="text-sm text-slate-400 italic py-8 text-center">Aucun objet métier — créez-en dans l\'onglet Objets métier, ou basculez « Par table technique ».</p>';
            ...
            getBoFacets(bo).forEach(st =>
                (st.elements || []).forEach(fe =>
                    rows.push({
                        el: fe,
                        mappings: fe.col ? [{ table: st.table, col: fe.col }] : [],
                        facet: st.name,
                        stId: st.id
                    })
                )
            ...
            const glossOpts = cur =>
                ...
                state.governance.glossary
                    .map(g => `<option value="${g.id}" ${g.id === cur ? 'selected' : ''}>${escapeHTML(g.term)}</option>`)
                    .join('');
            ...
            const sections = [{ st: null, rows: rows.filter(r => !r.stId) }].concat(
                facets.map(st => ({ st, rows: rows.filter(r => r.stId === st.id) }))
            );
            ...
            const grpHtml = g => {
                ...
                const prov = one
                    ? m
                        ? escapeHTML(m.table + '.' + m.col)
                        : '<span class="italic text-amber-500">non rattaché à une colonne</span>'
                    : escapeHTML(cols.join(' · ')) || '<span class="italic text-amber-500">colonnes non rattachées</span>';
                const sampleBtn = one
                    ? m
                        ? `<button onclick="boAttrSampleOne('${bo.id}','${r0.el.id}')" class="text-[11px] px-1.5 py-1 rounded border border-indigo-200 text-indigo-700 bg-white hover:bg-indigo-50 shrink-0" title="Prendre de vrais exemples dans ${escapeHTML(m.table + '.' + m.col)}">⟳</button>`
                        : ''
                    : cols.length
                      ? `<button onclick="boFoldSample('${bo.id}','${ids}')" class="text-[11px] px-1.5 py-1 rounded border border-indigo-200 text-indigo-700 bg-white hover:bg-indigo-50 shrink-0" title="Prendre de vrais exemples dans les ${cols.length} colonnes repliées">⟳</button>`
                      : '';
                const vlCell = one
                    ? m
                        ? vlSelectHtml(
                              m.table,
                              m.col,
                              (((state.governance.dictionary[m.table] || {}).columns || {})[m.col] || {}).valueListId
                          )
                        : '<span class="text-[10px] text-slate-300 italic">rattachez une colonne</span>'
                ...
                return `<tr class="hover:bg-slate-50 align-top">
                    ...
                    <td class="p-2 whitespace-nowrap">${boValueCountBadgeHtml(bo, g)}
                        <div class="mt-1">${boMultiSelectHtml(
                            boAttrMulti(bo, g).decl,
                            one
                                ? `boAttrWrite('${bo.id}','${stId}','${r0.el.id}','multi',this.value); renderGovernance()`
                                : `boFoldWrite('${bo.id}','${stId}','${ids}','multi',this.value)`,
                            one
                                ? 'Déclarez ici combien de valeurs cet attribut peut prendre pour un(e) ' + bo.name
                                : 'Déclaration appliquée aux ' + g.members.length + ' colonnes repliées de cet attribut'
                        )}</div></td>
                    ...
                    <td class="p-2">${
                        one
                            ? attrTermsCellHtml(bo, stId, r0.el.id)
                            : `<select onchange="${wr('term')}" class="border border-slate-200 p-1.5 rounded text-xs bg-white max-w-[130px]" title="Terme appliqué aux ${g.members.length} colonnes repliées">${glossOpts(val('term'))}</select>${(() => {
                                  const oth = Array.from(
                                      new Set(
                                          g.members.flatMap(mm =>
                                              termsOfAttr(bo.id, mm.el.id)
                                                  .filter(t2 => t2.id !== val('term'))
                                                  .map(t2 => t2.term)
                                          )
                                      )
                                  );
                                  return oth.length
                                      ? `<div class="flex flex-wrap gap-1 mt-1">${oth.map(x => `<span class="text-[10px] bg-indigo-50 border border-indigo-200 text-indigo-800 rounded-full px-1.5 py-0.5 font-bold">📖 ${escapeHTML(x)}</span>`).join('')}</div>`
                                      : '';
                              })()}`
                    }</td>
            ...
            sections.forEach(sec => {
                ...
                groups.forEach(g => {
                    nShown++;
                    if (g.fold) nFolded += g.members.length;
                    rowsHtml += grpHtml(g);
                });
            });
            const _bl = typeof govLockAttr === 'function' ? govLockAttr(govCanEditBo(bo), govCanProposeBo(bo)) : '';
            return (
                (_bl ? '<div data-gov-lock="1">' + govLockBand("l'objet « " + bo.name + ' »', boDomainOf(bo)) : '<div>') +
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    ...
                    ${
                        md
                            ? `<div><label class="text-xs font-bold text-slate-500 block mb-1">Référent (Data Steward)</label><input type="text" value="${escapeHTML(md.steward || '')}" placeholder="Personne référente" onchange="updateDictField('${escapeHTML(master.name)}','steward',this.value)" class="w-full border border-slate-300 p-2 rounded text-sm bg-white"></div>
                ...
                <div class="border border-slate-200 rounded-lg overflow-x-auto"><table class="w-full text-left text-sm">
                    <thead class="bg-slate-50 text-[10px] uppercase font-bold text-slate-500"><tr><th class="p-2">Attribut métier <span class="normal-case font-medium text-slate-300">(et sa provenance)</span></th>
                    <th class="p-2" title="Combien de valeurs cet attribut peut prendre dans une occurrence — déclarable ici. La répétition du GROUPE, elle, est indiquée sur la ligne du composant.">Nombre de valeurs</th>
                    <th class="p-2">Définition</th>
                    <th class="p-2">Exemples</th>
                    <th class="p-2">Sensibilité</th>
                    <th class="p-2">Terme du glossaire</th>
                    <th class="p-2">🎚️ Liste de valeurs</th>
                    </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100">${rowsHtml || '<tr><td colspan="7" class="p-4 text-xs text-slate-400 italic text-center">Aucun attribut — définissez la structure de l\'objet dans sa fiche.</td></tr>'}</tbody>
                        </table>
                        </div>
                        </div>`
            );
        ...
        function ensureDictEntry(tableName) {
            ...
            if (table)
                table.headers.forEach(h => {
                    if (!d[tableName].columns[h])
                        d[tableName].columns[h] = { definition: '', technicalType: '', sensitivity: '', term: '' };
                });
            return d[tableName];
        }
        ...
        function renderGovernance() {
            ...
            const crumb = el('govCrumb'),
                acts = el('govActions');
            const govTabs = (NAV_PHASES.find(p => p.id === 'gov') || { tabs: [] }).tabs;
            const curT =
                govTabs.find(t => t.g === (govState.tab === 'flow' ? 'lineage' : govState.tab)) ||
                govTabs.find(t => t.g === govState.tab) ||
                null;
            if (crumb)
                crumb.innerHTML = curT
                    ? `${govIsReadOnly() ? '👁 Consultation' : escapeHTML(curT.fam || 'Gouvernance')} › <b>${escapeHTML(curT.label)}</b>`
                    : '';
            if (acts)
                acts.innerHTML = `<button onclick="giOpen()" aria-label="Import en masse de la gouvernance" class="text-[11.5px] font-bold px-3 py-1.5 rounded-full border border-indigo-200 bg-indigo-50 text-indigo-700 hover:bg-indigo-100" title="Importer en masse définitions, propriétaires, termes… depuis un fichier">⬆ Import en masse</button>`;
            const famBar = el('govFamTabs');
            if (famBar) famBar.innerHTML = '';
            const govContentElement = el('govContent');
            if (govState.tab === 'dictionary') govContentElement.innerHTML = renderGovDictionary();
            else if (govState.tab === 'objects') {
                govContentElement.innerHTML = renderGovObjects();
                ...
                if (selBo)
                    getBoHierarchies(selBo).forEach(h2 => {
                        if (h2.typeCol) fillBoHierLevelValues(selBo.id, h2.id);
                    });
                if (selBo) fillBoVolumetry(selBo.id, 'bo-vol-' + selBo.id);
            } else if (govState.tab === 'model') {
                govContentElement.innerHTML =
                    '<div class="flex justify-end gap-2 mb-2">' +
                    graphToolbarHtml('model') +
                    '<button data-ro="keep" onclick="exportGraphImage(\'model\')" class="text-[11px] bg-white border border-slate-300 px-2.5 py-1 rounded font-bold text-slate-600 hover:bg-slate-100">📷 Exporter l\'image</button></div><div id="modelGraphWrap" class="border border-slate-200 rounded-xl bg-slate-50/50 h-[550px]" style="background-image: radial-gradient(#e2e8f0 1px, transparent 1px); background-size: 20px 20px;"></div><p class="text-xs text-slate-400 mt-3">Diagramme du modèle métier : 🏛️ objets (vert), ◆ compositions (ce qui fait partie de l\'objet), → références entre objets, cardinalités sur les liens. Construit via l\'assistant "Concevoir depuis le modèle de données".</p>';
                renderObjectModelDiagram();
                requestAnimationFrame(() => reflowGraph('model'));
            } else if (govState.tab === 'perimeters') govContentElement.innerHTML = renderGovPerimeters();
            else if (govState.tab === 'glossary') govContentElement.innerHTML = renderGovGlossary();
            else if (govState.tab === 'vlists') govContentElement.innerHTML = renderValueLists();
            else if (govState.tab === 'usecases') {
                govState.tab = 'assets';
                govContentElement.innerHTML = renderGovAssets();
            } else if (govState.tab === 'assets') govContentElement.innerHTML = renderGovAssets();
            else if (govState.tab === 'privacy') govContentElement.innerHTML = renderGovPrivacy();
            else if (govState.tab === 'catalog') govContentElement.innerHTML = renderGovCatalog();
            else if (govState.tab === 'history') govContentElement.innerHTML = renderGovHistory();
            else if (govState.tab === 'people') govContentElement.innerHTML = renderGovPeople();
            else if (govState.tab === 'review') govContentElement.innerHTML = renderGovReview();
            else if (govState.tab === 'srcwatch') govContentElement.innerHTML = renderSrcWatch();
            else if (govState.tab === 'flow') govContentElement.innerHTML = renderLineageFlow();
            else if (govState.tab === 'share') {
                govState.tab = 'catalog';
                govContentElement.innerHTML = renderGovCatalog();
                openBackupCenter();
            } else if (govState.tab === 'lineage') {
                const _am = govState.lineageView === 'attr';
                const _bos = state.governance.businessObjects || [];
                govContentElement.innerHTML =
                    '<div id="lineageFsWrap"><div class="flex items-center gap-2 mb-2 flex-wrap">' +
                    ...
                    '<button onclick="govState.lineageView=\'global\'; renderGovernance()" class="px-3 py-1.5 ' +
                    (_am ? 'bg-white text-slate-500 hover:bg-slate-50' : 'bg-indigo-600 text-white') +
                    '">🌐 Vue globale</button>' +
                    '<button onclick="govState.lineageView=\'attr\'; renderGovernance()" class="px-3 py-1.5 ' +
                    (_am ? 'bg-emerald-600 text-white' : 'bg-white text-slate-500 hover:bg-slate-50') +
                    '">🔬 Par attribut</button></div>' +
                    (_am
                        ? '<select onchange="govState.lineageAttrBo=this.value; renderLineageGraph()" class="border border-emerald-300 rounded-lg px-2 py-1.5 text-xs font-bold bg-emerald-50/50">' +
                          _bos
                              .map(
                                  b2 =>
                                      '<option value="' +
                                      b2.id +
                                      '"' +
                                      (govState.lineageAttrBo === b2.id ? ' selected' : '') +
                                      '>🏛️ ' +
                                      escapeHTML(b2.name) +
                                      '</option>'
                              )
                          ...
                          escapeHTML(govState.lineageAttrQ || '') +
                          '" oninput="govState.lineageAttrQ=this.value; attrFlowApplyFocus()" placeholder="🔍 filtrer les attributs…" class="text-[11px] border border-slate-300 rounded px-2 py-1.5 w-44 bg-white">'
                        : '') +
                    '</div>' +
                    '<div class="flex justify-end items-center gap-2.5 mb-2 flex-wrap bg-white/90 rounded-lg px-2 py-1' +
                    (_am ? ' hidden' : '') +
                    '">' +
                    [
                        'srcs,📄 Sources,lineageSrcs',
                        'bos,🏛 Objets,lineageBos',
                        'apps,🖥 Applications,lineageApps',
                        'procs,⚙️ Processus,lineageProcs'
                    ]
                        .map(x => {
                            const [, lbl2, gk] = x.split(',');
                            return (
                                '<label class="flex items-center gap-1 text-[11px] text-slate-600 cursor-pointer"><input type="checkbox" ' +
                                (govState[gk] !== false ? 'checked' : '') +
                                ' onchange="govState.' +
                                gk +
                                '=this.checked; renderLineageGraph()"> ' +
                                lbl2 +
                                '</label>'
                            );
                        })
                        .join('') +
                    '<label class="flex items-center gap-1 text-[11px] text-slate-600 cursor-pointer" title="Par défaut, un fichier source est intégré à l\'application qui le produit. Décochez pour afficher chaque fichier séparément."><input type="checkbox" ' +
                    (govState.lineageFoldSrc !== false ? 'checked' : '') +
                    ' onchange="govState.lineageFoldSrc=this.checked; renderLineageGraph()"> 📦 Sources dans l\'appli</label>' +
                    '<label class="flex items-center gap-1 text-[11px] text-emerald-700 font-bold cursor-pointer" title="Affiche les attributs des objets métier avec leur colonne source"><input type="checkbox" ' +
                    (govState.lineageBoDetail ? 'checked' : '') +
                    ' onchange="govState.lineageBoDetail=this.checked; renderLineageGraph()"> 🔬 Attributs des objets</label><input list="linGotoList" placeholder="🔍 aller à…" onchange="lineageGoto(this.value); this.value=\'\'" class="text-[11px] border border-slate-300 rounded px-2 py-1 w-36 bg-white"><datalist id="linGotoList"></datalist>' +
                    graphToolbarHtml('lineage') +
                    '<button onclick="lineageFullscreen()" class="text-[11px] bg-white border border-slate-300 px-2.5 py-1 rounded font-bold text-slate-600 hover:bg-slate-100" title="Plein écran">⛶ Plein écran</button><button onclick="exportGraphImage(\'lineage\')" class="text-[11px] bg-white border border-slate-300 px-2.5 py-1 rounded font-bold text-slate-600 hover:bg-slate-100">📷</button></div><div id="lineageHint" class="hidden text-xs bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-lg px-3 py-1.5 mb-2"></div><div id="lineageGraphWrap" class="border border-slate-200 rounded-xl bg-slate-50/50 h-[550px]" style="background-image: radial-gradient(#e2e8f0 1px, transparent 1px); background-size: 20px 20px;"></div><p class="text-xs text-slate-400 mt-3">Bleu : fichiers sources · Cyan : API · Violet : extractions (dérivées) · Vert : objets métier. Flèches violettes : alimentation (lineage) · Traits gris pointillés : relations du modèle de données · Flèches ambre : table utilisée par un processus · <strong>👑 Vert épais : source maître → objet</strong> · Vert pointillé : contributeur · <strong>Sarcelle : objet → destinataire (diffusion)</strong> · <strong>🖥 Gris foncé : application (produit)</strong> · <strong>⚙️ Orange : processus (consomme)</strong> · 💡 <strong>Cliquez un nœud</strong> pour mettre en évidence toute sa chaîne de bout en bout (re-cliquez pour effacer) · <strong>double-cliquez-le</strong> pour ouvrir sa fiche · <strong>cliquez un trait</strong> pour voir ce qui y passe.</p></div>';
                renderLineageGraph();
                requestAnimationFrame(() => reflowGraph('lineage'));
            }
            lucide.createIcons();
            govProjectValues();
        ...
        function renderGovDictionary() {
            ...
            if (!names.length)
                return '<p class="text-sm text-slate-400 italic py-8 text-center">Chargez d\'abord des tables (onglet Sources) pour documenter la donnée.</p>';
            ...
            const wf = wfStats();
            const wfBar = wf.all.length
                ? `<div class="flex items-center gap-2 mb-4 flex-wrap text-xs bg-slate-50 border border-slate-200 rounded-lg px-3 py-2">
            ...
            if (govState.dictWfFilter && wf.toReview.length) {
                return (
                    modeBar +
                    wfBar +
                    `<div class="space-y-2">${wf.toReview
                        .map(
                            x => `<div class="flex items-center gap-2 border border-amber-200 bg-amber-50/40 rounded-lg p-3 text-sm">
            ...
            const inp = (field, val, ph) =>
                `<input type="text" value="${escapeHTML(val || '')}" placeholder="${ph}" onchange="updateDictField('${escapeHTML(tn)}','${field}',this.value)" class="w-full border border-slate-300 p-2 rounded text-sm bg-white">`;
            const selOpts = (opts, cur) => opts.map(o => `<option ${o === cur ? 'selected' : ''}>${o}</option>`).join('');
            const glossOpts = cur =>
                '<option value="">— Aucun terme —</option>' +
                state.governance.glossary
            ...
            (table ? table.headers : Object.keys(dictionaryEntry.columns)).forEach(h => {
                const term = dictionaryEntry.columns[h] || {};
                colRows += `<tr class="hover:bg-slate-50">
                    <td class="p-2 font-semibold text-slate-700 text-xs whitespace-nowrap">${escapeHTML(h)}</td>
                    <td class="p-2"><input type="text" value="${escapeHTML(term.definition || '')}" placeholder="${(() => {
                        const r = dictColResolved(tn, h, 'definition');
                        return r.from ? escapeHTML(r.value).replace(/"/g, '&quot;') : 'D\u00e9finition m\u00e9tier...';
                    })()}" onchange="updateDictColField('${escapeHTML(tn)}','${escapeHTML(h.replace(/'/g, "\\'"))}','definition',this.value)" class="w-full border border-slate-200 p-1.5 rounded text-xs">${(() => {
                        const r = dictColResolved(tn, h, 'definition');
                        return !term.definition && r.from
                            ? `<div class="text-[9px] text-emerald-600 mt-0.5">d\u00e9fini sur l'objet <b>${escapeHTML(r.from)}</b> - ${escapeHTML(r.attr)}</div>`
                            : '';
                    })()}${typeof propBadgeHtml === 'function' ? propBadgeHtml('dictcol', { tn, col: h }, 'definition') : ''}</td>
                    <td class="p-2"><input type="text" value="${escapeHTML(term.technicalType || '')}" placeholder="ex: VARCHAR(10)" onchange="updateDictColField('${escapeHTML(tn)}','${escapeHTML(h.replace(/'/g, "\\'"))}','technicalType',this.value)" class="w-28 border border-slate-200 p-1.5 rounded text-xs font-mono"></td>
                    <td class="p-2"><input type="text" value="${escapeHTML(term.examples || '')}" placeholder="ex: PAR01 ; LYO02..." onchange="updateDictColField('${escapeHTML(tn)}','${escapeHTML(h.replace(/'/g, "\\'"))}','examples',this.value)" class="w-44 border border-slate-200 p-1.5 rounded text-xs ${term.examplesAuto ? 'bg-indigo-50/50' : ''}" title="${term.examplesAuto ? 'Échantillonné automatiquement depuis la source' : 'Saisie manuelle'}"></td>
                    <td class="p-2"><select onchange="updateDictColField('${escapeHTML(tn)}','${escapeHTML(h.replace(/'/g, "\\'"))}','sensitivity',this.value)" class="border border-slate-200 p-1.5 rounded text-xs bg-white"><option value="">—</option>${selOpts(SENSITIVITY_OPTS, term.sensitivity)}</select>${(() => {
                        const r = dictColResolved(tn, h, 'sensitivity');
                        return !term.sensitivity && r.from
                            ? `<div class="text-[9px] text-emerald-600 mt-0.5" title="Défini sur l'objet métier — c'est là qu'il faut le modifier">hérite : ${escapeHTML(r.value)}</div>`
                            : '';
                    })()}</td>
                    <td class="p-2"><select onchange="updateDictColField('${escapeHTML(tn)}','${escapeHTML(h.replace(/'/g, "\\'"))}','term',this.value)" class="border border-slate-200 p-1.5 rounded text-xs bg-white max-w-[140px]">${glossOpts(term.term)}</select>${colTermsPillsHtml(tn, h, term.term)}</td>
                    <td class="p-2">${vlSelectHtml(tn, h, term.valueListId)}</td>
                </tr>`;
            });
            ...
            return (
                modeBar +
                wfBar +
                (typeof govLockAttr === 'function' && govLockAttr(govCanEdit(_tdom), govCanPropose(_tdom))
                    ? '<div data-gov-lock="1">' + govLockBand('la source « ' + tn + ' »', _tdom)
                    : '<div>') +
                `
                <div class="flex items-center gap-4 mb-5">
                    ...
                    <span class="text-xs px-2 py-1 rounded font-bold ${dictionaryEntry.status === 'Validé' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'}">${dictionaryEntry.status}</span>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    <div class="md:col-span-3"><label class="text-xs font-bold text-slate-500 block mb-1">Description fonctionnelle de l'objet</label><textarea onchange="updateDictField('${escapeHTML(tn)}','description',this.value)" placeholder="À quoi sert cette donnée, dans quel processus métier..." class="w-full border border-slate-300 p-2 rounded text-sm bg-white h-16">${escapeHTML(dictionaryEntry.description || '')}</textarea>${typeof propBadgeHtml === 'function' ? propBadgeHtml('dict', { tn }, 'description') : ''}</div>
                    <div><label class="text-xs font-bold text-slate-500 block mb-1">Domaine métier <span class="text-[9px] text-slate-400">(= thème de la source, qui valide)</span></label>${(() => {
                        const table = tableByName(tn);
                        return table
                            ? govDomainSelectHtml(
                                  String(table.theme || '').trim(),
                                  `updateTableTheme('${table.id}', this.value); renderGovernance()`,
                                  'dict-domain',
                                  govFeatureOn() && !govCanEdit(String(table.theme || '').trim())
                              ) + (govFeatureOn() && !govCanEdit(String(table.theme || '').trim()) ? govLockedHint() : '')
                            : '<span class="text-xs text-slate-400">—</span>';
                    })()}</div>
                    <div><label class="text-xs font-bold text-slate-500 block mb-1">Propriétaire métier (Owner)</label>${inp('owner', dictionaryEntry.owner, 'Direction / personne')}</div>
                    <div><label class="text-xs font-bold text-slate-500 block mb-1">Référent (Data Steward)</label>${inp('steward', dictionaryEntry.steward, 'Personne référente')}</div>
                    <div><label class="text-xs font-bold text-slate-500 block mb-1">Système source <span class="text-[9px] text-slate-400">(= application du lineage)</span></label><input type="text" list="dictAppsList" value="${escapeHTML(dictionaryEntry.sourceSystem || '')}" placeholder="ex: SAP, GMAO, export Excel..." onchange="updateDictField('${escapeHTML(tn)}','sourceSystem',this.value)" class="w-full border border-slate-300 p-2 rounded text-sm bg-white"><datalist id="dictAppsList">${(
                        state.governance.assets || []
                    )
                        .filter(a => a.kind === 'app')
                        .map(a => `<option value="${escapeHTML(a.name)}">`)
                        .join('')}</datalist></div>
                    <div><label class="text-xs font-bold text-slate-500 block mb-1">Fréquence de mise à jour</label>${inp('updateFrequency', dictionaryEntry.updateFrequency, 'ex: quotidienne, mensuelle')}</div>
                    <div><label class="text-xs font-bold text-slate-500 block mb-1">Sensibilité globale</label><select onchange="updateDictField('${escapeHTML(tn)}','sensitivity',this.value)" class="w-full border border-slate-300 p-2 rounded text-sm bg-white">${selOpts(SENSITIVITY_OPTS, dictionaryEntry.sensitivity)}</select></div>
                    <div><label class="text-xs font-bold text-slate-500 block mb-1">Statut de la fiche ${wfBadge(dictionaryEntry)}</label><select onchange="wfSetDictStatus('${escapeHTML(tn)}',this.value)" class="w-full border border-slate-300 p-2 rounded text-sm bg-white">${selOpts(STATUS_OPTS, dictionaryEntry.status)}</select>${wfHistoryHtml(dictionaryEntry)}</div>
                </div>
                ...
                <div class="border border-slate-200 rounded-lg overflow-x-auto">
                    <table class="w-full text-left text-sm">
                        <thead class="bg-slate-50 text-[10px] uppercase font-bold text-slate-500"><tr><th class="p-2">Colonne</th>
                        <th class="p-2">Définition fonctionnelle</th>
                        <th class="p-2">Type technique</th>
                        <th class="p-2">Exemples de valeurs</th>
                        <th class="p-2">Sensibilité</th>
                        <th class="p-2">Terme glossaire</th>
                        <th class="p-2">🎚️ Liste de valeurs</th>
                        </tr>
                        </thead>
        ...
        function updateDictField(tn, f, v) {
            ensureDictEntry(tn)[f] = v;
            persistAppState();
        }
