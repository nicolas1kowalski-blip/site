        ...
        function renderGovDictionary() {
            ...
            return modeBar + wfBar + (typeof govLockAttr === 'function' && govLockAttr(govCanEdit(_tdom), govCanPropose(_tdom)) ? '<div data-gov-lock="1">' + govLockBand('la source « ' + tn + ' »', _tdom) : '<div>') + `
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    <div class="md:col-span-3"><label class="text-xs font-bold text-slate-500 block mb-1">Description fonctionnelle de l'objet</label><textarea onchange="updateDictField('${escapeHTML(tn)}','description',this.value)" placeholder="À quoi sert cette donnée, dans quel processus métier..." class="w-full border border-slate-300 p-2 rounded text-sm bg-white h-16">${escapeHTML(d.description || '')}</textarea>${typeof propBadgeHtml === 'function' ? propBadgeHtml('dict', { tn }, 'description') : ''}</div>
                    <div><label class="text-xs font-bold text-slate-500 block mb-1">Domaine métier <span class="text-[9px] text-slate-400">(= thème de la source, qui valide)</span></label>${(() => { const tt = tableByName(tn); return tt ? govDomainSelectHtml(String(tt.theme || '').trim(), `updateTableTheme('${tt.id}', this.value); renderGovernance()`, 'dict-domain', govFeatureOn() && !govCanEdit(String(tt.theme || '').trim())) + (govFeatureOn() && !govCanEdit(String(tt.theme || '').trim()) ? govLockedHint() : '') : '<span class="text-xs text-slate-400">—</span>'; })()}</div>
