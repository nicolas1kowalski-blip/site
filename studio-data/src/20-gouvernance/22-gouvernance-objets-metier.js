        ...
        function computeMultiSourceAttributes() {
            ...
            names.forEach(n => {
                ...
                (table ? table.headers : []).forEach(h => {
                    (byCol[h] = byCol[h] || []).push(n);
                });
            });
            return Object.entries(byCol)
                .filter(([, tabs]) => tabs.length >= 2)
                .map(([col, tabs]) => {
                    const tableOwners = Array.from(
                        new Set(tabs.map(tn => (state.governance.dictionary[tn] || {}).owner).filter(Boolean))
                    );
                    let elementOwner = null,
                        objectOwner = null,
                        objName = null,
                        elmName = null;
                    state.governance.businessObjects.forEach(bo =>
                        (bo.elements || []).forEach(elm => {
                    ...
                    return {
                        ...
                        owner,
                        ownerSource,
                        conflictOwners: !elementOwner && !objectOwner && tableOwners.length > 1,
                        objName,
                        elmName
                    };
                })
        ...
        function renderGovObjects() {
                ...
                if (govState.showMulti) {
                    const rows = multi
                        .map(m => {
                            ...
                            if (!m.owner)
                                status =
                                    '<span class="text-[10px] uppercase font-bold px-2 py-0.5 rounded bg-red-100 text-red-700">⚠️ Sans propriétaire</span>';
                            else if (m.conflictOwners)
                                status = `<span class="text-[10px] uppercase font-bold px-2 py-0.5 rounded bg-amber-100 text-amber-800">⚠️ Propriétaires divergents</span>`;
                            else
                                status = `<span class="text-[10px] uppercase font-bold px-2 py-0.5 rounded bg-emerald-100 text-emerald-700">✅ ${escapeHTML(m.owner)} <span class="normal-case font-medium">(via ${m.ownerSource})</span></span>`;
                            ...
                            return `<tr class="hover:bg-slate-50"><td class="p-2.5 font-bold text-sm text-slate-800">${escapeHTML(m.col)}</td><td class="p-2.5 text-xs text-slate-500">${m.tables.map(escapeHTML).join(' · ')}</td>
                                <td class="p-2.5">${status}</td>
                                <td class="p-2.5">${action}</td>
                                </tr>`;
                        })
                        .join('');
                    html += `<div class="border border-t-0 border-amber-200 rounded-b-lg overflow-x-auto bg-white"><table class="w-full text-left"><thead class="bg-slate-50 text-[10px] uppercase font-bold text-slate-500"><tr><th class="p-2.5">Attribut</th>
                        <th class="p-2.5">Présent dans</th>
                        <th class="p-2.5">Propriétaire</th>
                        <th class="p-2.5">Objet métier</th>
                        </tr>
                        </thead>
            ...
            if (!bos.length)
                return (
                    html +
                    '<p class="text-sm text-slate-400 italic py-10 text-center">Aucun objet métier. Initialisez-en un depuis une source ci-dessus.</p>'
                );
            ...
            const listHtml = bos
                .map(bo => {
                    ...
                    if (!bo.globalOwner)
                        badge =
                            '<span class="text-[9px] uppercase font-bold px-1.5 py-0.5 rounded bg-red-100 text-red-600">sans propriétaire</span>';
                    else if (sources.length && !masters.length && !hasCtxRules)
                        badge =
                            '<span class="text-[9px] uppercase font-bold px-1.5 py-0.5 rounded bg-amber-100 text-amber-700">sans maître</span>';
                    else if (hasCtxRules)
                        badge =
                            '<span class="text-[9px] uppercase font-bold px-1.5 py-0.5 rounded bg-blue-100 text-blue-700">maîtrise contextuelle</span>';
                    else
                        badge =
                            '<span class="text-[9px] uppercase font-bold px-1.5 py-0.5 rounded bg-emerald-100 text-emerald-700">ok</span>';
            ...
            bo.structure.forEach(s => {
                ...
                if (!Array.isArray(s.elements)) {
                    ...
                    bo.elements = (bo.elements || []).filter(elm => {
                        ...
                        const mapping = (elm.mappings || []).find(mm => mm.table === s.table);
                        if (mapping) {
                            moved.push({
                                id: elm.id || 'fe_' + generateId(),
                                name: elm.name,
                                owner: elm.owner || '',
                                col: mapping.col
                            });
        ...
        function boElToggleUsedBy(boId, elId, assetId, on) {
            ...
            if (!bo || !assetId) return;
            const elm = (bo.elements || []).find(e => e.id === elId);
            if (!elm) return;
            elm.usedBy = elm.usedBy || [];
            if (on) {
                if (!elm.usedBy.includes(assetId)) elm.usedBy.push(assetId);
            } else elm.usedBy = elm.usedBy.filter(x => x !== assetId);
            persistAppState();
            renderGovernance();
        }
        ...
        function boAllAttrRows(bo) {
            ...
            getBoFacets(bo).forEach(st =>
                (st.elements || []).forEach(fe => rows.push({ el: fe, facet: st.name, stId: st.id }))
            );
            return rows;
        }
        ...
        function renderBoUsageByApp(bo, rows, assets) {
            ...
            const tgl = r =>
                (r.stId
                    ? `boFacetElToggleUsedBy('${bo.id}','${r.stId}','${r.el.id}'`
                    : `boElToggleUsedBy('${bo.id}','${r.el.id}'`) + `,'${cur.id}',this.checked)`;
            ...
            <div class="border border-slate-200 rounded-lg overflow-y-auto max-h-[520px]"><table class="w-full text-left text-xs">
                <thead class="bg-slate-50 text-[10px] uppercase font-bold text-slate-500 sticky top-0 z-10"><tr>
                    <th class="p-2 w-10 text-center">${ASSET_KINDS[cur.kind][0]}</th><th class="p-2">Attribut</th><th class="p-2">Autres usages déclarés</th></tr>
                        </thead>
                <tbody class="divide-y divide-slate-100">${rows
                    .map(r => {
                        const attribute = r.el;
                        const used = attribute.usedBy || [];
                        const others = used
                            .filter(id => id !== cur.id)
                            .map(assetById)
                            .filter(Boolean);
                        return `<tr class="hover:bg-slate-50 ${used.length ? '' : 'bg-amber-50/30'}">
                        <td class="p-2 text-center"><input type="checkbox" ${used.includes(cur.id) ? 'checked' : ''} onchange="${tgl(r)}" aria-label="${escapeHTML(attribute.name)} utilisé par ${escapeHTML(cur.name)}" class="w-4 h-4 rounded"></td>
                        <td class="p-2 font-bold text-slate-700">${escapeHTML(attribute.name)}${r.facet ? ` <span class="text-[9px] text-emerald-600 font-bold">◆ ${escapeHTML(r.facet)}</span>` : ''}${used.length ? '' : ' <span title="Aucun usage déclaré">😴</span>'}</td>
        ...
        function renderBoUsageMatrix(bo) {
            ...
            if (!assets.length)
                return '<p class="text-sm text-slate-400 italic py-6 text-center">Déclarez d\'abord des applications et des processus dans <strong>Gouvernance ▸ Applis & processus</strong>.</p>';
            const rows = boAllAttrRows(bo);
            if (!rows.length)
                return '<p class="text-sm text-slate-400 italic py-6 text-center">Aucun attribut — définissez la structure de l\'objet d\'abord.</p>';
            ...
            if (govState.usageView === 'byapp')
                return (
                    viewBar +
                    renderBoUsageByApp(bo, rows, assets) +
            ...
            <div class="border border-slate-200 rounded-lg overflow-x-auto max-h-[520px] overflow-y-auto"><table class="w-full text-left text-xs">
                <thead class="bg-slate-50 text-[10px] uppercase font-bold text-slate-500 sticky top-0 z-10"><tr>
                    ...
                    ${assets
                        .map(
                            a => `<th class="p-1.5 text-center bg-slate-50 min-w-[70px]"><div class="font-black">${ASSET_KINDS[a.kind][0]}</div><div class="normal-case leading-tight">${escapeHTML(a.name)}</div>
                        <div class="mt-0.5"><button onclick="${tglAll(a.id, true)}" class="text-emerald-500 hover:text-emerald-700" title="Tout cocher">✔</button> <button onclick="${tglAll(a.id, false)}" class="text-slate-300 hover:text-red-500" title="Tout décocher">✕</button></div>
                        </th>`
                        )
                        .join('')}
                ...
                <tbody class="divide-y divide-slate-100">${rows
                    .map(r => {
                        const attribute = r.el;
                        const used = attribute.usedBy || [];
                        const diff = attribute.sourceApp && attribute.sourceApp !== globalSrc;
                        const setF = r.stId
                            ? `boFacetElSetField('${bo.id}','${r.stId}','${attribute.id}'`
                            : `boElSetField('${bo.id}','${attribute.id}'`;
                        const tgl = aid =>
                            (r.stId
                                ? `boFacetElToggleUsedBy('${bo.id}','${r.stId}','${attribute.id}'`
                                : `boElToggleUsedBy('${bo.id}','${attribute.id}'`) + `,'${aid}',this.checked)`;

                        return `<tr class="hover:bg-slate-50 ${used.length ? '' : 'bg-amber-50/30'}">
                        <td class="p-2 font-bold text-slate-700 whitespace-nowrap">${escapeHTML(attribute.name)}${r.facet ? `<br><span class="text-[9px] text-emerald-600 font-bold">◆ ${escapeHTML(r.facet)}</span>` : ''}${used.length ? '' : ' <span title="Aucun usage déclaré">😴</span>'}</td>
                        <td class="p-2"><select onchange="${setF},'sourceApp',this.value)" class="w-full border p-1 rounded text-[11px] ${diff ? 'border-amber-400 bg-amber-50 font-bold' : 'border-slate-200 bg-white'}"><option value="">${globalA ? '= objet (' + escapeHTML(globalA.name) + ')' : "(celle de l'objet)"}</option>${apps.map(a => `<option value="${a.id}" ${attribute.sourceApp === a.id ? 'selected' : ''}>${escapeHTML(a.name)}</option>`).join('')}</select>${diff ? '<span class="text-[9px] font-black text-amber-600">≠ objet</span>' : ''}</td>
                        ${assets.map(a => `<td class="p-1.5 text-center"><input type="checkbox" ${used.includes(a.id) ? 'checked' : ''} onchange="${tgl(a.id)}" class="w-4 h-4 rounded ${a.kind === 'app' ? 'text-slate-600' : 'text-orange-500'}" title="${escapeHTML(attribute.name)} utilisé par ${escapeHTML(a.name)}"></td>`).join('')}
                        <td class="p-1.5 text-center"><button onclick="openAttrLineage('${bo.id}','${r.stId || ''}','${attribute.id}')" class="text-indigo-500 hover:text-indigo-700 font-bold" title="Lineage de cet attribut">🕸</button></td>
                    </tr>`;
                    })
                    .join('')}</tbody></table></div>
        ...
        function boUsageMatrixAll(boId, assetId, on) {
            ...
            boAllAttrRows(bo).forEach(r => {
                r.el.usedBy = r.el.usedBy || [];
                if (on) {
                    if (!r.el.usedBy.includes(assetId)) r.el.usedBy.push(assetId);
                } else r.el.usedBy = r.el.usedBy.filter(x => x !== assetId);
            });
            persistAppState();
            renderGovernance();
        }
        ...
        function openAttrLineage(boId, stId, elId) {
            const bo = state.governance.businessObjects.find(x => x.id === boId);
            if (!bo) return;
            const facet = stId ? getBoFacets(bo).find(x => x.id === stId) : null;
            const e2 = facet ? (facet.elements || []).find(x => x.id === elId) : (bo.elements || []).find(x => x.id === elId);
            if (!e2) return;
            ...
            if (attrLineageGraph) {
                try {
                    attrLineageGraph.destroy();
                } catch (e3) {}
                attrLineageGraph = null;
            }
        ...
        function buildAttrLineageGraph(bo, st, e2) {
            ...
            if (srcA) {
                nodes.push({
                    ...
                    title: '🖥 ' + srcA.name,
                    content:
                        e2.sourceApp && e2.sourceApp !== ((bo.producedBy || [])[0] || '')
                            ? "≠ source de l'objet"
                            : "source de l'objet",
                    fill: '#e2e8f0',
                    stroke: '#334155'
            ...
            if (!lineageFilesOn()) {
                ...
                maps.forEach(m => {
                    const own = appOwnerOfSource(m.table) || srcA;
                    if (own) {
                        if (!owners.has(own.id)) owners.set(own.id, { a: own, via: [] });
                        owners.get(own.id).via.push(m.table + '.' + m.col);
                    }
                });
                ...
                owners.forEach(({ a, via }) => {
                    ...
                    if (!nodes.some(n => n.id === id))
                        nodes.push({
                            ...
                            type: 'studio-rich-node',
                            title: '🖥 ' + a.name,
                            content: via.length ? 'via ' + via.join(', ') : "source de l'objet",
                            fill: '#e2e8f0',
                            stroke: '#334155'
                        });
                    else {
                        const node = nodes.find(x => x.id === id);
                        if (via.length) node.content = 'via ' + via.join(', ');
                    }
                    edges.push({
                        id: 'ep' + a.id,
                        source: id,
                        target: attrId,
                        label: 'produit',
                        style: { stroke: '#334155', lineWidth: 2, endArrow: { path: '', fill: '#334155' } }
                    });
                });
                if (!owners.size) {
                    nodes.push({
                        id: 'nosrc',
                        type: 'studio-rich-node',
                        title: '❔ aucune application source',
                        content: maps.length ? maps.map(m => m.table + '.' + m.col).join(', ') : 'attribut non alimenté',
                        fill: '#fef9c3',
                        stroke: '#ca8a04'
                    });
                    edges.push({
                        ...
                        target: attrId,
                        label: '',
                        style: { stroke: '#eab308', lineDash: [3, 3] }
                    });
                }
            } else
                maps.forEach((m, i) => {
                    ...
                    edges.push({
                        ...
                        target: attrId,
                        label: 'alimente',
                        style: { stroke: '#2563eb', endArrow: { path: '', fill: '#2563eb' } }
                    });
                });
            if (lineageFilesOn() && !maps.length && srcA)
                edges.push({
                    id: 'e0',
                    source: 'as:' + srcA.id,
                    target: attrId,
            ...
            (e2.usedBy || []).forEach(id => {
                ...
                nodes.push({
                    ...
                    content: asset.criticality || '',
                    fill: asset.kind === 'app' ? '#e2e8f0' : '#ffedd5',
                    stroke: asset.kind === 'app' ? '#334155' : '#ea580c'
                });
                edges.push({
                    id: 'eu' + id,
                    source: attrId,
                    target: 'use:' + id,
                    label: 'utilisé par',
                    style: { stroke: '#ea580c', endArrow: { path: '', fill: '#ea580c' } }
                });
            });
            if (!(e2.usedBy || []).length) {
                nodes.push({
                    id: 'nouse',
                    type: 'studio-rich-node',
                    title: '😴 aucun usage déclaré',
                    content: 'candidat au nettoyage ?',
                    fill: '#fef9c3',
                    stroke: '#ca8a04'
                });
                edges.push({
        ...
        function buildColumnLineageGraph(tbl, col) {
            ...
            for (const bo of governance.businessObjects || []) {
                for (const attribute of bo.elements || []) {
                    if ((attribute.mappings || []).some(m => m.table === tbl && m.col === col)) {
                        buildColumnLineageGraph._viaAttr = true;
                        return buildAttrLineageGraph(bo, null, attribute);
                    }
                }
                for (const st of getBoFacets(bo)) {
                    for (const attribute of st.elements || []) {
                        if (st.table === tbl && attribute.col === col) {
                            buildColumnLineageGraph._viaAttr = true;
                            return buildAttrLineageGraph(bo, st, attribute);
                        }
                    }
                }
            }
            ...
            const sys = String((governance.dictionary[tbl] || {}).sourceSystem || '')
                .trim()
                .toLowerCase();
            const srcA = sys
                ? (governance.assets || []).find(a => a.kind === 'app' && String(a.name).trim().toLowerCase() === sys)
                : null;
            if (srcA) {
                nodes.push({
                    id: 'as:' + srcA.id,
            ...
            (governance.businessObjects || [])
                .filter(bo => (bo.sources || []).some(s2 => s2.table === tbl))
                .forEach(bo => {
                    nodes.push({
                        id: 'bo:' + bo.id,
                        type: 'studio-rich-node',
                        title: '🏛️ ' + bo.name,
                        content: 'objet métier',
                        fill: '#dcfce7',
                        stroke: '#16a34a'
                    });
                    edges.push({
            ...
            (governance.assets || [])
                .filter(
                    a =>
                        a.kind === 'process' &&
                        ((a.tables || []).includes(tbl) || (a.columns || []).some(c => c.table === tbl && c.col === col))
                )
                .forEach(a => {
                    nodes.push({
                        id: 'use:' + a.id,
        ...
        function lineageFilesOn() {
            if (govState.lineageFiles === undefined) {
                try {
                    govState.lineageFiles = localStorage.getItem('sd_lineage_files') === '1';
                } catch (e) {
        ...
        function lineageFilesSet(on) {
            ...
            try {
                localStorage.setItem('sd_lineage_files', on ? '1' : '0');
            } catch (e) {}
        ...
        function buildBoLineageGraph(bo, opts) {
            ...
            add({
                ...
                type: 'studio-rich-node',
                title: '🏛️ ' + bo.name,
                content: nAttr + ' attribut(s)' + (bo.globalOwner ? ' · ' + bo.globalOwner : ''),
                fill: '#dcfce7',
                stroke: '#16a34a'
            });
            const appNode = a => ({
                id: 'as:' + a.id,
                type: 'studio-rich-node',
                title: (a.kind === 'process' ? '⚙️ ' : '🖥 ') + a.name,
                content: a.kind === 'process' ? 'processus' : a.criticality || 'application',
                fill: a.kind === 'process' ? '#ffedd5' : '#e2e8f0',
                stroke: a.kind === 'process' ? '#ea580c' : '#334155'
            });
            ...
            (bo.sources || [])
                ...
                .forEach((s2, i) => {
                    ...
                    if (!files && own) {
                        ...
                        add(appNode(own));
                        producers.delete(own.id);
                        (via[own.id] = via[own.id] || []).push(s2.table);
                    ...
                    edges.push({
                        ...
                        source: tid,
                        target: boId,
                        label: ROLE[s2.role] || 'alimente',
                        style: {
                            stroke: '#2563eb',
                            lineWidth: s2.role === 'maitre' ? 2 : 1,
                            endArrow: { path: '', fill: '#2563eb' }
                        }
                    });
                    if (own) {
                        add(appNode(own));
            ...
            Object.keys(via).forEach(id => {
                const node = nodes.find(x => x.id === 'as:' + id);
                if (node) node.content = 'via ' + via[id].join(', ');
            });
            ...
            (governance.assets || []).forEach(a => {
                if ((a.boIds || []).includes(bo.id) && !cons.has(a.id) && !seen.has('as:' + a.id)) cons.set(a.id, 'rattaché à');
            });
            const useCnt = {};
            (boAllAttrRows(bo) || []).forEach(r => (r.el.usedBy || []).forEach(id => (useCnt[id] = (useCnt[id] || 0) + 1)));
            Object.keys(useCnt).forEach(id => {
                if (!cons.has(id)) cons.set(id, 'utilise ' + useCnt[id] + ' attribut(s)');
            });
            cons.forEach((lbl, id) => {
                const asset = assetById(id);
                if (!asset) return;
                ...
                .forEach((s2, i) => {
                    if (!files) {
                        ...
                        if (own) {
                            ...
                            const node = nodes.find(x => x.id === 'as:' + own.id);
            ...
            if (!edges.some(e => e.source === boId)) {
                add({
                    id: 'nouse',
                    type: 'studio-rich-node',
                    title: '😴 aucun usage déclaré',
                    content: 'consommateurs à renseigner',
                    fill: '#fef9c3',
                    stroke: '#ca8a04'
                });
                edges.push({
            ...
            if (!edges.some(e => e.target === boId)) {
                add({
                    id: 'nosrc',
                    type: 'studio-rich-node',
                    title: '❔ aucune source déclarée',
                    content: 'fiche ▸ Sources',
                    fill: '#fef9c3',
                    stroke: '#ca8a04'
                });
                edges.push({
        ...
        function boFacetElToggleUsedBy(boId, stId, feId, assetId, on) {
            const bo = state.governance.businessObjects.find(x => x.id === boId);
            if (!bo || !assetId) return;
            const facets = getBoFacets(bo).find(x => x.id === stId);
            if (!facets) return;
            const attribute = (facets.elements || []).find(e => e.id === feId);
            if (!attribute) return;
            attribute.usedBy = attribute.usedBy || [];
            if (on) {
                if (!attribute.usedBy.includes(assetId)) attribute.usedBy.push(assetId);
            } else attribute.usedBy = attribute.usedBy.filter(x => x !== assetId);
            persistAppState();
            renderGovernance();
        }
        ...
        function boAttrCardinality(bo, row) {
            ...
            const facets = getBoFacets(bo).find(x => x.id === row.stId);
            const card = (facets && facets.cardinality) || '1–N';
            return { multi: /[–-]\s*N$/.test(card), card, via: facets ? facets.name : null };
        }
        ...
        function boFoldRows(rows) {
            ...
            (rows || []).forEach(r => {
                ...
                let g = byKey.get(key);
                if (!g) {
                    g = { fold: true, base: sp.base, members: [], idx: [], stId: r.stId || '', facet: r.facet };
                    byKey.set(key, g);
                    out.push(g);
                }
                g.members.push(r);
            ...
            return out.map(g =>
                g.fold && g.members.length < 2
                    ? { fold: false, base: g.members[0].el.name, members: g.members, idx: [], stId: g.stId, facet: g.facet }
                    : g
            );
        }
        function boFoldMax(g) {
            return Math.max(g.members.length, ...(g.idx.length ? g.idx : [1]));
        }
        function boFoldIds(g) {
            return g.members.map(m => m.el.id).join(',');
        }
        function boFoldField(g, f) {
            for (const m of g.members) if (m.el[f]) return m.el[f];
            return '';
        }
        function boFoldCols(bo, g) {
            const out = [];
            g.members.forEach(m => {
                const mp = (m.mappings || m.el.mappings || [])[0];
                if (mp) out.push(mp.table + '.' + mp.col);
            });
            return out;
        }
        ...
        function boFoldWrite(boId, stId, ids, f, v) {
            String(ids)
                .split(',')
                .filter(Boolean)
                .forEach(id => boAttrWrite(boId, stId, id, f, v));
            renderGovernance();
        }
        ...
        function boFoldRename(boId, stId, ids, v) {
            ...
            if (!text) return renderGovernance();
            String(ids)
                .split(',')
                .filter(Boolean)
                .forEach(id => {
                    const mapping = boAttrMapOf(bo, id);
                    if (!mapping) return;
                    const nm = String(mapping.el.name || '').trim(),
                        sp = boRepeatSplit(nm);
                    boAttrWrite(boId, stId, id, 'name', text + (sp ? nm.slice(sp.base.length) : ''));
                });
            renderGovernance();
        }
        ...
        function boMultiNorm(v) {
            const text = String(v == null ? '' : v)
                ...
                .toLowerCase();
            if (!text) return '';
            if (['n', '*', 'oui', 'plusieurs', 'multi', 'multivalue', 'multivalué', 'n valeurs'].includes(text)) return 'n';
            if (['1', 'non', 'unique', 'mono', '1 valeur'].includes(text)) return '1';
            const k = parseInt(text, 10);
            if (isFinite(k) && k >= 1 && k <= 999 && String(k) === text.replace(/\s/g, '')) return String(k);
        ...
        function boAttrMulti(bo, g) {
            ...
            return {
                ...
                from,
                decl,
                label: !multi ? '1 valeur' : max ? '1 à ' + max + ' valeurs' : 'plusieurs valeurs'
            };
        }
        // Combien de valeurs pour CET attribut, à l'intérieur d'UNE occurrence de son groupe.
        function boValueCountBadgeHtml(bo, g) {
            const c = boAttrMulti(bo, g),
                cols = boFoldCols(bo, g);
            const why =
                c.from === 'déclaré'
                    ? 'Déclaré sur l’objet métier.'
                    : g.fold
                      ? `Déduit de la structure technique : ${g.members.length} colonnes numérotées${cols.length ? ' (' + cols.join(', ') + ')' : ''}.`
        ...
        function boMultiSelectHtml(cur, onch, title) {
            return (
                `<select onchange="${onch}" class="border border-slate-200 rounded text-[10px] p-0.5 bg-white" title="${escapeHTML(title || 'Combien de valeurs cet attribut peut prendre pour une occurrence de son objet')}">` +
                MULTI_OPTS.map(([v, l]) => `<option value="${v}" ${v === cur ? 'selected' : ''}>${l}</option>`).join('') +
                '</select>'
            );
        }
        ...
        function boToggleFold() {
            govState.dictFold = govState.dictFold === false;
            renderGovernance();
        }
        ...
        async function boFoldSample(boId, ids) {
            ...
            try {
                ...
                for (const id of list) {
                    const mapping = boAttrMapOf(bo, id);
                    if (!mapping || !mapping.map) continue;
                    cols.push(mapping.map.table + '.' + mapping.map.col);
                    let v = [];
                    try {
                        v = await boSampleValues(conn, mapping.map.table, mapping.map.col, 3);
                    } catch (e) {
                        continue;
                    }
                    v.forEach(x => {
                ...
                return showError('Échantillonnage impossible : ' + e.message);
            }
            ...
            list.forEach(id => {
                const mapping = boAttrMapOf(bo, id);
                if (mapping) {
                    mapping.el.examples = val;
                    mapping.el.examplesAuto = true;
                }
            });
            persistAppState();
            renderGovernance();
        ...
        function boAttrMapOf(bo, elId) {
            ...
            for (const st of getBoFacets(bo))
                for (const attribute of st.elements || [])
                    if (attribute.id === elId)
                        return {
                            el: attribute,
                            stId: st.id,
                            map: attribute.col ? { table: st.table, col: attribute.col } : null
                        };
            return null;
        }
        async function boSampleValues(conn, tableName, col, n) {
            const table = tableByName(tableName);
            if (!table || table.status !== 'ready') throw new Error(`source « ${tableName} » non chargée`);
            const raw = `TRIM(CAST(${sqlIdent(col)} AS VARCHAR))`;
            const sql = `SELECT v FROM (SELECT ${raw} AS v FROM ${sqlIdent(duckTableName(table.id))}
        ...
        async function boAttrSampleOne(boId, elId) {
            const bo = (state.governance.businessObjects || []).find(x => x.id === boId);
            if (!bo) return;
            const found = boAttrMapOf(bo, elId);
            if (!found) return;
            if (!found.map)
                return showError(
                    "Cet attribut n'est rattaché à aucune colonne : rattachez-le d'abord dans la fiche de l'objet (Structure)."
                );
            try {
                const { conn } = await getDB();
                ...
                if (!vals.length)
                    return showError(
                        `« ${found.el.name} » : la colonne ${found.map.table}.${found.map.col} ne contient aucune valeur renseignée.`
                    );
        ...
        async function boAttrSampleAll(boId) {
            ...
            try {
                ...
                for (const row of rows) {
                    const found = boAttrMapOf(bo, row.el.id);
                    if (!found || !found.map) {
                        unmapped++;
                        continue;
                    }
                    // On ne remplace jamais un exemple saisi à la main : ce serait détruire du travail.
                    if (found.el.examples && !found.el.examplesAuto) {
                        kept++;
                        continue;
                    }
                    let vals = [];
                    try {
                        vals = await boSampleValues(conn, found.map.table, found.map.col, 4);
                    } catch (e) {
                        none++;
                        continue;
                    }
        ...
        function boAttrLiftFromColumn(bo) {
            ...
            getBoFacets(bo).forEach(st =>
                (st.elements || []).forEach(fe => lift(fe, fe.col ? { table: st.table, col: fe.col } : null))
            );
        ...
        function dictColResolved(tableName, col, field) {
            ...
            for (const bo of state.governance.businessObjects || []) {
                ...
                for (const st of getBoFacets(bo))
                    for (const attribute of st.elements || [])
                        if (st.table === tableName && attribute.col === col && attribute[field])
                            return { value: attribute[field], from: bo.name, attr: attribute.name };
            }
        ...
        function boMapPickToggle(elmId) {
            const element = el('bo-map-' + elmId);
            if (!element) return;
            element.classList.toggle('hidden');
            const btn = el('bo-map-btn-' + elmId);
            if (btn) btn.classList.toggle('on', !element.classList.contains('hidden'));
            if (!element.classList.contains('hidden')) {
                const element = el('bo-tbl-' + elmId);
                if (element) {
                    populateBoMapCols(elmId, element.value);
                    element.focus();
                }
            }
        }
        function boElementRowHtml(bo, elm, names) {
            ...
            const defTbl = master && names.includes(master.name) ? master.name : names[0];
            ...
            return `<tr class="hover:bg-slate-50" ondragover="reorderDragOver(event,'boel')" ondragleave="reorderDragLeave(event)" ondrop="reorderDrop(event,'boel','${elm.id}',(f,t)=>boElReorder('${bo.id}',f,t))">
                <td class="text-center"><span class="bo-handle"><span draggable="true" ondragstart="reorderDragStart(event,'boel','${elm.id}')" ondragend="reorderDragEnd(event)" class="cursor-grab text-slate-400 hover:text-emerald-600 select-none px-0.5" title="Glisser pour ordonner par importance">⠿</span>${[
                    ['top', '⤒', 'Tout en haut'],
                    ['up', '▲', 'Monter'],
                    ['down', '▼', 'Descendre'],
                    ['bottom', '⤓', 'Tout en bas']
                    ...
                    .map(
                        ([w, ic, ti]) =>
                            `<button onclick="boMoveEl('${bo.id}','${elm.id}','${w}')" title="${ti}">${ic}</button>`
                    )
                    .join('')}</span></td>
        ...
        function boAttrTableHead(cols) {
            return `<colgroup>${cols.map(c => `<col style="width:${c[1]}">`).join('')}</colgroup><thead class="bg-slate-50 text-[10px] uppercase font-bold text-slate-500"><tr>${cols.map(c => `<th class="${c[3] || ''}" ${c[2] ? `title="${escapeHTML(c[2])}"` : ''}>${c[0]}</th>`).join('')}</tr>
                </thead>`;
        }
        function boElementsTableHtml(bo, elems, names, emptyMsg) {
            ...
            const head = boAttrTableHead([
                ['', '78px', 'Glisser ou ⤒▲▼⤓ pour ordonner'],
                ['Nom métier', '27%'],
                ['Alimenté par', 'auto', 'Colonnes techniques qui alimentent cet attribut'],
                ['Propriétaire', '15%'],
                ['Nb valeurs', '14%', "Combien de valeurs cet attribut peut prendre pour une occurrence de l'objet"],
                ['🔌', '46px', "Applications et processus qui l'utilisent", 'text-center'],
                ['', '32px']
            ]);
            return `<div class="border border-slate-200 rounded-lg overflow-x-auto"><table class="bo-attr-tbl text-left">${head}<tbody class="divide-y divide-slate-100">${elems.map(e => boElementRowHtml(bo, e, names)).join('')}</tbody>
                </table>
                </div>`;
        }
        const CARD_OPTS = ['1–1', '1–N', 'N–1', 'N–N'];
        function facetScopeLabel(sc) {
            return `${sc.col} ${SCOPE_OPS[sc.op] || sc.op}${sc.op === 'empty' || sc.op === 'notempty' ? '' : ' "' + sc.val + '"'}`;
        }
        ...
        function facetElementRowHtml(bo, st, fe) {
            ...
            const cols = table ? table.headers : [];
        ...
        function renderBoFacetCard(bo, st, names, open) {
            ...
            const scopeChips = (st.scope || []).length
                ? (st.scope || [])
                      .map(
                          (sc, ci) =>
                              `<span class="text-[11px] bg-amber-50 border border-amber-200 text-amber-800 rounded-full px-2 py-0.5 flex items-center gap-1"><strong>${escapeHTML(sc.col)}</strong> ${SCOPE_OPS[sc.op] || sc.op}${sc.op === 'empty' || sc.op === 'notempty' ? '' : ` <strong>"${escapeHTML(sc.val)}"</strong>`} <button onclick="removeFacetScope('${bo.id}','${st.id}',${ci})" class="text-amber-400 hover:text-red-500">✕</button></span>`
                      )
                      .join('')
            ...
            const attrTable = attrs.length
                ? `<div class="border border-slate-200 rounded-lg overflow-x-auto"><table class="bo-attr-tbl text-left">${boAttrTableHead(
                      [
                          ['Nom métier', '28%'],
                          ['Colonne technique', 'auto', 'Colonne de ' + st.table + ' qui alimente cet attribut'],
                          ['Propriétaire', '17%'],
                          [
                              'Nb valeurs',
                              '16%',
                              'Combien de valeurs cet attribut peut prendre dans une occurrence du composant'
                          ],
                          ['🔌', '46px', "Applications et processus qui l'utilisent", 'text-center'],
                          ['', '32px']
                      ]
                  )}<tbody class="divide-y divide-slate-100">${attrs.map(fe => facetElementRowHtml(bo, st, fe)).join('')}</tbody></table></div>`
                : '<p class="text-xs text-slate-400 italic">Aucun attribut.</p>';
            const filterSummary = (st.scope || []).length
                ? (st.scope || []).map(facetScopeLabel).join(' · ')
                : 'toute la table';
            const appliesSummary = (st.applies || []).length
                ? ` · <span class="text-purple-600">si ${(st.applies || []).map(facetScopeLabel).map(escapeHTML).join(' et ')}</span>`
                : '';
                ...
                <div class="p-2 space-y-2">
                    <div>
                        ...
                        <div class="flex flex-wrap items-end gap-1.5 bg-slate-50 border border-slate-100 rounded p-1.5">
                            ...
                            <select id="fac-op-${st.id}" onchange="el('fac-val-${st.id}').classList.toggle('hidden', this.value==='empty'||this.value==='notempty')" class="border border-slate-300 p-1 rounded text-[11px] bg-white">${Object.entries(
                                SCOPE_OPS
                            )
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap items-end gap-1.5 bg-purple-50/40 border border-purple-100 rounded p-1.5">
                            ...
                            <select id="fac-app-op-${st.id}" onchange="el('fac-app-val-${st.id}').classList.toggle('hidden', this.value==='empty'||this.value==='notempty')" class="border border-slate-300 p-1 rounded text-[11px] bg-white">${Object.entries(
                                SCOPE_OPS
                            )
                                .map(([v, l]) => `<option value="${v}">${l}</option>`)
                                .join('')}</select>
        ...
        function renderBoStructure(bo, names) {
            ...
            html += `<details class="border border-slate-200 rounded-lg bg-white mb-3 group" ${core.length <= 6 ? 'open' : ''}>
                <summary class="bg-slate-50/80 border-b border-slate-100 px-3 py-2 cursor-pointer flex items-center gap-2 flex-wrap list-none">
                    ...
                    ${
                        core.length
                            ? `<span class="text-[10px] text-slate-400 truncate max-w-[50%]">${core
                                  .slice(0, 5)
                                  .map(e2 => escapeHTML(e2.name))
                                  .join(' · ')}${core.length > 5 ? ' …' : ''}</span>`
                            : ''
                    }
            ...
            if (facets.length)
                html += `<div class="text-[11px] font-bold text-slate-600 mb-1.5">◆ Composants / facettes (${facets.length}) <span class="text-slate-400 font-medium">— cliquez pour déplier</span></div>`;
            facets.forEach(st => {
                html += renderBoFacetCard(bo, st, names, facets.length === 1);
            });
            // -- Ajouter une facette --
            ...
            if (bo.references.length) {
                html += `<div class="flex flex-wrap gap-1.5 mb-2">${bo.references
                    .map((rf, i) => {
        ...
        function addBoStructure(boId) {
            const bo = state.governance.businessObjects.find(x => x.id === boId);
            if (!bo) return;
            const tbl = el('bo-comp-tbl-' + boId).value,
                card = el('bo-comp-card-' + boId).value;
            const nm = (el('bo-comp-name-' + boId).value || '').trim() || tbl;
            if (!tbl) return;
            const table = tableByName(tbl);
            ...
            bo.structure.push({
                ...
                name: nm,
                kind: 'composition',
                cardinality: card || '1–N',
                scope: [],
                elements: (table ? table.headers : []).map(h => ({ id: 'fe_' + generateId(), name: h, owner: '', col: h }))
            });
            persistAppState();
            renderGovernance();
        ...
        function updateBoFacet(boId, stId, f, v) {
            const bo = state.governance.businessObjects.find(x => x.id === boId);
            const facet = bo && (bo.structure || []).find(x => x.id === stId);
            if (!facet) return;
            facet[f] = v;
            if (f === 'table') {
                // la table change : réinitialise les colonnes des attributs et vide le filtre
                const table = tableByName(v);
                facet.elements = (table ? table.headers : []).map(h => ({
                    id: 'fe_' + generateId(),
                    name: h,
                    owner: '',
                    col: h
        ...
        function removeBoStructure(boId, stId) {
            const bo = state.governance.businessObjects.find(x => x.id === boId);
            if (!bo) return;
            const facet = (bo.structure || []).find(x => x.id === stId);
            bo.structure = (bo.structure || []).filter(s => s.id !== stId);
            // Détache la source si plus aucune facette n'utilise cette table.
            if (facet && !(bo.structure || []).some(s => s.table === facet.table)) {
                bo.sources = (bo.sources || []).filter(s => !(s.table === facet.table && s.role !== 'maitre'));
            }
            persistAppState();
            renderGovernance();
        }
        function addFacetElement(boId, stId) {
            ...
            const facet = bo && (bo.structure || []).find(x => x.id === stId);
            if (!facet) return;
            const table = tableByName(facet.table);
            facet.elements = facet.elements || [];
            facet.elements.push({
                id: 'fe_' + generateId(),
                name: 'Nouvel attribut',
                owner: '',
                col: (table ? table.headers[0] : '') || ''
            });
            persistAppState();
            renderGovernance();
        }
        function updateFacetElement(boId, stId, feId, f, v) {
            const bo = state.governance.businessObjects.find(x => x.id === boId);
            const facet = bo && (bo.structure || []).find(x => x.id === stId);
            const fe = facet && (facet.elements || []).find(x => x.id === feId);
            if (fe) {
                fe[f] = v;
                persistAppState();
            }
        ...
        function removeFacetElement(boId, stId, feId) {
            ...
            const facet = bo && (bo.structure || []).find(x => x.id === stId);
            if (!facet) return;
            facet.elements = (facet.elements || []).filter(x => x.id !== feId);
            persistAppState();
            renderGovernance();
        }
        function addFacetScope(boId, stId) {
            ...
            const facet = bo && (bo.structure || []).find(x => x.id === stId);
            if (!facet) return;
            const col = el('fac-col-' + stId).value,
                element = el('fac-op-' + stId).value;
            const val = element === 'empty' || element === 'notempty' ? '' : el('fac-val-' + stId).value.trim();
            if (!col) return;
            if (element !== 'empty' && element !== 'notempty' && !val)
                return showError('Indiquez la valeur du filtre (ex : PRINCIPALE).');
            facet.scope = facet.scope || [];
            if (facet.scope.some(c => c.col === col && c.op === element && c.val === val))
                return showError('Ce filtre est déjà présent.');
            facet.scope.push({ col, op: element, val });
            persistAppState();
            renderGovernance();
        }
        ...
        function addFacetApplies(boId, stId) {
            ...
            const facet = bo && (bo.structure || []).find(x => x.id === stId);
            if (!facet) return;
            const col = el('fac-app-col-' + stId).value,
                element = el('fac-app-op-' + stId).value;
            const val = element === 'empty' || element === 'notempty' ? '' : el('fac-app-val-' + stId).value.trim();
            if (!col) return;
            if (element !== 'empty' && element !== 'notempty' && !val) return showError('Indiquez la valeur (ex : ENTREPOT).');
            facet.applies = facet.applies || [];
            if (facet.applies.some(c => c.col === col && c.op === element && c.val === val))
                return showError("Cette condition d'applicabilité existe déjà.");
            facet.applies.push({ col, op: element, val });
            persistAppState();
            renderGovernance();
        }
        ...
        async function fillFacetAppliesVals(boId, stId) {
            ...
            const list = el('fac-app-vals-' + stId);
            const colSel = el('fac-app-col-' + stId);
            if (!table || !list || !colSel) return;
        ...
        async function fillFacetScopeVals(boId, stId) {
            ...
            if (!facet) return;
            const table = tableByName(facet.table);
            const list = el('fac-vals-' + stId);
            const colSel = el('fac-col-' + stId);
            if (!table || !list || !colSel) return;
            const col = colSel.value;
            if (!col) return;
            list.innerHTML = '';
        ...
        function addBoReference(boId) {
            const bo = state.governance.businessObjects.find(x => x.id === boId);
            if (!bo) return;
            const tgt = el('bo-ref-tgt-' + boId).value,
                card = el('bo-ref-card-' + boId).value;
            if (!tgt) return;
            bo.references = Array.isArray(bo.references) ? bo.references : [];
        ...
        function removeBoReference(boId, idx) {
            const bo = state.governance.businessObjects.find(x => x.id === boId);
            if (!bo) return;
            bo.references = bo.references || [];
            bo.references.splice(idx, 1);
            persistAppState();
            renderGovernance();
        }
        ...
        const BO_FILTERS = [
            ['all', 'Tous'],
            ['nomap', '⚠ Non alimentés'],
            ['nodef', '✎ Sans définition'],
            ['nouse', '😴 Sans usage']
        ];
        function boAttrHasMap(r) {
            return r.stId ? !!r.el.col : (r.el.mappings || []).length > 0;
        }
        function boAttrProv(r) {
            return r.stId
                ? r.el.col
                    ? r.facet + '.' + r.el.col
                    : ''
                : (r.el.mappings || []).map(m => m.table + '.' + m.col).join(' · ');
        }
        function boFilterOk(bo, r) {
            ...
            const normalized = catNorm(govState.boQ || '');
            if (normalized && !catNorm([r.el.name, boAttrProv(r), r.el.definition || ''].join(' ')).includes(normalized))
                return false;
            return true;
        }
        ...
        function boCompleteness(bo) {
            ...
            const noMap = rows.filter(r => !boAttrHasMap(r)).length,
                noDef = rows.filter(r => !r.el.definition).length,
                noUse = rows.filter(r => !(r.el.usedBy || []).length).length;
            const checks = [
                { w: 15, ok: !!(bo.definition || '').trim(), lbl: 'Écrire la définition', act: `boFocus('bo-def-${bo.id}')` },
                {
                    w: 15,
                    ok: !!(bo.globalOwner || '').trim(),
                    lbl: 'Désigner un propriétaire',
                    act: `boFocus('bo-own-${bo.id}')`
                },
                {
                    w: 15,
                    ok: (bo.sources || []).some(x => x.role === 'maitre'),
                    lbl: 'Désigner une source maître',
                    act: `setBoTab('sources')`
                },
                { w: 10, ok: rows.length > 0, lbl: 'Ajouter des attributs', act: `setBoTab('structure')` },
                {
                    w: 15,
                    ok: rows.length > 0 && noMap === 0,
                    lbl: noMap + ' attribut(s) non alimenté(s)',
                    act: `boGoFilter('nomap')`
                },
                {
                    w: 15,
                    ok: rows.length > 0 && noDef === 0,
                    lbl: noDef + ' attribut(s) sans définition',
                    act: `boGoFilter('nodef')`
                },
                ...(hasAssets
                    ? [
                          {
                              w: 10,
                              ok: rows.length > 0 && noUse === 0,
                              lbl: noUse + ' attribut(s) sans usage',
                              act: `boGoFilter('nouse')`
                          }
                      ]
                    : []),
            ...
            const tot = checks.reduce((a, c) => a + c.w, 0),
                got = checks.filter(c => c.ok).reduce((a, c) => a + c.w, 0);
            return {
                score: Math.round((100 * got) / tot),
                todos: checks.filter(c => !c.ok),
        ...
        function boSetStructView(v) {
            govState.structView = v;
            renderGovernance();
        }
        function boSelect(kind, stId, elId) {
            govState.boSel = { kind, stId: stId || '', elId: elId || '' };
            renderGovernance();
            const boDetailElement = el('boDetail');
            if (boDetailElement && window.innerWidth < 1100)
                boDetailElement.scrollIntoView({ behavior: 'smooth', block: 'start' });
        }
        ...
        function boListKey(ev, boId) {
            ...
            if (items[nx]) {
                const k = JSON.parse(items[nx].getAttribute('data-sel'));
                boSelect(k.kind, k.stId, k.elId);
                requestAnimationFrame(() => {
                    const boListElement = el('boList');
                    if (boListElement) boListElement.focus();
                });
            }
        }
        function boCompletenessHtml(bo) {
            const completeness = boCompleteness(bo);
            const cls = completeness.score < 40 ? 'lo' : completeness.score < 75 ? 'mid' : '';
            return `<div class="flex items-center gap-3 flex-wrap mt-3">
                ...
                <b class="text-base ${completeness.score < 40 ? 'text-red-600' : completeness.score < 75 ? 'text-amber-700' : 'text-emerald-700'}">${completeness.score} %</b>
                <div class="bo-prog ${cls}" role="progressbar" aria-valuenow="${completeness.score}" aria-valuemin="0" aria-valuemax="100"><span style="width:${completeness.score}%"></span></div>
                <span class="text-[11px] text-slate-500 whitespace-nowrap">${completeness.done}/${completeness.all} critères</span></div>
                <div class="flex items-center gap-1.5 flex-wrap mt-2">${
                    completeness.todos.length
                        ? completeness.todos
                              .map(
                                  t =>
                                      `<button data-ro="keep" onclick="${t.act}" class="bo-todo" title="Cliquer pour y aller">→ ${escapeHTML(t.lbl)}</button>`
                              )
                              .join('')
                        : '<span class="bo-todo ok">✓ Rien ne manque — la fiche est complète</span>'
                }</div>`;
        }
        ...
        function renderBoWorkspace(bo, names) {
            ...
            const exists = k =>
                !k
                    ? false
                    : k.kind === 'attr'
                      ? rows.some(r => r.el.id === k.elId)
                      : k.kind === 'facet'
                        ? getBoFacets(bo).some(st => st.id === k.stId)
                        : true;
            if (!exists(cur))
                cur = shown.length ? { kind: 'attr', stId: shown[0].stId || '', elId: shown[0].el.id } : { kind: 'none' };
            const isOn = (kind, stId, elId) =>
                cur.kind === kind && (cur.stId || '') === (stId || '') && (cur.elId || '') === (elId || '');
            const hasAssets = (state.governance.assets || []).length > 0;
            const item = r => {
                const on = isOn('attr', r.stId, r.el.id);
                const map = boAttrHasMap(r);
            ...
            list += core.length
                ? core.map(item).join('')
                : `<div class="text-[11px] text-slate-400 italic px-3 py-2">${rows.some(r => !r.stId) ? 'Aucun attribut ne correspond au filtre.' : 'Aucun attribut propre — « + Attribut ».'}</div>`;
            getBoFacets(bo).forEach(st => {
            ...
            let detail = '';
            if (cur.kind === 'attr') {
                const row = rows.find(x => x.el.id === cur.elId);
                detail = row ? boAttrFormHtml(bo, row, names) : '';
            } else if (cur.kind === 'facet') {
                const facets = getBoFacets(bo).find(x => x.id === cur.stId);
                detail = facets ? boFacetFormHtml(bo, facets, names) : '';
            } else if (cur.kind === 'refs') detail = boRefsFormHtml(bo);
            else if (cur.kind === 'newfacet') detail = boNewFacetFormHtml(bo, names);
            else
                detail = `<div class="bo-empty">🧩 Aucun attribut pour l'instant.<br><span class="text-[12px]">Cliquez sur <b>+ Attribut</b> pour créer le premier, ou sur <b>+ Facette</b> pour rattacher un composant (adresse, contact…).</span></div>`;
            return (
                toolbar +
                `<div class="bo-ws">
        ...
        function boAddAttrAndSelect(boId) {
            ...
            bo.elements.push(e2);
            persistAppState();
            govState.boFilter = 'all';
            govState.boQ = '';
            govState.boSel = { kind: 'attr', stId: '', elId: e2.id };
            renderGovernance();
            boFocus('boF-name');
        }
        function boAttrWriteR(boId, stId, elId, f, v, rerender) {
            boAttrWrite(boId, stId, elId, f, v);
            if (rerender) renderGovernance();
        }
        ...
        function boAttrFormHtml(bo, r, names) {
            ...
            const facet = stId ? getBoFacets(bo).find(x => x.id === stId) : null;
            const W = (f, rer) =>
                `boAttrWriteR('${bo.id}','${stId}','${attribute.id}','${f}',this.value,${rer ? 'true' : 'false'})`;
            const assets = state.governance.assets || [];
            const apps = assets.filter(a => a.kind === 'app');
            const globalSrc = (bo.producedBy || [])[0] || '';
            const multi = boMultiNorm(attribute.multi) || '';
            const cnt = boAttrMulti(bo, { fold: false, members: [{ el: attribute, stId }], idx: [], stId });
            const hasMap = boAttrHasMap(r);
            const sensOpts = cur =>
                '<option value="">—</option>' +
                SENSITIVITY_OPTS.map(o => `<option ${o === cur ? 'selected' : ''}>${o}</option>`).join('');
            // provenance
            let prov = '';
            if (facet) {
                const cols = (tableByName(facet.table) || { headers: [] }).headers;
                prov = `<div class="flex items-center gap-2 flex-wrap"><span class="bo-map-chip" title="${escapeHTML(facet.table)}"><span class="tb">${escapeHTML(facet.table)}</span></span><span class="text-slate-400">▸</span>
                    <select onchange="${W('col', true)}" class="border border-slate-300 rounded-lg px-2 py-1.5 text-xs bg-white font-mono" aria-label="Colonne technique"><option value="">— aucune —</option>${cols.map(h => `<option ${h === attribute.col ? 'selected' : ''}>${escapeHTML(h)}</option>`).join('')}</select>
                    <span class="text-[11px] text-slate-500">colonne de la facette « ${escapeHTML(facet.name)} »</span></div>`;
            } else {
                const maps = attribute.mappings || [];
                const master = boMasterTable(bo);
                const defTbl = master && names.includes(master.name) ? master.name : names[0];
                const cols = defTbl ? (tableByName(defTbl) || { headers: [] }).headers : [];
                prov = `<div class="flex items-center gap-1.5 flex-wrap mb-2">${maps.map((m, i) => boMapChipHtml(m, `removeBoMapping('${bo.id}','${attribute.id}',${i})`)).join('')}${maps.length ? '' : '<span class="bo-map-none">⚠ non alimenté — choisissez la colonne technique ci-dessous</span>'}</div>
                    <div class="bo-map-pick"><span class="lbl">Ajouter une colonne</span>
                        <select id="bo-tbl-${attribute.id}" onchange="populateBoMapCols('${attribute.id}',this.value)" class="border border-slate-300 bg-white" aria-label="Table">${names.map(n => `<option ${n === defTbl ? 'selected' : ''}>${escapeHTML(n)}</option>`).join('')}</select>
                        <span class="text-slate-400 text-xs">▸</span>
                        <select id="bo-col-${attribute.id}" class="border border-slate-300 bg-white" aria-label="Colonne">${cols.map(h => `<option>${escapeHTML(h)}</option>`).join('')}</select>
                        <button onclick="addBoMapping('${bo.id}','${attribute.id}')" class="bg-emerald-600 text-white">Ajouter</button></div>
            ...
            const tgl = aid =>
                stId
                    ? `boFacetElToggleUsedBy('${bo.id}','${stId}','${attribute.id}','${aid}',this.checked)`
                    : `boElToggleUsedBy('${bo.id}','${attribute.id}','${aid}',this.checked)`;
            const used = attribute.usedBy || [];
            const usage = assets.length
                ? `<div class="flex gap-1.5 flex-wrap">${assets.map(a => `<label class="bo-use ${used.includes(a.id) ? 'on' : ''}"><input type="checkbox" ${used.includes(a.id) ? 'checked' : ''} onchange="${tgl(a.id)}">${ASSET_KINDS[a.kind] ? ASSET_KINDS[a.kind][0] : ''} ${escapeHTML(a.name)}</label>`).join('')}</div>
                <div class="bo-grid mt-3"><div class="bo-fld"><label>Source spécifique de cet attribut</label><select onchange="${W('sourceApp', true)}"><option value="">${globalSrc && assetById(globalSrc) ? '= objet (' + escapeHTML(assetById(globalSrc).name) + ')' : "= source de l'objet"}</option>${apps.map(a => `<option value="${a.id}" ${attribute.sourceApp === a.id ? 'selected' : ''}>${escapeHTML(a.name)}</option>`).join('')}</select><div class="hint">À renseigner seulement si cet attribut vient d'une autre application que l'objet.</div>
                    </div>
                    </div>`
            ...
            const order = stId
                ...
                : `<span class="bo-handle ml-auto">${[
                      ...
                      .map(
                          ([w, ic, ti]) =>
                              `<button onclick="boMoveEl('${bo.id}','${attribute.id}','${w}')" class="text-slate-500 hover:text-emerald-700 px-1.5 py-1 rounded border border-slate-200 bg-white text-xs" title="${ti}">${ic}</button>`
                      )
                      .join(' ')}</span>`;
            const del = stId
                ? `removeFacetElement('${bo.id}','${stId}','${attribute.id}')`
                : `removeBoElement('${bo.id}','${attribute.id}')`;
            return `<div class="bo-dhead"><span class="text-lg" aria-hidden="true">${stId ? '◆' : '🔹'}</span><div class="min-w-0"><h3 class="truncate">${escapeHTML(attribute.name)}</h3>
                <div class="sub">${stId ? 'Attribut du composant « ' + escapeHTML(facet.name) + ' »' : 'Attribut propre de « ' + escapeHTML(bo.name) + ' »'}${hasMap ? ' · alimenté par ' + escapeHTML(boAttrProv(r)) : ' · <b class="text-amber-700">non alimenté</b>'}</div>
                </div>${order}
                <button onclick="openAttrLineage('${bo.id}','${stId}','${attribute.id}')" class="text-xs bg-indigo-50 border border-indigo-300 text-indigo-800 px-2.5 py-1.5 rounded-lg font-bold hover:bg-indigo-100 whitespace-nowrap" title="Graphe source → colonne → attribut → usages">🕸 Lineage</button>
            ...
            <div class="bo-sect"><h4>Identité</h4>
                    <div class="bo-grid">
                <div class="bo-fld" style="grid-column:span 2"><label for="boF-name">Nom métier</label><input id="boF-name" type="text" value="${escapeHTML(attribute.name)}" onchange="${W('name', true)}" placeholder="ex : Date de fin de construction"></div>
                <div class="bo-fld"><label>Propriétaire</label><input type="text" value="${escapeHTML(attribute.owner || '')}" onchange="${W('owner', false)}" placeholder="${escapeHTML(bo.globalOwner ? '= ' + bo.globalOwner : "— celui de l'objet —")}">${propBadgeHtml('attr', { boId: bo.id, stId, elId: attribute.id }, 'owner')}</div>
                <div class="bo-fld"><label>Nombre de valeurs</label>${boMultiSelectHtml(multi, `boSetMulti('${bo.id}','${stId}','${attribute.id}',this.value)`, 'Combien de valeurs cet attribut peut prendre pour un(e) ' + bo.name).replace('class="', 'style="width:100%;min-height:34px;font-size:12.5px;padding:7px 9px;border-radius:8px" class="')}${!multi && cnt.multi ? `<div class="hint">Observé : ${escapeHTML(cnt.label)} (colonnes numérotées) — déclarez-le pour le figer.</div>` : ''}${propBadgeHtml('attr', { boId: bo.id, stId, elId: attribute.id }, 'multi')}</div>
            </div>
                    </div>
            <div class="bo-sect"><h4>Sens métier</h4>
                    <div class="bo-grid">
                <div class="bo-fld" style="grid-column:1/-1"><label>Définition</label><textarea onchange="${W('definition', true)}" placeholder="Ce que désigne cet attribut, en langage métier — ce que verra un consommateur de la donnée dans le catalogue.">${escapeHTML(attribute.definition || '')}</textarea>${propBadgeHtml('attr', { boId: bo.id, stId, elId: attribute.id }, 'definition')}</div>
                <div class="bo-fld" style="grid-column:span 2"><label>Exemples de valeurs</label><div class="flex gap-1.5"><input type="text" value="${escapeHTML(attribute.examples || '')}" onchange="${W('examples', false)}" placeholder="ex : PARIS ; LYON" ${attribute.examplesAuto ? 'title="Valeurs prises dans la source rattachée"' : ''}>${hasMap ? `<button onclick="boAttrSampleOne('${bo.id}','${attribute.id}')" class="text-xs bg-white border border-indigo-300 text-indigo-700 px-2.5 rounded-lg font-bold hover:bg-indigo-50 whitespace-nowrap" title="Prendre les valeurs les plus fréquentes dans la source">🎲 Échantillonner</button>` : ''}</div>${attribute.examplesAuto ? '<div class="hint">Échantillonnés automatiquement depuis la source.</div>' : ''}${propBadgeHtml('attr', { boId: bo.id, stId, elId: attribute.id }, 'examples')}</div>
                <div class="bo-fld"><label>Sensibilité</label><select onchange="${W('sensitivity', false)}">${sensOpts(attribute.sensitivity || '')}</select>${propBadgeHtml('attr', { boId: bo.id, stId, elId: attribute.id }, 'sensitivity')}</div>
                <div class="bo-fld" style="grid-column:1/-1"><label>Termes du glossaire (noms métier, synonymes)</label>${termTagsHtml('attr', { boId: bo.id, elId: attribute.id })}<div class="hint">Tapez un terme existant ou un nouveau puis Entrée : il devient un synonyme cherchable dans le catalogue.</div>
                    </div>
            </div>
                    </div>
        ...
        function boFacetFormHtml(bo, st, names) {
            ...
            const scopeChips = (st.scope || []).length
                ? (st.scope || [])
                      .map(
                          (sc, ci) =>
                              `<span class="text-[11px] bg-amber-50 border border-amber-200 text-amber-800 rounded-full px-2 py-0.5 inline-flex items-center gap-1"><strong>${escapeHTML(sc.col)}</strong> ${SCOPE_OPS[sc.op] || sc.op}${sc.op === 'empty' || sc.op === 'notempty' ? '' : ` <strong>"${escapeHTML(sc.val)}"</strong>`} <button onclick="removeFacetScope('${bo.id}','${st.id}',${ci})" class="text-amber-400 hover:text-red-500">✕</button></span>`
                      )
                      .join('')
                : '<span class="text-[11px] text-slate-500 italic">Toute la table — filtrez si cette table porte plusieurs composants (ex : TYPE = PRINCIPALE).</span>';
            const appChips = (st.applies || []).length
                ? (st.applies || [])
                      .map(
                          (sc, ci) =>
                      ...
                      .join('')
                : '<span class="text-[11px] text-slate-500 italic">Vaut pour tous les ' +
                  escapeHTML(boMasterTable(bo) ? boMasterTable(bo).name : 'objets') +
                  '.</span>';
            const mh = boMasterTable(bo) ? boMasterTable(bo).headers : [];
            return `<div class="bo-dhead"><span class="text-lg" aria-hidden="true">◆</span><div class="min-w-0"><h3 class="truncate">${escapeHTML(st.name)}</h3>
                <div class="sub">Composant de « ${escapeHTML(bo.name)} » · ${(st.elements || []).length} attribut(s) · vue de ${escapeHTML(st.table)}</div>
                </div>
            ...
            <div class="bo-sect"><h4>Définition du composant</h4>
                    <div class="bo-grid">
            ...
            <div class="bo-sect"><h4>Filtre fonctionnel — ce qui définit la facette</h4>
                ...
                <div class="flex flex-wrap items-center gap-1.5 bg-slate-50 border border-slate-200 rounded-lg p-2">
                    ...
                    <select id="fac-op-${st.id}" onchange="el('fac-val-${st.id}').classList.toggle('hidden', this.value==='empty'||this.value==='notempty')" class="border border-slate-300 p-1.5 rounded text-xs bg-white">${Object.entries(
                        SCOPE_OPS
                    )
                        .map(([v, l]) => `<option value="${v}">${l}</option>`)
                        .join('')}</select>
                    ...
                    <button onclick="addFacetScope('${bo.id}','${st.id}')" class="text-xs bg-indigo-600 text-white px-3 py-1.5 rounded-lg font-bold">+ Filtre</button></div>
                        </div>
            <div class="bo-sect"><h4>S'applique uniquement si (objet principal)</h4>
                ...
                <div class="flex flex-wrap items-center gap-1.5 bg-purple-50/40 border border-purple-100 rounded-lg p-2">
                    ...
                    <select id="fac-app-op-${st.id}" onchange="el('fac-app-val-${st.id}').classList.toggle('hidden', this.value==='empty'||this.value==='notempty')" class="border border-slate-300 p-1.5 rounded text-xs bg-white">${Object.entries(
                        SCOPE_OPS
                    )
                        .map(([v, l]) => `<option value="${v}">${l}</option>`)
                        .join('')}</select>
                    ...
                    <button onclick="addFacetApplies('${bo.id}','${st.id}')" class="text-xs bg-purple-600 text-white px-3 py-1.5 rounded-lg font-bold">+ Applicabilité</button></div>
                        </div>
        ...
        function boNewFacetFormHtml(bo, names) {
            return `<div class="bo-dhead"><span class="text-lg" aria-hidden="true">◆</span><div><h3>Nouveau composant (facette)</h3><div class="sub">Une facette = une table technique filtrée + un nom métier + une cardinalité (ex : « Adresse principale » = ADRESSE où TYPE = PRINCIPALE, 1–1).</div>
                </div>
                </div>
            <div class="bo-sect"><div class="bo-grid">
                ...
                <div class="bo-fld"><label>Cardinalité — 1 ${escapeHTML(bo.name)} contient…</label><select id="bo-comp-card-${bo.id}">${CARD_OPTS.map(o => `<option ${o === '1–N' ? 'selected' : ''}>${o}</option>`).join('')}</select></div>
                    ...
                    <div class="mt-3 flex gap-2"><button onclick="boAddFacetAndSelect('${bo.id}')" class="text-xs bg-emerald-600 text-white px-4 py-2 rounded-lg font-bold">+ Créer la facette</button><button onclick="boSelect('none')" class="text-xs bg-white border border-slate-300 text-slate-600 px-3 py-2 rounded-lg font-bold">Annuler</button></div>
                </div>`;
        }
        function boAddFacetAndSelect(boId) {
            ...
            addBoStructure(boId);
            const facets = getBoFacets(bo).find(x => !before.includes(x.id));
            if (facets) {
                govState.boSel = { kind: 'facet', stId: facets.id, elId: '' };
                renderGovernance();
            }
        }
        function boRefsFormHtml(bo) {
            bo.references = Array.isArray(bo.references) ? bo.references : [];
            const otherBos = state.governance.businessObjects.filter(b => b.id !== bo.id);
            return `<div class="bo-dhead"><span class="text-lg" aria-hidden="true">🔗</span><div><h3>Objets référencés</h3><div class="sub">Objets métier séparés liés à « ${escapeHTML(bo.name)} » (ex : un Emplacement référence un Site).</div>
                </div>
                </div>
            <div class="bo-sect">${
                bo.references.length
                    ? `<div class="flex flex-wrap gap-1.5 mb-3">${bo.references
                          .map((rf, i) => {
                              const target = state.governance.businessObjects.find(b => b.id === rf.boId);
                          ...
                          .join('')}</div>`
                    : '<p class="text-[12px] text-slate-500 italic mb-3">Aucune référence.</p>'
            }
                ${
                    otherBos.length
                        ? `<div class="bo-grid"><div class="bo-fld"><label>Référencer un objet</label><select id="bo-ref-tgt-${bo.id}">${otherBos.map(b => `<option value="${b.id}">${escapeHTML(b.name)}</option>`).join('')}</select></div>
                ...
                <div class="bo-fld"><label>&nbsp;</label><button onclick="addBoReference('${bo.id}')" class="text-xs bg-teal-600 text-white px-3 py-2 rounded-lg font-bold w-full">+ Référence</button></div>
                    </div>`
                        : '<p class="text-[12px] text-slate-500 italic">Créez un second objet métier pour pouvoir le référencer.</p>'
                }</div>`;
        }
        function renderBoDetail(bo, names) {
            ...
            const tabs = [
                ...
                [
                    'usage',
                    '🔌 Applis & usages',
                    (state.governance.assets || []).length
                        ? boAllAttrRows(bo).filter(r => !(r.el.usedBy || []).length).length || null
                        : null
                ],
                ['audit', '🔎 Audit', null]
            ];
            ...
            if (cur === 'structure') html += renderBoWorkspace(bo, names);
                ...
                html += `<div class="mb-3 border border-slate-200 rounded-lg p-2.5 bg-slate-50/60">
                    ...
                    ${_qr.length ? '<div class="space-y-1">' + _qr.map(r => `<div class="text-xs flex items-center gap-2"><span class="font-bold text-slate-700">${escapeHTML(r.name)}</span><span class="text-[10px] text-slate-400">${QR_TYPES[r.type] || r.type}${r.bo === bo.id ? " · sur l'objet" : r.table ? ' · ' + escapeHTML(r.table) : ''}${r.enabled === false ? ' · désactivée' : ''}</span>${r.last ? `<span class="ml-auto font-bold ${r.last.fails === 0 ? 'text-emerald-600' : 'text-amber-600'}">${Math.round(100 * r.last.rate)} %</span>` : '<span class="ml-auto text-[10px] text-slate-300">non exécutée</span>'}</div>`).join('') + '</div>' : '<p class="text-[11px] text-slate-400 italic">Aucune règle de qualité rattachée. Créez-en dans <b>📏 Règles &amp; score</b> en choisissant <b>cet objet métier</b> (ou une de ses tables) comme cible — elle apparaîtra ici et sera évaluée par l\'audit.</p>'}
                        ...
                        <div id="bo-global-audit-${bo.id}"></div>`;
            }

            html += '</div></div>';
            return html;
        }
        function setBoTab(tab) {
            govState.boTab = tab;
            renderGovernance();
        }
        function openBoAudit(boId) {
            govState.boTab = 'audit';
            renderGovernance();
            requestAnimationFrame(() => auditBusinessObject(boId));
        }
        ...
        function renderBoSourcesBlock(bo, names) {
            ...
            const hasCtxRules = ((bo.contextRules || {}).rules || []).length > 0;
            let alert = '';
            if (sources.length && masters.length === 0 && !hasCtxRules)
                alert =
                    '<span class="text-[10px] uppercase font-bold px-2 py-0.5 rounded bg-red-100 text-red-700">⚠️ Aucune source maître désignée</span>';
            else if (masters.length > 1 && !hasCtxRules)
                alert = `<span class="text-[10px] uppercase font-bold px-2 py-0.5 rounded bg-amber-100 text-amber-800">⚠️ ${masters.length} sources maîtres en conflit — utilisez les règles contextuelles ci-dessous ?</span>`;
            else if (hasCtxRules)
                alert = `<span class="text-[10px] uppercase font-bold px-2 py-0.5 rounded bg-blue-100 text-blue-700">⚖️ Maîtrise contextuelle active (${(bo.contextRules.rules || []).length} règle(s))</span>`;
            const chips = sources
                .map(s => {
                    const roleColor =
                        s.role === 'maitre'
                    ...
                    <select onchange="updateBoSourceRole('${bo.id}','${escapeHTML(s.table.replace(/'/g, "\\'"))}',this.value)" class="bg-transparent text-xs font-bold border-0 cursor-pointer">${Object.keys(
                        BO_ROLES
                    )
                        .map(r => `<option value="${r}" ${r === s.role ? 'selected' : ''}>${BO_ROLES[r]}</option>`)
                        .join('')}</select>
            ...
            const flow =
                masters.length && sources.some(s => s.role === 'destinataire')
                    ? `<div class="text-xs text-slate-500 mt-2">Circulation : <strong class="text-emerald-700">${masters.map(m => escapeHTML(m.table)).join(', ')}</strong> → <strong>🏛️ ${escapeHTML(bo.name)}</strong> → <strong class="text-slate-600">${sources
                          .filter(s => s.role === 'destinataire')
                          .map(s => escapeHTML(s.table))
                          .join(', ')}</strong></div>`
                    : '';
            return `<div class="border border-slate-200 rounded-lg p-3 bg-slate-50/50">
                ...
                <div class="flex flex-wrap items-center gap-1.5">
                    ...
                    <select id="bo-src-role-${bo.id}" class="border border-slate-200 p-1 rounded text-xs bg-white">${Object.keys(
                        BO_ROLES
                    )
                        .map(r => `<option value="${r}">${BO_ROLES[r]}</option>`)
                        .join('')}</select>
                ...
                <div class="mt-2 space-y-1.5">
                    ${sources
                        .map((s, si) => {
                            const table = tableByName(s.table);
                        ...
                        ${(s.filters || [])
                            .map((f, fi) => {
                                const dlId = `bofval-${bo.id}-${si}-${fi}`;
                                return `<div class="flex items-center gap-1 mt-1 pl-4 flex-wrap">
                ...
                ${(() => {
                    const att = [];
                    (state.governance.perimeters || []).forEach(p2 => {
                        if ((p2.boIds || []).includes(bo.id)) att.push('🗂 ' + p2.name);
                    });
                    (state.governance.glossary || []).forEach(g2 => {
                        if ((g2.boIds || []).includes(bo.id)) att.push('📖 ' + g2.term);
                    });
                    (state.governance.assets || []).forEach(u2 => {
                        if ((u2.boIds || []).includes(bo.id)) att.push((u2.kind === 'process' ? '⚙️ ' : '🖥 ') + u2.name);
                    });
                    return att.length
                        ? `<div class="text-[11px] text-slate-500 mt-2"><span class="text-[10px] uppercase font-bold text-slate-400">Rattaché à :</span> ${att.map(escapeHTML).join(' · ')}</div>`
                        : '';
                })()}
            </div>`;
        }
        ...
        function boSourceWhere(bo, tableName) {
            const src = (bo.sources || []).find(s2 => s2.table === tableName);
            if (!src) return '';
            const valid = (src.filters || []).filter(
                c => c.col && c.op && (['empty', 'nempty'].includes(c.op) || String(c.val || '').length)
            );
            let sql = '';
            valid.forEach(c => {
                const conditionSql = qualCondSql(c);
                if (!conditionSql) return;
                sql = sql ? `(${sql}) ${c.conn === 'OR' ? 'OR' : 'AND'} ${conditionSql}` : conditionSql;
            });
            return sql;
        }
        function boSrcAddFilter(boId, si) {
            const bo = state.governance.businessObjects.find(x => x.id === boId);
            if (!bo || !bo.sources[si]) return;
            (bo.sources[si].filters = bo.sources[si].filters || []).push({ col: '', op: 'eq', val: '', conn: 'AND' });
            persistAppState();
            renderGovernance();
        }
        function boSrcSetFilter(boId, si, fi, f, v) {
            const bo = state.governance.businessObjects.find(x => x.id === boId);
            const flt = bo && bo.sources[si] && (bo.sources[si].filters || [])[fi];
            if (!flt) return;
            flt[f] = v;
            persistAppState();
            if (f === 'op' || f === 'col') renderGovernance();
        }
        function boSrcDelFilter(boId, si, fi) {
            const bo = state.governance.businessObjects.find(x => x.id === boId);
            if (!bo || !bo.sources[si]) return;
            bo.sources[si].filters.splice(fi, 1);
            persistAppState();
            renderGovernance();
        }
        ...
        async function fillBoVolumetry(boId, containerId, opts) {
            ...
            if (!names.length) {
                cont.innerHTML =
                    '<span class="text-[11px] text-slate-400 italic">Aucune source chargée — volumétrie indisponible.</span>';
                return;
            }
            ...
            try {
                ...
                for (const name of names) {
                    const tb = tableByName(name);
                    if (!tb) continue;
                    const w = boSourceWhere(bo, name);
                    try {
                        const res = await conn.query(
                            `SELECT COUNT(*)::BIGINT AS n FROM ${sqlIdent(duckTableName(tb.id))}${w ? ' WHERE ' + w : ''}`
                        );
                        counts.push({
                            name: name,
                            n: Number(arrowResultToObjects(res)[0].n),
                            master: table && name === table.name,
                            filtered: !!w
                        });
                    } catch (e) {
                        counts.push({ name: name, n: null, master: table && name === table.name });
                    }
                }
                ...
                cont.innerHTML = `<span class="text-[11px] text-red-600">Comptage impossible : ${escapeHTML(e.message)}</span>`;
            }
        }
            ...
            if (!Array.isArray(bo.hierarchies)) {
                ...
                if (bo.hierarchy && bo.hierarchy.enabled) {
                    ...
                    bo.hierarchies.push({
                        ...
                        name: 'Hiérarchie 1',
                        mode: old.mode || 'self',
                        childCol: old.childCol || '',
                        parentKeyCol: old.parentKeyCol || '',
                        linkTable: old.linkTable || '',
                        linkChildCol: old.linkChildCol || '',
                        linkParentCol: old.linkParentCol || '',
                        typeCol: old.typeCol || '',
                        maxDepth: '',
                        levels: []
                    });
                    // ancien modèle : liste ordonnée -> parents admis = niveau précédent
                    (old.levels || []).forEach((lv, i) =>
                        bo.hierarchies[0].levels.push({ name: lv, parents: i > 0 ? [old.levels[i - 1]] : [] })
                    );
                }
                delete bo.hierarchy;
            }
        ...
        function buildDetectedHierarchy(t) {
            ...
            if (hi.self) {
                ...
                h.childCol = g.childPtr;
                h.parentKeyCol = g.parentKey;
            } else {
                h.mode = 'link';
                ...
                if (linkT) {
                    ...
                    const rels = state.relations.filter(
                        r =>
                            r.sourceCol &&
                            r.targetCol &&
                            ((r.sourceTable === linkT.id && r.targetTable === t.id) ||
                                (r.targetTable === linkT.id && r.sourceTable === t.id))
                    );
                    if (rels.length >= 2) {
        ...
        function renderBoHierarchy(bo) {
            ...
            const table = boMasterTable(bo);
            const detected = table ? isHierTable(tableHierarchyInfo(), table.id) : false;
            let html = `<div class="border border-emerald-200 rounded-lg p-3 bg-emerald-50/30">
                <div class="flex items-center gap-2 flex-wrap mb-1">
                    ...
                    <button onclick="addBoHierarchy('${bo.id}')" class="ml-auto text-xs bg-emerald-600 text-white px-3 py-1.5 rounded-lg font-bold" ${table ? '' : 'disabled title="Rattachez d\'abord une source maître"'}>+ Hiérarchie ${detected && !hiers.length ? '(pré-remplie depuis le modèle de données)' : ''}</button>
                </div>`;
            hiers.forEach(h => {
                html += renderBoHierCard(bo, h, table);
            });
            return html + '</div>';
        }
        function renderBoHierCard(bo, h, t) {
            const headers = t ? t.headers : [];
            const colSel = (field, val, ph) =>
                `<select onchange="updateBoHier('${bo.id}','${h.id}','${field}',this.value)" class="border border-slate-300 p-1.5 rounded text-xs bg-white"><option value="">${ph}</option>${headers.map(c => `<option ${val === c ? 'selected' : ''}>${escapeHTML(c)}</option>`).join('')}</select>`;
            const lt = h.linkTable ? tableByName(h.linkTable) : null;
            const linkColSel = (field, val, ph) =>
                `<select onchange="updateBoHier('${bo.id}','${h.id}','${field}',this.value)" class="border border-slate-300 p-1.5 rounded text-xs bg-white"><option value="">${ph}</option>${(lt ? lt.headers : []).map(c => `<option ${val === c ? 'selected' : ''}>${escapeHTML(c)}</option>`).join('')}</select>`;
            ...
            return `<div class="border-2 border-emerald-300 rounded-lg p-3 bg-emerald-50/40 mb-2">
                ...
                <div class="flex flex-wrap items-end gap-2 mb-2 text-xs">
                    <div><label class="text-[9px] uppercase font-bold text-slate-400 block">Table de liaison</label><select onchange="updateBoHier('${bo.id}','${h.id}','linkTable',this.value)" class="border border-slate-300 p-1.5 rounded text-xs bg-white"><option value="">choisir...</option>${readyTableNames()
                        .map(n => `<option ${h.linkTable === n ? 'selected' : ''}>${escapeHTML(n)}</option>`)
                        .join('')}</select></div>
                ...
                <div class="border-t border-emerald-100 pt-2 mt-1">
                    <div class="flex flex-wrap items-end gap-2 mb-1.5">
                        ...
                        ${
                            h.typeCol
                                ? `<div><label class="text-[9px] uppercase font-bold text-slate-400 block">Ajouter un niveau</label>
        ...
        function addBoHierarchy(boId) {
            ...
            if (!table) return;
            const hiers = getBoHierarchies(bo);
            const detected = !hiers.length ? buildDetectedHierarchy(table) : null;
            hiers.push(
                detected || {
                    id: 'bh_' + generateId(),
                    name: 'Hiérarchie ' + (hiers.length + 1),
                    mode: 'self',
                    childCol: '',
                    parentKeyCol: '',
                    linkTable: '',
        ...
        function removeBoHierarchy(boId, hierId) {
            const bo = state.governance.businessObjects.find(x => x.id === boId);
            if (!bo) return;
            bo.hierarchies = getBoHierarchies(bo).filter(x => x.id !== hierId);
            persistAppState();
            renderGovernance();
        }
        function updateBoHier(boId, hierId, f, v) {
            const { h } = findBoHier(boId, hierId);
            if (!h) return;
            h[f] = v;
            persistAppState();
            renderGovernance();
        ...
        function addBoHierLevel(boId, hierId) {
            const { h } = findBoHier(boId, hierId);
            if (!h) return;
            const inp = el('bo-hier-lvl-' + hierId);
        ...
        function removeBoHierLevel(boId, hierId, i) {
            const { h } = findBoHier(boId, hierId);
            if (!h) return;
            const removed = h.levels.splice(i, 1)[0];
            h.levels.forEach(l => (l.parents = (l.parents || []).filter(p => p !== removed.name)));
            persistAppState();
            renderGovernance();
        }
        function toggleBoHierLevelParent(boId, hierId, i, parentName, checked) {
            const { h } = findBoHier(boId, hierId);
            if (!h || !h.levels[i]) return;
            const lv = h.levels[i];
            lv.parents = lv.parents || [];
            if (checked) {
                if (!lv.parents.includes(parentName)) lv.parents.push(parentName);
            } else lv.parents = lv.parents.filter(p => p !== parentName);
            persistAppState();
            renderGovernance();
        }
        async function fillBoHierLevelValues(boId, hierId) {
            const { bo, h } = findBoHier(boId, hierId);
            if (!bo || !h || !h.typeCol) return;
            const table = boMasterTable(bo);
            const list = el('bo-hier-lvls-' + hierId);
            if (!table || table.status !== 'ready' || !list) return;
            try {
                const { conn } = await getDB();
                const raw = `TRIM(CAST(${sqlIdent(h.typeCol)} AS VARCHAR))`;
                const res = await conn.query(
                    `SELECT ${raw} AS v, COUNT(*)::BIGINT AS c FROM ${sqlIdent(duckTableName(table.id))} WHERE ${sqlIdent(h.typeCol)} IS NOT NULL AND ${raw} <> '' GROUP BY 1 ORDER BY c DESC LIMIT 20`
                );
                list.innerHTML = arrowResultToObjects(res)
                    .map(r => `<option value="${escapeHTML(r.v)}">`)
                    .join('');
        ...
        async function auditBoHierarchy(boId, hierId) {
            ...
            if (!bo || !h) return;
            const table = boMasterTable(bo);
            const out = el('bo-hier-audit-' + hierId);
            if (!out || !table) return;
            ...
            try {
                ...
                if (h.mode === 'link') {
                    const table = tableByName(h.linkTable);
                    if (!table || !h.linkChildCol || !h.linkParentCol || !h.parentKeyCol)
                        throw new Error('Configuration de la table de liaison incomplète.');
                    const L = sqlIdent(duckTableName(table.id));
                ...
                const unknownExpr =
                    h.typeCol && levels.length
                        ? `(${typeU('c')} IS NOT NULL AND ${typeU('c')} NOT IN (${knownLevels.join(', ')}))`
                        : 'FALSE';
                const res = await conn.query(`
                    ...
                    ${parentJoinSql}
                `);
                const rows = arrowResultToObjects(res)[0];
                ...
                if (maxD > 0 && h.mode !== 'link') {
                    ...
                    `);
                    const rows = arrowResultToObjects(dres)[0];
                    const beyond = Number(rows.beyond);
                    depthHtml = `<div class="p-2 border rounded-lg ${beyond > 0 ? 'bg-red-50 border-red-200' : 'bg-white'}"><div class="text-sm font-black ${beyond > 0 ? 'text-red-600' : 'text-slate-700'}">${Number(rows.maxd)}${Number(rows.maxd) > cap - 1 ? '+' : ''} / ${maxD}</div>
                        <div class="text-[9px] uppercase font-bold text-slate-400">profondeur max (${beyond.toLocaleString('fr-FR')} au-delà)</div>
                        </div>`;
                }
                const tile = (lbl, v, bad) =>
                    `<div class="p-2 border rounded-lg ${Number(v) > 0 && bad ? 'bg-red-50 border-red-200' : 'bg-white'}"><div class="text-sm font-black ${Number(v) > 0 && bad ? 'text-red-600' : 'text-slate-700'}">${Number(v).toLocaleString('fr-FR')}</div>
                        <div class="text-[9px] uppercase font-bold text-slate-400">${lbl}</div>
                        </div>`;
                // Consultation / export des lignes en défaut : une requête par anomalie détectée.
                const rowSel = pred =>
                    `SELECT DISTINCT c.* EXCLUDE (__rn) FROM ${parentJoinSql.replace(/^FROM /, '')} WHERE ${pred}`;
                const hierInspect = [];
                if (Number(rows.orphans) > 0)
                    hierInspect.push({
                        lbl: 'orphelins',
                        sql: rowSel(`${childExpr} IS NOT NULL AND ${norm('p', h.parentKeyCol)} IS NULL`)
                    });
                if (Number(rows.selfloops) > 0)
                    hierInspect.push({ lbl: 'auto-boucles', sql: rowSel(`${childExpr} = ${norm('c', h.parentKeyCol)}`) });
                if (Number(rows.lvlviol) > 0) hierInspect.push({ lbl: 'parents non admis', sql: rowSel(violExpr) });
                if (Number(rows.unknowntypes) > 0) hierInspect.push({ lbl: 'types hors niveaux', sql: rowSel(unknownExpr) });
                if (depthHtml.includes('bg-red-50') && maxD > 0 && h.mode !== 'link') {
                    // des lignes dépassent la profondeur max
                    const cap = maxD + 3;
                    hierInspect.push({
                        lbl: 'au-delà de la profondeur max',
                ...
                out.innerHTML = `<div class="grid grid-cols-3 md:grid-cols-7 gap-2">
                    ${tile('lignes', rows.total, false)}
                    ${tile('racines', rows.roots, false)}
                    ${tile('orphelins (parent inexistant)', rows.orphans, true)}
                    ${tile('auto-boucles', rows.selfloops, true)}
                    ${tile('parents non admis', rows.lvlviol, true)}
                    ${tile('types hors niveaux', rows.unknowntypes, true)}
                    ${depthHtml}
                </div>
                ${inspectRow}
                ...
                out.innerHTML = `<p class="text-xs text-red-600">Audit impossible : ${escapeHTML(e.message)}</p>`;
                return null;
            }
        }
        ...
        async function auditBusinessObject(boId) {
            ...
            const checks = [
                ...
                [
                    'Rattaché à un périmètre métier',
                    governance.perimeters.some(
                        p => (p.boIds || []).includes(bo.id) || (table && (p.tables || []).includes(table.name))
                    )
                ],
                [
                    'Utilisé par au moins une application ou un processus',
                    (governance.assets || []).some(
                        a2 =>
                            (a2.boIds || []).includes(bo.id) ||
                            (table &&
                                ((a2.tables || []).includes(table.name) ||
                                    (a2.columns || []).some(c => c.table === table.name)))
                    ) ||
                        (bo.consumedBy || []).length > 0 ||
            ...
            if (hiers.length) {
                html +=
                    `<div class="text-[10px] uppercase font-bold text-slate-400 mb-1 mt-2">🌳 Hiérarchies (${hiers.length})</div>` +
                    hiers
                        .map(
                            h =>
                                `<div class="mb-2"><div class="text-[11px] font-bold text-slate-600 mb-1">${escapeHTML(h.name || 'Hiérarchie')}${h.maxDepth ? ` <span class="text-slate-400 font-medium">(profondeur max ${h.maxDepth})</span>` : ''}</div>
                                    <div id="bo-hier-audit-${h.id}"></div>
                                    </div>`
                        )
                        .join('');
            }
            // 3. Règles métier touchant TOUT le périmètre de l'objet (maître + composants + sources)
            const rules = (governance.rules || []).filter(r => objTables.has(r.parentTable) || objTables.has(r.childTable));
            if (rules.length) {
                html += `<div class="text-[10px] uppercase font-bold text-slate-400 mb-1 mt-2">⚖️ Règles métier du périmètre (${rules.length}) <span class="normal-case font-medium text-slate-300">— ${Array.from(objTables).map(escapeHTML).join(', ')}</span></div>
                    <div id="bo-rules-results-${boId}" class="space-y-1"><p class="text-xs text-slate-400">Exécution...</p>
                    </div>`;
            }
            ...
            if (qRules.length) {
                html += `<div class="text-[10px] uppercase font-bold text-slate-400 mb-1 mt-2">📏 Règles de qualité (${qRules.length})</div><div id="bo-qr-results-${boId}" class="space-y-1"><p class="text-xs text-slate-400">Exécution...</p>
                    </div>`;
            }
            ...
            if (facets11.length) {
                html += `<div class="text-[10px] uppercase font-bold text-slate-400 mb-1 mt-2">🎭 Facettes 1–1 (${facets11.length})</div><div id="bo-facets-results-${boId}" class="space-y-1"><p class="text-xs text-slate-400">Vérification...</p>
                    </div>`;
            }
            html += '</div>';
            out.innerHTML = html;
            ...
            await runNoSpill(__auditConn, async () => {
                ...
                for (const h of hiers) {
                    try {
                        await auditBoHierarchy(boId, h.id);
                    } catch (e) {}
                }
                if (rules.length) {
                    ...
                    for (const rule of rules) {
                        try {
                            ...
                            let insp = '';
                            if (res.viol > 0) {
                                const vs = bizRuleViolRowsSql(rule);
                                if (vs) insp = qualInspectButtons(registerQualInspect(`Règle : ${bizRuleLabel(rule)}`, vs));
                            }
                            parts.push(
                                `<div class="text-xs ${res.viol > 0 ? 'text-red-600' : 'text-emerald-700'}">${res.viol > 0 ? '❌' : '✅'} ${escapeHTML(bizRuleLabel(rule))} — ${res.viol.toLocaleString('fr-FR')} violation(s) sur ${res.total.toLocaleString('fr-FR')} ${escapeHTML(rule.parentTable)}${res.examples.length ? ` <span class="text-slate-400">(ex : ${res.examples.map(escapeHTML).join(', ')})</span>` : ''}${insp}</div>`
                            );
                        } catch (e) {
                            parts.push(
                                `<div class="text-xs text-red-600">⚠️ Règle inexécutable : ${escapeHTML(e.message)}</div>`
                            );
                        }
                    }
                ...
                if (qRules.length) {
                    const qout = el('bo-qr-results-' + boId);
                    const qparts = [];
                    for (const qRule of qRules) {
                        try {
                            const ev = await qrEvalRule(qRule, __auditConn);
                            if (!ev.ran) {
                                qparts.push(
                                    `<div class="text-xs text-slate-400">➖ ${escapeHTML(qRule.name)} — non applicable (colonne/table non résolue)</div>`
                                );
                                continue;
                            }
                            const rate = Math.round(100 * ev.rate);
                            const bad = ev.fails > 0;
                            let insp = '';
                            if (bad) {
                                const vs = qrInspectSql(qRule);
                                if (vs)
                                    insp = qualInspectButtons(
                                        registerQualInspect(`Règle qualité : ${qRule.name}`, vs, qrExplainHtml(qRule))
                                    );
                            }
                            qparts.push(
                                `<div class="text-xs ${bad ? 'text-red-600' : 'text-emerald-700'}">${bad ? '❌' : '✅'} ${escapeHTML(qRule.name)} <span class="text-slate-400">(${QR_TYPES[qRule.type] || qRule.type}${qRule.bo === bo.id ? " · sur l'objet" : qRule.table ? ' · ' + escapeHTML(qRule.table) : ''})</span> — ${bad ? `${ev.fails.toLocaleString('fr-FR')} échec(s) sur ${ev.total.toLocaleString('fr-FR')} (${rate} %)` : `conforme (${ev.total.toLocaleString('fr-FR')} lignes)`}${insp}</div>`
                            );
                        } catch (e) {
                            qparts.push(
                                `<div class="text-xs text-red-600">⚠️ ${escapeHTML(qRule.name)} : ${escapeHTML(e.message)}</div>`
                            );
                        }
                    }
                    if (qout)
                        qout.innerHTML = qparts.join('') || '<p class="text-xs text-slate-400">Aucune règle applicable.</p>';
                    persistAppState();
                }
        ...
        function bizRuleLabel(r) {
            ...
            return (
                r.label ||
                `1 ${r.parentTable} doit avoir ${expectTxt} ${r.n} ${r.childTable}${r.cond ? ` [${r.cond.col} ${SCOPE_OPS[r.cond.op] || r.cond.op} "${r.cond.val}"]` : ''}`
            );
        }
        async function auditBusinessRule(rule) {
            const { conn } = await getDB();
            const pT = tableByName(rule.parentTable),
                cT = tableByName(rule.childTable);
            if (!pT || !cT) throw new Error(`Tables de la règle non chargées (${rule.parentTable} / ${rule.childTable}).`);
            const norm = (a, c) => `NULLIF(UPPER(TRIM(CAST(${a}.${sqlIdent(c)} AS VARCHAR))), '')`;
            const condSql =
                rule.cond && rule.cond.col
                    ? ' AND ' + scopeCondSql(rule.cond).replaceAll(sqlIdent(rule.cond.col), 'x.' + sqlIdent(rule.cond.col))
                    : '';
            ...
            `);
            const rows = arrowResultToObjects(res)[0];
            const exs = (Array.isArray(rows.exs) ? rows.exs : rows.exs && rows.exs.toArray ? rows.exs.toArray() : [])
                .filter(Boolean)
                .slice(0, 5);
            return { total: Number(rows.total), viol: Number(rows.viol || 0), examples: exs };
        }
        function renderBizRules() {
            const cont = el('bizRulesList');
            if (!cont) return;
            const rules = state.governance.rules || [];
            const validRels = state.relations.filter(
                r => r.sourceCol && r.targetCol && state.tables[r.sourceTable] && state.tables[r.targetTable]
            );
            el('bizRuleRel').innerHTML = validRels
                .map(
                    r =>
                        `<option value="${r.id}">${escapeHTML(state.tables[r.sourceTable].name)}.${escapeHTML(r.sourceCol)} ↔ ${escapeHTML(state.tables[r.targetTable].name)}.${escapeHTML(r.targetCol)}</option>`
                )
                .join('');
            bizRuleRelChanged();
            if (!rules.length) {
                cont.innerHTML = '<p class="text-xs text-slate-400 italic">Aucune règle définie.</p>';
                return;
            }
            cont.innerHTML = rules
                .map(r => {
        ...
        function updateBizRule(id, field, value) {
            const rule = (state.governance.rules || []).find(x => x.id === id);
            if (!rule) return;
            if (field === 'expect') rule.expect = value;
            else if (field === 'n') rule.n = Math.max(0, parseInt(value) || 0);
            else if (field === 'label') rule.label = (value || '').trim();
            else if (field === 'condVal') {
                if (rule.cond) rule.cond.val = (value || '').trim();
            }
            persistAppState();
            renderBizRules();
        }
        function bizRuleRelChanged() {
            ...
            const sN = state.tables[rel.sourceTable].name,
                tN = state.tables[rel.targetTable].name;
        ...
        function bizRuleDirChanged() {
            ...
            el('bizRuleCondCol').innerHTML =
                '<option value="">— sans condition —</option>' +
                childT.headers.map(h => `<option>${escapeHTML(h)}</option>`).join('');
            el('bizRuleCondVal').value = '';
            el('bizRuleCondVals').innerHTML = '';
        }
        async function bizRuleCondColChanged() {
            const rel = state.relations.find(r => r.id === el('bizRuleRel').value);
            if (!rel) return;
            const childT = el('bizRuleDir').value === 's' ? state.tables[rel.targetTable] : state.tables[rel.sourceTable];
            const col = el('bizRuleCondCol').value;
            const list = el('bizRuleCondVals');
            if (!col || !list) return;
            list.innerHTML = '';
            try {
                const { conn } = await getDB();
                const raw = `TRIM(CAST(${sqlIdent(col)} AS VARCHAR))`;
                const res = await conn.query(
                    `SELECT ${raw} AS v, COUNT(*)::BIGINT AS c FROM ${sqlIdent(duckTableName(childT.id))} WHERE ${sqlIdent(col)} IS NOT NULL AND ${raw} <> '' GROUP BY 1 ORDER BY c DESC LIMIT 20`
                );
                list.innerHTML = arrowResultToObjects(res)
                    .map(r => `<option value="${escapeHTML(r.v)}">`)
                    .join('');
        ...
        function addBizRule() {
            ...
            const condCol = el('bizRuleCondCol').value,
                condVal = el('bizRuleCondVal').value.trim();
            ...
            state.governance.rules.push({
                id: 'rul_' + generateId(),
                parentTable: parentT.name,
                parentCol: isS ? rel.sourceCol : rel.targetCol,
                childTable: childT.name,
                childCol: isS ? rel.targetCol : rel.sourceCol,
                cond: condCol ? { col: condCol, op: '=', val: condVal } : null,
                expect: el('bizRuleExpect').value,
                n: parseInt(el('bizRuleN').value) || 1,
                label: ''
            });
            persistAppState();
            renderBizRules();
        ...
        function removeBizRule(id) {
            state.governance.rules = (state.governance.rules || []).filter(r => r.id !== id);
            persistAppState();
            renderBizRules();
        }
        async function testBizRule(id, btn) {
            const rule = (state.governance.rules || []).find(r => r.id === id);
            if (!rule) return;
            const out = el('bizrule-res-' + id);
            ...
            try {
                ...
                out.innerHTML = `<p class="text-xs text-red-600 mt-1">Inexécutable : ${escapeHTML(e.message)}</p>`;
            } finally {
        ...
        function renderBoContextRules(bo, names) {
            ...
            const tableOpts = sel =>
                (sourceTables.length ? sourceTables : names)
                    .map(n => `<option ${n === sel ? 'selected' : ''}>${escapeHTML(n)}</option>`)
                    .join('');
            ...
            if (cr.elementId && ctxElem) {
                html += (cr.rules || [])
                    .map(
                        r => `<div class="flex flex-wrap items-center gap-2 mb-1.5 bg-white border border-blue-100 rounded-lg p-2">
        ...
        async function fillCtxRuleValues(boId) {
            const bo = state.governance.businessObjects.find(x => x.id === boId);
            if (!bo) return;
            const list = el('ctx-vals-' + boId);
            if (!list) return;
            const ctxElem = (bo.elements || []).find(e2 => e2.id === (bo.contextRules || {}).elementId);
            const map = ctxElem && (ctxElem.mappings || [])[0];
            if (!map) return;
            const table = tableByName(map.table);
            if (!table || table.status !== 'ready') return;
            try {
                const { conn } = await getDB();
                const raw = `TRIM(CAST(${sqlIdent(map.col)} AS VARCHAR))`;
                const res = await conn.query(
                    `SELECT ${raw} AS v, COUNT(*)::BIGINT AS c FROM ${sqlIdent(duckTableName(table.id))} WHERE ${sqlIdent(map.col)} IS NOT NULL AND ${raw} <> '' GROUP BY 1 ORDER BY c DESC LIMIT 20`
                );
                list.innerHTML = arrowResultToObjects(res)
                    .map(r => `<option value="${escapeHTML(r.v)}">`)
                    .join('');
        ...
        function updateBoContext(boId, elementId) {
            const bo = state.governance.businessObjects.find(x => x.id === boId);
            if (!bo) return;
            bo.contextRules = bo.contextRules || { elementId: '', rules: [] };
            bo.contextRules.elementId = elementId;
            persistAppState();
            renderGovernance();
        }
        function addBoContextRule(boId) {
            const bo = state.governance.businessObjects.find(x => x.id === boId);
            if (!bo) return;
            bo.contextRules = bo.contextRules || { elementId: '', rules: [] };
            bo.contextRules.rules.push({
                id: 'cr_' + generateId(),
                value: '',
                masterTable: ((bo.sources || [])[0] || {}).table || '',
                owner: ''
            });
            persistAppState();
            renderGovernance();
        }
        function updateBoContextRule(boId, ruleId, f, v) {
            const bo = state.governance.businessObjects.find(x => x.id === boId);
            const r = bo && ((bo.contextRules || {}).rules || []).find(x => x.id === ruleId);
            if (r) {
                r[f] = v;
                persistAppState();
            }
        }
        function removeBoContextRule(boId, ruleId) {
            const bo = state.governance.businessObjects.find(x => x.id === boId);
            if (bo && bo.contextRules) {
                bo.contextRules.rules = bo.contextRules.rules.filter(x => x.id !== ruleId);
                persistAppState();
                renderGovernance();
            }
        }
        ...
        async function analyzeContextCoverage(boId) {
            const bo = state.governance.businessObjects.find(x => x.id === boId);
            if (!bo) return;
            const out = el('bo-ctx-coverage-' + boId);
            if (!out) return;
            ...
            if (!map) {
                out.innerHTML =
                    '<p class="text-xs text-red-600">L\'attribut de contexte doit être mappé sur au moins une colonne physique (Alimenté par).</p>';
                return;
            }
            const table = tableByName(map.table);
            if (!table || table.status !== 'ready') {
                out.innerHTML = `<p class="text-xs text-red-600">Table "${escapeHTML(map.table)}" non chargée.</p>`;
                return;
            }
            ...
            try {
                ...
                const raw = `TRIM(CAST(${sqlIdent(map.col)} AS VARCHAR))`;
                const res = await conn.query(
                    `SELECT ${raw} AS v, COUNT(*)::BIGINT AS c FROM ${sqlIdent(duckTableName(table.id))} WHERE ${sqlIdent(map.col)} IS NOT NULL AND ${raw} <> '' GROUP BY 1 ORDER BY c DESC LIMIT 50`
                );
                const vals = arrowResultToObjects(res);
                const ruleVals = new Set(
                    (cr.rules || [])
                        .map(r =>
                            String(r.value || '')
                                .trim()
                ...
                out.innerHTML = `<p class="text-xs text-red-600">Analyse impossible : ${escapeHTML(e.message)}</p>`;
            }
        }
        ...
        function updateBusinessObject(id, f, v) {
            const bo = state.governance.businessObjects.find(x => x.id === id);
            if (bo) {
                bo[f] = v;
                persistAppState();
            }
        }
        function removeBusinessObject(id) {
            state.governance.businessObjects = state.governance.businessObjects.filter(x => x.id !== id);
            persistAppState();
            renderGovernance();
        }
        function addBoElement(boId) {
            const bo = state.governance.businessObjects.find(x => x.id === boId);
            if (!bo) return;
            bo.elements = bo.elements || [];
            bo.elements.push({ id: 'be_' + generateId(), name: 'Nouvel attribut', owner: '', mappings: [] });
            persistAppState();
            renderGovernance();
        }
        function updateBoElement(boId, elmId, f, v) {
            const bo = state.governance.businessObjects.find(x => x.id === boId);
            const elm = bo && (bo.elements || []).find(e => e.id === elmId);
            if (elm) {
                elm[f] = v;
                persistAppState();
            }
        }
        function removeBoElement(boId, elmId) {
            const bo = state.governance.businessObjects.find(x => x.id === boId);
            if (bo) {
                bo.elements = (bo.elements || []).filter(e => e.id !== elmId);
                persistAppState();
                renderGovernance();
            }
        }
        function populateBoMapCols(elmId, tableName) {
            const table = tableByName(tableName);
            const element = el('bo-col-' + elmId);
            if (element)
                element.innerHTML = (table ? table.headers : []).map(h => `<option>${escapeHTML(h)}</option>`).join('');
        }
        function addBoMapping(boId, elmId) {
            const bo = state.governance.businessObjects.find(x => x.id === boId);
            const elm = bo && (bo.elements || []).find(e => e.id === elmId);
            if (!elm) return;
            const tbl = el('bo-tbl-' + elmId).value,
                col = el('bo-col-' + elmId).value;
            if (!tbl || !col) return;
            elm.mappings = elm.mappings || [];
            if (!elm.mappings.some(m => m.table === tbl && m.col === col)) elm.mappings.push({ table: tbl, col });
            persistAppState();
            renderGovernance();
        }
        function removeBoMapping(boId, elmId, idx) {
            const bo = state.governance.businessObjects.find(x => x.id === boId);
            const elm = bo && (bo.elements || []).find(e => e.id === elmId);
            if (elm && elm.mappings) {
                elm.mappings.splice(idx, 1);
                persistAppState();
                renderGovernance();
            }
        }
        ...
        function createObjectFromAttribute(col) {
            const tabs = readyTableNames().filter(n => {
                const table = tableByName(n);
                return table && table.headers.includes(col);
            });
            const bo = {
                id: 'bo_' + generateId(),
                name: 'Objet — ' + col,
            ...
            renderGovernance();
            showSuccess(
                `Objet métier créé pour "${col}" (${tabs.length} sources mappées). Nommez maintenant son propriétaire global.`
            );
        }
        ...
        function openBoDesigner() {
            govState.designer = { rootId: '', analysis: null };
            renderGovernance();
        }
        function closeBoDesigner() {
            govState.designer = null;
            renderGovernance();
        }

        function renderBoDesigner() {
            const designer = govState.designer;
            ...
            let html = `<div class="bg-white border-2 border-emerald-200 rounded-xl p-5">
                ...
                <div class="flex items-end gap-2 mb-4">
                    ...
                    <select id="designer-root" class="border border-slate-300 p-2 rounded text-sm bg-white">${tables.map(t2 => `<option value="${t2.id}" ${designer.rootId === t2.id ? 'selected' : ''}>${escapeHTML(t2.name)}</option>`).join('')}</select></div>
            ...
            if (designer.analysis) {
                const root = state.tables[designer.rootId];
                const selfRel = selfRelationFor(designer.rootId);
                if (selfRel)
                    html += `<p class="text-xs text-emerald-800 bg-emerald-50 border border-emerald-200 rounded-lg p-2.5 mb-3">🌳 <strong>${escapeHTML(root.name)}</strong> est hiérarchique (auto-relation ${escapeHTML(selfRel.sourceCol)} ↔ ${escapeHTML(selfRel.targetCol)}) : ses lignes forment un arbre (ex : site → bâtiment → niveau). Pensez aux <strong>profils de clé par type</strong> dans l'audit Doublons, et au composant de clé "colonne du parent".</p>`;
                if (!designer.analysis.length) {
                    ...
                    html += designer.analysis
                        .map(
                            (a, i) => `<div class="border border-slate-200 rounded-lg p-3 mb-2 bg-slate-50/50">
                ...
                html += `<div class="flex items-end gap-2 mt-4 pt-3 border-t border-slate-100">
                    ...
                    <input type="text" id="designer-name" value="${escapeHTML(designer.suggestedName || '')}" class="border border-slate-300 p-2 rounded text-sm w-64"></div>
        ...
        async function analyzeBoDesign() {
            const designer = govState.designer;
            if (!designer) return;
            designer.rootId = el('designer-root').value;
            const root = state.tables[designer.rootId];
            if (!root) return;
            const btn = el('btnDesignAnalyze');
            btn.disabled = true;
            btn.textContent = 'Mesure des cardinalités...';
            const rels = state.relations.filter(
                r =>
                    r.sourceCol &&
                    r.targetCol &&
                    (r.sourceTable === designer.rootId || r.targetTable === designer.rootId) &&
                    r.sourceTable !== r.targetTable
            );
            const analysis = [];
            try {
                for (const relation of rels) {
                    const otherId = relation.sourceTable === designer.rootId ? relation.targetTable : relation.sourceTable;
                    const other = state.tables[otherId];
                    if (!other || other.status !== 'ready') continue;
                    if (!relation.measured) {
                        try {
                            relation.measured = await measureRelation(relation);
                        } catch (e) {
                            /* moteur indisponible : suggestion sans mesure */
                        }
                    }
                    const m = relation.measured;
                    const rootIsSource = relation.sourceTable === designer.rootId;
                    const rootMax = m ? (rootIsSource ? m.smax : m.tmax) : 1;
                    const otherMax = m ? (rootIsSource ? m.tmax : m.smax) : 1;
                    const degree = state.relations.filter(
                        x =>
                            x.id !== relation.id &&
                            x.sourceCol &&
                            x.targetCol &&
                            (x.sourceTable === otherId || x.targetTable === otherId)
                    ).length;
                    // Côté N exclusif à la racine -> fait probablement partie de l'objet ; partagé ou côté 1 -> référentiel.
                    const suggestion =
                        otherMax > 1 && degree === 0 ? 'composition' : rootMax > 1 || degree > 0 ? 'reference' : 'composition';
                    const cardText = `${rootMax <= 1 ? '1' : 'N'}–${otherMax <= 1 ? '1' : 'N'}`;
                    const detail = m
                        ? `max ${otherMax} ligne(s) par clé · ${degree ? degree + ' autre(s) table(s) y sont liées' : 'liée uniquement à la racine'}`
                        : 'cardinalité non mesurable (moteur indisponible)';
                    analysis.push({
                        relId: relation.id,
                        tblId: otherId,
                        tblName: other.name,
                ...
                persistAppState();
                designer.analysis = analysis;
                designer.suggestedName = cleanFileName(root.name)
                    .replace(/[_-]+/g, ' ')
                ...
                btn.disabled = false;
                btn.textContent = '📐 Analyser le sous-graphe';
            }
        }

        function generateObjectFromDesign() {
            const designer = govState.designer;
            if (!designer || !designer.rootId) return;
            const root = state.tables[designer.rootId];
            if (!root) return;
            ...
            const bo = {
                ...
                elements: root.headers.map(h => ({
                    id: 'be_' + generateId(),
                    name: h,
                    owner: '',
                    mappings: [{ table: root.name, col: h }]
                })),
                contextRules: { elementId: '', rules: [] },
                structure: [],
                references: []
            ...
            try {
                const dh = buildDetectedHierarchy(root);
                if (dh) bo.hierarchies = [dh];
            } catch (e) {}
            (designer.analysis || []).forEach(a => {
                ...
                if (a.choice === 'composition') {
                    ...
                    const refName = cleanFileName(a.tblName)
                        .replace(/[_-]+/g, ' ')
                        .replace(/^\w/, c => c.toUpperCase());
                    let ref = state.governance.businessObjects.find(
                        b =>
                            b.name.toLowerCase() === refName.toLowerCase() ||
                            (b.sources || []).some(s2 => s2.table === a.tblName)
                    );
                    if (!ref) {
                        ...
                        ref = {
                            ...
                            globalOwner: '',
                            contributors: [],
                            sources: [{ table: a.tblName, role: 'maitre' }],
                            elements: (refTable ? refTable.headers : []).map(h => ({
                                id: 'be_' + generateId(),
                                name: h,
                                owner: '',
                                mappings: [{ table: a.tblName, col: h }]
                            })),
                            contextRules: { elementId: '', rules: [] },
                            structure: [],
                            references: []
                    ...
                    bo.references.push({
                        boId: ref.id,
                        viaLabel: rel ? `${rel.sourceCol} = ${rel.targetCol}` : '',
                        cardinality: a.cardText
                    });
                }
            });
            govState.designer = null;
            ...
            renderRelationsList();
            showSuccess(
                `Objet "${name}" généré depuis le modèle de données (${bo.structure.length} composition(s), ${bo.references.length} référence(s)).`
            );
        }
        ...
        function renderObjectModelDiagram() {
            const wrap = el('modelGraphWrap');
            if (!wrap) return;
            try {
                _renderObjectModelImpl(wrap);
            } catch (e) {
                console.error('Model render error', e);
                wrap.innerHTML = graphUnavailableHtml("Le modèle objet n'a pas pu être affiché : " + escapeHTML(e.message));
                return;
            }
        ...
        function _renderObjectModelImpl(wrap) {
            ...
            bos.forEach(bo => {
                ...
                nodes.push({
                    id: 'bo:' + bo.id,
                    type: 'studio-uml-node',
                    title: (isHier ? '🌳 ' : '') + '🏛️ ' + bo.name,
                    rows: boRows,
                    headFill: '#dcfce7',
                    titleFill: '#14532d',
                    stroke: isHier ? '#059669' : '#16a34a'
                });
                hiers.forEach((h2, hi) => {
                    edges.push({
                        id: 'hier:' + bo.id + ':' + hi,
                        ...
                        target: 'bo:' + bo.id,
                        type: 'loop',
                        loopCfg: { position: 'top', dist: 40 + hi * 22 },
                        label:
                            '🌳 ' +
                            (h2.name || 'hiérarchie') +
                            (h2.mode === 'link' ? ' (via ' + (h2.linkTable || 'liaison') + ')' : '') +
                            (h2.maxDepth ? ' ≤' + h2.maxDepth + ' niv.' : ''),
                        style: { stroke: '#059669', lineWidth: 1.5 }
                    });
                    ...
                    lvls.forEach(lv => {
                        nodes.push({
                            ...
                            type: 'studio-rich-node',
                            title: lv.name,
                            content: (lv.parents || []).length ? '' : 'racine',
                            fill: '#f0fdf4',
                            stroke: '#6ee7b7'
                        });
                        if (!(lv.parents || []).length)
                            edges.push({
                                id: 'er' + lvlId(lv.name),
                                source: 'bo:' + bo.id,
                                target: lvlId(lv.name),
                                label: h2.name || 'niveaux',
                                style: { stroke: '#6ee7b7', lineDash: [3, 3], endArrow: { path: '', fill: '#6ee7b7' } }
                            });
                        (lv.parents || []).forEach(pn => {
                            if (lvls.some(x => x.name === pn))
                                edges.push({
                                    id: 'ep' + lvlId(lv.name) + ':' + pn,
                                    source: lvlId(pn),
                                    target: lvlId(lv.name),
                                    label: 'contient',
                                    style: { stroke: '#6ee7b7', lineDash: [3, 3], endArrow: { path: '', fill: '#6ee7b7' } }
                ...
                getBoFacets(bo).forEach((c2, j) => {
                    ...
                    const filterTxt = (c2.scope || []).length
                        ? (c2.scope || []).map(facetScopeLabel).join(' · ')
                        : 'toute la table';
                    ...
                    const appliesTxt = (c2.applies || []).length
                        ? ' [si ' + (c2.applies || []).map(facetScopeLabel).join(' et ') + ']'
                        : '';
                    ...
                    edges.push({
                        ...
                        source: 'bo:' + bo.id,
                        target: cid,
                        label: `${c2.cardinality || ''}${appliesTxt}`,
                        style: { stroke: '#16a34a', lineWidth: 1.6, startArrow: { path: '', fill: '#16a34a' }, endArrow: false }
                    });
                });
                (bo.references || []).forEach((rf, j) => {
                    ...
                    edges.push({
                        ...
                        source: 'bo:' + bo.id,
                        target: 'bo:' + rf.boId,
                        label: `→ ${rf.cardinality || ''}${rf.viaLabel ? ' (' + rf.viaLabel + ')' : ''}`,
                        style: { stroke: '#0d9488', lineDash: [6, 3], endArrow: { path: '', fill: '#0d9488' } }
                    });
                });
            });
            if (modelGraph) {
            ...
            if (!nodes.length) {
                wrap.innerHTML =
                    '<p class="text-sm text-slate-400 italic text-center pt-24">Aucun objet métier — utilisez l\'assistant 🧬 "Concevoir depuis le modèle de données" dans l\'onglet Objets métier.</p>';
                return;
            }
            modelGraph = createSvgGraph(
                wrap,
                { type: 'dagre', rankdir: 'LR', nodesep: 28, ranksep: 100 },
                {
                    width: wrap.clientWidth || 900,
                    height: wrap.clientHeight || 550,
                    defaultEdge: {
                        type: 'cubic-horizontal',
                        style: { stroke: '#94a3b8', lineWidth: 1.4 },
                        labelCfg: {
                            autoRotate: true,
                            style: {
                                fill: '#475569',
                                fontSize: 10,
                                background: { fill: '#fff', stroke: '#e2e8f0', padding: [2, 4, 2, 4], radius: 3 }
                            }
                        }
                    }
                }
        ...
        function openBoWizard(tableName) {
            ...
            existing.innerHTML =
                '<option value="__new__">➕ Créer un nouvel objet métier</option>' +
                state.governance.businessObjects
                    .map(
                        bo =>
                            `<option value="${bo.id}" ${suggestions[0] && suggestions[0].id === bo.id ? 'selected' : ''}>🏛️ ${escapeHTML(bo.name)}</option>`
                    )
                    .join('');
            el('boWizNameWrap').classList.toggle('hidden', existing.value !== '__new__');
            el('boWizName').value = tableName
        ...
        function attachSourceToObject(bo, tableName, role, genElements) {
            ...
            const source = bo.sources.find(s => s.table === tableName);
            if (source) source.role = role;
            else bo.sources.push({ table: tableName, role });
            if (genElements) {
                const table = tableByName(tableName);
                if (!table) return;
                bo.elements = bo.elements || [];
                table.headers.forEach(h => {
                    let elm = bo.elements.find(
                        e2 => e2.name.toLowerCase() === h.toLowerCase() || (e2.mappings || []).some(m => m.col === h)
                    );
                    if (!elm) {
                        elm = { id: 'be_' + generateId(), name: h, owner: '', mappings: [] };
                        bo.elements.push(elm);
                    }
                    if (!(elm.mappings || []).some(m => m.table === tableName && m.col === h))
                        elm.mappings.push({ table: tableName, col: h });
                });
            }
        }
        ...
        function confirmBoWizard() {
            ...
            if (existingId === '__new__') {
                ...
                if (state.governance.businessObjects.some(b => b.name.toLowerCase() === name.toLowerCase()))
                    return showError(
                        `Un objet métier "${name}" existe déjà : sélectionnez-le dans la liste pour y rattacher la source.`
                    );
                bo = {
                    id: 'bo_' + generateId(),
                    name,
            ...
            if (existingId === '__new__') {
                try {
                    const dh = buildDetectedHierarchy(tableByName(boWizardTable));
                    if (dh) bo.hierarchies = [dh];
                } catch (e) {}
            }
            ...
            persistAppState();
            govState.selectedBoId = bo.id;
            if (!el('step-9').classList.contains('hidden')) {
        ...
        function addBoSource(boId) {
            const bo = state.governance.businessObjects.find(x => x.id === boId);
            if (!bo) return;
            const tbl = el('bo-src-tbl-' + boId).value,
                role = el('bo-src-role-' + boId).value;
            if (!tbl) return;
        ...
        function updateBoSourceRole(boId, tableName, role) {
            const bo = state.governance.businessObjects.find(x => x.id === boId);
            const s = bo && (bo.sources || []).find(x => x.table === tableName);
            if (s) {
                s.role = role;
                boSyncAppsFromSources();
                persistAppState();
                lfSyncFromData({ silent: true });
                renderGovernance();
            }
        ...
        function updateBoApp(boId, appId) {
            const bo = state.governance.businessObjects.find(x => x.id === boId);
            if (!bo) return;
            bo.appId = appId || '';
            persistAppState();
        ...
        function removeBoSource(boId, tableName) {
            const bo = state.governance.businessObjects.find(x => x.id === boId);
            if (bo) {
                bo.sources = (bo.sources || []).filter(s => s.table !== tableName);
                boSyncAppsFromSources();
                persistAppState();
                lfSyncFromData({ silent: true });
                renderGovernance();
