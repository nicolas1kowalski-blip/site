        ...
        const _v12tLayout = v12ExtractLayout;
        v12ExtractLayout = function () {
        ...
        const _v12tDrop = duckDropTable;
        duckDropTable = async function (tId) { if (v12State.cmpRunning && /^cmp_/.test(String(tId))) { if (v12State.cmpKeep && v12State.cmpKeep !== tId) { try { await _v12tDrop(v12State.cmpKeep); } catch (e) {} } v12State.cmpKeep = tId; return; } return _v12tDrop.apply(this, arguments); };
        const _v12tRunComp = runComparison;
        ...
        const _v12tEmpty = v12EmptyBand;
        v12EmptyBand = function (n) { if (v12TmpList().length) { const sec = el('step-' + n); const b = sec && sec.querySelector('.v12-band.info'); if (b) b.remove(); return; } return _v12tEmpty.apply(this, arguments); };
        const _v12tAfter = v12After;
        v12After = function () { const r = _v12tAfter.apply(this, arguments); try { v12TmpTopBtn(); if (currentTab === 7) v12TmpCompareAfter(); if (v12State.tmpIds.length) v12TmpDecorate(); } catch (e) {} return r; };
        window.addEventListener('beforeunload', e => { if (v12TmpList().length) { e.preventDefault(); e.returnValue = ''; } });
        // ---- recherche, aide, accueil ----
        const _v12tIndex = v11Index;
        v11Index = function () { const out = _v12tIndex.apply(this, arguments); out.push({ grp: 'Actions', ic: '⏳', label: 'Jeux temporaires (résultats et fichiers de la session)', key: 'jeux temporaires session fichier extérieur résultat', go: () => v12TmpOpen() }); v12TmpList().forEach(t => out.push({ grp: 'Actions', ic: '⏳', label: 'Jeu temporaire : ' + t.name, sub: v12TmpKindLabel(t) + ' · ' + (t.rows || 0).toLocaleString('fr-FR') + ' lignes', key: 'jeu temporaire ' + t.name, go: () => v12TmpOpen() })); return out; };
