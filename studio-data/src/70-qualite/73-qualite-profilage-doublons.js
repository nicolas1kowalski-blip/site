        ...
        function getKeyProfiles(tableName) {
            const dictionaryEntry = ensureDictEntry(tableName);
            if (!Array.isArray(dictionaryEntry.keyProfiles)) {
                dictionaryEntry.keyProfiles = [];
                if (Array.isArray(dictionaryEntry.functionalKey) && dictionaryEntry.functionalKey.length) {
                    dictionaryEntry.keyProfiles.push({
                        id: 'kp_' + generateId(),
                        scope: [],
                        parts: dictionaryEntry.functionalKey
                    });
                }
                delete dictionaryEntry.functionalKey;
            }
            // Migration : ancien périmètre simple (typeCol/typeVal) -> liste de conditions composite.
            dictionaryEntry.keyProfiles.forEach(pr => {
            ...
            return dictionaryEntry.keyProfiles;
        }
        // Hiérarchie : l'objet est-il lié à lui-même dans le modèle de données (ex : DK_CODE_PARENT -> DK_CODE) ?
        function selfRelationFor(tId) {
            return (
                state.relations.find(r => r.sourceCol && r.targetCol && r.sourceTable === tId && r.targetTable === tId) || null
            );
        }
        ...
        function fkPartLabel(p, auditedName) {
            if (p.hier)
                return `<strong>${escapeHTML(p.col)}</strong> <span class="text-slate-400">— 🌳 du <strong class="text-slate-600">parent hiérarchique</strong> <span class="text-slate-300">(${escapeHTML(p.viaChild)} → ${escapeHTML(p.viaParent)})</span></span>`;
            if (p.table === auditedName)
                return `<strong>${escapeHTML(p.col)}</strong> <span class="text-slate-400">— colonne de la table auditée</span>`;
        ...
        function profileScopeLabel(pr) {
            ...
            return sc
                .map(c => `${c.col} ${SCOPE_OPS[c.op] || c.op}${c.op === 'empty' || c.op === 'notempty' ? '' : ` "${c.val}"`}`)
                .join(' ET ');
        }
        ...
        function renderDupSection() {
            const cont = el('qual-sub-dups');
            if (!cont) return;
            const tId = currentProfilingTableId;
            const target = state.tables[tId];
            if (!target) {
                cont.innerHTML =
                    '<p class="text-sm text-slate-400 italic py-8 text-center">Lancez d\'abord un audit sur une table.</p>';
                return;
            }
            ...
            const profileCards = profiles
                .map(pr => {
                    ...
                    ${
                        (pr.parts || []).length
                            ? `<div class="mt-2 space-y-1">${pr.parts
                                  .map(
                                      (p, i) =>
                                          `<div class="flex items-center gap-2 text-xs bg-slate-50 border border-slate-100 rounded p-1.5"><span class="w-5 h-5 rounded-full bg-indigo-600 text-white text-[10px] font-black flex items-center justify-center flex-shrink-0">${i + 1}</span><span>${fkPartLabel(p, tn)}</span>${
                                              editing
                                                  ? `<select onchange="updateFkPartMatch('${pr.id}',${i},this.value)" title="Tolérance d'appariement de ce composant" class="ml-auto border border-slate-200 p-0.5 rounded text-[10px] bg-white">${[
                                                        ['fuzzy', '🌫 flou autorisé'],
                                                        ['norm', '≈ casse/accents tolérés'],
                                                        ['exact', '🎯 strict (exact)']
                                                    ]
                                                        .map(
                                                            ([v, l]) =>
                                                                `<option value="${v}" ${(p.match || 'fuzzy') === v ? 'selected' : ''}>${l}</option>`
                                                        )
                                                        .join(
                                                            ''
                                                        )}</select><button onclick="removeFkPart('${pr.id}',${i})" class="text-slate-300 hover:text-red-500">✕</button>`
                                                  : `${(p.match || 'fuzzy') !== 'fuzzy' ? `<span class="ml-auto text-[9px] uppercase font-bold text-slate-400">${p.match === 'exact' ? '🎯 strict' : '≈ normalisé'}</span>` : ''}`
                                          }</div>`
                                  )
                                  .join('')}</div>`
                            : '<p class="text-xs text-slate-300 italic mt-1.5">Aucun composant — éditez ce profil pour composer sa clé.</p>'
                    }
                </div>`;
                })
                .join('');
            ...
            if (editPr) {
                ...
                const modes = [
                    ...
                    [
                        'linked',
                        `Colonne d'un fichier lié${linked.length ? '' : ' (aucune relation du modèle de données)'}`,
                        linked.length > 0
                    ],
                    [
                        'hier',
                        `🌳 Colonne du parent hiérarchique${selfRel ? '' : " (pas d'auto-relation du modèle de données)"}`,
                        !!selfRel
                    ]
                ];
                let builderBody = '';
                ...
                editorHtml = `<div class="bg-white border-2 border-indigo-300 rounded-xl p-4 mt-3">
                    ...
                    <div class="mb-3 pb-3 border-b border-slate-100">
                        ...
                        <div class="flex flex-wrap items-end gap-2 bg-slate-50 border border-slate-100 rounded-lg p-2">
                            ...
                            <select id="fk-scope-op" onchange="el('fk-scope-val').classList.toggle('hidden', this.value === 'empty' || this.value === 'notempty')" class="border border-slate-300 p-1.5 rounded text-xs bg-white">${Object.entries(
                                SCOPE_OPS
                            )
        ...
        function addKeyProfile() {
            const target = state.tables[currentProfilingTableId];
            if (!target) return;
        ...
        function removeKeyProfile(id) {
            const target = state.tables[currentProfilingTableId];
            if (!target) return;
            const dictionaryEntry = ensureDictEntry(target.name);
            dictionaryEntry.keyProfiles = (dictionaryEntry.keyProfiles || []).filter(p => p.id !== id);
        ...
        function addScopeCond(profileId) {
            const target = state.tables[currentProfilingTableId];
            if (!target) return;
            const pr = getKeyProfiles(target.name).find(p => p.id === profileId);
            if (!pr) return;
            const col = el('fk-scope-col').value,
                element = el('fk-scope-op').value,
                val = el('fk-scope-val').value.trim();
            if (element !== 'empty' && element !== 'notempty' && !val)
                return showError('Indiquez la valeur de la condition (ex : SITE).');
            pr.scope = pr.scope || [];
            const cond = { col, op: element, val: element === 'empty' || element === 'notempty' ? '' : val };
            if (pr.scope.some(c => c.col === cond.col && c.op === cond.op && c.val === cond.val))
                return showError('Cette condition est déjà dans le périmètre.');
            pr.scope.push(cond);
            persistAppState();
            renderDupSection();
        }
        ...
        async function fkScopeColChanged() {
            ...
            try {
                ...
                const res = await conn.query(
                    `SELECT ${raw} AS v, COUNT(*)::BIGINT AS c FROM ${sqlIdent(duckTableName(target.id))} WHERE ${sqlIdent(col)} IS NOT NULL AND ${raw} <> '' GROUP BY 1 ORDER BY c DESC LIMIT 20`
                );
        ...
        function fkTableChanged() {
            const table = state.tables[el('fk-tbl').value];
            if (!table) return;
            el('fk-col').innerHTML = table.headers.map(h => `<option>${escapeHTML(h)}</option>`).join('');
            el('fk-condcol').innerHTML = table.headers.map(h => `<option>${escapeHTML(h)}</option>`).join('');
            el('fk-condval-list').innerHTML = '';
            el('fk-condval').value = '';
        }
        async function fkCondColChanged() {
            const table = state.tables[el('fk-tbl').value];
            const col = el('fk-condcol').value;
            const list = el('fk-condval-list');
        ...
        function addFkPart() {
            ...
            if (dupUi.mode === 'self') {
                ...
                const selfRel = selfRelationFor(target.id);
                if (!selfRel) return;
                ...
                const table = state.tables[el('fk-tbl').value];
                if (!table) return;
                part = { table: table.name, col: el('fk-col').value };
                if (el('fk-usecond').checked) {
                    const condCol = el('fk-condcol').value,
                        condVal = el('fk-condval').value.trim();
                    if (!condVal) return showError('Indiquez la valeur du filtre (ex : GENERAL), ou décochez la condition.');
                    part.whereCol = condCol;
                    part.whereVal = condVal;
                }
            }
            if (pr.parts.some(p => JSON.stringify({ ...p }) === JSON.stringify({ ...part })))
                return showError('Ce composant est déjà dans la clé de ce profil.');
            pr.parts.push(part);
            persistAppState();
            renderDupSection();
        }
        ...
        async function refreshKeyPreview() {
            ...
            try {
                ...
                const res = await conn.query(
                    `WITH src AS (${srcSql}) SELECT ke FROM src WHERE TRIM(REPLACE(ke, chr(31), '')) <> '' LIMIT 3`
                );
                const rows = arrowResultToObjects(res);
                if (!rows.length) {
                    box.innerHTML =
                        '<p class="text-[10px] text-amber-600">Aucune ligne sur ce périmètre — vérifiez la valeur du type.</p>';
                    return;
                }
                box.innerHTML =
                    `<div class="text-[10px] uppercase font-bold text-slate-400 mb-1">Aperçu de la clé sur vos données (${escapeHTML(profileScopeLabel(pr))})</div>` +
        ...
        function renderKeySegments(ke, parts) {
            ...
            return segs
                .map((s, i) => {
                    const part = parts[i];
                    const lbl = part ? (part.hier ? '🌳 ' + part.col : part.col) : 'clé';
        ...
        function buildDupSrcSql(tId, parts, sampleSql, mode, profile) {
            ...
            parts.forEach((p, i) => {
                if (p.hier) {
                    const alias = 'h' + i;
                    joins.push(
                        `LEFT JOIN ${sqlIdent(duckTableName(tId))} ${alias} ON ${normJoin(alias, p.viaParent)} = ${normJoin('b', p.viaChild)}`
                    );
                    exprs.push(`${alias}.${sqlIdent(p.col)}`);
                    extraSelects.push(`${alias}.${sqlIdent(p.col)} AS ${sqlIdent(p.col + ' · parent')}`);
                    return;
                }
                if (p.table === target.name) {
                    exprs.push(`b.${sqlIdent(p.col)}`);
                    return;
                }
                const table = tableByName(p.table);
                if (!table) throw new Error(`Table "${p.table}" non chargée.`);
                const rel = state.relations.find(
                    r =>
                        r.sourceCol &&
                        r.targetCol &&
                        ((r.sourceTable === tId && r.targetTable === table.id) ||
                            (r.targetTable === tId && r.sourceTable === table.id))
                );
                if (!rel)
                    throw new Error(
                        `Aucune relation du modèle de données entre "${target.name}" et "${p.table}" (étape 2) : impossible de joindre ce composant de clé.`
                    );
                ...
                const alias = 'f' + i;
                const cond = p.whereCol
                    ? `AND UPPER(TRIM(CAST(x.${sqlIdent(p.whereCol)} AS VARCHAR))) = ${sqlLiteral(String(p.whereVal).toUpperCase())}`
                    : '';
                joins.push(`LEFT JOIN (
                    SELECT * FROM ${sqlIdent(duckTableName(table.id))} x
                    WHERE 1=1 ${cond}
            ...
            const normExprK = e =>
                `regexp_replace(strip_accents(UPPER(COALESCE(TRIM(CAST(${e} AS VARCHAR)), ''))), '[^A-Z0-9]', '', 'g')`;
            ...
            const selectCols = mode === 'rows' ? `b.*${extraSelects.length ? ', ' + extraSelects.join(', ') : ''}, ` : '';
        ...
        async function analyzeDuplicates() {
            const tId = currentProfilingTableId;
            const target = state.tables[tId];
            const out = el('dupResults');
            const btn = el('btnDupRun');
            ...
            try {
                ...
                for (let count = 0; count < profiles.length; count++) {
                    const pr = profiles[count];
                    out.innerHTML = `<p class="text-xs text-indigo-700">Analyse du profil ${count + 1}/${profiles.length} (${escapeHTML(profileScopeLabel(pr))})...</p>`;
                    ...
                    const rows = arrowResultToObjects(exactRes)[0];
                    ...
                    const nearTop = arrowResultToObjects(nearRes).map(r => ({
                        kn: r.kn,
                        c: Number(r.c),
                        exs: (Array.isArray(r.exs) ? r.exs : r.exs && r.exs.toArray ? r.exs.toArray() : []).slice(0, 4)
                    }));
                    ...
                    if (hasFuzzy) {
                        ...
                        fuzzy = arrowResultToObjects(fuzzyRes).map(r => ({
                            e1: r.e1,
                            e2: r.e2,
                            n1: Number(r.n1),
                            n2: Number(r.n2),
                    ...
                    results.push({
                        profile: {
                            ...
                            parts: JSON.parse(JSON.stringify(pr.parts))
                        },
                        totalRows,
                        exact: { groups: Number(rows.grps), rows: Number(rows.rws), top: exactTop },
                        near: { groups: nearGroups, top: nearTop },
                        fuzzy
                    });
                }
            ...
            R.profiles.forEach((res, pi) => {
                ...
                if (totalIssues > 0) {
                    html += `<div class="grid grid-cols-3 gap-3 mb-3">
                        <div class="p-2.5 border rounded-xl ${res.exact.groups ? 'bg-red-50 border-red-200' : 'bg-slate-50'}"><div class="text-xl font-black ${res.exact.groups ? 'text-red-600' : 'text-emerald-600'}">${res.exact.groups.toLocaleString('fr-FR')}</div>
                            <div class="text-[10px] font-bold text-slate-600">groupes stricts (${res.exact.rows.toLocaleString('fr-FR')} lignes)</div>
                            </div>
                        <div class="p-2.5 border rounded-xl ${res.near.groups ? 'bg-amber-50 border-amber-200' : 'bg-slate-50'}"><div class="text-xl font-black ${res.near.groups ? 'text-amber-600' : 'text-emerald-600'}">${res.near.groups.toLocaleString('fr-FR')}</div>
                            <div class="text-[10px] font-bold text-slate-600">quasi identiques (casse/accents)</div>
                            </div>
                        <div class="p-2.5 border rounded-xl ${res.fuzzy.length ? 'bg-purple-50 border-purple-200' : 'bg-slate-50'}"><div class="text-xl font-black ${res.fuzzy.length ? 'text-purple-600' : 'text-emerald-600'}">${res.fuzzy.length.toLocaleString('fr-FR')}</div>
                            <div class="text-[10px] font-bold text-slate-600">paires suspectes (≥ ${R.threshold})</div>
                            </div>
                    </div>`;
                    const card = (
                        kind,
                        ...
                        badgeCls,
                        body
                    ) => `<div class="border border-slate-200 rounded-xl bg-white mb-2 overflow-hidden">
                    ...
                    if (res.exact.top.length) {
                        ...
                        html += res.exact.top
                            .map((r, i) => card('exact', i, `× ${r.c}`, 'bg-red-100 text-red-700', seg(r.ke)))
                            .join('');
                    }
                        ...
                        html += res.near.top
                            .map((r, i) =>
                                card(
                                    ...
                                    r.exs.map(seg).join('<div class="text-[10px] text-slate-300 my-0.5">≈ variante :</div>')
                                )
                            )
                            .join('');
                    }
                        ...
                        html += res.fuzzy
                            ...
                            .map((r, i) =>
                                card(
                                    ...
                                    `${seg(r.e1)}<div class="text-[10px] text-slate-300 my-0.5">ressemble à :</div>${seg(r.e2)}`
                                )
                            )
                            .join('');
                    }
        ...
        async function toggleDupRows(pi, kind, i) {
            const box = el(`dup-rows-${pi}-${kind}-${i}`);
            const btn = el(`btn-dup-${pi}-${kind}-${i}`);
            if (!box || !lastDupResults) return;
            if (box.dataset.loaded) {
                box.classList.toggle('hidden');
                btn.textContent = box.classList.contains('hidden') ? '👁 Voir les lignes' : 'Masquer';
                return;
            }
            box.classList.remove('hidden');
            box.innerHTML = '<p class="text-xs text-indigo-600">Chargement des lignes...</p>';
            btn.textContent = 'Masquer';
            const R = lastDupResults;
            const res = R.profiles[pi];
            try {
                const { conn } = await getDB();
                const rowsSql = buildDupSrcSql(R.tId, res.profile.parts, R.sampleSql, 'rows', res.profile);
                const fetch2 = async (field, value) => {
                    const queryResult = await conn.query(
                        `WITH src AS (${rowsSql}) SELECT * FROM src WHERE ${field} = ${sqlLiteral(value)} LIMIT 30`
                    );
                    return arrowResultToObjects(queryResult);
                };
                let html = '';
                ...
                box.innerHTML = `<p class="text-xs text-red-600">Chargement impossible : ${escapeHTML(e.message)}</p>`;
            }
        }
        ...
        function renderDupRowsTable(rows, parts) {
            ...
            parts.forEach(p =>
                keyCols.add(p.hier ? p.col + ' · parent' : p.table === R.table ? p.col : p.col + ' · ' + p.table)
            );
            ...
            const head = ordered
                .map(
                    c =>
                        `<th class="p-2 whitespace-nowrap ${keyCols.has(c) ? 'bg-indigo-100 text-indigo-800' : ''}">${keyCols.has(c) ? '🔑 ' : ''}${escapeHTML(c)}</th>`
                )
                .join('');
            const body = rows
                .map(
                    r =>
                        `<tr class="hover:bg-white">${ordered.map(c => `<td class="p-2 whitespace-nowrap max-w-[220px] truncate ${keyCols.has(c) ? 'bg-indigo-50/60 font-bold text-indigo-900' : 'text-slate-600'}" title="${escapeHTML(r[c] === null || r[c] === undefined ? '' : String(r[c]))}">${escapeHTML(r[c] === null || r[c] === undefined || r[c] === '' ? '—' : String(r[c]))}</td>`).join('')}</tr>`
                )
                .join('');
            return `<div class="overflow-x-auto border border-slate-200 rounded-lg bg-white"><table class="w-full text-left text-xs"><thead class="bg-slate-50 text-[9px] uppercase font-bold text-slate-500"><tr>${head}</tr>
                </thead>
        ...
        async function exportDupCsv(btn) {
            ...
            try {
                ...
                for (const res of lastDupResults.profiles) {
                    ...
                    const srcSql = buildDupSrcSql(
                        lastDupResults.tId,
                        res.profile.parts,
                        lastDupResults.sampleSql,
                        'keys',
                        res.profile
                    );
                    const exAll = arrowResultToObjects(
                        await conn.query(
                            `WITH src AS (${srcSql}) SELECT ke, COUNT(*)::BIGINT AS c FROM src WHERE ${notEmpty} GROUP BY 1 HAVING COUNT(*) > 1 ORDER BY c DESC`
                        )
                    );
                    exAll.forEach(r => rows.push([scope, 'EXACT', clean(r.ke), '', '', Number(r.c)].map(escapeCSV).join(';')));
                    const nearAll = arrowResultToObjects(
                        await conn.query(
                            `WITH src AS (${srcSql}) SELECT kn, COUNT(*)::BIGINT AS c, list(DISTINCT ke) AS exs FROM src WHERE ${notEmpty} GROUP BY 1 HAVING COUNT(*) > 1 AND COUNT(DISTINCT ke) > 1 ORDER BY c DESC`
                        )
                    );
                    nearAll.forEach(r => {
                        const exs = Array.isArray(r.exs) ? r.exs : r.exs && r.exs.toArray ? r.exs.toArray() : [];
                        rows.push(
                            [scope, 'NORMALISE', exs.map(clean).join(' || '), '', '', Number(r.c)].map(escapeCSV).join(';')
                        );
                    });
                    const hasFuzzy = res.profile.parts.some(p => (p.match || 'fuzzy') === 'fuzzy');
                    if (hasFuzzy) {
                        const fzAll = arrowResultToObjects(
                            await conn.query(`WITH src AS (${srcSql}),
                            d AS (SELECT kb, kf, MIN(ke) AS ex, COUNT(*)::BIGINT AS n FROM src WHERE ${notEmpty} AND LENGTH(kf) >= 3 GROUP BY 1, 2)
                            SELECT a.ex AS e1, b.ex AS e2, a.n AS n1, b.n AS n2, jaro_winkler_similarity(a.kf, b.kf) AS s
                            FROM d a JOIN d b ON a.kb = b.kb AND left(a.kf, 4) = left(b.kf, 4) AND a.kf < b.kf
                        ...
                        fzAll.forEach(r =>
                            rows.push(
                                [scope, 'FLOU', clean(r.e1), clean(r.e2), Number(r.s).toFixed(3), Number(r.n1) + Number(r.n2)]
                                    .map(escapeCSV)
                                    .join(';')
                            )
                        );
                ...
                downloadTextFile(
                    `Doublons_cles_${lastDupResults.table.replace(/[^a-zA-Z0-9]/g, '_')}_${Date.now()}.csv`,
                    rows.join('\n')
                );
                ...
                if (btn) {
                    btn.disabled = false;
                    btn.textContent = '⬇ Toutes les clés en double (CSV)';
                }
            }
        }
        ...
        async function collectDupRowsByType() {
            ...
            for (const res of lastDupResults.profiles) {
                ...
                const pushRows = (rowsArr, type, groupOf) => {
                    rowsArr.forEach(r => {
                        if (!R2.cols)
                            R2.cols = Object.keys(r).filter(
                                c => c !== 'ke' && c !== 'kn' && c !== 'kb' && c !== 'kf' && c !== '__pid' && c !== '__sim'
                            );
                        R2.types[type].push([
                            scope,
                            groupOf(r),
                            String(r.ke).split(SEP).join(' | '),
                            ...R2.cols.map(c => r[c])
                        ]);
                    });
                };
                pushRows(
                    arrowResultToObjects(
                        await conn.query(`WITH src AS (${rowsSql})
                    SELECT * FROM src WHERE ${notEmpty} AND ke IN (SELECT ke FROM src WHERE ${notEmpty} GROUP BY 1 HAVING COUNT(*) > 1)
                    ORDER BY ke LIMIT 50000`)
                    ),
                    'EXACT',
                    r => String(r.ke).split(SEP).join(' | ')
                );
                pushRows(
                    arrowResultToObjects(
                        await conn.query(`WITH src AS (${rowsSql})
                    ...
                    ORDER BY kn, ke LIMIT 50000`)
                    ),
                    'NORMALISE',
                    r => String(r.kn).split(SEP).join(' | ')
                );
                ...
                if (hasFuzzy) {
                    pushRows(
                        ...
                        ORDER BY pn.__pid LIMIT 50000`)
                        ),
                        'FLOU',
                        r => `paire ${Number(r.__pid)} (similarité ${(Number(r.__sim) * 100).toFixed(1)} %)`
                    );
                }
            }
            return R2;
        ...
        async function exportDupRowsCsv(btn) {
            ...
            try {
                ...
                if (!total) {
                    showSuccess('Aucune ligne en double à exporter.');
                } else {
                    const out = [
                        ['PROFIL', 'TYPE', 'GROUPE', 'CLE_FONCTIONNELLE', ...(R2.cols || [])].map(escapeCSV).join(';')
                    ];
                    for (const type of ['EXACT', 'NORMALISE', 'FLOU'])
                        R2.types[type].forEach(row => out.push([row[0], type, ...row.slice(1)].map(escapeCSV).join(';')));
                    downloadTextFile(
                        `Doublons_lignes_${lastDupResults.table.replace(/[^a-zA-Z0-9]/g, '_')}_${Date.now()}.csv`,
                        out.join('\n')
                    );
                    showSuccess(
                        `Export : ${total} ligne(s) — ${R2.types.EXACT.length} stricte(s), ${R2.types.NORMALISE.length} normalisée(s), ${R2.types.FLOU.length} floue(s).`
                    );
                }
            } catch (e) {
                showError('Export impossible : ' + e.message);
                ...
                if (btn) {
                    btn.disabled = false;
                    btn.textContent = '⬇ Toutes les lignes en double (CSV)';
                }
            }
        }
        ...
        async function exportDupRowsXlsx(btn) {
            ...
            if (typeof XLSX === 'undefined')
                return showError("Export Excel indisponible (bibliothèque non chargée) — utilisez l'export CSV.");
            if (btn) {
                btn.disabled = true;
                btn.textContent = 'Export...';
            }
            try {
                ...
                const synth = [
                    ['Table', lastDupResults.table],
                    ['Exporté le', new Date().toLocaleString('fr-FR')],
                    ['Seuil de similarité (flou)', lastDupResults.threshold],
                    [],
                    ['Onglet', 'Rapprochement', 'Lignes'],
                    ['Stricts', 'Clé strictement identique', R2.types.EXACT.length],
                    [
                        'Casse-accents tolérés',
                        'Identique une fois casse, accents, espaces et ponctuation tolérés',
                        R2.types.NORMALISE.length
                    ],
                    [
                        'Paires floues',
                        'Paires suspectes au-dessus du seuil (les 2 lignes de chaque paire)',
                        R2.types.FLOU.length
                    ]
                ];
                ...
                const sheets = [
                    ['EXACT', 'Stricts'],
                    ['NORMALISE', 'Casse-accents tolérés'],
                    ['FLOU', 'Paires floues']
                ];
                ...
                showSuccess(
                    `📗 Excel exporté : ${R2.types.EXACT.length} stricte(s), ${R2.types.NORMALISE.length} normalisée(s), ${R2.types.FLOU.length} floue(s) — un onglet par rapprochement.`
                );
            } catch (e) {
                ...
                if (btn) {
                    btn.disabled = false;
                    btn.textContent = '📗 Lignes en double (Excel, 3 onglets)';
                }
            }
        }
        ...
        function exportAuditCsv() {
            const stats = currentProfilingStats;
            if (!stats) return showError("Lancez d'abord un audit.");
            const head = [
                'colonne',
                'type_semantique',
                'completude_pct',
                ...
                'dates_futur',
                'dates_avant_1900',
                'dates_invraisemblables',
                'ratio_cardinalite_pct',
                'cle_candidate',
                'colonne_constante',
                'recopie_de',
                'espaces_debut_fin',
            ...
            Object.values(stats.columns).forEach(c => {
                rows.push(
                    [
                        ...
                        c.nullCount,
                        c.distinctCount,
                        c.patterns && c.patterns[0] ? c.patterns[0].pattern : '',
                        c.patterns && c.patterns[0] ? c.patterns[0].pct.toFixed(1) : '',
                        c.patternDistinct || '',
                        c.min !== null && c.min !== undefined ? c.min : '',
                        c.max !== null && c.max !== undefined ? c.max : '',
                        c.mean ? c.mean.toFixed(4) : '',
                        c.stdDev ? c.stdDev.toFixed(4) : '',
                        c.minLen,
                        c.maxLen,
                        c.avgLen ? c.avgLen.toFixed(1) : '',
                        c.p05 != null ? c.p05 : '',
                        c.p25 != null ? c.p25 : '',
                        ...
                        c.datesAncient || 0,
                        c.datesFar || 0,
                        c.cardinalityRatio != null ? (100 * c.cardinalityRatio).toFixed(2) : '',
                        c.isCandidateKey ? 'oui' : '',
                        c.isConstant ? 'oui' : '',
                        c.duplicateOf || '',
                        c.spaceIssues || 0,
                        c.multiSpace || 0,
                        ...
                        c.mad != null ? c.mad : '',
                        c.valueListName || '',
                        c.outsideList != null ? c.outsideList : '',
                        (c.frequentValues || [])
                            .slice(0, 5)
                            .map(f => `${f.val} (${f.count})`)
                            .join(' | ')
            ...
            downloadTextFile(`Audit_${stats.tableName.replace(/[^a-zA-Z0-9]/g, '_')}_${Date.now()}.csv`, rows.join('\n'));
        }
        function exportAuditJson() {
            const stats = currentProfilingStats;
            if (!stats) return showError("Lancez d'abord un audit.");
            const blob = new Blob([JSON.stringify({ exportedAt: new Date().toISOString(), ...stats }, null, 2)], {
                type: 'application/json'
            });
            const anchorElement = document.createElement('a');
            anchorElement.href = URL.createObjectURL(blob);
            anchorElement.download = `Audit_${stats.tableName.replace(/[^a-zA-Z0-9]/g, '_')}_${Date.now()}.json`;
            document.body.appendChild(anchorElement);
            anchorElement.click();
            anchorElement.remove();
        }
        ...
        function renderProfilingReport(stats) {
            ...
            kpis.innerHTML = `
                <div class="bg-indigo-50 border border-indigo-100 p-4 rounded-xl shadow-sm"><div class="text-[10px] uppercase font-bold text-indigo-500">Lignes total</div>
                <div class="text-xl font-black mt-1">${stats.totalRows.toLocaleString('fr-FR')}</div>
                    </div>
                <div class="bg-emerald-50 border border-emerald-100 p-4 rounded-xl shadow-sm"><div class="text-[10px] uppercase font-bold text-emerald-500">Densité de données</div>
                    <div class="text-xl font-black mt-1">${density.toFixed(1)} %</div>
                    </div>
                <div class="bg-amber-50 border border-amber-100 p-4 rounded-xl shadow-sm"><div class="text-[10px] uppercase font-bold text-amber-500">Lignes Doublons (strictes)</div>
                    <div class="text-xl font-black mt-1">${stats.duplicatesApprox ? '≈ ' : ''}${stats.duplicateRows.toLocaleString('fr-FR')}</div>
                    <div class="text-[10px] text-slate-400 mt-0.5">${((stats.duplicateRows / stats.totalRows) * 100).toFixed(1)} %${stats.duplicatesApprox ? ' · estimation (mémoire limitée)' : ''}</div>${
                        (stats.funcDupCols || []).length
                            ? `<div class="text-[11px] mt-2 pt-2 border-t border-amber-200/60"><span class="font-bold text-amber-700">${stats.funcDuplicatesApprox ? '≈ ' : ''}${stats.funcDuplicateRows.toLocaleString('fr-FR')}</span> <span class="text-slate-500">hors clés techniques</span><div class="text-[9px] text-slate-400 mt-0.5 leading-tight" title="Colonnes quasi 100 % uniques, ignorées pour ce comptage : ${escapeHTML(stats.funcDupCols.join(', '))}">sans ${escapeHTML(stats.funcDupCols.slice(0, 3).join(', '))}${stats.funcDupCols.length > 3 ? ` +${stats.funcDupCols.length - 3}` : ''}</div>
                    </div>`
                            : ''
                    }</div>
                <div class="bg-red-50 border border-red-100 p-4 rounded-xl shadow-sm"><div class="text-[10px] uppercase font-bold text-red-500">Lignes Vides</div>
                    <div class="text-xl font-black mt-1">${stats.emptyRows.toLocaleString('fr-FR')}</div>
                    </div>
            `;
            ...
            for (const cName in stats.columns) {
                ...
                if (col.completeness < 90)
                    alertsList.push(
                        `<div class="flex items-center gap-2 text-xs bg-red-50 border border-red-100 text-red-700 px-3 py-2 rounded-lg"><i data-lucide="alert-circle" class="w-4 h-4 flex-shrink-0"></i><span>La colonne <strong>${escapeHTML(cName)}</strong> a une complétude faible de ${col.completeness.toFixed(1)}% (${col.nullCount} vides).</span></div>`
                    );
                if (col.semanticType === 'Email' && col.numEmail < stats.totalRows - col.nullCount) {
                    ...
                    alertsList.push(
                        `<div class="flex items-center gap-2 text-xs bg-amber-50 border border-amber-100 text-amber-800 px-3 py-2 rounded-lg"><i data-lucide="alert-triangle" class="w-4 h-4 flex-shrink-0"></i><span>La colonne <strong>${escapeHTML(cName)}</strong> contient ${failCount} ligne(s) ne respectant pas le format Email.</span></div>`
                    );
                }
                if (col.semanticType === 'Téléphone' && col.numPhone < stats.totalRows - col.nullCount) {
                    ...
                    alertsList.push(
                        `<div class="flex items-center gap-2 text-xs bg-amber-50 border border-amber-100 text-amber-800 px-3 py-2 rounded-lg"><i data-lucide="alert-triangle" class="w-4 h-4 flex-shrink-0"></i><span>La colonne <strong>${escapeHTML(cName)}</strong> contient ${failCount} ligne(s) déviantes du format Téléphone.</span></div>`
                    );
                }
                ...
                if (col.patterns && col.patterns.length > 1 && col.patterns[0].pct >= 80) {
                    ...
                    if (deviants > 0)
                        alertsList.push(
                            `<div class="flex items-center gap-2 text-xs bg-purple-50 border border-purple-100 text-purple-800 px-3 py-2 rounded-lg"><i data-lucide="regex" class="w-4 h-4 flex-shrink-0"></i><span>La colonne <strong>${escapeHTML(cName)}</strong> suit majoritairement le format <code class="bg-white border border-purple-200 px-1 rounded font-mono">${escapeHTML(col.patterns[0].pattern)}</code> (${col.patterns[0].pct.toFixed(1)}%) mais <strong>${deviants.toLocaleString('fr-FR')} valeur(s)</strong> s'en écartent (${col.patternDistinct} formats distincts).</span></div>`
                        );
                }
                // Alertes hygiène de saisie
                const hyg = [];
                if (col.spaceIssues > 0)
                    hyg.push(`${col.spaceIssues.toLocaleString('fr-FR')} valeur(s) avec espaces en début/fin`);
                if (col.multiSpace > 0) hyg.push(`${col.multiSpace.toLocaleString('fr-FR')} avec espaces multiples`);
                if (col.placeholders > 0)
                    hyg.push(`${col.placeholders.toLocaleString('fr-FR')} valeur(s) "bouche-trou" (N/A, -, ?, INCONNU...)`);
                if (col.caseDupGroups > 0)
                    hyg.push(
                        `${col.caseDupGroups.toLocaleString('fr-FR')} valeur(s) ne différant que par la casse (PARIS/Paris)`
                    );
                if (col.outliers > 0)
                    hyg.push(`${col.outliers.toLocaleString('fr-FR')} valeur(s) numériques aberrantes (> 3 écarts-types)`);
                if (hyg.length)
                    alertsList.push(
                        `<div class="flex items-center gap-2 text-xs bg-sky-50 border border-sky-100 text-sky-800 px-3 py-2 rounded-lg"><i data-lucide="eraser" class="w-4 h-4 flex-shrink-0"></i><span>Hygiène de <strong>${escapeHTML(cName)}</strong> : ${hyg.join(' · ')}.</span></div>`
                    );
            }

            if (alertsList.length === 0) {
                alertsDiv.innerHTML = `<p class="text-xs text-green-700 italic">Aucune alerte sémantique détectée sur cet échantillon.</p>`;
            } else {
                alertsDiv.innerHTML = `<div class="grid grid-cols-1 md:grid-cols-2 gap-2">${alertsList.join('')}</div>`;
            }
            // V9.5.1 : section repliée par défaut — le compteur dit ce qu'elle contient sans l'ouvrir.
            const qualAlertsCountElement = el('qualAlertsCount');
            if (qualAlertsCountElement) {
                qualAlertsCountElement.textContent = alertsList.length ? alertsList.length + ' alerte(s)' : 'aucune';
                qualAlertsCountElement.className =
                    'text-[11px] font-bold rounded-full px-2 py-0.5 normal-case ' +
                    (alertsList.length ? 'bg-amber-100 text-amber-800' : 'bg-emerald-100 text-emerald-800');
            }

            const tbody = el('profilingColumnsTableBody');
            const tName2 = state.tables[currentProfilingTableId]
                ? state.tables[currentProfilingTableId].name
                : stats.tableName || '';
            let _tbodyHtml = '';
            let _nAlert = 0,
                _hygTotal = 0;
            for (const cName in stats.columns) {
                const columnStats = stats.columns[cName];
                const pct = columnStats.completeness;
                const [qs, ql] = uxColQuality(columnStats);
                if (qs !== 'ok') _nAlert++;
                _hygTotal +=
                    (columnStats.placeholders || 0) +
                    (columnStats.multiSpace || 0) +
                    (columnStats.caseDupGroups || 0) +
                    (columnStats.outliers || 0);
                ...
                const nameHtml = d
                    ...
                    : escapeHTML(cName);
                const buckets = (columnStats.frequentValues || []).slice(0, 10).map(v => Number(v.count) || 0);
                const fv0 = columnStats.frequentValues && columnStats.frequentValues[0];
                const sample = fv0 ? String(fv0.val !== undefined ? fv0.val : fv0.value) : '';
                _tbodyHtml += `
                    <tr data-col="${escapeHTML(cName)}" tabindex="0" aria-label="Détail de la colonne ${escapeHTML(cName)}">
                        <td class="p-3 font-semibold text-slate-800"><span class="flex items-center gap-2">${typeIconUx(columnStats.semanticType)}<span>${nameHtml}</span></span></td>
                        <td class="p-3 text-[11px] text-slate-500">${escapeHTML(uxTypeOf(columnStats.semanticType)[2])}<div class="text-[9.5px] text-slate-300 font-mono">${escapeHTML(columnStats.physicalType || '')}</div>
                            </td>
                        <td class="p-3">${cellMeter(pct)}</td>
                        <td class="p-3 text-xs text-slate-600 font-medium text-right">${columnStats.distinctCount.toLocaleString('fr-FR')}${columnStats.hasMoreDistinct ? '+' : ''}</td>
            ...
            tbody.querySelectorAll('tr').forEach(tr => {
                ...
                tr.addEventListener('keydown', e2 => {
                    if (e2.key === 'Enter') open2();
                });
            });
            const nCols2 = Object.keys(stats.columns).length;
            const uxProfilTfootElement = el('uxProfilTfoot');
            if (uxProfilTfootElement)
                uxProfilTfootElement.textContent = `${nCols2} colonne(s) · ${Number(stats.totalRows || 0).toLocaleString('fr-FR')} ligne(s) auditée(s) · ${_nAlert} colonne(s) en alerte · audit du ${new Date().toLocaleString('fr-FR')}`;
            ...
            if (tiles) {
                ...
                tiles.innerHTML = `
                    <div class="tile-ux"><div class="lab">🎯 Score qualité (règles)</div><div class="big" style="color:${score == null ? 'var(--faint)' : meterColor(score)}">${score == null ? '—' : score}<span class="text-sm" style="color:var(--faint)">${score == null ? '' : '/100'}</span></div>
                        <div class="sub">${score == null ? 'Déclarez des règles dans 📏 Règles & score' : rulesT.length + ' règle(s) évaluée(s)'}</div>
                        </div>
                    <div class="tile-ux"><div class="lab">✓ Complétude moyenne</div>
                            <div class="big" style="color:${meterColor(avgC)}">${avgC.toFixed(0)}<span class="text-sm" style="color:var(--faint)">%</span></div>
                        <div class="sub">${okCols} colonne(s) ≥ 95 %</div>
                        </div>
                    <div class="tile-ux"><div class="lab">⚠️ Colonnes en alerte</div>
                            <div class="big" style="color:${_nAlert ? 'var(--bad)' : 'var(--ok)'}">${_nAlert}</div>
                        <div class="sub">sur ${nCols2} colonne(s) profilée(s)</div>
                        </div>
                    <div class="tile-ux"><div class="lab">🧹 Valeurs à nettoyer</div>
                            <div class="big" style="color:${_hygTotal ? 'var(--warn)' : 'var(--ok)'}">${_hygTotal.toLocaleString('fr-FR')}</div>
                        <div class="sub">bouche-trous, casse, espaces, aberrantes</div>
                        </div>`;
            }
            try {
                renderContextBar();
            ...
            for (const cName in stats.columns) {
                ...
                _selHtml += `
                    <button onclick="showColumnProfilingDetail('${escapeHTML(cName.replace(/'/g, "\\'"))}')" id="btn-col-select-${cName}" class="w-full text-left p-2.5 rounded-lg text-xs font-bold text-slate-700 hover:bg-indigo-50 border border-transparent transition-colors flex justify-between items-center">
                        <span class="truncate pr-2">${escapeHTML(cName)}</span><span class="text-[9px] px-1.5 py-0.5 rounded bg-indigo-50 border border-indigo-100 text-indigo-700 uppercase">${columnStats.semanticType}</span>
                    </button>
                `;
            }
            listSelector.innerHTML = _selHtml;

            const firstCol = Object.keys(stats.columns)[0];
            if (firstCol) showColumnProfilingDetail(firstCol);
            lucide.createIcons();
        }
        ...
        function showColumnProfilingDetail(cName) {
            ...
            document.querySelectorAll("[id^='btn-col-select-']").forEach(btn => {
                btn.className =
                    'w-full text-left p-2.5 rounded-lg text-xs font-bold text-slate-700 hover:bg-indigo-50 border border-transparent transition-colors flex justify-between items-center';
            });
            const activeBtn = el(`btn-col-select-${cName}`);
            if (activeBtn)
                activeBtn.className =
                    'w-full text-left p-2.5 rounded-lg text-xs font-bold bg-indigo-50 border-indigo-200 text-indigo-800 transition-colors flex justify-between items-center shadow-sm';
            ...
            if (c.semanticType === 'Numérique') {
                statsGridHtml = `
                    ...
                    ${
                        ...
                        <div class="grid grid-cols-5 gap-2">${[
                            ...
                            .map(
                                ([l, v]) =>
                                    `<div class="p-2 border rounded-lg ${l === 'Médiane' ? 'bg-indigo-50 border-indigo-200' : 'bg-white'}"><span class="text-[9px] text-slate-500 uppercase block font-bold">${l}</span><span class="text-[12px] font-black ${l === 'Médiane' ? 'text-indigo-700' : 'text-slate-700'}">${v == null ? '—' : Number(v).toLocaleString('fr-FR', { maximumFractionDigits: 2 })}</span></div>`
                            )
                            .join('')}</div>
                        ${
                            (() => {
                                ...
                                return scale > 0 && Math.abs(c.median - c.mean) > scale;
                            })()
                                ? `<div class="text-[10px] text-amber-700 mt-1.5">⚠ Moyenne (${c.mean.toLocaleString('fr-FR', { maximumFractionDigits: 2 })}) et médiane (${c.median.toLocaleString('fr-FR', { maximumFractionDigits: 2 })}) s'écartent nettement : distribution asymétrique, la moyenne est tirée par les extrêmes.</div>`
                                : ''
                        }
                    </div>`
                            : ''
                ...
                statsGridHtml = `
                    ...
                    ${
                        c.semanticType === 'Date/Heure' && (c.dateMin || c.dateMax)
                ...
                c.frequentValues.forEach(f => {
                    ...
                    frequenciesHtml += `
                        <div>
                            ...
                            <div class="w-full bg-slate-100 h-2 rounded-full overflow-hidden"><div class="bg-indigo-600 h-2 rounded-full" style="width: ${relativeWidth}%"></div>
                                </div>
                        </div>
                    `;
                });
            ...
            if (c.patterns && c.patterns.length) {
                ...
                const extra =
                    c.patternDistinct > c.patterns.length
                        ? ` <span class="normal-case font-medium text-slate-400">(${c.patternDistinct} formats distincts au total)</span>`
                        : '';
                patternsHtml =
                    `<div class="mb-6"><h5 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Formats détectés${extra}</h5>
                    ...
                    c.patterns
                        ...
                        <div>
                            ...
                            <div class="w-full bg-slate-100 h-2 rounded-full overflow-hidden"><div class="${i === 0 ? 'bg-purple-600' : 'bg-amber-400'} h-2 rounded-full" style="width: ${(p.count / maxPat) * 100}%"></div>
                                </div>
                        </div>`
                        )
                        .join('') +
            ...
            let colAnomaliesHtml = '';
            const kindIdx = {};
            try {
                const entries = qualAnomalyEntries(currentProfilingStats, currentProfilingTableId).filter(e => e.col === cName);
                colAnomaliesHtml =
                    `<div class="mb-6"><h5 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">🔍 Anomalies de cet attribut — voir & extraire</h5>` +
                    (entries.length
                        ? `<div class="border border-amber-200 rounded-xl overflow-hidden"><table class="w-full text-left text-xs"><tbody class="divide-y divide-amber-100">` +
                          entries
                              .map(e => {
                                  const esql = currentProfilingWhere
                                      ? `SELECT * FROM (${e.sql}) __flt WHERE ${currentProfilingWhere}`
                                      : e.sql;
                                  const idx = registerQualInspect(`${e.col} — ${e.kind}`, esql);
                                  kindIdx[e.kind] = idx;
                                  return `<tr class="hover:bg-amber-50/40"><td class="p-2.5">${escapeHTML(e.kind)}</td><td class="p-2.5 text-right font-black text-amber-700 whitespace-nowrap">${e.n.toLocaleString('fr-FR')}</td>
                                      <td class="p-2.5 whitespace-nowrap">${qualInspectButtons(idx)}</td>
                                      </tr>`;
                              })
                              .join('') +
                        ...
                        : '<p class="text-xs text-emerald-700 bg-emerald-50 border border-emerald-100 rounded-lg p-2.5">✔ Aucune anomalie détectée sur cet attribut.</p>') +
                    `</div>`;
            } catch (e) {
                colAnomaliesHtml = '';
            }
            // Tuile d'hygiène cliquable → ouvre directement les lignes concernées.
            const idxOfPrefix = pre => {
                const k = Object.keys(kindIdx).find(x => x.startsWith(pre));
                return k !== undefined ? kindIdx[k] : null;
            };
            panel.innerHTML = `
                <div class="flex items-center gap-3 border-b pb-4 mb-5"><div class="p-3 bg-indigo-100 rounded-lg text-indigo-700"><i data-lucide="${typeIcon}"></i></div>
                    <div><h4 class="text-lg font-black text-slate-800">${escapeHTML(c.name)}</h4>
                    <span class="text-xs text-indigo-700 font-bold bg-indigo-50 px-2 py-0.5 rounded border border-indigo-100 uppercase">${c.semanticType}</span></div>
                    </div>
                <div class="grid grid-cols-2 gap-4 mb-6">
                    <div class="border rounded-xl p-3 bg-slate-50/50"><span class="text-xs text-slate-500 block uppercase font-bold mb-1">Taux de remplissage</span><div class="flex items-center gap-2"><span class="text-xl font-black" style="color:${meterColor(fillRate)}">${fmtPctUx(fillRate)}</span><span class="text-xs text-slate-400">${currentProfilingStats && currentProfilingStats.totalRows ? Math.max(0, currentProfilingStats.totalRows - c.nullCount).toLocaleString('fr-FR') + ' renseignée(s) · ' : ''}${Number(c.nullCount).toLocaleString('fr-FR')} vide(s)</span></div>
                        </div>
                    <div class="border rounded-xl p-3 bg-slate-50/50"><span class="text-xs text-slate-500 block uppercase font-bold mb-1">Cardinalité</span><div class="flex items-center gap-2"><span class="text-xl font-black text-slate-800">${c.distinctCount.toLocaleString('fr-FR')}${c.hasMoreDistinct ? '+' : ''}</span><span class="text-xs text-slate-400">valeur(s) distincte(s)</span></div>
                        </div>
                </div>
                ${(() => {
                    const tn2 = (state.tables[currentProfilingTableId] || {}).name;
                    if (!tn2) return '';
                    ...
                    return `<div class="mb-4 text-[11px] ${box} border rounded-lg px-3 py-2 flex items-center gap-2 flex-wrap">
                        ...
                        <span>${
                            bound
                                ...
                                : "Si cet attribut ne prend que des valeurs d'une liste figée, rattachez-le : l'audit contrôlera l'appartenance au référentiel au lieu de signaler les codes rares."
                        }</span>
                    </div>`;
                })()}
                ...
                ${(() => {
                    ...
                    if (c.isCandidateKey)
                        tags.push([
                            'bg-emerald-50 border-emerald-200 text-emerald-800',
                            '🔑 Clé candidate',
                            'Toujours renseignée et sans doublon : cette colonne peut identifier une ligne à elle seule.'
                        ]);
                    if (c.isConstant)
                        tags.push([
                            'bg-amber-50 border-amber-200 text-amber-800',
                            '▪ Colonne constante',
                            "Une seule valeur sur toute la table : elle n'apporte aucune information."
                        ]);
                    if (c.duplicateOf)
                        tags.push([
                            'bg-amber-50 border-amber-200 text-amber-800',
                            '⧉ Recopie de « ' + escapeHTML(c.duplicateOf) + ' »',
                            'Valeurs strictement identiques à celles de cette autre colonne, ligne à ligne.'
                        ]);
                    if (
                        !c.isConstant &&
                        !c.isCandidateKey &&
                        c.cardinalityRatio != null &&
                        c.cardinalityRatio > 0 &&
                        c.cardinalityRatio < 0.02 &&
                        c.distinctCount > 1
                    )
                        tags.push([
                            'bg-sky-50 border-sky-200 text-sky-800',
                            '≡ Peu de valeurs (' + c.distinctCount.toLocaleString('fr-FR') + ')',
                            'Faible cardinalité : bonne candidate pour une liste de valeurs autorisées.'
                        ]);
                    return tags.length
                        ? `<div class="flex flex-wrap gap-2 mb-5">${tags.map(([cl, l, ti]) => `<span title="${ti}" class="text-[11px] font-bold px-2.5 py-1 rounded-lg border ${cl}">${l}</span>`).join('')}</div>`
                        : '';
                })()}
                ...
                <div class="mb-6"><h5 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Hygiène de saisie</h5>
                    <div class="grid grid-cols-2 md:grid-cols-5 gap-2">
                        ${[
                            ['Espaces déb./fin', c.spaceIssues, 'Espaces début/fin'],
                            ['Espaces multiples', c.multiSpace, 'Espaces multiples'],
                            ['Bouche-trous', c.placeholders, 'Valeurs bouche-trous'],
                            ['Variantes de casse', c.caseDupGroups, 'Casse incohérente'],
                            ['Aberrantes (3σ)', c.outliers, 'Valeurs aberrantes'],
                            ['Aberrantes (écart interq.)', c.outliersIqr, 'Valeurs atypiques'],
                            ['Extrêmes (score MAD)', c.outliersMad, 'Valeurs extrêmes'],
                            ['Aberrantes dans leur groupe', c.outliersCtx, 'Valeurs aberrantes dans leur groupe']
                        ]
                            .map(([lbl, v, pre]) => {
                                const ix = (v || 0) > 0 ? idxOfPrefix(pre) : null;
                                const clickable = ix !== null;
                ...
                ${
                    currentProfilingTableId
                        ? `<div class="mb-6 border border-teal-200 bg-teal-50/40 rounded-xl p-4">
        ...
        async function analyzeFunctionalPatterns(colName) {
            const tId = currentProfilingTableId;
            const target = state.tables[tId];
            const out = el('funcPatternsResult');
            const btn = el('btnFuncPat');
            if (!target || !out) return;
            btn.disabled = true;
            const sampleOption = el('qualSampleSize').value;
            ...
            try {
                ...
                if (totalLab === 0) {
                    out.innerHTML = '<p class="text-xs text-slate-500 italic">Colonne vide sur l\'échantillon analysé.</p>';
                    btn.disabled = false;
                    return;
                }
                ...
                const freqTokens = arrowResultToObjects(tokRes)
                    .map(r => ({ word: r.tk, count: Number(r.c), pct: (Number(r.c) / totalLab) * 100 }))
                    .filter(t => t.pct >= 10)
                    .slice(0, 8);
                ...
                const objectWords = [];
                const addWords = (label, origin) =>
                    String(label || '')
                        .toUpperCase()
                        .split(/[^A-Z0-9À-Þ]+/)
                        .forEach(w => {
                            if (w.length >= 3 && !objectWords.some(o => o.word === w)) objectWords.push({ word: w, origin });
                        });
                ...
                if (objectWords.length) {
                    const exprs = objectWords.map(
                        (o, i) =>
                            `SUM(CASE WHEN regexp_matches(lv, '(^|[^A-Z0-9À-Þ])${regexEscape(o.word)}([^A-Z0-9À-Þ]|$)') THEN 1 ELSE 0 END)::BIGINT AS w${i}`
                    );
                    const objRes = await conn.query(`WITH lab AS (${labSql}) SELECT ${exprs.join(', ')} FROM lab`);
                    const row = arrowResultToObjects(objRes)[0];
                    objectWords.forEach((o, i) => {
                        const number = Number(row['w' + i]);
                        if (number / totalLab >= 0.05)
                            objNameHits.push({ ...o, count: number, pct: (number / totalLab) * 100 });
                    });
                ...
                const candidates = [];
                scopeTables.forEach(t =>
                    t.headers.forEach(h => {
                        if (!(t.id === tId && h === colName)) candidates.push({ tId: t.id, tName: t.name, col: h });
                    })
                );
                ...
                for (let i = 0; i < candidates.length; i++) {
                    ...
                    try {
                        ...
                        const nvals = Number(rows.nvals),
                            cN = Number(rows.c_n),
                            sN = Number(rows.s_n);
                        if (nvals < 2 || nvals > 100000) continue;
                        const cPct = (cN / totalLab) * 100,
                            sPct = (sN / totalLab) * 100;
                        if (cPct >= 20) results.push({ ...cand, cPct, sPct });
                    } catch (e) {
                        /* colonne candidate inutilisable (type exotique) : on l'ignore */
                    }
                }
                ...
                if (freqTokens.length) {
                    ...
                    html +=
                        `<div class="mb-4"><div class="text-[10px] uppercase font-bold text-teal-800 mb-1.5">🔁 Mots récurrents dans les libellés de la table</div>` +
                        freqTokens
                            .map(
                                t2 => `<div class="mb-1.5">
                            ...
                            <div class="w-full bg-white border border-teal-100 h-1.5 rounded-full overflow-hidden"><div class="bg-slate-400 h-1.5 rounded-full" style="width: ${(t2.count / maxTok) * 100}%"></div>
                                </div>
                        </div>`
                            )
                            .join('') +
                ...
                if (objNameHits.length) {
                    html +=
                        `<div class="mb-4"><div class="text-[10px] uppercase font-bold text-teal-800 mb-1.5">🏷️ Nom d'objet retrouvé dans les libellés</div>` +
                        objNameHits
                            .map(
                                o =>
                                    `<div class="text-xs mb-1">Le mot <code class="bg-white border border-teal-200 px-1 rounded font-mono font-bold">${escapeHTML(o.word)}</code> <span class="text-slate-400">(${escapeHTML(o.origin)})</span> apparaît dans <strong>${o.pct.toFixed(1)}%</strong> des libellés (${o.count.toLocaleString('fr-FR')}).</div>`
                            )
                            .join('') +
                        '</div>';
                }
                ...
                if (scopeTables.length === 1 && state.relations.length === 0)
                    html += `<p class="text-[10px] text-slate-400 mb-2">Aucune relation du modèle de données définie (étape 2) : le croisement est limité aux colonnes de la table auditée.</p>`;
                if (!top.length) {
                    ...
                    html +=
                        top
                            ...
                            <div class="w-full bg-white border border-teal-100 h-2 rounded-full overflow-hidden"><div class="${isStart ? 'bg-teal-600' : 'bg-teal-400'} h-2 rounded-full" style="width: ${Math.min(100, pct)}%"></div>
                                </div>
                        </div>`;
                            })
                            .join('') +
                        `<p class="text-[10px] text-slate-400 mt-2">Correspondance par mot entier (insensible à la casse). Les valeurs multi-mots sont détectées en début de libellé (avant un séparateur), pas en milieu de texte.</p>
                            </div>`;
                }
                out.innerHTML = html;
            } catch (e) {
        ...
        async function runQualityAudit(retried) {
            const tId = el('qualTable').value,
                col = el('qualCol').value,
                sampleOption = el('qualSampleSize').value;
            ...
            currentProfilingGroupCol =
                el('qualGroupCol') && el('qualGroupCol').value !== col ? (el('qualGroupCol') || {}).value || '' : '';

            el('qualResultArea').classList.remove('hidden');
            el('qualLoading').classList.remove('hidden');
            el('qualDashboard').classList.add('hidden');
            el('btnRunQual').disabled = true;
            bgTaskStart(`Audit de "${state.tables[tId] ? state.tables[tId].name : ''}" en cours`);

            const qualLoadingText = el('qualLoadingText'),
                qualProgressBar = el('qualProgressBar');
            qualProgressBar.style.width = '10%';
            qualLoadingText.innerText = 'Calcul du Data Profiling déterministe local...';
            ...
            try {
                ...
                try {
                    await renderDesignedSourcePanel(tId);
                } catch (ePanel) {
                    console.warn(ePanel);
                }
                        ...
                        for (const rule of rules) {
                            try {
                                ...
                                if (res.viol > 0) {
                                    ...
                                    if (vs)
                                        inspectBtns = qualInspectButtons(
                                            registerQualInspect(`Règle : ${bizRuleLabel(rule)}`, vs)
                                        );
                                }
                                div.innerHTML =
                                    `<div class="font-bold text-xs uppercase mb-0.5">Règle métier (modèle de données)</div>` +
                                ...
                                console.warn('Règle métier non vérifiable', rule, e2);
                            }
                        }
                    }
                } catch (e1) {
                    console.warn('Audit des règles métier ignoré :', e1);
                }
                ...
                if (!retried && isCsvScanError(e)) {
                    ...
                    if (fixed) {
                        qualAuditWhereOverride = whereSql;
                        return runQualityAudit(true);
                    }
                }
                showError('Erreur lors du profilage : ' + e.message);
                el('qualResultArea').classList.add('hidden');
            } finally {
                el('btnRunQual').disabled = false;
            }
        }
