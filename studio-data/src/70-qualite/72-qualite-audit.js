        ...
        function populateQualTables() {
            ...
            if (boSel) {
                ...
                boSel.innerHTML = bos.length
                    ? bos.map(b => `<option value="${b.id}">${escapeHTML(b.name)}</option>`).join('')
                    : '<option value="">— aucun objet métier défini —</option>';
            }
            renderQualFilters('file');
            renderQualFilters('bo');
        }
        ...
        async function auditFacetCardinality(bo, st) {
            ...
            const child = tableByName(st.table);
            if (!master || !child) throw new Error('Tables non chargées.');
            const rel = state.relations.find(
                r =>
                    r.sourceCol &&
                    r.targetCol &&
                    ((r.sourceTable === master.id && r.targetTable === child.id) ||
                        (r.targetTable === master.id && r.sourceTable === child.id))
            );
            ...
            const condSql = (st.scope || []).length
                ? ' AND ' +
                  (st.scope || []).map(c => scopeCondSql(c).replaceAll(sqlIdent(c.col), 'x.' + sqlIdent(c.col))).join(' AND ')
                : '';
            // Applicabilité : la facette ne concerne que les lignes maîtres d'une catégorie/type/nature donnée.
            const appliesSql = (st.applies || []).length
                ? ' AND ' +
                  (st.applies || []).map(c => scopeCondSql(c).replaceAll(sqlIdent(c.col), 'p.' + sqlIdent(c.col))).join(' AND ')
                : '';
            const { conn } = await getDB();
            const norm = (a, c) => `NULLIF(UPPER(TRIM(CAST(${a}.${sqlIdent(c)} AS VARCHAR))), '')`;
            const wMaster = boSourceWhere(bo, master.name),
                wChild = boSourceWhere(bo, st.table);
            ...
            return {
                ...
                missing: Number(rows.missing || 0),
                multi: Number(rows.multi || 0),
                examples: exs
            };
        }
        async function profileBusinessObject() {
            ...
            try {
                ...
                el('qualTable').value = master.id;
                handleQualTableChange();
                const boSample = el('qualBoSampleSize');
                if (boSample) el('qualSampleSize').value = boSample.value;
                ...
                qualAuditWhereOverride = [declaredW, adhocW]
                    .filter(Boolean)
                    .map(x => `(${x})`)
                    .join(' AND ');
                await runQualityAudit();
                const alerts = el('profilingAlerts');
                if (!alerts) return;
                ...
                head.innerHTML = `<div class="font-black text-sm text-emerald-800">🏛️ Audit de l'objet métier "${escapeHTML(bo.name)}"</div><div class="text-[11px] text-slate-500 mt-0.5">Table maître profilée : <strong>${escapeHTML(master.name)}</strong> — gouvernance, règles métier, hiérarchies et facettes 1–1 vérifiées ci-dessous.</div>
                ...
                showError("Profiling de l'objet impossible : " + e.message);
            } finally {
                btn.disabled = false;
                bgTaskEnd();
            }
        ...
        function handleQualTableChange() {
            ...
            if (tId && state.tables[tId]) {
                colSelect.innerHTML =
                    '<option value="">Toutes les colonnes (Audit global)</option>' +
                    state.tables[tId].headers.map(h => `<option value="${escapeHTML(h)}">${escapeHTML(h)}</option>`).join('');
            } else {
            ...
            if (gSel) {
                ...
                gSel.innerHTML =
                    ...
                    (t ? t.headers.map(h => `<option value="${escapeHTML(h)}">${escapeHTML(h)}</option>`).join('') : '');
            }
            renderQualFilters('file');
        }
        ...
        const QUAL_FILTER_OPS = [
            { v: 'eq', t: '= égal à' },
            { v: 'neq', t: '≠ différent de' },
            { v: 'contains', t: 'contient' },
            { v: 'ncontains', t: 'ne contient pas' },
            { v: 'starts', t: 'commence par' },
            { v: 'ends', t: 'finit par' },
            { v: 'empty', t: 'est vide' },
            { v: 'nempty', t: "n'est pas vide" },
            { v: 'gt', t: '> supérieur à' },
            { v: 'gte', t: '≥ supérieur ou égal' },
            { v: 'lt', t: '< inférieur à' },
            { v: 'lte', t: '≤ inférieur ou égal' }
        ];
        const qualFilters = { file: [], bo: [] };

        function qualFilterTableId(scope) {
            if (scope === 'file') return el('qualTable') ? el('qualTable').value : null;
            const bo = (state.governance.businessObjects || []).find(
                b => b.id === (el('qualBoSelect') ? el('qualBoSelect').value : '')
            );
        ...
        function removeQualFilterRow(scope, i) {
            qualFilters[scope].splice(i, 1);
            renderQualFilters(scope);
        }
        ...
        function renderQualFilters(scope) {
            const box = el('qualFilterRows-' + scope);
            if (!box) return;
            const rows = qualFilters[scope];
            const headers = qualFilterHeaders(scope);
            if (!rows.length) {
                box.innerHTML = `<p class="text-[11px] text-slate-400 italic">Aucun filtre — toutes les lignes sont auditées.</p>`;
                return;
            }
            box.innerHTML = rows
                .map((c, i) => {
                    ...
                    const colOpts = pool
                        .map(h => `<option value="${escapeHTML(h)}" ${h === c.col ? 'selected' : ''}>${escapeHTML(h)}</option>`)
                        .join('');
                    const opOpts = QUAL_FILTER_OPS.map(
                        o => `<option value="${o.v}" ${o.v === c.op ? 'selected' : ''}>${o.t}</option>`
                    ).join('');
        ...
        function qualCondSql(c) {
            ...
            switch (c.op) {
                case 'eq':
                    return `${raw} = ${lit(c.val)}`;
                case 'neq':
                    return `${raw} IS DISTINCT FROM ${lit(c.val)}`;
                case 'contains':
                    return `${raw} ILIKE ${lit('%' + c.val + '%')}`;
                case 'ncontains':
                    return `(${raw} NOT ILIKE ${lit('%' + c.val + '%')} OR ${id} IS NULL)`;
                case 'starts':
                    return `${raw} ILIKE ${lit(c.val + '%')}`;
                case 'ends':
                    return `${raw} ILIKE ${lit('%' + c.val)}`;
                case 'empty':
                    return `(${id} IS NULL OR TRIM(${raw}) = '')`;
                case 'nempty':
                    return `(${id} IS NOT NULL AND TRIM(${raw}) <> '')`;
                case 'gt':
                case 'gte':
                case 'lt':
                case 'lte': {
        ...
        function buildQualWhere(scope) {
            const valid = qualFilters[scope].filter(
                c => c.col && c.op && (['empty', 'nempty'].includes(c.op) || String(c.val || '').length)
            );
            let sql = '';
            valid.forEach(c => {
                const frag = qualCondSql(c);
                if (!frag) return;
                if (!sql) sql = frag;
        ...
        function registerQualInspect(label, sql, explain) {
            qualInspectRegistry.push({ label, sql, explain });
            return qualInspectRegistry.length - 1;
        }
        ...
        function qualInspectModal() {
            ...
            openUxDrawer({
                sem: '',
                title: 'Lignes en anomalie',
                sub: "Consultation sans quitter l'écran — Échap pour fermer",
                body: `<div id="qualInspectModalHead" class="bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 flex items-center gap-2 flex-wrap text-xs mb-3"></div>
                    <div id="qualInspectModalBody" class="overflow-auto border border-slate-100 rounded-lg"></div>`,
                foot: ''
            });
        }
        function closeQualInspectModal() {
        ...
        async function qualViewInspect(idx, btn, offset) {
            ...
            head.innerHTML =
                `<strong class="text-amber-800">${escapeHTML(entry.label)}</strong><button onclick="closeQualInspectModal()" class="ml-auto text-amber-600 hover:text-red-600 font-black text-base leading-none px-1">✕</button>` +
            ...
            try {
                ...
                head.innerHTML =
                    `<strong class="text-amber-800">${escapeHTML(entry.label)}</strong>
                ...
                if (!rows.length && offset === 0) {
                    body.innerHTML =
                        '<p class="text-xs text-emerald-700 p-4">Aucune ligne en anomalie (peut différer de l\'audit si celui-ci portait sur un échantillon).</p>';
                    return;
                }
                const cols = rows.length ? Object.keys(rows[0]) : [];
                body.innerHTML = `<table class="w-full text-left text-[11px]"><thead class="bg-slate-100 text-slate-600 font-bold sticky top-0 z-10"><tr>${cols.map(c => `<th class="p-2 whitespace-nowrap">${escapeHTML(c)}</th>`).join('')}</tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100">${rows.map(rw => `<tr class="hover:bg-amber-50/50">${cols.map(c => `<td class="p-2 whitespace-nowrap">${escapeHTML(rw[c] == null ? '' : String(rw[c]))}</td>`).join('')}</tr>`).join('')}</tbody>
                    </table>`;
            } catch (e) {
                body.innerHTML = `<p class="text-xs text-red-600 p-4">Consultation impossible : ${escapeHTML(e.message)}</p>`;
            }
        }
        async function qualExportInspect(idx, btn) {
            ...
            try {
                ...
                anchorElement.download = `Anomalies_${entry.label.replace(/[^a-zA-Z0-9]+/g, '_').slice(0, 40)}_${Date.now()}.csv`;
        ...
        function qualAnomalyEntries(stats, tId) {
            ...
            if (stats.duplicateRows > 0) {
                const dupExpr = `md5(concat_ws(chr(1), ${table.headers.map(h => `COALESCE(${rawOf(h)}, chr(0))`).join(', ')}))`;
                entries.push({
                    col: '(toute la ligne)',
                    kind:
                        'Lignes strictement identiques' +
                        (stats.duplicatesApprox ? ' (≈ estimation — la consultation donne le vrai décompte)' : ''),
                    n: stats.duplicateRows,
                    sql: `SELECT * EXCLUDE (__rn) FROM ${T} WHERE ${dupExpr} IN (SELECT ${dupExpr.replace(/\bsrc\b/g, '')} FROM ${T} GROUP BY 1 HAVING COUNT(*) > 1)`
                });
            }
            ...
            if (stats.funcDuplicateRows > 0 && (stats.funcDupCols || []).length) {
                const funcHeaders = table.headers.filter(h => !stats.funcDupCols.includes(h));
                if (funcHeaders.length) {
                    ...
                    entries.push({
                        ...
                        kind:
                            `Lignes identiques hors clés (${stats.funcDupCols.slice(0, 3).join(', ')}${stats.funcDupCols.length > 3 ? '…' : ''} ignorées)` +
                            (stats.funcDuplicatesApprox ? ' (≈ estimation)' : ''),
            ...
            if (stats.emptyRows > 0)
                entries.push({
                    col: '(toute la ligne)',
                    kind: 'Lignes entièrement vides',
                    n: stats.emptyRows,
                    sql: sel(table.headers.map(h => `(${sqlIdent(h)} IS NULL OR ${trimOf(h)} = '')`).join(' AND '))
                });
            Object.values(stats.columns).forEach(c => {
                const h = c.name;
                if (!table.headers.includes(h)) return;
                const vlBound = vlBindingFor(table.name, h); // V6.27 : liste de valeurs rattachée à cet attribut
                if (c.spaceIssues > 0)
                    entries.push({
                        col: h,
                        kind: 'Espaces début/fin',
                        n: c.spaceIssues,
                        sql: sel(`${sqlIdent(h)} IS NOT NULL AND ${rawOf(h)} <> '' AND ${rawOf(h)} <> ${trimOf(h)}`)
                    });
                if (c.multiSpace > 0)
                    entries.push({
                        col: h,
                        kind: 'Espaces multiples',
                        n: c.multiSpace,
                        sql: sel(`${notBlank(h)} AND regexp_matches(${trimOf(h)}, '  +')`)
                    });
                if (c.placeholders > 0)
                    entries.push({
                        col: h,
                        kind: 'Valeurs bouche-trous (N/A, -, ?...)',
                        n: c.placeholders,
                        sql: sel(
                            `${notBlank(h)} AND UPPER(${trimOf(h)}) IN ('N/A','NA','NULL','-','--','?','X','XX','INCONNU','NC','TBD','NONE','AUCUN','VIDE','#N/A','SANS','A DEFINIR','À DÉFINIR')`
                        )
                    });
                if (c.caseDupGroups > 0)
                    entries.push({
                        col: h,
                        kind: 'Casse incohérente (même valeur, casses différentes)',
                        n: c.caseDupGroups,
                        sql: sel(
                            `${notBlank(h)} AND UPPER(${trimOf(h)}) IN (SELECT UPPER(${trimOf(h)}) FROM ${T} WHERE ${notBlank(h)} GROUP BY 1 HAVING COUNT(DISTINCT ${trimOf(h)}) > 1)`
                        )
                    });
                if (c.outliers > 0 && c.stdDev > 0 && !vlBound) {
                    ...
                    entries.push({
                        col: h,
                        kind: `Valeurs aberrantes (hors moyenne ± 3σ)`,
                        n: c.outliers,
                        sql: sel(
                            `${numE} IS NOT NULL AND (${numE} < ${c.mean - 3 * c.stdDev} OR ${numE} > ${c.mean + 3 * c.stdDev})`
                        )
                    });
                }
                ...
                if (c.outliersIqr > 0 && c.iqrLow != null && c.iqrHigh != null && !vlBound) {
                    ...
                    entries.push({
                        col: h,
                        kind: `Valeurs atypiques (hors écart interquartile)`,
                        n: c.outliersIqr,
                        sql: sel(`${numE} IS NOT NULL AND (${numE} < ${c.iqrLow} OR ${numE} > ${c.iqrHigh})`)
                    });
                }
                if (c.outliersMad > 0 && c.madSigma > 0 && !vlBound) {
                    ...
                    entries.push({
                        col: h,
                        kind: 'Valeurs extrêmes (score robuste MAD > 3,5)',
                        n: c.outliersMad,
                ...
                if (c.outliersCtx > 0 && stats.groupCol && !vlBound) {
                    ...
                    entries.push({
                        col: h,
                        kind: `Valeurs aberrantes dans leur groupe (${stats.groupCol})`,
                        n: c.outliersCtx,
                ...
                if (c.datesFar > 0) {
                    ...
                    entries.push({
                        col: h,
                        kind: "Dates invraisemblables (plus d'un siècle dans le futur)",
                        n: c.datesFar,
                        sql: sel(`${dE} > CURRENT_DATE + INTERVAL 100 YEAR`)
                    });
                }
                if (c.datesAncient > 0) {
                    const dE = `COALESCE(TRY_CAST(${tdNormExpr('date', `CAST(${sqlIdent(h)} AS VARCHAR)`)} AS DATE), CAST(TRY_CAST(CAST(${sqlIdent(h)} AS VARCHAR) AS TIMESTAMP) AS DATE))`;
                    entries.push({
                        col: h,
                        kind: 'Dates antérieures à 1900',
                        n: c.datesAncient,
                        sql: sel(`${dE} < DATE '1900-01-01'`)
                    });
                }
                ...
                if (typRe && validCount > typN)
                    entries.push({
                        col: h,
                        kind: `Ne respecte pas le type ${c.semanticType}`,
                        n: validCount - typN,
                        sql: sel(`${notBlank(h)} AND NOT regexp_matches(${trimOf(h)}, '${typRe}')`)
                    });
                if (c.semanticType === 'Date/Heure' && validCount > c.numDate)
                    entries.push({
                        col: h,
                        kind: 'Ne respecte pas le type Date',
                        n: validCount - c.numDate,
                        sql: sel(
                            `${notBlank(h)} AND NOT (regexp_matches(${trimOf(h)}, '${re.date1}') OR regexp_matches(${trimOf(h)}, '${re.date2}'))`
                        )
                    });
                ...
                if (vlBound) {
                    ...
                    if (cond && (c.outsideList || 0) > 0)
                        entries.push({
                            col: h,
                            kind: `Valeur hors référentiel « ${vlBound.name} »`,
                            n: c.outsideList,
                            sql: sel(`${notBlank(h)} AND ${cond}`)
                        });
                ...
                if (!enumLike && (c.patterns || []).length > 1 && c.patterns[0].pct >= 60) {
                    ...
                    entries.push({
                        col: h,
                        kind: `Format hors motif dominant "${c.patterns[0].pattern}"`,
                        n: devN,
                        sql: sel(`${notBlank(h)} AND ${maskE} <> ${sqlLiteral(c.patterns[0].pattern)}`)
                    });
                }
            });
            return entries;
        ...
        function renderQualAnomalyInspector(stats, tId) {
            const box = el('qualAnomalyList');
            if (!box) return;
            const entries = qualAnomalyEntries(stats, tId);
            const qualInspCountElement = el('qualInspCount');
            if (qualInspCountElement) {
                qualInspCountElement.textContent = entries.length ? entries.length + ' anomalie(s)' : 'aucune';
                qualInspCountElement.className =
                    'text-[11px] font-bold rounded-full px-2 py-0.5 normal-case ' +
                    (entries.length ? 'bg-amber-200 text-amber-900' : 'bg-emerald-100 text-emerald-800');
            }
            if (!entries.length) {
                box.innerHTML = '<p class="text-xs text-emerald-700">✔ Aucune anomalie d\'hygiène ou de format détectée.</p>';
                return;
            }
            box.innerHTML =
                `<div class="border border-slate-200 rounded-lg overflow-x-auto"><table class="w-full text-left text-xs"><thead class="bg-slate-50 text-[10px] uppercase font-bold text-slate-500"><tr><th class="p-2">Colonne</th>
                    <th class="p-2">Anomalie</th>
                    <th class="p-2 text-right">Lignes (échantillon audité)</th>
                ...
                entries
                    .map(e => {
                        ...
                        return `<tr class="hover:bg-amber-50/40"><td class="p-2 font-bold">${escapeHTML(e.col)}</td><td class="p-2">${escapeHTML(e.kind)}</td><td class="p-2 text-right font-black text-amber-700">${e.n == null ? '—' : e.n.toLocaleString('fr-FR')}</td>
                            <td class="p-2"><div>${qualInspectButtons(idx)}</div>
                            </td></tr>`;
                    })
                    .join('') +
        ...
        async function performDeterministicProfiling(tId, sampleLimit, col, whereSql = '') {
            ...
            const allColsBlankExpr = table.headers
                .map(h => `(${sqlIdent(h)} IS NULL OR TRIM(CAST(${sqlIdent(h)} AS VARCHAR)) = '')`)
                .join(' AND ');
            ...
            let duplicateRows = 0,
                duplicatesApprox = false;
            try {
                const dRes = await conn.query(
                    `SELECT (COUNT(*) - COUNT(DISTINCT ${dupExpr}))::BIGINT AS d FROM ${sourceExpr} AS src`
                );
                duplicateRows = Math.max(0, Number(arrowResultToObjects(dRes)[0].d));
            } catch (eDup) {
                const dRes = await conn.query(
                    `SELECT (COUNT(*) - approx_count_distinct(${dupExpr}))::BIGINT AS d FROM ${sourceExpr} AS src`
                );
                duplicateRows = Math.max(0, Number(arrowResultToObjects(dRes)[0].d));
            ...
            const funcDupCols = [];
            let funcDuplicateRows = 0,
                funcDuplicatesApprox = false;
            try {
                const KB = 16;
                const cand = [];
                for (let count = 0; count < table.headers.length; count += KB) {
                    const sl = table.headers.slice(count, count + KB);
                    ...
                    const rows = arrowResultToObjects(
                        await conn.query(`SELECT ${exprs.join(', ')} FROM ${sourceExpr} AS src`)
                    )[0];
                    sl.forEach((h, k) => {
                        const number = Number(rows['n' + k]),
                            d = Number(rows['d' + k]);
                        if (number >= totalRows * 0.8 && number > 0 && d / number >= 0.7) cand.push(h);
                    });
                }
                for (const h of cand) {
                    ...
                    const rows = arrowResultToObjects(
                        await conn.query(
                            `SELECT COUNT(${raw})::BIGINT AS n, COUNT(DISTINCT ${raw})::BIGINT AS d FROM ${sourceExpr} AS src`
                        )
                    )[0];
                    if (Number(rows.n) > 0 && Number(rows.d) / Number(rows.n) >= 0.995) funcDupCols.push(h);
                }
                ...
                if (funcDupCols.length && funcHeaders.length) {
                    ...
                    try {
                        const rows = arrowResultToObjects(
                            await conn.query(
                                `SELECT (COUNT(*) - COUNT(DISTINCT ${dupExpr2}))::BIGINT AS d FROM ${sourceExpr} AS src`
                            )
                        )[0];
                        funcDuplicateRows = Math.max(0, Number(rows.d));
                    } catch (e2) {
                        const rows = arrowResultToObjects(
                            await conn.query(
                                `SELECT (COUNT(*) - approx_count_distinct(${dupExpr2}))::BIGINT AS d FROM ${sourceExpr} AS src`
                            )
                        )[0];
                        funcDuplicateRows = Math.max(0, Number(rows.d));
                        funcDuplicatesApprox = true;
                    }
                }
            } catch (eK) {
                console.warn('Détection des clés techniques ignorée :', eK);
            }
            ...
            for (let start = 0; start < targetHeaders.length; start += BATCH) {
                ...
                const res = await conn.query(
                    `SELECT ${colExprs.join(', ')} FROM (SELECT ${proj.join(', ')} FROM ${sourceExpr} AS src) q`
                );
            ...
            try {
                ...
                for (let start = 0; start < cands.length; start += BATCH) {
                    ...
                    const ex = slice.map(i => {
                        const t2 = `TRIM(CAST(${sqlIdent(targetHeaders[i])} AS VARCHAR))`;
                        const nb = `(${t2} IS NOT NULL AND ${t2} <> '')`;
                        return `(COUNT(DISTINCT CASE WHEN ${nb} THEN ${t2} END) - COUNT(DISTINCT CASE WHEN ${nb} THEN UPPER(${t2}) END))::BIGINT AS x${i}`;
                    });
                    const rows = arrowResultToObjects(await conn.query(`SELECT ${ex.join(', ')} FROM ${sourceExpr} AS src`))[0];
                    slice.forEach(i => {
                        colRow[`c${i}_casex`] = Math.max(0, Number(rows[`x${i}`] || 0));
                    });
                }
            } catch (eC) {
                console.warn('Recompte exact des variantes de casse ignoré :', eC);
            }

            const stats = {
                tableName: table.name,
                totalRows,
                emptyRows: Number(g.empty_rows),
                duplicateRows: Number(g.duplicate_rows),
                duplicatesApprox,
                funcDupCols,
                funcDuplicateRows,
                funcDuplicatesApprox,
                totalCells: totalRows * targetHeaders.length,
                filledCells: 0,
                completnessRate: 0,
                columns: {}
            };
            ...
            targetHeaders.forEach((h, i) => {
                ...
                const numNum = Number(colRow[`c${i}_num`]),
                    numDate = Number(colRow[`c${i}_date`]),
                    numEmail = Number(colRow[`c${i}_email`]),
                    numPhone = Number(colRow[`c${i}_phone`]);
                ...
                const mean = 0,
                    stdDev = 0,
                    minVal = null,
                    maxVal = null; // remplis par les passes ciblées
                ...
                stats.columns[h] = {
                    ...
                    numEmail,
                    numPhone,
                    semanticType: colType,
                    physicalType: colType === 'Numérique' ? 'float' : 'string',
                    completeness: validCount > 0 ? (validCount / totalRows) * 100 : 0,
                    distinctCount: Number(colRow[`c${i}_distinct`]),
                    hasMoreDistinct: false,
                    ...
                    max: maxVal,
                    sum: mean * numNum,
                    spaceIssues: Number(colRow[`c${i}_sp`] || 0),
                    multiSpace: Number(colRow[`c${i}_msp`] || 0),
                    ...
                    isCandidateKey:
                        validCount > 0 && validCount === totalRows && Number(colRow[`c${i}_distinct`]) === validCount,
                    duplicateOf: null,
                    frequentValues: []
                };
            });
            ...
            for (let start = 0; start < numOnly.length; start += 12) {
                ...
                const proj = slice.map(
                    (h, k) => `TRY_CAST(REPLACE(TRIM(CAST(${sqlIdent(h)} AS VARCHAR)), ',', '.') AS DOUBLE) AS v${start + k}`
                );
                const agg = slice.map((h, k) => {
                    const j = start + k;
                ...
                const row = arrowResultToObjects(
                    await conn.query(`SELECT ${agg.join(', ')} FROM (SELECT ${proj.join(', ')} FROM ${sourceExpr} AS src) q`)
                )[0];
                slice.forEach((h, k) => {
                    const j = start + k;
                    const columnStats = stats.columns[h];
                    const nz = x => (row[x] != null ? Number(row[x]) : null);
                    columnStats.mean = nz(`n${j}_mean`) || 0;
                    columnStats.stdDev = nz(`n${j}_sd`) || 0;
                    columnStats.min = nz(`n${j}_min`);
                    columnStats.max = nz(`n${j}_max`);
                    columnStats.sum = columnStats.mean * columnStats.numNum;
                    columnStats.p05 = nz(`n${j}_p05`);
                    columnStats.p25 = nz(`n${j}_p25`);
                    columnStats.median = nz(`n${j}_p50`);
                    columnStats.p75 = nz(`n${j}_p75`);
                    columnStats.p95 = nz(`n${j}_p95`);
                });
            }
            ...
            for (let start = 0; start < dateOnly.length; start += 12) {
                ...
                const proj = slice.map((h, k) => {
                    const raw2 = `CAST(${sqlIdent(h)} AS VARCHAR)`;
                ...
                slice.forEach((h, k) => {
                    const j = start + k;
                    const columnStats = stats.columns[h];
                    const validCount2 = totalRows - columnStats.nullCount;
                    ...
                    if (validCount2 > 0 && parsed / validCount2 < 0.8) {
                        columnStats.semanticType = 'Texte';
                        columnStats.physicalType = 'string';
                        columnStats.dateMin = null;
                        columnStats.dateMax = null;
                        columnStats.datesFuture = 0;
                        columnStats.datesAncient = 0;
                        columnStats.datesFar = 0;
                        columnStats.dateDemoted = true;
                        columnStats.dateParsedPct = validCount2 ? (100 * parsed) / validCount2 : 0;
                        return;
                    }
                    columnStats.dateMin = row[`x${j}_min`] != null ? String(row[`x${j}_min`]).slice(0, 10) : null;
                    columnStats.dateMax = row[`x${j}_max`] != null ? String(row[`x${j}_max`]).slice(0, 10) : null;
                    columnStats.datesFuture = Number(row[`x${j}_fut`] || 0);
                    columnStats.datesAncient = Number(row[`x${j}_old`] || 0);
                    columnStats.datesFar = Number(row[`x${j}_far`] || 0);
                });
            }
            ...
            const numCols = targetHeaders.filter(h => {
                const columnStats = stats.columns[h];
                return columnStats.semanticType === 'Numérique' && columnStats.stdDev > 0;
            });
            if (numCols.length) {
                const outExprs = numCols.map((h, i) => {
                    const columnStats = stats.columns[h];
                    const lo = columnStats.mean - 3 * columnStats.stdDev,
                        hi = columnStats.mean + 3 * columnStats.stdDev;
                ...
                numCols.forEach((h, i) => {
                    stats.columns[h].outliers = Number(outRow['o' + i] || 0);
                });
            }
            ...
            const iqrCols = targetHeaders.filter(h => {
                const columnStats = stats.columns[h];
                return (
                    columnStats.semanticType === 'Numérique' &&
                    columnStats.p25 != null &&
                    columnStats.p75 != null &&
                    columnStats.p75 > columnStats.p25
                );
            });
            if (iqrCols.length) {
                const iqrExprs = iqrCols.map((h, i) => {
                    const columnStats = stats.columns[h];
                    const iqr = columnStats.p75 - columnStats.p25;
                    const lo = columnStats.p25 - 1.5 * iqr,
                        hi = columnStats.p75 + 1.5 * iqr;
                    const numE = `TRY_CAST(REPLACE(TRIM(CAST(${sqlIdent(h)} AS VARCHAR)), ',', '.') AS DOUBLE)`;
                    stats.columns[h].iqrLow = lo;
                    stats.columns[h].iqrHigh = hi;
                    return `SUM(CASE WHEN ${numE} IS NOT NULL AND (${numE} < ${lo} OR ${numE} > ${hi}) THEN 1 ELSE 0 END)::BIGINT AS q${i}`;
                });
                const iqrRes = arrowResultToObjects(
                    await conn.query(`SELECT ${iqrExprs.join(', ')} FROM ${sourceExpr} AS src`)
                )[0];
                iqrCols.forEach((h, i) => {
                    stats.columns[h].outliersIqr = Number(iqrRes['q' + i] || 0);
                });
            }
            ...
            const exactCand = targetHeaders.filter(h => {
                const columnStats = stats.columns[h];
                const vc = totalRows - columnStats.nullCount;
                return vc > 0 && (columnStats.distinctCount <= 2 || columnStats.distinctCount >= vc * 0.8);
            });
            if (exactCand.length) {
                for (let start = 0; start < exactCand.length; start += 8) {
                    const slice = exactCand.slice(start, start + 8);
                    const ex = slice.map(
                        (h, k) => `COUNT(DISTINCT NULLIF(TRIM(CAST(${sqlIdent(h)} AS VARCHAR)), ''))::BIGINT AS e${start + k}`
                    );
                    const row = arrowResultToObjects(await conn.query(`SELECT ${ex.join(', ')} FROM ${sourceExpr} AS src`))[0];
                    slice.forEach((h, k) => {
                        const columnStats = stats.columns[h];
                        const vc = totalRows - columnStats.nullCount;
                        const number = Number(row['e' + (start + k)] || 0);
                        columnStats.distinctExact = number;
                        columnStats.cardinalityRatio = vc > 0 ? number / vc : 0;
                        columnStats.isConstant = vc > 0 && number === 1;
                        columnStats.isCandidateKey = vc > 0 && vc === totalRows && number === vc;
                    });
                }
            }
            ...
            const madCols = targetHeaders.filter(h => {
                const columnStats = stats.columns[h];
                return columnStats.semanticType === 'Numérique' && columnStats.median != null;
            });
            if (madCols.length) {
                for (let start = 0; start < madCols.length; start += 8) {
                    const slice = madCols.slice(start, start + 8);
                    const ex = slice.map((h, k) => {
                        const columnStats = stats.columns[h];
                        const numE = `TRY_CAST(REPLACE(TRIM(CAST(${sqlIdent(h)} AS VARCHAR)), ',', '.') AS DOUBLE)`;
                        return `approx_quantile(ABS(${numE} - ${columnStats.median}), 0.5) AS m${start + k}`;
                    });
                    const row = arrowResultToObjects(await conn.query(`SELECT ${ex.join(', ')} FROM ${sourceExpr} AS src`))[0];
                    slice.forEach((h, k) => {
                        const columnStats = stats.columns[h];
                        ...
                        columnStats.mad = mad;
                        columnStats.madSigma = mad > 0 ? 1.4826 * mad : 0;
                    });
                }
                // Comptage des lignes au-delà du seuil usuel (score robuste > 3,5)
                const scored = madCols.filter(h => stats.columns[h].madSigma > 0);
                if (scored.length) {
                    const ex = scored.map((h, i) => {
                        const columnStats = stats.columns[h];
                        const numE = `TRY_CAST(REPLACE(TRIM(CAST(${sqlIdent(h)} AS VARCHAR)), ',', '.') AS DOUBLE)`;
                        return `SUM(CASE WHEN ${numE} IS NOT NULL AND ABS(${numE} - ${columnStats.median}) / ${columnStats.madSigma} > 3.5 THEN 1 ELSE 0 END)::BIGINT AS s${i}`;
                    });
                    const row = arrowResultToObjects(await conn.query(`SELECT ${ex.join(', ')} FROM ${sourceExpr} AS src`))[0];
                    scored.forEach((h, i) => {
                        stats.columns[h].outliersMad = Number(row['s' + i] || 0);
                    });
                }
            }
            ...
            const grpCol =
                currentProfilingGroupCol && targetHeaders.includes(currentProfilingGroupCol) ? currentProfilingGroupCol : '';
            ...
            if (grpCol) {
                ...
                for (const h of ctxCols) {
                    ...
                    try {
                        ...
                        stats.columns[h].ctxError = String(e.message || e);
                    }
                }
            }
            ...
            for (const h of targetHeaders) {
                const L = vlBindingFor(table.name, h);
                if (!L) continue;
                const cond = vlOutsideCond(L, sqlIdent(h));
                if (!cond) {
                    stats.columns[h].vlError = 'Liste incomplète';
                    continue;
                }
                try {
                    const rows = arrowResultToObjects(
                        await conn.query(
                            `SELECT SUM(CASE WHEN ${cond} THEN 1 ELSE 0 END)::BIGINT AS n FROM ${sourceExpr} AS src`
                        )
                    )[0];
                    stats.columns[h].outsideList = Number(rows.n || 0);
                    stats.columns[h].valueListName = L.name;
                } catch (e) {
                    stats.columns[h].vlError = String(e.message || e);
                }
            }
            ...
            const dupCand = targetHeaders.filter(h => {
                const columnStats = stats.columns[h];
                return !columnStats.isConstant && columnStats.distinctCount > 1;
            });
            const pairs = [];
            for (let i = 0; i < dupCand.length && pairs.length < 80; i++) {
                for (let j = i + 1; j < dupCand.length && pairs.length < 80; j++) {
                    const A = stats.columns[dupCand[i]],
                        B = stats.columns[dupCand[j]];
                    if (A.distinctCount === B.distinctCount && A.nullCount === B.nullCount && A.semanticType === B.semanticType)
                        pairs.push([dupCand[i], dupCand[j]]);
                }
            }
            if (pairs.length) {
                const nk = h => `NULLIF(UPPER(TRIM(CAST(${sqlIdent(h)} AS VARCHAR))), '')`;
                const pExprs = pairs.map(
                    ([x, y], i) => `SUM(CASE WHEN ${nk(x)} IS DISTINCT FROM ${nk(y)} THEN 1 ELSE 0 END)::BIGINT AS p${i}`
                );
                const pRes = arrowResultToObjects(await conn.query(`SELECT ${pExprs.join(', ')} FROM ${sourceExpr} AS src`))[0];
                pairs.forEach(([x, y], i) => {
                    if (Number(pRes['p' + i] || 0) === 0) {
            ...
            const groupByExpr =
                sampleLimit === 'all' && totalRows > FREQ_CAP
                    ? `(SELECT * FROM ${tName}${whereClause} LIMIT ${FREQ_CAP})`
                    : sourceExpr;
            ...
            for (const h of targetHeaders) {
                ...
                stats.columns[h].frequentValues = rows.map(r => ({
                    val: r.v,
                    count: Number(r.c),
                    percentage: (Number(r.c) / totalRows) * 100
                }));
            }
            ...
            for (const h of targetHeaders) {
                ...
                const columnStats = stats.columns[h];
                const validCount = totalRows - columnStats.nullCount;
                columnStats.patternDistinct = rows.length ? Number(rows[0].ndist) : 0;
                columnStats.patterns = rows.map(r => ({
                    pattern: r.p,
                    count: Number(r.c),
                    pct: validCount > 0 ? (Number(r.c) / validCount) * 100 : 0
                }));
            }
        ...
        function switchQualSubTab(tab) {
            ...
            const off =
                'py-2.5 px-4 font-medium border-b-2 border-transparent text-slate-500 hover:text-indigo-600 text-sm whitespace-nowrap';
