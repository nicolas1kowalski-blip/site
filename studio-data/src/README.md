...
| `9G-v12-extraction-correctifs.js` | Correctifs Extraire : `v12xTool` (fermer la vue graphique par son bouton désactive `graphMode`), `adv-link-via` ajouté à `V12_VIA_ANY_SELECTS`, `buildAdvSql` / `advPlanJoins` (ancrages de toutes les routes : `c._anchors`, `v12State.linkExtra`), `advExpandCol` (synthèses sur routes réunies en `UNION ALL`, transposition par `list(... ORDER BY rn)` + `list_extract`), `v12CleanErr` sur `showError`, `advPreview`, `advQuality` ; repli sans débordement disque (`v12QueryNoSpill`, `v12ConnNoSpill` via `getDB` pendant `advCount`, `advPreview`, `advQuality`, `advGenerate`, `advColumnsOf`, `v12TmpFromSql`, `v12TmpFromTable`) |
| `9F-v12-liens-lisibles.js` | Liens lisibles dans tous les graphes SVG : `createSvgGraph.router` (`v12LnRoute` : point d'attache par lien réparti sur le côté de la case dans l'ordre vertical de l'autre extrémité, tracé orthogonal à angles arrondis `v12LnPath`, couloir vertical / horizontal propre à chaque lien `v12LnLane`, crochet par le haut quand deux cases se chevauchent) et `createSvgGraph.afterDraw` (`v12LnDecor` : calque `.utop`, lien ou liens d'une case mis en avant au survol et pendant le déplacement). Interrupteur `v12LinesSet` (`sd_v12_ortho`) dans la palette |
| `9E-v12-amont-complet.js` | Remonter jusqu'au début : `v12UpExtend(gr, centerId)` enveloppe `buildBoLineageGraph`, `buildAttrLineageGraph` et `buildColumnLineageGraph` (donc l'onglet Objets et le catalogue) et, niveau par niveau, ajoute les sources de chaque élément amont (`v12UpSourcesOf` : objets amont → leur propre graphe amont, attribut d'origine sans colonne → son objet, application → fichiers lus, producteurs, alimentations de la carte des flux, processus → applications, fichier → producteur, tables conçues, `governance.lineage`, flux) ; nœuds marqués `deep` / `first` (« début de la chaîne ») ; panneau « Depuis le début » (`v12UpPaths`, `v12UpPanelHtml`) ; interrupteur `v12UpSet` (`sd_v12_deep`) dans la barre du lineage et la fiche catalogue ; synthèse V12.7 enrichie du niveau |
| `9D-v12-restitutions.js` | `ASSET_KINDS.report` (restitution) : catalogue (sous-titre), export / import en masse, dossier de gouvernance (sections Restitutions et origines via un crochet `Blob` temporaire, `v12WithBlobHook`), cartes dans `renderGovAssets`, champs `producedBy` / `deliveredTo` / `recipients` / `frequency` / `format`, extension des graphes (`v12RepExtend`) et de la carte des flux ; lineage repliable (`v12LinFold` par sens et type, `v12LinRender` avec barre Réduire / Tout développer / seuil, ouverture d'un groupe au clic) |
