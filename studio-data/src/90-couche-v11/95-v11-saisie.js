        ...
        function v11WizardRender() {
            ...
            const people = typeof govPeople === 'function' ? govPeople() : [];
            const steps = ['Nom et domaine', 'Source et attributs', 'Propriétaire'];
            let body = '';
            if (v11Wiz.step === 0)
                body = `<div class="v11-fld"><label>Nom de l'objet</label><input type="text" id="wz-name" value="${escapeHTML(d.name)}" placeholder="ex : Client, Facture, Site…" onkeydown="if(event.key==='Enter') v11WizardNext()"><div class="hint">Un nom au singulier, tel que le métier le dit.</div>
                    </div>
                <div class="v11-fld"><label>Domaine métier</label><select id="wz-dom"><option value="">— aucun —</option>${govDomains()
                    .map(x => `<option ${x === d.domain ? 'selected' : ''}>${escapeHTML(x)}</option>`)
                    .join(
                        ''
                    )}</select><input type="text" id="wz-dom-new" class="mt-1" placeholder="ou un nouveau domaine…" value=""><div class="hint">Le domaine détermine qui valide.</div>
                        </div>
        ...
        function v11WizCol(h, on) {
            const c = v11Wiz.d.cols || (v11Wiz.d.cols = []);
            if (on) {
                if (!c.includes(h)) c.push(h);
            } else v11Wiz.d.cols = c.filter(x => x !== h);
            const lab = el('v11Modal').querySelector('.v11-fld label span');
            if (lab)
                lab.textContent =
                    '(' + v11Wiz.d.cols.length + ' / ' + (tableByName(v11Wiz.d.table) || { headers: [] }).headers.length + ')';
        }
        function v11WizardCollect() {
            const d = v11Wiz.d;
            if (v11Wiz.step === 0) {
                d.name = (el('wz-name') || {}).value || '';
                d.domain = ((el('wz-dom-new') || {}).value || '').trim() || (el('wz-dom') || {}).value || '';
                d.definition = (el('wz-def') || {}).value || '';
            } else if (v11Wiz.step === 1) {
                if (el('wz-free')) d.free = el('wz-free').value;
            } else {
                d.owner = (el('wz-own') || {}).value || '';
                d.status = (el('wz-status') || {}).value || 'Brouillon';
            }
        }
        function v11WizardNext() {
            v11WizardCollect();
            if (v11Wiz.step === 0 && !v11Wiz.d.name.trim()) {
                v11Toast("Donnez un nom à l'objet.", 'err');
                return;
            }
            v11Wiz.step = Math.min(2, v11Wiz.step + 1);
            v11WizardRender();
        }
        function v11WizardPrev() {
            v11WizardCollect();
            v11Wiz.step = Math.max(0, v11Wiz.step - 1);
            v11WizardRender();
        }
        function v11WizardFinish() {
            ...
            if (!d.name.trim()) {
                v11Wiz.step = 0;
                return v11WizardRender();
            }
            if (d.domain && typeof govAddDomain === 'function' && !govDomains().includes(d.domain)) {
                const governance = state.governance;
                governance.domainList = governance.domainList || [];
                governance.domainList.push(d.domain);
            }
            const els = d.table
                ? (d.cols || []).map(h => ({
                      id: 'be_' + generateId(),
                      name: h,
                      owner: '',
                      mappings: [{ table: d.table, col: h }]
                  }))
                : d.free
                      .split('\n')
                      .map(x => x.trim())
                      .filter(Boolean)
                      .map(n => ({ id: 'be_' + generateId(), name: n, owner: '', mappings: [] }));
            const bo = {
                id: 'bo_' + generateId(),
                name: d.name.trim(),
                definition: d.definition.trim(),
                domain: d.domain,
                globalOwner: d.owner.trim(),
                status: d.status,
                contributors: [],
                sources: d.table ? [{ table: d.table, role: 'maitre' }] : [],
                elements: els,
                structure: [],
            ...
            if (d.table && typeof boSyncAppsFromSources === 'function') {
                try {
        ...
        function v11BulkOpen(boId) {
            ...
            if (!rows.length) return v11Toast('Aucun attribut sur cet objet.', 'info');
            ...
            v11ModalOpen(`<div class="flex items-center gap-3"><h3>Actions groupées</h3><span class="text-xs text-slate-500">${escapeHTML(bo.name)} · ${rows.length} attribut(s)</span><span class="flex-grow"></span><button class="v11-btn" onclick="v11ModalClose()">✕</button></div>
                ...
                <div class="v11-checks">${rows.map(r => `<label><input type="checkbox" value="${r.el.id}" data-st="${r.stId || ''}" ${r.el.definition ? 'data-def="1"' : ''}>${escapeHTML(r.el.name)}${r.facet ? ' ◆ ' + escapeHTML(r.facet) : ''}</label>`).join('')}</div>
                    </div>
                ...
                <div class="v11-fld" id="bk-val-wrap"><label>Valeur</label><span id="bk-val"></span></div>
                    </div>
        ...
        function v11BulkField() {
            const act = (el('bk-act') || {}).value;
            const w = el('bk-val');
            if (!w) return;
            const D = window._v11BulkData || {};
            if (act === 'sensitivity')
                w.innerHTML = `<select id="bk-v">${(D.sens || []).map(o => `<option>${o}</option>`).join('')}</select>`;
            else if (act === 'term')
                w.innerHTML = `<input type="text" id="bk-v" list="bk-terms" placeholder="terme existant ou nouveau"><datalist id="bk-terms">${(D.terms || []).map(t => `<option value="${escapeHTML(t)}">`).join('')}</datalist>`;
            else if (act === 'usedBy')
                w.innerHTML = `<select id="bk-v">${(D.assets || []).map(a => `<option value="${a.id}">${escapeHTML(a.name)}</option>`).join('') || '<option value="">— aucune application déclarée —</option>'}</select>`;
            else if (act === 'definition')
                w.innerHTML = `<textarea id="bk-v" rows="2" placeholder="Définition commune"></textarea>`;
            else w.innerHTML = `<input type="text" id="bk-v" placeholder="Nom ou direction">`;
            el('bk-hint').textContent =
                typeof govFeatureOn === 'function' &&
                govFeatureOn() &&
                !govCanEditBo((state.governance.businessObjects || []).find(b => b.id === govState.selectedBoId))
                    ? 'Vos modifications seront des propositions, une par attribut.'
                    : '';
        }
        function v11BulkApply(boId) {
            ...
            if (!bo) return;
            const act = el('bk-act').value;
            const v = (el('bk-v') || {}).value || '';
            const sel = Array.from(document.querySelectorAll('#v11Modal .v11-checks input:checked')).map(i => ({
                elId: i.value,
                stId: i.dataset.st || ''
            }));
            ...
            sel.forEach(s => {
                if (act === 'term') {
                    termTagAdd('attr', { boId, elId: s.elId }, v);
                    count++;
                } else if (act === 'usedBy') {
                    const attributeRow = (boAllAttrRows(bo) || []).find(x => x.el.id === s.elId);
                    const cur = (attributeRow && attributeRow.el.usedBy) || [];
                    if (!cur.includes(v)) {
                        boAttrWrite(boId, s.stId, s.elId, 'usedBy', [...cur, v]);
                        count++;
                    }
                } else {
                    boAttrWrite(boId, s.stId, s.elId, act, v);
                    count++;
                }
            });
            v11ModalClose();
            renderGovernance();
            v11Toast(count + ' attribut(s) mis à jour.', 'ok');
        }
        // ---- Glisser-déposer : colonne de source → attribut ----
        function v11AddMapping(boId, elId, table, col) {
            const bo = (state.governance.businessObjects || []).find(b => b.id === boId);
            const elm = bo && (bo.elements || []).find(e => e.id === elId);
            if (!elm) return;
            elm.mappings = elm.mappings || [];
            if (elm.mappings.some(m => m.table === table && m.col === col)) return;
            elm.mappings.push({ table, col });
            persistAppState();
            renderGovernance();
        }
        if (typeof govWrapBoAction === 'function') govWrapBoAction('v11AddMapping', 'Rattacher une colonne', 0);
        function v11ColsPanelHtml(bo) {
            const tabs = (bo.sources || []).map(s => s.table).filter(n => tableByName(n));
            if (!tabs.length) return '';
            const used = new Set();
            (bo.elements || []).forEach(e => (e.mappings || []).forEach(m => used.add(m.table + '.' + m.col)));
            const chips = tabs.flatMap(tn =>
                (tableByName(tn).headers || [])
                    .filter(h => !used.has(tn + '.' + h))
                    .map(
                        h =>
                            `<span class="v11-drag" draggable="true" data-tbl="${escapeHTML(tn)}" data-col="${escapeHTML(h)}" title="Glisser sur un attribut pour le rattacher">▫ ${escapeHTML(h)}<span class="text-[9px] opacity-60">${escapeHTML(tn)}</span></span>`
                    )
            );
        ...
        function v11DragInit() {
            ...
            if (!det || det.querySelector('.v11-cols')) return;
            const html = v11ColsPanelHtml(bo);
            if (!html) return;
            det.insertAdjacentHTML('afterbegin', html);
            det.querySelectorAll('.v11-drag').forEach(ch =>
                ch.addEventListener('dragstart', e => {
                    e.dataTransfer.setData('text/plain', JSON.stringify({ tbl: ch.dataset.tbl, col: ch.dataset.col }));
            ...
            document.querySelectorAll('#govContent .bo-item[data-sel]').forEach(row => {
                let s;
                try {
                    s = JSON.parse(row.getAttribute('data-sel'));
                } catch (e) {
                    return;
                }
                if (!s || s.kind !== 'attr' || s.stId) return; // attributs propres seulement (les facettes ont une colonne unique)
                const m = [null, '', s.elId];
                row.addEventListener('dragover', e => {
                    e.preventDefault();
                    row.classList.add('v11-drop');
                });
                row.addEventListener('dragleave', () => row.classList.remove('v11-drop'));
                row.addEventListener('drop', e => {
                    e.preventDefault();
                    row.classList.remove('v11-drop');
                    let d;
                    try {
                    ...
                    if (d && d.tbl && d.col) {
                        v11AddMapping(bo.id, m[2], d.tbl, d.col);
                        v11Toast(d.col + ' rattachée.', 'ok');
                    }
                });
            });
        }
