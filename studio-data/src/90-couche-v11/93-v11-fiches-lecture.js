        ...
        Studio.extend('catEditInObject', (_v11_cat) => function () { const e2 = _catFicheCtx; if (e2 && e2.bo) v11State.edit['bo:' + e2.bo] = true; return _v11_cat.apply(this, arguments); });
        ...
        Studio.extend('termTagOpen', (_v11_termTagOpen) => function (id) { v11State.selTerm = id; const t = (state.governance.glossary || []).find(x => x.id === id); if (t) v11Recent('term', id, t.term); return _v11_termTagOpen.apply(this, arguments); });
        ...
        function v11Acts(kind, id, ent, extra) {
            const can = v11Can(kind, ent); const ro = typeof govIsReadOnly === 'function' && govIsReadOnly();
        ...
        Studio.extend('renderBoDetail', (_v11_renderBoDetail) => function (bo, names) {
        ...
        [['removeBusinessObject', id => ((state.governance.businessObjects || []).find(b => b.id === id) || {}).name, 'l\'objet', () => state.governance.businessObjects], ['removeGlossaryTerm', id => ((state.governance.glossary || []).find(t => t.id === id) || {}).term, 'le terme', () => state.governance.glossary], ['removeGovAsset', id => (assetById(id) || {}).name, 'l\'application ou le processus', () => state.governance.assets]].forEach(([nm, nameOf, what, coll]) => Studio.extend(nm, (o) => function (id) { const name = nameOf(id); if (name && !_govBypass && !confirm('Supprimer ' + what + ' « ' + name + ' » ?')) return; const before = (coll() || []).length; const r = o.apply(this, arguments); if (name && (coll() || []).length < before) v11Toast('« ' + name + ' » supprimé(e).', 'info', { action: () => v11Undo(), actionLabel: '⟲ Annuler' }); return r; }, { motif: 'confirmation et annulation d\'une suppression' }));
        // ---- Fiche complète depuis le catalogue ----
        Studio.extend('catOpenFiche', (_v11_cof) => function () { const r = _v11_cof.apply(this, arguments); const e2 = _catFicheCtx; const foot = el('uxDrawerFoot'); if (e2 && e2.bo && foot && !foot.querySelector('.v11-fullbtn')) foot.insertAdjacentHTML('afterbegin', `<button data-ro="keep" class="v11-fullbtn flex-1 bg-white border border-slate-300 rounded-lg py-2 text-xs font-bold text-slate-600 hover:bg-slate-50" onclick="closeUxDrawer(); v11State.edit['bo:${e2.bo}']=false; v11GoBo('${e2.bo}')">📄 Fiche complète</button>`); return r; });
        // ---- Dupliquer ----
        ...
        ['addBusinessObject', 'createObjectFromAttribute', 'generateObjectFromDesign'].forEach(nm => Studio.extend(nm, (o) => function () { const before = new Set((state.governance.businessObjects || []).map(b => b.id)); const r = o.apply(this, arguments); let any = false; (state.governance.businessObjects || []).forEach(b => { if (!before.has(b.id)) { v11State.edit['bo:' + b.id] = true; any = true; } }); if (any && currentTab === 9) renderGovernance(); return r; }, { motif: 'un objet fraîchement créé s\'ouvre en modification' }));
        Studio.extend('addGlossaryTerm', (_v11_addGlossaryTerm) => function () { const before = new Set((state.governance.glossary || []).map(t => t.id)); const r = _v11_addGlossaryTerm.apply(this, arguments); let any = false; (state.governance.glossary || []).forEach(t => { if (!before.has(t.id)) { v11State.edit['term:' + t.id] = true; any = true; } }); if (any) renderGovernance(); return r; });
        Studio.extend('addGovAsset', (_v11_addGovAsset) => function () { const before = new Set((state.governance.assets || []).map(a => a.id)); const r = _v11_addGovAsset.apply(this, arguments); let any = false; (state.governance.assets || []).forEach(a => { if (!before.has(a.id)) { v11State.edit['asset:' + a.id] = true; any = true; } }); if (any) renderGovernance(); return r; });
