        ...
        if (typeof v12xTool === 'function') {
            Studio.extend('v12xTool', (_v12gTool) => function (name) {
        ...
        Studio.extend('buildAdvSql', (_v12gBuild) => function (spec) {
            v12State.linkExtra = [];
            try {
        ...
        Studio.extend('advPlanJoins', (_v12gPlan) => function (baseId, needs) {
        ...
        Studio.extend('advExpandCol', (_v12gExpand) => function (map, c, hierRef) {
            if (c && c.kind === 'link') {
        ...
        Studio.extend('showError', (_v12gErr) => function (msg) { return _v12gErr.call(this, v12CleanErr(msg)); });
        ['advPreview', 'advQuality'].forEach(nm => Studio.extend(nm, (o) => async function () { const r = await o.apply(this, arguments); try { const box = el(nm === 'advPreview' ? 'adv-preview' : 'adv-quality'); const p = box && box.querySelector('p.text-red-600'); if (p && /<\s*(!doctype|html|body)\b/i.test(p.textContent)) p.textContent = (nm === 'advPreview' ? 'Aperçu impossible : ' : 'Bilan impossible : ') + v12CleanErr(p.textContent); } catch (e) {} return r; }, { motif: 'erreurs contenant une page HTML rendues lisibles' }));
        ...
        v12State.noSpill = 0;
        Studio.extend('getDB', (_v12gGetDB) => async function () { const r = await _v12gGetDB.apply(this, arguments); if (v12State.noSpill > 0 && r && r.conn) return Object.assign({}, r, { conn: v12ConnNoSpill(r.conn) }); return r; });
        ['advCount', 'advPreview', 'advQuality', 'advGenerate', 'advColumnsOf', 'v12TmpFromSql', 'v12TmpFromTable'].forEach(nm => Studio.extend(nm, (o) => async function () { v12State.noSpill++; try { return await o.apply(this, arguments); } finally { v12State.noSpill--; } }, { motif: 'requêtes relancées en mémoire pure en cas de débordement disque' }));
