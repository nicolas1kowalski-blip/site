        ...
        Studio.extend('renderCockpit', (_v12_renderCockpit) => function () { const c = el('cockpitContent'); if (!c) return; c.innerHTML = v12HomeHtml(); try { v11SortableTables(c); } catch (e) {} });
        ...
        Studio.extend('getDB', (_v12_getDB) => async function () { try { const r = await _v12_getDB.apply(this, arguments); v12State.engineDown = false; return r; } catch (e) { v12State.engineDown = true; v12State.engineMsg = e.message || String(e); v12EngineBand(); throw e; } });
        ...
        Studio.extend('renderContextBar', (_v12_renderContextBar) => function () { const r = _v12_renderContextBar.apply(this, arguments); try { v12Crumb(); } catch (e) {} return r; });
        Studio.extend('v11NavKey', (_v12_navKey) => function () { return currentTab === 9 ? _v12_navKey() : JSON.stringify({ t: currentTab }); });
        Studio.extend('v11HistGo', (_v12_histGo) => function (d) { const h = v11State.hist; const j = h.i + d; if (j < 0 || j >= h.stack.length) return; let s; try { s = JSON.parse(h.stack[j]); } catch (e) { return; } if (s && s.t && s.t !== 9) { h.i = j; h.nav = true; try { switchTab(s.t); } finally { h.nav = false; } v11HistButtons(); return; } return _v12_histGo.apply(this, arguments); });
        ...
        Studio.extend('switchTab', (_v12_switchTab) => function (num) { const r = _v12_switchTab.apply(this, arguments); v12After(); return r; });
        ['renderTables', 'renderAdvExtract', 'renderQualityRules', 'renderDashboards', 'renderRecipes', 'renderLinkage', 'renderTablesDesign', 'renderTimeSeries', 'renderBrowserTableHTML', 'renderProfilingReport', 'renderCompKeys', 'renderExpConfigCols'].forEach(nm => Studio.extend(nm, (o) => function () { const r = o.apply(this, arguments); try { if (currentTab !== 9) { v12FsButtons(); v12StickyBars(); if (nm === 'renderTables') { v12SourcesAfter(); v12EmptyBand(currentTab); if (currentTab === 11) renderCockpit(); } v11SortableTables(el('step-' + currentTab)); } } catch (e) {} return r; }, { motif: 'plein écran, barres collantes, tri des tableaux après chaque rendu' }));
        // ---- Recherche : tous les écrans et les actions de l'application ----
        Studio.extend('v11Index', (_v12_index) => function () {
