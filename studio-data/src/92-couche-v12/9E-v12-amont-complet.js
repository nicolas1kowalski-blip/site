        ...
        Studio.extend('buildBoLineageGraph', (_v12upBo) => function (bo) { const gr = _v12upBo.apply(this, arguments); try { if (bo) v12UpExtend(gr, 'bo:' + bo.id); } catch (e) {} return gr; });
        Studio.extend('buildAttrLineageGraph', (_v12upAttr) => function (bo, st, e2, _depth) { const gr = _v12upAttr.apply(this, arguments); try { if (!_depth && e2) v12UpExtend(gr, 'attr:' + e2.id); } catch (e) {} return gr; });
        Studio.extend('buildColumnLineageGraph', (_v12upCol) => function (tbl, col) { const gr = _v12upCol.apply(this, arguments); try { if (!buildColumnLineageGraph._viaAttr) v12UpExtend(gr, 'col:main'); } catch (e) {} return gr; });
        ...
        Studio.extend('v12LinBarHtml', (_v12upBar) => function () { return _v12upBar.apply(this, arguments).replace(/<\/span>\s*$/, '') + v12UpToggleHtml() + '</span>'; });
        ...
        Studio.extend('v12BoLinSynth', (_v12upSynth) => function (bo, gr) { const rows = _v12upSynth.apply(this, arguments); try { rows.forEach(r => { const n = gr.nodes.find(x => x.id === r.id); if (n && n.deep) { r.dir = 'amont'; r.how = (r.how ? r.how + ' · ' : '') + 'niveau ' + n.deep + (n.first ? ' · début de la chaîne' : ''); } }); } catch (e) {} return rows; });
        Studio.extend('v12LinKind', (_v12upKind) => function (n) { if (/^(?:src|ext):/.test(String(n.id))) return 'autre'; return _v12upKind.apply(this, arguments); });
        ...
        Studio.extend('v11Index', (_v12upIdx) => function () { const items = _v12upIdx.apply(this, arguments); try { if (Array.isArray(items)) items.push({ grp: 'Actions', ic: '⇠', label: v12UpOn() ? 'Lineage : ne montrer que le premier niveau amont' : 'Lineage : remonter jusqu\'au début', sub: 'graphes d\'objet, d\'information et de colonne', key: 'lineage amont début chaîne remonter', go: () => v12UpSet(!v12UpOn()) }); } catch (e) {} return items; });
