        ...
        const _v13qBo = openBoLineage; openBoLineage = function (boId) { const r = _v13qBo.apply(this, arguments); try { const bo = (state.governance.businessObjects || []).find(x => x.id === boId); if (bo) v13SentenceInject(v13Sentence(bo)); } catch (e) {} return r; };
        const _v13qAttr = openAttrLineage; openAttrLineage = function (boId, stId, elId) { const r = _v13qAttr.apply(this, arguments); try { const bo = (state.governance.businessObjects || []).find(x => x.id === boId); const rr = bo && (boAllAttrRows(bo) || []).find(x => x.el.id === elId); if (rr) v13SentenceInject(v13Sentence(bo, rr)); } catch (e) {} return r; };
        // ---- la question ----
        ...
        if (typeof v12UpOn === 'function') { const _v13qSent = v13Sentence; v13Sentence = function (bo, r) { let s = _v13qSent.apply(this, arguments); try { if (!v12UpOn() || !bo) return s; const gr = r ? buildAttrLineageGraph(bo, r.stId ? getBoFacets(bo).find(x => x.id === r.stId) : null, r.el) : buildBoLineageGraph(bo); const firsts = gr.nodes.filter(n => n.first && n.deep).map(n => v13Clean(n.title)); if (firsts.length) s = s.replace(/\.$/, '') + ' — tout au début : <b>' + firsts.slice(0, 4).map(escapeHTML).join(', ') + (firsts.length > 4 ? '…' : '') + '</b>.'; } catch (e) {} return s; }; }
