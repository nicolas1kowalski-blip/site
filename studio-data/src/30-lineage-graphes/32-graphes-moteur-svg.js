        ...
        function createSvgGraph(container) {
            ...
            const nodeSize = m => {
                ...
                const text = String(m.title || '').length,
                    cl = String(m.content || '').length;
                return [Math.min(260, Math.max(120, Math.max(text * 7.5, cl * 6) + 20)), m.content ? 46 : 30];
            };
            ...
            const layered = () => {
                ...
                ids.forEach(i => {
                    if (depth[i] === undefined) depth[i] = 0;
                });
                const colsMap = {};
                ids.forEach(i => (colsMap[depth[i]] = colsMap[depth[i]] || []).push(i));
                const order = Object.keys(colsMap)
                    .sort((a, b) => a - b)
                    .map(d => colsMap[d]);
                const preds = {},
                    succs = {};
                ...
                for (let count = 0; count < 4; count++) {
                    ...
                    (fwd ? order : order.slice().reverse()).forEach(col => {
                        col.sort((a, b) => {
                            const na = (fwd ? preds : succs)[a] || [],
                                nb = (fwd ? preds : succs)[b] || [];
                ...
                const packs = order.map(col => {
                    ...
                    const subs = [];
                    for (let count = 0; count < nSub; count++) subs.push(col.slice(count * per, (count + 1) * per));
            ...
            const edgePath = (a, b) => {
                if (b.cx - a.cx > (a.w + b.w) / 2 + 24) {
                    const p1 = { x: a.cx + a.w / 2, y: a.cy },
                        p2 = { x: b.cx - b.w / 2, y: b.cy };
                    const maximum = Math.max(40, (p2.x - p1.x) * 0.45);
                    return {
                        d: `M ${p1.x} ${p1.y} C ${p1.x + maximum} ${p1.y}, ${p2.x - maximum} ${p2.y}, ${p2.x} ${p2.y}`,
                        mx: (p1.x + p2.x) / 2,
                        my: (p1.y + p2.y) / 2
                    };
                }
                if (a.cx - b.cx > (a.w + b.w) / 2 + 24) {
                    const p1 = { x: a.cx - a.w / 2, y: a.cy },
                        p2 = { x: b.cx + b.w / 2, y: b.cy };
                    const maximum = Math.max(40, (p1.x - p2.x) * 0.45);
                    return {
                        d: `M ${p1.x} ${p1.y} C ${p1.x - maximum} ${p1.y}, ${p2.x + maximum} ${p2.y}, ${p2.x} ${p2.y}`,
                        mx: (p1.x + p2.x) / 2,
                        my: (p1.y + p2.y) / 2
                ...
                const p1 = svgBorderPoint(a, b.cx, b.cy),
                    p2 = svgBorderPoint(b, a.cx, a.cy);
            ...
            const draw = () => {
                ...
                if (missing.length === S.nodes.length) layered();
                    ...
                    missing.forEach(n => {
                        const ref = S.edges.find(
                            e => (e.target === n.id && S.pos[e.source]) || (e.source === n.id && S.pos[e.target])
                        );
                        const rp = ref ? S.pos[ref.source === n.id ? ref.target : ref.source] : { x: 0, y: 0 };
                        S.pos[n.id] = { x: rp.x + 260 + Math.random() * 60, y: rp.y + (Math.random() - 0.5) * 160 };
                    });
                const box = id => {
                    const position = S.pos[id],
                        s = S.size[id];
                ...
                const lblFor = e => {
                    if (!e.label) return '';
                    const kt = 't:' + e.label + '¦' + e.target,
                        ks = 's:' + e.label + '¦' + e.source;
                    if (lblCnt[kt] >= 3) {
                        if (lblDone.has(kt)) return '';
                        lblDone.add(kt);
                        return e.label + ' ×' + lblCnt[kt];
                    }
                    if (lblCnt[ks] >= 3) {
                        if (lblDone.has(ks)) return '';
                        lblDone.add(ks);
                        return e.label + ' ×' + lblCnt[ks];
                    }
                    return e.label;
                };
                let eSvg = '';
                S.edges.forEach(e => {
                    ...
                    const st = e.style || {};
                    const col = st.stroke || '#94a3b8';
                    const dash = st.lineDash ? ` stroke-dasharray="${st.lineDash.join(' ')}"` : '';
                    const lw = st.lineWidth || 1.4;
                ...
                let nSvg = '';
                S.nodes.forEach(m => {
                    const [w, h] = S.size[m.id];
                    const position = S.pos[m.id];
                    let inner;
                    if (m.type === 'studio-uml-node') {
                        ...
                        (m.rows || []).forEach(
                            (r, i) =>
                                (inner += `<text x="10" y="${headH + 12 + i * rowH}" font-size="10" fill="#475569">${esc(svgTrunc(r, w))}</text>`)
                        );
                    } else {
                    ...
                    nSvg += `<g class="usvgn" data-id="${esc(m.id)}" transform="translate(${position.x - w / 2},${position.y - h / 2})" style="cursor:grab">${inner}</g>`;
                });
                ...
                bind();
                // Crochet de couche (V12) : décoration après rendu (mise en avant des liens, calques). Sans effet si absent.
                if (createSvgGraph.afterDraw) {
                    try {
                        createSvgGraph.afterDraw(S, S.c.querySelector('svg.usvg'));
                    } catch (e) {}
                }
            };
            const apply = () => {
                const element = S.c.querySelector('.uvp');
                if (element) element.setAttribute('transform', `translate(${S.tx},${S.ty}) scale(${S.k})`);
                const sv = S.c.querySelector('svg.usvg');
                if (sv) sv.classList.toggle('usvg-far', S.k < 0.45);
            };
            const fire = (evt, id) => {
                (S.h[evt] || []).forEach(f => {
                    try {
            ...
            const moveNode = id => {
                const position = S.pos[id],
                    sz = S.size[id];
                const elx = (S._nEls || {})[id];
                if (!position || !sz || !elx) return;
                elx.setAttribute('transform', `translate(${position.x - sz[0] / 2},${position.y - sz[1] / 2})`);
                const box = i => {
                    const position = S.pos[i],
                        ss = S.size[i];
                    return { cx: position.x, cy: position.y, w: ss[0], h: ss[1] };
                };
            ...
            const bind = () => {
                const svg = S.c.querySelector('svg.usvg');
                if (!svg) return;
                if (S._mm) window.removeEventListener('mousemove', S._mm);
                if (S._mu) window.removeEventListener('mouseup', S._mu);
                const nEls = {};
                svg.querySelectorAll('.usvgn').forEach(g => (nEls[g.getAttribute('data-id')] = g));
                const byN = {};
                ...
                const pt = ev => {
                    ...
                    return { x: (ev.clientX - rect.left - S.tx) / S.k, y: (ev.clientY - rect.top - S.ty) / S.k };
                };
                let mode = null,
                    dragId = null,
                    last = null,
                ...
                window.addEventListener(
                    ...
                    (S._mu = () => {
                        ...
                        if (mode === 'node' && moved < 3 && dragId) {
                            const now = Date.now();
                            if (clicks[dragId] && now - clicks[dragId] < 350) {
                                fire('node:dblclick', dragId);
                                clicks[dragId] = 0;
                            } else {
                                clicks[dragId] = now;
                                fire('node:click', dragId);
                            }
                        }
                        mode = null;
                        dragId = null;
                ...
                svg.addEventListener(
                    ...
                    ev => {
                        ...
                        const maximum = Math.max(0.1, Math.min(3, S.k * f));
                        S.tx = mx - (mx - S.tx) * (maximum / S.k);
                        S.ty = my - (my - S.ty) * (maximum / S.k);
                        S.k = maximum;
                        apply();
                    },
                    { passive: false }
            ...
            const api = {
                data(d) {
                    S.nodes = ((d && d.nodes) || []).slice();
                    S.edges = ((d && d.edges) || []).slice();
                },
                render: draw,
                changeData(d) {
                    this.data(d);
                ...
                fitView() {
                    const ids = S.nodes.map(n => n.id).filter(i => S.pos[i]);
                    ...
                    const W = S.c.clientWidth || 800,
                        H = S.c.clientHeight || 450,
                        M = 40;
                    S.k = Math.max(
                        0.1,
                        Math.min(1.4, Math.min((W - 2 * M) / Math.max(1, x2 - x1), (H - 2 * M) / Math.max(1, y2 - y1)))
                    );
                    S.tx = (W - (x2 - x1) * S.k) / 2 - x1 * S.k;
                    S.ty = (H - (y2 - y1) * S.k) / 2 - y1 * S.k;
                    apply();
                },
                zoom(f) {
                    const W = S.c.clientWidth || 800,
                        H = S.c.clientHeight || 450;
                    const maximum = Math.max(0.1, Math.min(3, S.k * f));
                    S.tx = W / 2 - (W / 2 - S.tx) * (maximum / S.k);
                    S.ty = H / 2 - (H / 2 - S.ty) * (maximum / S.k);
                    S.k = maximum;
                    apply();
                },
                centerOn(id) {
                    ...
                    const W = S.c.clientWidth || 800,
                        H = S.c.clientHeight || 450;
                    if (S.k < 0.6) S.k = 0.9;
                    S.tx = W / 2 - position.x * S.k;
                    S.ty = H / 2 - position.y * S.k;
                    apply();
                },
                dimExcept(keep) {
                    S.c
                        .querySelectorAll('.usvgn')
                        .forEach(g => (g.style.opacity = !keep || keep.has(g.getAttribute('data-id')) ? '' : '0.12'));
                    S.c.querySelectorAll('.usvge').forEach(g => {
                        const s2 = g.getAttribute('data-s'),
                            t2 = g.getAttribute('data-t');
                        g.style.opacity = !keep || (keep.has(s2) && keep.has(t2)) ? '' : '0.08';
                    });
                },
                getWidth() {
                    return S.c.clientWidth || 800;
                },
                getHeight() {
                    return S.c.clientHeight || 450;
                },
                changeSize() {},
                get(k2) {
                    return k2 === 'destroyed' ? S.destroyed : undefined;
                },
                getNodes() {
                    return S.nodes.map(wrap);
                },
                getEdges() {
                    return S.edges.map(wrap);
                },
                findById(id) {
                    return wrap(S.nodes.find(n => n.id === id) || S.edges.find(e => e.id === id));
                },
                addItem(kind, m) {
                    (kind === 'node' ? S.nodes : S.edges).push(m);
                },
                updateItem(it, patch) {
                    const m =
                        it && it._m
                            ? it._m
                            : typeof it === 'string'
                              ? S.nodes.find(n => n.id === it) || S.edges.find(e => e.id === it)
                              : it && it.getModel
                                ? it.getModel()
                                : null;
                    if (m) Object.assign(m, patch);
                    clearTimeout(S._rt);
                    S._rt = setTimeout(draw, 30);
                },
                setItemState() {},
                refreshPositions: draw,
                on(evt, fn) {
                    (S.h[evt] = S.h[evt] || []).push(fn);
                },
                destroy() {
                    S.destroyed = true;
                    if (S._mm) window.removeEventListener('mousemove', S._mm);
                ...
                downloadFullImage(name) {
                    try {
                        ...
                        const element = S.c.querySelector('.uvp').cloneNode(true);
                        element.setAttribute('transform', `translate(${M - x1},${M - y1})`);
                        const out = `<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}"><defs><marker id="uArrow" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse"><path d="M0 0 L10 5 L0 10 z" fill="#94a3b8"/></marker></defs><rect width="${w}" height="${h}" fill="#fff"/>${new XMLSerializer().serializeToString(element)}</svg>`;
                        const img = new Image();
                        img.onload = () => {
                            ...
                            anchorElement.download = (name || 'graphe') + '.png';
