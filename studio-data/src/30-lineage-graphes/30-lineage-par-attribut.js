        ...
        function renderAttrFlow(wrap) {
            ...
            if (!bos.length) {
                wrap.innerHTML =
                    '<p class="p-8 text-sm text-slate-400 italic text-center">Créez un objet métier pour utiliser la vue par attribut.</p>';
                return;
            }
            ...
            rows.forEach(r => {
                ...
                const maps = r.stId
                    ? attribute.col
                        ? [{ table: facetTable, col: attribute.col }]
                        : []
                    : attribute.mappings || [];
                let srcApp = assetById(attribute.sourceApp) || assetById((bo.producedBy || [])[0]) || null;
                if (!srcApp && maps[0]) {
                    const sys = String((state.governance.dictionary[maps[0].table] || {}).sourceSystem || '')
                        .trim()
                        .toLowerCase();
                    if (sys)
                        srcApp =
                            (state.governance.assets || []).find(
                                x => x.kind === 'app' && String(x.name).trim().toLowerCase() === sys
                            ) || null;
                }
                attrs.push({
                    id: aid,
                    ...
                    maps,
                    srcApp,
                    diff: !!(attribute.sourceApp && attribute.sourceApp !== ((bo.producedBy || [])[0] || ''))
                });
                if (srcApp) prod.set(srcApp.id, srcApp);
                maps.forEach(m => {
                    const key = m.table + '.' + m.col;
                    cols.set(key, m);
                    links.push(['c:' + key, aid]);
                    if (srcApp) links.push(['p:' + srcApp.id, 'c:' + key]);
                });
                if (!maps.length && srcApp) links.push(['p:' + srcApp.id, aid]);
                (attribute.usedBy || []).forEach(id => {
            ...
            const pill = (fid, main, sub, cls) =>
                `<div data-fid="${escapeHTML(fid)}" onclick="attrFlowClick('${escapeHTML(fid)}')" class="af-pill cursor-pointer border rounded-lg px-2.5 py-1.5 mb-1.5 text-[11px] leading-tight ${cls}"><div class="font-bold truncate">${main}</div>${sub ? `<div class="text-[9px] opacity-70 truncate">${sub}</div>` : ''}</div>`;
            const deg = fid => links.filter(l => l[0] === fid || l[1] === fid).length;
            const colHtml = (title, items) =>
                `<div class="flex-1 min-w-[160px] relative z-10"><div class="text-[10px] uppercase font-bold text-slate-400 mb-1.5 text-center">${title}</div>${items.join('') || '<div class="text-[10px] text-slate-300 italic text-center py-3">—</div>'}</div>`;
            ...
            wrap.innerHTML = `<div class="p-3">
                ...
                <div id="afBoard" class="relative flex gap-8 items-start">
                    ...
                    ${colHtml(
                        ...
                        [...prod.values()]
                            .sort((x, y) => deg('p:' + y.id) - deg('p:' + x.id))
                            .map(a =>
                                pill(
                                    'p:' + a.id,
                                    '🖥 ' + escapeHTML(a.name),
                    ...
                    ${colHtml(
                        '📄 Colonnes techniques',
                        [...cols.entries()].map(([k, m]) =>
                            pill('c:' + k, escapeHTML(m.col), escapeHTML(m.table), 'bg-blue-50 border-blue-200 text-blue-800')
                        )
                    )}
                    ${colHtml(
                        "🏛️ Attributs de l'objet",
                        attrs.map(a =>
                            pill(
                                a.id,
                                escapeHTML(a.el.name) +
                                    (a.diff ? ' <span class="text-amber-600 font-black">≠</span>' : '') +
                                    ((a.el.usedBy || []).length ? '' : ' 😴'),
                                a.facet ? '◆ ' + escapeHTML(a.facet) : '',
                    ...
                    ${colHtml(
                        '🖥⚙️ Consommateurs',
                        [...cons.values()]
                            .sort((x, y) => deg('u:' + y.id) - deg('u:' + x.id))
                            .map(a =>
                                pill(
                                    'u:' + a.id,
                                    escapeHTML(assetLabel(a)),
                                    deg('u:' + a.id) + ' attribut(s)',
                                    a.kind === 'app'
                                        ? 'bg-slate-100 border-slate-300 text-slate-700'
        ...
        function attrFlowCard(fid) {
            ...
            const chip = (txt, cls) =>
                `<div class="text-[11px] ${cls} border rounded px-2 py-1 mb-1 truncate font-semibold">${txt}</div>`;
            const zone = (title, items, empty) =>
                `<div class="flex-1 min-w-[150px]"><div class="text-[9px] uppercase font-bold text-slate-400 mb-1">${title}</div>${items.length ? items.join('') : `<div class="text-[10px] text-slate-300 italic">${empty}</div>`}</div>`;
            const amont = [];
            if (a.app) amont.push(chip('🖥 ' + escapeHTML(a.app), 'bg-slate-100 border-slate-300 text-slate-700'));
            a.cols.forEach(c =>
                amont.push(
                    chip(
                        '📄 ' + escapeHTML(c.col) + ' <span class="opacity-60">(' + escapeHTML(c.table) + ')</span>',
                        'bg-blue-50 border-blue-200 text-blue-800'
                    )
                )
            );
            const aval = a.cons.map(c =>
                chip(
                    (c.kind === 'app' ? '🖥 ' : '⚙️ ') + escapeHTML(c.name),
                    c.kind === 'app'
                        ? 'bg-slate-100 border-slate-300 text-slate-700'
                        : 'bg-orange-50 border-orange-200 text-orange-800'
                )
            ...
            box.innerHTML = `<div class="flex items-center justify-between mb-2">
                ...
                <div class="flex items-stretch gap-2">
                    ...
                    <div class="flex-1 min-w-[130px]"><div class="text-[9px] uppercase font-bold text-slate-400 mb-1">Cet attribut</div>
                        <div class="text-[12px] bg-emerald-100 border border-emerald-300 text-emerald-900 rounded px-2 py-2 font-bold">🏛️ ${escapeHTML(a.name)}${a.facet ? `<div class="text-[9px] font-medium opacity-70">◆ ${escapeHTML(a.facet)}</div>` : ''}${a.diff ? '<div class="text-[9px] text-amber-600 font-bold">≠ source différente du producteur</div>' : ''}</div>
                            </div>
                    <div class="flex items-center text-slate-300 font-black">→</div>
        ...
        function attrFlowDraw() {
            const board = el('afBoard'),
                svg = el('afSvg');
            if (!board || !svg || !_afData) return;
            const keep = _afData.focusSet;
            const many = _afData.links.length > 30;
            const hint = el('afHint');
            if (hint) hint.classList.toggle('hidden', !!keep || !many);
            const els = {};
            board.querySelectorAll('.af-pill').forEach(x => (els[x.getAttribute('data-fid')] = x));
        ...
        function attrFlowClick(fid) {
            if (!_afData) return;
            if (_afData.focus === fid) {
                _afData.focus = null;
                _afData.focusSet = null;
                attrFlowCard(null);
                attrFlowDraw();
                return;
            }
        ...
        function attrFlowChain(startIds) {
            ...
            _afData.links.forEach(([f, t]) => {
                (fwd[f] = fwd[f] || []).push(t);
                (back[t] = back[t] || []).push(f);
            });
            const keep = new Set(startIds);
            let q = [...startIds];
            while (q.length) {
                const current = q.shift();
                (fwd[current] || []).forEach(v => {
                    if (!keep.has(v)) {
                        keep.add(v);
                        q.push(v);
                    }
        ...
        function attrFlowApplyFocus() {
            if (!_afData) return;
            const normalized = srcNorm(govState.lineageAttrQ || '');
            if (!normalized) {
                _afData.focus = null;
                _afData.focusSet = null;
                attrFlowDraw();
            ...
            board.querySelectorAll('.af-pill').forEach(x => {
                ...
                if (srcNorm(x.textContent).includes(normalized)) attrFlowChain([fid]).forEach(v => keep.add(v));
            });
            _afData.focus = '__search__';
            _afData.focusSet = keep;
            attrFlowCard(null);
            attrFlowDraw();
        }
        function _renderLineageImpl(wrap) {
            const nodes = [],
                edges = [],
                seen = new Set();
            const showSrcs = govState.lineageSrcs !== false,
                showBos = govState.lineageBos !== false;
            ...
            if (showSrcs)
                Object.values(state.tables)
                    ...
                    .forEach(t => {
                        ...
                        seen.add(t.name);
                        if (own) {
                            srcFold['tbl:' + t.name] = 'as:' + own.id;
                            foldOwners.add(own.id);
                            (foldNames[own.id] = foldNames[own.id] || []).push(t.name);
                            return;
                        }
                        const colors =
                            t.type === 'extraction'
                        ...
                        nodes.push({
                            ...
                            title: t.name,
                            content: (state.governance.dictionary[t.name] || {}).sourceSystem || '',
                            fill: colors[0],
                            stroke: colors[1]
                        });
                    });
            Object.keys(state.governance.lineage).forEach(derived => {
                ...
                (state.governance.lineage[derived].from || []).forEach(src => {
                    if (seen.has(src) && src !== derived)
                        edges.push({
                            id: 'lin:' + src + '>' + derived,
                            source: 'tbl:' + src,
                            target: 'tbl:' + derived,
                            label: 'alimente',
                            style: { stroke: '#7c3aed', lineWidth: 2, endArrow: { path: '', fill: '#7c3aed' } }
                        });
                });
            });
            state.relations.forEach(r => {
                ...
                if (sN && tN && r.sourceCol && r.targetCol && seen.has(sN) && seen.has(tN)) {
                    ...
                    edges.push({
                        ...
                        source: 'tbl:' + sN,
                        target: 'tbl:' + tN,
                        label: (isSelf ? '🌳 ' : '') + relEdgeLabel(r),
                        type: isSelf ? 'loop' : undefined,
                        loopCfg: isSelf ? { position: 'top', dist: 40 } : undefined,
                        style: isSelf ? { stroke: '#059669', lineWidth: 1.5 } : { stroke: '#cbd5e1', lineDash: [4, 3] }
                    });
                }
            });
            // ---- Applications (amont) & Processus (aval) : le lineage de bout en bout ----
            const showApps = govState.lineageApps !== false,
                showProcs = govState.lineageProcs !== false;
            ...
            if (showApps)
                Object.values(state.tables)
                    .filter(t => t.status === 'ready')
                    .forEach(t => {
                        const sys = String((state.governance.dictionary[t.name] || {}).sourceSystem || '')
                            .trim()
                            .toLowerCase();
                        if (!sys) return;
                        const asset = assets.find(x => x.kind === 'app' && String(x.name).trim().toLowerCase() === sys);
                        if (!asset) return;
                        usedAssets.add(asset.id);
                        edges.push({
                            id: 'app:' + asset.id + '>' + t.name,
                            source: 'as:' + asset.id,
                            target: 'tbl:' + t.name,
                            label: 'produit',
            ...
            (state.governance.businessObjects || []).forEach(bo => {
                (bo.producedBy || []).forEach(id => {
                    const asset = assetById(id);
                    if (!assetOk(asset)) return;
                    usedAssets.add(id);
                    edges.push({
                        id: 'prod:' + id + '>' + bo.id,
                        source: 'as:' + id,
                ...
                (bo.consumedBy || []).forEach(id => {
                    ...
                    edges.push({
                        ...
                        target: 'as:' + id,
                        label: 'consommé par',
                        style: { stroke: '#ea580c', lineWidth: 1.6, endArrow: { path: '', fill: '#ea580c' } }
                    });
                });
            });
            // Un processus s'appuie sur une ou plusieurs applications (v3.7).
            assets
                .filter(a2 => a2.kind === 'process')
                .forEach(pr => {
                    if (!assetOk(pr)) return;
                    (pr.appIds || []).forEach(id => {
                        const asset = assetById(id);
                        if (!assetOk(asset)) return;
                        usedAssets.add(id);
                        usedAssets.add(pr.id);
                        edges.push({
                            id: 'papp:' + id + '>' + pr.id,
                            source: 'as:' + id,
                            target: 'as:' + pr.id,
                            label: 'outille',
                            style: { stroke: '#94a3b8', lineDash: [4, 3], endArrow: { path: '', fill: '#94a3b8' } }
                        });
                    });
                    const linkedTables = new Set([...(pr.tables || []), ...(pr.columns || []).map(c => c.table)]);
                    linkedTables.forEach(tn => {
                        if (seen.has(tn)) {
                            usedAssets.add(pr.id);
                            edges.push({
                                id: 'use:' + pr.id + ':' + tn,
                                ...
                                target: 'as:' + pr.id,
                                label: 'utilisé par',
                                style: { stroke: '#d97706', endArrow: { path: '', fill: '#d97706' } }
                            });
                        }
                    });
                });
            assets.forEach(a => {
                if (!usedAssets.has(a.id)) return;
                const isApp = a.kind === 'app';
                ...
                nodes.push({
                    id: 'as:' + a.id,
                    type: 'studio-rich-node',
                    title: (isApp ? '🖥 ' : '⚙️ ') + a.name,
                    content: sub,
                    fill: isApp ? '#e2e8f0' : '#ffedd5',
                    stroke: isApp ? '#334155' : '#ea580c'
                });
            });
            if (showBos)
                state.governance.businessObjects.forEach(bo => {
                    ...
                    if (govState.lineageBoDetail && (bo.elements || []).length) {
                        const rows = (bo.elements || []).slice(0, 14).map(e2 => {
                            const m = (e2.mappings || [])[0];
                            const srcA =
                                e2.sourceApp && e2.sourceApp !== ((bo.producedBy || [])[0] || '')
                                    ? assetById(e2.sourceApp)
                                    : null;
                            return (
                                e2.name +
                                (m ? '  ←  ' + m.table + '.' + m.col : '') +
                                (srcA ? '  [src: ' + srcA.name + ']' : '') +
                                ((e2.usedBy || []).length || !(state.governance.assets || []).length ? '' : '  😴')
                            );
                        });
                        if ((bo.elements || []).length > 14)
                            rows.push('… +' + ((bo.elements || []).length - 14) + ' attribut(s)');
                        nodes.push({
                            id: 'bo:' + bo.id,
                        ...
                        nodes.push({
                            ...
                            type: 'studio-rich-node',
                            title: '🏛️ ' + bo.name,
                            content: bo.globalOwner ? 'Prop. : ' + bo.globalOwner : '⚠️ sans propriétaire',
                            fill: '#dcfce7',
                            stroke: '#16a34a'
                        });
                    ...
                    (bo.sources || []).forEach(s => {
                        if (!seen.has(s.table)) return;
                        declared.add(s.table);
                        if (s.role === 'maitre')
                            edges.push({
                                id: 'bos:' + bo.id + ':' + s.table,
                                source: 'tbl:' + s.table,
                                target: 'bo:' + bo.id,
                                label: '👑 maître',
                                style: { stroke: '#16a34a', lineWidth: 2.5, endArrow: { path: '', fill: '#16a34a' } }
                            });
                        else if (s.role === 'contributeur')
                            edges.push({
                                id: 'bos:' + bo.id + ':' + s.table,
                                source: 'tbl:' + s.table,
                                target: 'bo:' + bo.id,
                                label: 'contribue',
                                style: { stroke: '#16a34a', lineDash: [4, 3], endArrow: { path: '', fill: '#16a34a' } }
                            });
                        else
                            edges.push({
                                id: 'bos:' + bo.id + ':' + s.table,
                                ...
                                target: 'tbl:' + s.table,
                                label: 'diffusé vers',
                                style: { stroke: '#0d9488', endArrow: { path: '', fill: '#0d9488' } }
                            });
                    });
                    ...
                    ctxMasters.forEach(tn => {
                        if (seen.has(tn) && !globalMasters.has(tn))
                            edges.push({
                                id: 'boc:' + bo.id + ':' + tn,
                                source: 'tbl:' + tn,
                                target: 'bo:' + bo.id,
                                label: `👑 maître selon ${ctxElem ? ctxElem.name : 'contexte'}`,
                                style: {
                                    stroke: '#16a34a',
                                    lineWidth: 2,
                                    lineDash: [8, 4],
                                    endArrow: { path: '', fill: '#16a34a' }
                                }
                            });
                    });
                    // Tables mappées via les sous-éléments mais sans rôle déclaré : simple lien descriptif.
                    const linkedTables = new Set();
                    (bo.elements || []).forEach(e2 => (e2.mappings || []).forEach(m => linkedTables.add(m.table)));
                    linkedTables.forEach(tn => {
                        if (seen.has(tn) && !declared.has(tn) && !ctxMasters.has(tn))
                            edges.push({
                                id: 'bo:' + bo.id + ':' + tn,
                                source: 'tbl:' + tn,
                                target: 'bo:' + bo.id,
                                label: 'décrit',
                                style: { stroke: '#86efac', lineDash: [2, 3], endArrow: { path: '', fill: '#86efac' } }
                            });
                    });
                });
            ...
            const foldedEdges = [];
            edges.forEach(e => {
                const s = _fm(e.source),
                    t = _fm(e.target);
                if (s === t) return;
                const k = s + '¦' + t + '¦' + (e.label || '');
                if (_fseen.has(k)) return;
                _fseen.add(k);
            ...
            lineageGraph = createSvgGraph(
                wrap,
                { type: 'dagre', rankdir: 'LR', nodesep: 18, ranksep: 80 },
                {
        ...
        function lineageChainFocus(id) {
            ...
            if (window._linFocus === id) {
                window._linFocus = null;
                lineageGraph.dimExcept(null);
            ...
            const up = {},
                down = {};
            window._linData.edges.forEach(e => {
            ...
            const node = window._linData.nodes.find(n => n.id === id);
            if (hint) {
                hint.classList.remove('hidden');
                hint.innerHTML = `🔗 Chaîne de <strong>${escapeHTML(node ? node.title : id)}</strong> : ${keep.size} élément(s) de bout en bout (amont + aval). <button onclick="lineageChainFocus('${escapeHTML(id)}')" class="ml-2 text-[10px] bg-white border border-emerald-300 rounded px-1.5 py-0.5 font-bold">effacer</button>`;
            }
        }
        function lineageGoto(title) {
            if (!window._linData || !lineageGraph) return;
            const node = window._linData.nodes.find(n => n.title === title);
            if (!node) return;
