...
| Tests : 112 tests d'API (PGlite et PostgreSQL, dont les constructeurs SQL, l'extraction jointe, la qualité et la qualité avancée, les tables conçues, la gouvernance, le lineage, l'exploitation, le catalogue, la surveillance, la sauvegarde et l'analyse d'impact, le cockpit, les préparations, les statistiques et l'explorateur 360°, l'importation et la couverture), 94 assertions de bout en bout dans Chromium | **livré** | `api/test`, `tests` |
...
```bash
npm run tester:api                                  # 112 tests, PGlite
SD_POSTGRES_URL_TEST=postgres://… npm run tester:api # les mêmes sur PostgreSQL
npm run tester:e2e                                  # 94 assertions, Chromium (Playwright de l'environnement)
